  ...
  useEffect(() => {
    ...
    const loadProductWithRetry = async () => {
      ...
      for (let attempt = 0; attempt <= RETRY_CONFIG.maxRetries; attempt++) {
        try {
          console.log(`🔄 [ProductDataLoader] محاولة ${attempt + 1}/${RETRY_CONFIG.maxRetries + 1} لجلب المنتج: ${slug}`);
          ...
          if (responseData && responseData.product) {
            console.log(`✅ [ProductDataLoader] تم جلب المنتج بنجاح: ${responseData.product.name}`);
            ...
            // المنتج غير موجود
            console.log(`❌ [ProductDataLoader] المنتج غير موجود: ${slug}`);
          ...
          console.error(`❌ [ProductDataLoader] محاولة ${attempt + 1} فشلت:`, error);
          ...
          if (attempt === RETRY_CONFIG.maxRetries) {
            console.error(`❌ [ProductDataLoader] فشلت جميع المحاولات، آخر خطأ:`, lastError);
          ...
          console.log(`⏳ [ProductDataLoader] انتظار ${delay}ms قبل إعادة المحاولة...`);
