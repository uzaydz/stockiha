...
CREATE OR REPLACE FUNCTION create_pos_order_ultra_fast_with_fifo(
...
BEGIN
    ...
    LOOP
        ...
        ELSE
            ...
            PERFORM update_product_stock_safe(
                (v_item->>'product_id')::UUID,
                (v_item->>'quantity')::INTEGER
            );
    ...
    SELECT jsonb_build_object(
        ...
        'created_at', NOW(),
        'updated_at', NOW(),
        'message', 'تم إنشاء الطلب بنجاح مع FIFO محسن'
    ) INTO v_result;
...
CREATE INDEX IF NOT EXISTS idx_pos_orders_org_created_optimized 
ON pos_orders (organization_id, created_at DESC, status) 
WHERE status = 'completed';

CREATE INDEX IF NOT EXISTS idx_pos_order_items_order_optimized 
ON pos_order_items (order_id, organization_id, created_at DESC);

-- تعليق توضيحي
COMMENT ON FUNCTION create_pos_order_ultra_fast_with_fifo IS 'دالة محسنة جداً لإنشاء طلبيات نقطة البيع مع دعم FIFO - هدف الأداء: أقل من 500ms'; 