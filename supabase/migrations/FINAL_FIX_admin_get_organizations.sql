-- =====================================================
-- FINAL FIX: admin_get_organizations_with_subscriptions
-- حل نهائي لمشكلة auth.uid() returning NULL
-- =====================================================
-- المشكلة: SECURITY DEFINER يفقد سياق JWT
...
SECURITY INVOKER  -- ✅ التغيير الرئيسي: INVOKER بدلاً من DEFINER
...
BEGIN
  -- Get authenticated user ID - الآن سيعمل لأننا نستخدم SECURITY INVOKER
  v_auth_id := auth.uid();
...
COMMENT ON FUNCTION admin_get_organizations_with_subscriptions IS 'Get organizations with subscription data for super admin - uses SECURITY INVOKER to preserve auth context';

-- ✅ التحديث مكتمل
-- الفرق الرئيسي: SECURITY INVOKER بدلاً من SECURITY DEFINER
-- هذا يحافظ على سياق المصادقة (auth.uid() يعمل بشكل صحيح)
--
-- لتطبيق هذا التحديث:
-- 1. افتح Supabase SQL Editor
-- 2. الصق هذا الكود
-- 3. اضغط RUN
-- 4. أعد تحميل صفحة Organizations - يجب أن تعمل الآن!
