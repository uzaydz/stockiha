...
const getDefaultLanguageFromDatabase = async (useImmediateCache = false): Promise<string> => {
  ...
  try {
    ...
    if (subdomain === 'localhost' || subdomain === '127' || currentHost.includes('localhost')) {
      ...
      if (!storedOrgId) {
        console.log('🏠 التطوير المحلي: لا يوجد معرف مؤسسة محفوظ، استخدام اللغة الافتراضية');
      ...
      console.log('🏠 التطوير المحلي: استخدام معرف المؤسسة المحفوظ:', storedOrgId);
    }
    ...
    if (organizationId) {
      ...
      if (isUUID) {
        ...
        console.log('🔍 استخدام معرف المؤسسة المباشر:', subdomain);
        query = supabase
          .from('organization_settings')
          .select('default_language, organization_id')
