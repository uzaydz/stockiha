-- إصلاح مشكلة وظائف إعدادات النماذج (الإصدار الثالث)
-- المشكلة: 1. وظيفة uuid_nil() غير موجودة
--          2. الوظيفة غير متاحة عبر RPC في Supabase

-- إنشاء وظيفة uuid_nil() لتوفير UUID صفري
...
BEGIN
    ...
    DROP POLICY IF EXISTS form_settings_all_policy ON form_settings;
...
-- الكشف عن الوظيفة لواجهة RPC في Supabase
-- هذا يجعل الوظيفة متاحة عبر REST API في Supabase
GRANT EXECUTE ON FUNCTION upsert_form_settings(uuid, text, jsonb, jsonb, boolean, boolean, jsonb, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION upsert_form_settings(uuid, text, jsonb, jsonb, boolean, boolean, jsonb, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION upsert_form_settings(uuid, text, jsonb, jsonb, boolean, boolean, jsonb, uuid) TO anon;
