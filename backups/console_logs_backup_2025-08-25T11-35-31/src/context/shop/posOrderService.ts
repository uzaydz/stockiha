...
export async function createPOSOrder(orderData: POSOrderData): Promise<POSOrderResult> {
  ...
  console.log('🔍 [createPOSOrder] بيانات الطلب المستلمة في بداية الدالة:', {
  ...
  try {
    ...
    console.log('🔍 [posOrderService] بيانات الطلب المستلمة:', {
      subtotal: orderData.subtotal,
      total: orderData.total,
      discount: orderData.discount,
    ...
    console.log('🔍 [posOrderService] التحقق من employee_id:', {
      providedEmployeeId: orderData.employeeId,
      type: typeof orderData.employeeId,
      isEmpty: !orderData.employeeId || orderData.employeeId === ""
    });
      ...
      try {
        ...
        if (employeeExists) {
          ...
          if (employeeByAuthId) {
            ...
            console.log('✅ [posOrderService] تم العثور على الموظف بـ auth_user_id:', {
              providedId: orderData.employeeId,
              actualId: employeeByAuthId.id
            });
          } else {
            console.warn('🚨 [posOrderService] معرف الموظف غير موجود في قاعدة البيانات:', orderData.employeeId);
          }
        }
      } catch (error) {
        console.warn('🚨 [posOrderService] خطأ في التحقق من معرف الموظف:', error);
      }
    }

    console.log('🔍 [posOrderService] نتيجة التحقق من employee_id:', {
      validEmployeeId,
      willSaveAs: validEmployeeId || 'null'
    });
    ...
    console.log('🔍 [posOrderService] بيانات الطلب المحضرة:', {
      ...
      try {
        ...
        if (employeeExists) {
          ...
          if (employeeByAuthId) {
            validEmployeeId = employeeByAuthId.id;
            console.log('✅ [posOrderService] تم العثور على الموظف بـ auth_user_id:', {
              providedId: order.employeeId,
              actualId: employeeByAuthId.id
            });
          } else {
            console.warn('🚨 [posOrderService] معرف الموظف غير موجود في قاعدة البيانات:', order.employeeId);
          }
        }
      } catch (error) {
        console.warn('🚨 [posOrderService] خطأ في التحقق من معرف الموظف:', error);
