...
export class UnifiedRequestManager {
  ...
  static async getOrganizationSettings(orgId: string) {
    if (!orgId) {
      console.warn('⚠️ [UnifiedRequestManager] getOrganizationSettings: No orgId provided');
      return null;
    }
    
    console.log('🔍 [UnifiedRequestManager] Fetching settings for org:', orgId);
    
    return executeRequest(
      `unified_org_settings_${orgId}`,
      async () => {
        console.log('📡 [UnifiedRequestManager] Executing Supabase query for org:', orgId);
        ...
        console.log('📊 [UnifiedRequestManager] Supabase response:', {
          orgId,
          hasData: !!data,
          hasError: !!error,
        ...
        if (error) {
          console.error('❌ [UnifiedRequestManager] Error fetching settings:', error);
          throw error;
        }

        // إصلاح مهم: التأكد من إرجاع كائن وليس مصفوفة
        if (Array.isArray(data) && data.length > 0) {
          console.log('🔧 [UnifiedRequestManager] تحويل مصفوفة إلى كائن واحد');
          return data[0];
        }
