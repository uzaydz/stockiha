...
// import SyncManager from '../components/SyncManager'; // Replaced by new SyncManager
// ⚡ تم إزالة syncManager - PowerSync يتعامل مع المزامنة تلقائياً
...
export const SyncManagerWrapper = () => {
    ...
    useEffect(() => {
        if (shouldRenderByPath && isAuthed && currentOrganization?.id) {
            // 🚀 PowerSync يتعامل مع المزامنة تلقائياً - لا حاجة لـ syncManager
            // syncManager.start(currentOrganization.id).catch((error) => {
            //     console.error('[SyncManagerWrapper] Failed to start sync manager:', error);
            //     // لا نرمي الخطأ لأنه لا يجب أن يوقف التطبيق - المزامنة ستحاول مرة أخرى
            // });
        }
    }, [shouldRenderByPath, isAuthed, currentOrganization?.id]);
