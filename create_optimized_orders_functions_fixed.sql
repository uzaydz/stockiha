-- ==============================================
-- دوال محسنة لجلب بيانات الطلبيات بكفاءة عالية - نسخة مصححة
-- ==============================================
    ...
    WITH order_data AS (
        ...
        WHERE 
            ...
            AND (p_search_term IS NULL OR 
                 o.id ILIKE '%' || p_search_term || '%' OR
                 o.customer_order_number::text ILIKE '%' || p_search_term || '%')
...
-- 3. دالة تحديث حالة تأكيد الإتصال محسنة (مع ترتيب صحيح للمعاملات)
...
BEGIN
    ...
    SET 
        status = p_status,
        updated_at = NOW(),
        employee_id = COALESCE(p_user_id, employee_id)
    WHERE 
        id = ANY(p_order_ids) 
    ...
    RETURN QUERY SELECT v_updated_count, v_updated_count > 0;
END;
$$ LANGUAGE plpgsql;

-- 6. إنشاء فهارس محسنة للأداء (إذا لم تكن موجودة)
DO $$ 
BEGIN
    -- فهرس مركب للطلبيات حسب المنظمة والحالة والتاريخ
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_online_orders_org_status_created') THEN
        CREATE INDEX idx_online_orders_org_status_created 
        ON online_orders (organization_id, status, created_at DESC);
    END IF;
    
    -- فهرس مركب للطلبيات حسب المنظمة والعميل والتاريخ
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_online_orders_org_customer_created') THEN
        CREATE INDEX idx_online_orders_org_customer_created 
        ON online_orders (organization_id, customer_id, created_at DESC);
    END IF;
    
    -- فهرس لحالات تأكيد الإتصال
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_call_confirmation_statuses_org_default') THEN
        CREATE INDEX idx_call_confirmation_statuses_org_default 
        ON call_confirmation_statuses (organization_id, is_default DESC, name);
    END IF;
    
    -- فهرس لرقم الطلب للعميل
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_online_orders_customer_order_number') THEN
        CREATE INDEX idx_online_orders_customer_order_number 
        ON online_orders (organization_id, customer_order_number);
    END IF;
    
    -- فهرس للبحث النصي
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_online_orders_search') THEN
        CREATE INDEX idx_online_orders_search 
        ON online_orders USING gin(to_tsvector('arabic', COALESCE(notes, '')));
    END IF;
END $$;

-- 7. إنشاء view محسن للطلبيات مع جميع البيانات المرتبطة
...
SELECT 
    ...
    a.street_address as shipping_street,
...
-- 8. تحديث إحصائيات الجداول للحصول على أداء أفضل
...
DO $$ 
BEGIN
    RAISE NOTICE 'تم إنشاء جميع الدوال والفهارس المحسنة بنجاح!';
    RAISE NOTICE 'الدوال المتاحة:';
    RAISE NOTICE '- get_orders_with_details()';
    RAISE NOTICE '- get_orders_shared_data()';
    RAISE NOTICE '- update_order_call_confirmation_optimized()';
    RAISE NOTICE '- get_orders_stats_optimized()';
    RAISE NOTICE '- bulk_update_orders_status()';
END $$; 