...
import { module8Data } from '@/data/digitalMarketingModule8Data';
import ModuleNavigation from '@/components/courses/ModuleNavigation';
import type { Video } from '@/data/digitalMarketingModule8Data';

const DigitalMarketingModule8: React.FC = () => {
  const navigate = useNavigate();
  const [currentVideoIndex, setCurrentVideoIndex] = useState(0);
  const [completedVideos, setCompletedVideos] = useState<Set<number>>(new Set());
  const [isVideoLoaded, setIsVideoLoaded] = useState(true);

  const currentVideo = module8Data.videos[currentVideoIndex];
  const progress = (completedVideos.size / module8Data.videos.length) * 100;
  ...
  useEffect(() => {
    localStorage.setItem('module8_progress', JSON.stringify([...completedVideos]));
  }, [completedVideos]);
  ...
  const handleNextVideo = () => {
    if (currentVideoIndex < module8Data.videos.length - 1) {
    ...
    <POSPureLayout>
      <div className="container mx-auto px-4 py-6 max-w-7xl" dir="rtl">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-300 mb-3">
                  {module8Data.description}
                </p>
                <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                  <div className="flex items-center gap-1">
                    <PlayCircle className="w-4 h-4" />
                    <span>{module8Data.totalVideos} فيديو</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <Clock className="w-4 h-4" />
                    <span>{module8Data.totalDuration}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                      <span>الفيديو {currentVideoIndex + 1} من {module8Data.videos.length}</span>
              ...
              <div className="p-4 border-t border-gray-200 dark:border-gray-700">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    disabled={currentVideoIndex === module8Data.videos.length - 1}
                    className="flex items-center gap-2"
                  >
                    التالي
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
              <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  {completedVideos.size} من {module8Data.videos.length} مكتمل
                </p>
              </div>
              <div className="max-h-96 overflow-y-auto">
                {module8Data.videos.map((video, index) => (
                  <div
                    key={video.id}
        ...
        <ModuleNavigation
          ...
          totalVideos={module8Data.videos.length}
        />
      </div>
    </POSPureLayout>
  );
...
export default DigitalMarketingModule8;
