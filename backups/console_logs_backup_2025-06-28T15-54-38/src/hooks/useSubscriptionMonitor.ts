...
export const useSubscriptionMonitor = () => {
  ...
  const checkSubscriptionStatus = useCallback(async () => {
    ...
    try {
      ...
      if (monitorError) {
        console.error('خطأ في مراقبة الاشتراكات:', monitorError);
      ...
      if (monitorResult && monitorResult[0]?.total_fixed > 0) {
        console.log('تم إصلاح الاشتراكات:', monitorResult[0]);
      ...
      if (!syncError && syncResult && syncResult.length > 0) {
        ...
        if (orgUpdate) {
          console.log('تم تزامن بيانات المؤسسة:', orgUpdate);
  ...
  const fallbackSubscriptionCheck = useCallback(async () => {
    ...
    try {
      ...
      console.error('خطأ في التحقق الاحتياطي من الاشتراك:', error);
    }
  }, [organization, refreshOrganizationData]);
