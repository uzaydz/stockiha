        ...
        <CardContent className="p-3">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="image" className="space-y-3">
              {/* طريقة رفع الصورة */}
              <div className="space-y-2">
                <Label className="text-xs font-medium">طريقة إضافة الصورة</Label>
              ...
              {imageUploadMethod === 'upload' && (
                <div className="space-y-2">
                  <Label className="text-xs font-medium">رفع صورة جديدة</Label>
                  <ImageUploader
                    imageUrl={item.image || ''}
                    onImageUploaded={(url) => onItemChange(index, 'image', url)}
                    label="اختر صورة"
                    folder="product-benefits"
                    maxSizeInMB={2}
                    compact={true}
                    className="h-24"
                  />
              ...
              {item.image && (
                <div className="space-y-3 pt-2 border-t">
                  <Label className="text-xs font-medium">إعدادات الصورة</Label>
                  
                  {/* نمط عرض الصورة للعنصر الفردي */}
                  <div className="space-y-2">
                    <Label className="text-xs">نمط العرض</Label>
                    <Select
                      value={item.imageObjectFit || 'cover'}
                      onValueChange={(value) => onItemChange(index, 'imageObjectFit', value)}
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="none">أصلي</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <Label className="text-xs">
                        ارتفاع: <strong>{item.imageHeight || 128}px</strong>
                      </Label>
                    </div>
                    <Slider
                      min={60}
                      max={250}
                      step={10}
                      value={[item.imageHeight || 128]}
                      onValueChange={(value) => onItemChange(index, 'imageHeight', value[0])}
                      className="py-1"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="flex items-center justify-between">
                      <Label className="text-xs">تمرير</Label>
                      <Switch
                        checked={item.imageHoverEffect !== false}
                        onCheckedChange={(checked) => onItemChange(index, 'imageHoverEffect', checked)}
                      />
                    </div>
                    <div className="flex items-center justify-between">
                      <Label className="text-xs">ظل</Label>
                      <Switch
                        checked={item.imageShadow !== false}
                        onCheckedChange={(checked) => onItemChange(index, 'imageShadow', checked)}
                      />
                    </div>
                  </div>
                </div>
              ...
              {item.image && (
                <div className="space-y-1">
                  <Label className="text-xs font-medium">معاينة</Label>
                  <div 
                    className="border overflow-hidden bg-muted/30 relative group"
                    style={{
                      height: `${item.imageHeight || 100}px`,
                      borderRadius: `${item.imageBorderRadius || 6}px`
                    }}
                  >
                    <img 
                      src={item.image} 
                      ...
                      style={{ 
                        ...
                        transform: item.imageHoverEffect !== false ? 'scale(1)' : 'scale(1)',
                        boxShadow: item.imageShadow !== false ? '0 4px 6px -1px rgba(0, 0, 0, 0.1)' : 'none'
                      }}
                      onError={(e) => {
