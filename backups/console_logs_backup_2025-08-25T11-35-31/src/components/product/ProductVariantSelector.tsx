...
const ProductVariantSelector = memo<ProductVariantSelectorProps>(({
  ...
  console.log('🚀 [ProductVariantSelector] بيانات المنتج الواردة:', {
    hasProduct: !!product,
    productId: product?.id,
    productName: product?.name,
    hasVariants: product?.variants?.has_variants,
    colorsCount: product?.variants?.colors?.length || 0,
    colorsData: product?.variants?.colors?.map(color => ({
      id: color.id,
      name: color.name,
      imageUrl: color.image_url,
      hasImage: !!color.image_url,
      imageLength: color.image_url ? color.image_url.length : 0
    }))
  });

  // ✅ تم تحديث النظام لتحميل جميع صور الألوان دائماً
  console.log('✅ [ProductVariantSelector] يتم الآن تحميل جميع صور الألوان دائماً');
    ...
    >
      ...
      >
        ...
        <div className="flex flex-wrap gap-3">
          {/* 🚀 تسجيل معلومات البيانات الواردة للتشخيص */}
          {(() => {
            console.log('🔍 [ProductVariantSelector] البيانات الواردة:', {
              variantDataKeys: Object.keys(variantData),
              colorsCount: variantData.colors?.length || 0,
              firstColorKeys: variantData.colors?.[0] ? Object.keys(variantData.colors[0]) : null,
              hasColors: !!variantData.colors,
              colorsType: typeof variantData.colors
            });
            return null;
          })()}
          ...
          {variantData.colors.map((color) => {
            ...
            console.log('🔍 [ProductVariantSelector] معلومات اللون:', {
              colorId: color.id,
              colorName: color.name,
              imageUrl: color.image_url,
              colorUrlAlt: (color as any).imageUrl,
