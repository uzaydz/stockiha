...
import { getProductCompleteDataOptimized } from './productCompleteOptimized';

// دالة fallback لجلب بيانات المنتج بطريقة مباشرة
const getProductDataFallback = async (
  productIdentifier: string,
  options: { organizationId?: string; includeInactive?: boolean; dataScope?: string }
): Promise<ProductCompleteResponse | null> => {
  console.log('🔄 [getProductDataFallback] بدء جلب البيانات بطريقة مباشرة...');
  ...
  try {
    ...
    const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(productIdentifier);
    
    if (isUuid) {
      productQuery = productQuery.eq('id', productIdentifier);
    } else {
      productQuery = productQuery.eq('slug', productIdentifier);
    }
    ...
    if (productError) {
      console.error('❌ [getProductDataFallback] خطأ في جلب المنتج:', productError);
      throw productError;
    }
    ...
    console.log('✅ [getProductDataFallback] تم جلب المنتج:', product.name);

    // إرجاع بيانات مبسطة
    return {
      success: true,
      data_scope: 'basic' as DataScope,
      product: product as any,
      stats: null as any,
      meta: null as any
    } as ProductCompleteResponse;
  } catch (error) {
    console.error('🔴 [getProductDataFallback] خطأ في fallback:', error);
    throw error;
  }
};
...
export const getProductCompleteData = async (
  ...
  console.log('🔍 [getProductCompleteData] بدء استدعاء دالة جلب بيانات المنتج:', {
    productIdentifier,
    options,
  ...
  try {
    console.log('🔄 [getProductCompleteData] استدعاء RPC get_product_complete_data...');
    console.log('🔍 [DEBUG] استدعاء get_product_complete_data مع:', {
      productIdentifier,
    ...
    console.log('📝 [getProductCompleteData] معاملات RPC:', rpcParams);

    // إضافة timeout للـ RPC call
    const rpcCall = supabase.rpc('get_product_complete_data' as any, rpcParams);
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('RPC timeout after 15 seconds')), 15000)
    );

    console.log('⏱️ [getProductCompleteData] بدء RPC call مع timeout 15 ثانية...');

    const { data, error } = await Promise.race([rpcCall, timeoutPromise]) as any;

    console.log('📊 [getProductCompleteData] نتيجة RPC:', {
      hasData: !!data,
    ...
    if (error) {
      console.error('❌ [getProductCompleteData] خطأ من RPC:', {
        error,
    ...
    if (!data) {
      console.warn('⚠️ [getProductCompleteData] لم يتم إرجاع بيانات من RPC');
      return {
        success: false,
        error: {
          message: 'لم يتم إرجاع بيانات من قاعدة البيانات',
    ...
    console.log('✅ [getProductCompleteData] تم جلب البيانات بنجاح:', {
    ...
    console.log('🔍 [DEBUG] Full product data from Original RPC:', data.product);

    return data as ProductCompleteResponse;
  } catch (error) {
    console.error('🔴 [getProductCompleteData] خطأ عام في جلب بيانات المنتج:', {
      error,
    ...
    // محاولة fallback - جلب البيانات بطريقة مباشرة
    console.log('🔄 [getProductCompleteData] محاولة fallback مع استدعاءات مباشرة...');
    try {
      const fallbackResult = await getProductDataFallback(productIdentifier, options);
      if (fallbackResult) {
        console.log('✅ [getProductCompleteData] نجح fallback!');
        return fallbackResult;
      }
    } catch (fallbackError) {
      console.error('❌ [getProductCompleteData] فشل fallback أيضاً:', fallbackError);
    }
...
// 🚀 دالة ذكية تختار أفضل طريقة لجلب البيانات
export const getProductCompleteDataSmart = async (
  productIdentifier: string,
  options: {
    organizationId?: string;
    includeInactive?: boolean;
    dataScope?: DataScope;
    useOptimized?: boolean; // خيار لفرض استخدام الدالة المحسنة
  } = {}
): Promise<ProductCompleteResponse | null> => {
  const { useOptimized = true, ...restOptions } = options;
  
  // إذا كان المطلوب استخدام الدالة المحسنة
  if (useOptimized) {
    console.log('🚀 [getProductCompleteDataSmart] استخدام الدالة المحسنة أولاً...');
    try {
      const result = await getProductCompleteDataOptimized(productIdentifier, restOptions);
      if (result) {
        console.log('✅ [getProductCompleteDataSmart] نجحت الدالة المحسنة');
        return result;
      }
    } catch (error) {
      console.warn('⚠️ [getProductCompleteDataSmart] فشلت الدالة المحسنة، التراجع للعادية...');
    }
  }
  
  // التراجع للدالة العادية
  console.log('🔄 [getProductCompleteDataSmart] استخدام الدالة العادية...');
  return await getProductCompleteData(productIdentifier, restOptions);
};
...
export const getDefaultColor = (product: CompleteProduct): ProductColor | null => {
  ...
  const defaultColor = product.variants.colors.find(color => color.is_default && (color.quantity || 0) > 0);
  if (defaultColor) {
    return defaultColor;
  }
  
  // ثانياً: البحث عن أول لون متوفر في المخزون
  const firstAvailableColor = product.variants.colors.find(color => (color.quantity || 0) > 0);
  if (firstAvailableColor) {
    return firstAvailableColor;
  }
  
  // ثالثاً: إذا كانت جميع الألوان نفدت، أرجع اللون الافتراضي المحدد
  const originalDefault = product.variants.colors.find(color => color.is_default);
  if (originalDefault) {
    return originalDefault;
  }
  
  // أخيراً: أرجع الأول متاح
  const fallback = product.variants.colors[0] || null;
  if (fallback) {
  }
  return fallback;
};
...
export const getDefaultSize = (color: ProductColor): ProductSize | null => {
  ...
  const defaultSize = color.sizes.find(size => size.is_default && (size.quantity || 0) > 0);
  if (defaultSize) {
    return defaultSize;
  }
  
  // ثانياً: البحث عن أول مقاس متوفر في المخزون
  const firstAvailableSize = color.sizes.find(size => (size.quantity || 0) > 0);
  if (firstAvailableSize) {
    return firstAvailableSize;
  }
  
  // ثالثاً: إذا كانت جميع المقاسات نفدت، أرجع المقاس الافتراضي المحدد
  const originalDefault = color.sizes.find(size => size.is_default);
  if (originalDefault) {
    return originalDefault;
  }
  
  // أخيراً: أرجع الأول متاح
  const fallback = color.sizes[0] || null;
  if (fallback) {
  }
  return fallback;
};
...
export const getVariantPrice = (
  ...
  // دعنا نفحص الألوان حتى لو لم يكن use_variant_prices مفعل
  if (colorId) {
    ...
    if (color) {
      if (sizeId && color.sizes && color.sizes.length > 0) {
        ...
        if (size && size.price !== null && size.price !== undefined && size.price > 0) {
          return size.price;
        }
      }

      // استخدام سعر اللون حتى لو لم يكن use_variant_prices مفعل
      if (color.price !== null && color.price !== undefined && color.price > 0) {
        return color.price;
      }
    }
  }

  // فقط إذا لم نجد سعر صالح للمتغيرات، استخدم السعر الأساسي
...
export const getFinalPrice = (
  ...
  // تأكد من أن الكمية لا تقل عن 1 (لتجنب مشكلة السعر = 0)
  const safeQuantity = Math.max(1, quantity);

  const basePrice = getVariantPrice(product, colorId, sizeId);
  
  const wholesalePrice = getWholesalePrice(product, safeQuantity);
  ...
  if (wholesalePrice !== null) {
    ...
    isWholesale = true;
    wholesaleTier = product.wholesale_tiers.find(tier => safeQuantity >= tier.min_quantity);
  } else {
  }

  const totalPrice = finalPrice * safeQuantity;
  const originalTotalPrice = basePrice * safeQuantity;
  ...
  const result = {
    ...
    compareAtPrice: compareAtPrice ? compareAtPrice * safeQuantity : undefined,
    isWholesale,
    wholesaleTier,
