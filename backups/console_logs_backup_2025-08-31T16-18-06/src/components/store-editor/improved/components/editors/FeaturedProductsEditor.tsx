...
export const FeaturedProductsEditor: React.FC<FeaturedProductsEditorProps> = ({
  ...
  // Logging للتتبع
  console.log('🎯 FeaturedProductsEditor Debug:', {
    organizationId,
    propOrganizationId,
    settings: {
  ...
  const loadAvailableProducts = async () => {
    console.log('🔄 FeaturedProductsEditor: Loading products for org:', organizationId)

    if (!organizationId) {
      console.error('❌ FeaturedProductsEditor: No organizationId available')
    ...
    try {
      console.log('📡 FeaturedProductsEditor: Calling getProducts API...')
      const products = await getProducts(organizationId)
      console.log('✅ FeaturedProductsEditor: Received products:', products?.length || 0)
      ...
      console.error('❌ FeaturedProductsEditor: Failed to load products:', error)
    ...
    const loadProductsWithCache = async () => {
      ...
      if (cachedProducts && isCacheValid) {
        try {
          ...
          setAvailableProducts(products);
          console.log('📦 FeaturedProductsEditor: Loaded from cache:', products.length);
          return;
        } catch (error) {
          console.error('❌ FeaturedProductsEditor: Cache parsing error:', error);
        }
      }
