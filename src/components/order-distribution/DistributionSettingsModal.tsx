...
import { X, Save, CheckCircle, Loader2, Users, Package, Info } from 'lucide-react';
...
export const DistributionSettingsModal: React.FC<DistributionSettingsModalProps> = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-[95vw] md:max-w-4xl h-[95vh] md:h-auto md:max-h-[90vh] p-0">
        <div className="flex flex-col h-full">
          <DialogHeader className="px-4 md:px-6 pt-4 md:pt-6 pb-4 border-b">
            <DialogTitle className="flex items-center gap-3 text-xl md:text-2xl">
              ...
              <span>إعدادات خطة {plan.name}</span>
            </DialogTitle>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
            <TabsList className="mx-4 md:mx-6 grid grid-cols-2 md:grid-cols-4 gap-1">
              ...
              <div className="px-4 md:px-6 pb-6">
                <TabsContent value="overview" className="space-y-4 mt-4">
                  <div className="space-y-4">
                    <div>
                      <h3 className="text-lg font-semibold mb-2">{planInfo.title}</h3>
          ...
          <div className="flex justify-end gap-3 px-4 md:px-6 py-4 border-t bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
