/**
 * CustomersOptimized - Customers Page
 * ============================================================
...
interface CustomersOptimizedProps extends POSSharedLayoutControls {}
...
const CustomersOptimized: React.FC<CustomersOptimizedProps> = ({
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[CustomersOptimized] forceSync error:', err);
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white text-2xl font-bold">
                  {dialogState.selectedCustomer.name.charAt(0).toUpperCase()}
                </div>
                <div>
                  <h3 className="text-lg font-semibold">{dialogState.selectedCustomer.name}</h3>
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                <div>
                  ...
                  <p className="font-medium">{dialogState.selectedCustomer.phone || '—'}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">البريد الإلكتروني</p>
                  <p className="font-medium truncate">{dialogState.selectedCustomer.email || '—'}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-xs text-muted-foreground mb-1">العنوان</p>
                  <p className="font-medium">{dialogState.selectedCustomer.address || '—'}</p>
                </div>
                {dialogState.selectedCustomer.nif && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">الرقم الجبائي (NIF)</p>
                    <p className="font-medium font-numeric">{dialogState.selectedCustomer.nif}</p>
                  </div>
                )}
                {dialogState.selectedCustomer.rc && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">السجل التجاري (RC)</p>
                    <p className="font-medium font-numeric">{dialogState.selectedCustomer.rc}</p>
                  </div>
                )}
              </div>
