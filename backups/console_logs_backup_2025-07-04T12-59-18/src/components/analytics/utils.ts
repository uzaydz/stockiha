...
export const getDateRangePreset = (preset: string): DateRange => {
  ...
  console.log('📅 [getDateRangePreset] Calculating range for preset:', { 
    preset, 
    localNow: now,
    utcTime: new Date(utcTime),
    algerianDate,
    timezoneOffset: now.getTimezoneOffset(),
    algerianYear: algerianDate.getFullYear(),
    algerianMonth: algerianDate.getMonth(),
    algerianDay: algerianDate.getDate()
  });
  ...
  switch (preset) {
    ...
    case 'year':
      ...
      break;
    default:
      console.warn('⚠️ [getDateRangePreset] Unknown preset, using month:', preset);
      result = { 
        from: new Date(Date.UTC(algerianDate.getFullYear(), algerianDate.getMonth(), 1, 0, 0, 0)),
        to: new Date(Date.UTC(algerianDate.getFullYear(), algerianDate.getMonth() + 1, 0, 23, 59, 59, 999))
      };
  ...
  console.log('📊 [getDateRangePreset] Calculated range:', {
    preset,
    from: result.from.toISOString(),
    to: result.to.toISOString(),
    fromLocal: result.from.toLocaleString(),
    toLocal: result.to.toLocaleString(),
    fromDate: `${result.from.getUTCFullYear()}-${(result.from.getUTCMonth()+1).toString().padStart(2,'0')}-${result.from.getUTCDate().toString().padStart(2,'0')}`,
    toDate: `${result.to.getUTCFullYear()}-${(result.to.getUTCMonth()+1).toString().padStart(2,'0')}-${result.to.getUTCDate().toString().padStart(2,'0')}`
  });
