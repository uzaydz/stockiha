...
const MaxStorePage: React.FC<MaxStorePageProps> = ({ 
  ...
  const fetchStoreData = useCallback(async () => {
    try {
      ...
      console.log(`🚀 بدء تحميل متجر Max لـ: ${subdomain}`);
  ...
  useEffect(() => {
    if (existingStoreData) {
      ...
      if (convertedData) {
        ...
        setLoading(false);
        setIsVisible(true);
        console.log('✅ تم استخدام البيانات الموجودة - تجنب الاستدعاء المزدوج');
        return;
      }
    }
  ...
  useEffect(() => {
    ...
    console.log('🎨 تطبيق ثيم المتجر Max مع ThemeContext:', {
    ...
    console.log('🎨 الألوان المستخدمة:', {
      primaryColor,
      secondaryColor,
      primaryHSL: hexToHsl(primaryColor),
      secondaryHSL: hexToHsl(secondaryColor),
      primaryForeground: getContrastColor(primaryColor),
      secondaryForeground: getContrastColor(secondaryColor)
    });
    ...
    Object.entries(colorProperties).forEach(([property, value]) => {
      ...
      console.log(`🎨 تم تطبيق ${property}: ${value}`);
    });
    ...
    setTimeout(() => {
      forceRerender();
      console.log('🎨 تم تطبيق جميع ألوان الثيم بنجاح مع إعادة الرسم');
    }, 200);
    ...
    if (settings.theme_mode && settings.theme_mode !== theme) {
      ...
      console.log('🔄 تحديث ثيم ThemeContext إلى:', storeTheme);
    ...
    setTimeout(() => {
      ...
      elementsToUpdate.forEach(el => {
        ...
        console.log(`✅ تم تطبيق CSS المخصص مع إجبار الألوان على ${elementsToUpdate.length} عنصر`);
      } catch (error) {
    ...
    setTimeout(() => {
      try {
        ...
        console.log(`🎯 تم إجبار تطبيق الألوان على ${heroButtons.length} زر في الهيرو`);
      } catch (error) {
        console.error('❌ خطأ في تطبيق الألوان على أزرار الهيرو:', error);
      }
    }, 300);
    ...
    console.log('🎨 تم تطبيق جميع ألوان الثيم بنجاح');
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('⚠️ خطأ في تطبيق JavaScript المخصص:', err);
    }
  }, [storeData]);
  ...
  return (
    <MaxErrorBoundary>
      <Helmet>
        ...
        <meta name="author" content={storeData.organization_details?.name} />
