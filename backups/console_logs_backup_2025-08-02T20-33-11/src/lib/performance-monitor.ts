...
class PerformanceMonitor {
  ...
  trackApiCall(endpoint: string, duration: number) {
    ...
    if (timeSinceLastCall < 1000 && currentCount > 0) { // أقل من ثانية
      console.warn(`⚠️ [Performance] استدعاء متكرر لـ ${endpoint} بعد ${timeSinceLastCall}ms`);
    }

    // تحذير من الاستدعاءات المفرطة
    if (currentCount > 10) {
      console.warn(`🚨 [Performance] استدعاءات مفرطة لـ ${endpoint}: ${currentCount} استدعاءات`);
    }
  }
  ...
  printReport() {
    ...
    console.log(`إجمالي النقاط النهائية: ${stats.totalEndpoints}`);
    console.log(`إجمالي الاستدعاءات: ${stats.totalCalls}`);
    console.log(`متوسط وقت الاستجابة: ${stats.averageResponseTime.toFixed(2)}ms`);
    
    console.log('أكثر النقاط النهائية استدعاءً:');
    stats.mostCalledEndpoints.forEach((item, index) => {
      console.log(`${index + 1}. ${item.endpoint}: ${item.count} استدعاءات (متوسط: ${item.averageTime.toFixed(2)}ms)`);
    });
    
    console.log('الاستدعاءات الأخيرة:');
    stats.recentCalls.forEach(item => {
      console.log(`- ${item.endpoint}: منذ ${item.timeSinceLastCall}ms`);
    });
    console.groupEnd();
  }
}
