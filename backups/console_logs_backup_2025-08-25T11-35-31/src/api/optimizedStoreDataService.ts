  ...
  try {
    ...
    const [categoriesResult, productsResult, componentsResult] = await Promise.all([
      supabase
        ...
        .select('id, name, slug, image_url, icon, description, is_active, updated_at, product_count')
    ...
    if (existingData) {
      // تطبيق cache busting على صور الفئات
      const categoriesWithCacheBusting = (categoriesResult.data || []).map(category => {
      ...
      const updatedData: StoreInitializationData = {
        ...existingData,
        categories: categoriesWithCacheBusting,
