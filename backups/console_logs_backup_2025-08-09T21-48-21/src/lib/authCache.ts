...
export const getCachedAuth = async (): Promise<{ user: User | null; session: Session | null }> => {
  // إذا كان الكاش صالحاً أعده فوراً
  if (isCacheValid(authCache)) {
    return { user: authCache!.user, session: authCache!.session };
  }

  // إذا كان هناك طلب جارٍ، استخدمه لمنع التكرار
  if (pendingAuthRequest) {
    const result = await pendingAuthRequest;
    return { user: result.user, session: result.session };
  }

  // نفّذ طلباً واحداً وشارك نتيجته
