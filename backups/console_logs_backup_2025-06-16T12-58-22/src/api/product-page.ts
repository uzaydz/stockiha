    ...
    async () => {
      try {
        ...
        if (functionError) {
          ...
          console.warn('[ProductPage] Edge Function failed, trying RPC fallback:', functionError);
          
          try {
            console.log('[ProductPage] Trying RPC fallback with:', { slug, organizationId });
            const { data: rpcData, error: rpcError } = await supabase.rpc('get_complete_product_data', {
              p_slug: slug,
              p_org_id: organizationId,
            });
            console.log('[ProductPage] RPC Response:', { rpcData, rpcError });
            
            if (rpcError) {
              console.error('[ProductPage] RPC Error:', rpcError);
              throw new Error('فشل في الطريقة البديلة أيضاً');
            }
            
            if (!rpcData) {
              console.error('[ProductPage] RPC returned no data');
            ...
            console.log('[ProductPage] RPC Fallback successful');
            return fallbackData;
            
          } catch (fallbackError) {
            console.error('[ProductPage] Both Edge Function and RPC fallback failed:', fallbackError);
