...
export async function fixMissingUser(): Promise<{
  ...
  try {
    ...
    if (existingUser && !existingUser.auth_user_id) {
      console.log('User exists but auth_user_id is null, updating...');
      ...
      if (updateError) {
        console.error('خطأ في تحديث auth_user_id:', updateError);
        return {
          success: false,
    ...
    if (existingUser && existingUser.auth_user_id) {
      console.log('User already exists and is properly configured:', existingUser);
      return {
        success: true,
    ...
    if (createError) {
      ...
      if (createError.code === '23505') {
        console.log('User already exists, attempting to fix auth_user_id...');
        ...
        if (updateError) {
          console.error('خطأ في تحديث المستخدم الموجود:', updateError);
          return {
            success: false,
    ...
    console.error('خطأ في إصلاح المستخدم المفقود:', error);
    return {
      success: false,
      ...
      try {
        ...
        if (!dbUser) {
          ...
          if (createError) {
            ...
            fixed++;
            console.log(`تم إنشاء المستخدم: ${authUser.email}`);
          }
        }
      } catch (error) {
