...
export const SmartProviderWrapper: React.FC<SmartProviderWrapperProps> = ({ children }) => {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      ...
      }, 200); // زيادة التأخير لتقليل الطلبات المكررة
      
      return () => clearTimeout(timeoutId);
    }
  }, [location.pathname, pageType, config]);

  // 🎨 تطبيق الخطوط عند تغيير المسار (لحل مشكلة Lazy Loading)
  useEffect(() => {
    // إضافة تأخير صغير للتأكد من تحميل المكونات أولاً
    const fontApplyTimeout = setTimeout(() => {
      // إجبار إعادة تطبيق الخطوط على جميع العناصر
      const allElements = document.querySelectorAll('*');
      allElements.forEach(element => {
        if (element instanceof HTMLElement) {
          element.style.fontFamily = "'TajawalForced', 'Tajawal', 'Arial Unicode MS', 'Tahoma', 'Arial', sans-serif";
        }
      });
      
      // إضافة class للجسم للتأكد
      document.body.classList.add('tajawal-forced');
      
      if (process.env.NODE_ENV === 'development') {
        console.log('🎨 تم إعادة تطبيق الخطوط بعد تغيير المسار');
      }
    }, 100);
    
    return () => clearTimeout(fontApplyTimeout);
  }, [location.pathname]);
