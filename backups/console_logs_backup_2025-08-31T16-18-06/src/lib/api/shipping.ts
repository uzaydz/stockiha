...
// Cache for shipping providers to prevent repeated API calls
const shippingProvidersCache = new Map<string, { data: ActiveShippingProvider[]; timestamp: number }>();
const SHIPPING_CACHE_DURATION = 5 * 60 * 1000; // 5 minutes
...
export async function getActiveShippingProvidersForOrg(
  ...
  try {
    // Check cache first
    const cached = shippingProvidersCache.get(organizationId);
    ...
    // Update cache
    shippingProvidersCache.set(organizationId, {
      data: result,
      timestamp: Date.now()
    });
