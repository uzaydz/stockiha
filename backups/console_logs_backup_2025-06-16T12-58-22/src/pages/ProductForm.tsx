...
const ProductForm = () => {
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      ...
      console.log('الألوان الأصلية:', productColors);
      console.log('الألوان بعد التنظيف:', colorsToSubmit);
  ...
  const onInvalid = useCallback((errors: any) => {
    ...
    console.log('Current productColors state:', productColors);
    console.log('Current form colors value:', form.getValues('colors'));
  ...
  useEffect(() => {
    if (productColors.length > 0) {
      console.log('🔄 تحديث النموذج بالألوان المحملة:', productColors.length, 'ألوان');
