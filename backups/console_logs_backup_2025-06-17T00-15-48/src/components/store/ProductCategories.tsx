...
const ProductCategories = ({
  ...
  const displayedCategories = useMemo(() => {
    console.log('🔍 ProductCategories: Processing categories with settings:', {
      useRealCategories,
      optimizedCategoriesCount: optimizedCategories?.length || 0,
      selectionMethod: settings.selectionMethod,
      selectedCategories: settings.selectedCategories,
      displayCount: settings.displayCount,
      maxCategories: settings.maxCategories,
      previewCategories: settings._previewCategories?.length || 0
    });
    ...
    if (settings._previewCategories && settings._previewCategories.length > 0) {
      console.log('✅ ProductCategories: Using preview categories from editor');
    ...
    if (useRealCategories && categoriesToUse && categoriesToUse.length > 0) {
      console.log('✅ ProductCategories: Using real categories from optimized data');
      let processedCategories = mapRealCategoriesToExtended(categoriesToUse);
      
      console.log('🔧 ProductCategories: Processed categories:', processedCategories.map(c => ({ id: c.id, name: c.name })));
      ...
      if (settings.selectionMethod === 'manual' && settings.selectedCategories && settings.selectedCategories.length > 0) {
        console.log('🎯 ProductCategories: Applying manual selection filter');
        console.log('🎯 ProductCategories: Selected category IDs:', settings.selectedCategories);
        
        processedCategories = processedCategories.filter(cat => {
          const isSelected = settings.selectedCategories!.includes(cat.id);
          console.log(`🔍 Category ${cat.name} (${cat.id}): ${isSelected ? 'SELECTED' : 'FILTERED OUT'}`);
          return isSelected;
        });
        
        console.log('✅ ProductCategories: Manual selection result:', processedCategories.map(c => ({ id: c.id, name: c.name })));
      } else if (settings.selectionMethod === 'popular') {
        console.log('📈 ProductCategories: Applying popular sorting');
        processedCategories = processedCategories.sort((a, b) => b.productsCount - a.productsCount);
      } else if (settings.selectionMethod === 'newest') {
        console.log('🆕 ProductCategories: Applying newest sorting');
        // يمكن إضافة ترتيب حسب تاريخ الإنشاء لاحقاً
        processedCategories = processedCategories.reverse();
      } else {
        console.log('🔄 ProductCategories: Using automatic/default selection');
      }
      
      // تحديد العدد المطلوب عرضه
      const displayCount = settings.displayCount || settings.maxCategories || 6;
      console.log(`📊 ProductCategories: Limiting to ${displayCount} categories`);
      
      const finalCategories = processedCategories.slice(0, displayCount);
      console.log('🎉 ProductCategories: Final categories to display:', finalCategories.map(c => ({ id: c.id, name: c.name })));
      
      return finalCategories;
    }
    
    // استخدام الفئات الافتراضية كخيار أخير
    console.log('⚠️ ProductCategories: Using default categories (no real data available)');
    const displayCount = settings.displayCount || settings.maxCategories || 6;
    const defaultResult = defaultCategories.slice(0, displayCount);
    console.log('📝 ProductCategories: Default categories result:', defaultResult.map(c => ({ id: c.id, name: c.name })));
