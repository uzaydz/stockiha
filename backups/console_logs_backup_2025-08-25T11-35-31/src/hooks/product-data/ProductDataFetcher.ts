...
export async function fetchUnifiedProductData(
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log(`🚀 [ProductDataFetcher] جلب بيانات المنتج:`, { productId, organizationId, dataScope, forceRefresh });
  }
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (productResponse.product?.variants?.colors) {
        console.log('🔍 [ProductDataFetcher] معلومات صور الألوان المستلمة:', productResponse.product.variants.colors.map(color => ({
          id: color.id,
          name: color.name,
      ...
      console.log('✅ [ProductDataFetcher] تم جلب بيانات المنتج بنجاح');
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error('❌ [ProductDataFetcher] خطأ في جلب بيانات المنتج:', error);
    }
    throw error;
  }
}
...
export async function fetchWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= retryAttempts; attempt++) {
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.warn(`⚠️ [ProductDataFetcher] محاولة ${attempt} فشلت، إعادة المحاولة...`);
      }
    }
  }
...
export async function fetchEnhancedProductData(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error('❌ [ProductDataFetcher] فشل في جلب البيانات المحسنة:', error);
    }
    throw error;
  }
}
