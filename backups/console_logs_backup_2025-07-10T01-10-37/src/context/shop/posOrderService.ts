...
export async function createPOSOrder(orderData: POSOrderData): Promise<POSOrderResult> {
  ...
  try {
    ...
    console.log('🚀 بدء إنشاء طلب POS بالدالة المحسنة مع حماية UUID...', {
      itemsCount: orderItems.length,
      total: orderData.total,
      organizationId: orderData.organizationId
    });

    console.log('📋 البيانات المرسلة:', JSON.stringify(cleanedParams, null, 2));

    // 🔍 تتبع مصدر الرقم 666 - نقطة 1: قبل الإرسال
    console.log('🔍 [DEBUG-1] تتبع customer_order_number قبل الإرسال:', {
      organizationId: orderData.organizationId,
      employeeId: orderData.employeeId,
      totalAmount: orderData.total,
      timestamp: new Date().toISOString(),
      location: 'posOrderService.createPOSOrder - before RPC call'
    });
    ...
    if (error) {
      console.error('❌ خطأ في إنشاء طلب POS:', error);
      console.error('📋 المعاملات المرسلة:', cleanedParams);
      console.error('📋 البيانات الأصلية:', orderData);
      
      // محاولة fallback للدالة القديمة في حالة عدم وجود الدالة الجديدة
      if (error.code === '42883' || error.message?.includes('function') || error.message?.includes('does not exist')) {
        console.log('🔄 محاولة استخدام الدالة القديمة كـ fallback...');
        ...
        console.log('✅ تم إنشاء طلب POS بنجاح باستخدام fallback:', {
          ...
          databaseProcessingTime: `${fallbackOrderResult.processing_time_ms}ms`,
          method: 'fallback'
        });
    ...
    console.log('🔍 [DEBUG-2] تتبع customer_order_number بعد استقبال النتيجة من قاعدة البيانات:', {
      rawResult: result,
      orderResultCustomerOrderNumber: orderResult?.customer_order_number,
      orderResultSuccess: orderResult?.success,
      orderResultOrderId: orderResult?.order_id,
      orderResultSlug: orderResult?.slug,
      timestamp: new Date().toISOString(),
      location: 'posOrderService.createPOSOrder - after RPC response'
    });

    if (!orderResult?.success) {
      console.error('❌ فشل في إنشاء طلب POS:', result);
    ...
    console.log('✅ تم إنشاء طلب POS بنجاح مع حماية UUID:', {
    ...
    console.log('🔍 [DEBUG-3] تتبع customer_order_number قبل إرجاع النتيجة النهائية:', {
      orderResultCustomerOrderNumber: orderResult.customer_order_number,
      finalReturnValue: orderResult.customer_order_number,
      orderResultOrderId: orderResult.order_id,
      orderResultSlug: orderResult.slug,
      timestamp: new Date().toISOString(),
      location: 'posOrderService.createPOSOrder - before return'
    });
    ...
    console.error('❌ خطأ عام في إنشاء طلب POS:', error);
...
async function updateInventoryForOrder(items: OrderItem[], orderId?: string, organizationId?: string) {
  ...
  console.log('📦 بدء تحديث المخزون للطلب...', { 
    itemsCount: items.length, 
    orderId,
    method: 'FIFO_OPTIMIZED_CORRECTED'
  });

  for (const item of items) {
    if (!item.productId) {
      console.warn('⚠️ تخطي عنصر بدون معرف منتج:', item);
      continue;
    }
    ...
    try {
      ...
      if (fifoError || !fifoResult?.success) {
        console.warn('⚠️ فشل FIFO، استخدام الطريقة البسيطة:', {
          productId: item.productId,
          error: fifoError?.message || fifoResult?.error
        });
      ...
      console.error('❌ خطأ في تحديث مخزون المنتج:', {
        productId: item.productId,
  ...
  const endTime = performance.now();
  console.log(`✅ انتهى تحديث المخزون في ${(endTime - startTime).toFixed(2)}ms`);
}
