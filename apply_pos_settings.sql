...
-- تشغيل هذا الملف لإضافة نظام إعدادات نقطة البيع إلى قاعدة البيانات

-- تحقق من وجود جدول organizations أولاً
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'organizations') THEN
        RAISE EXCEPTION 'جدول organizations غير موجود. يرجى إنشاؤه أولاً';
    END IF;
END
$$;

-- تحقق من وجود جدول users أولاً
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
        RAISE EXCEPTION 'جدول users غير موجود. يرجى إنشاؤه أولاً';
    END IF;
END
$$;

-- إنشاء Extension UUID إذا لم يكن موجوداً
...
DO $$
BEGIN
    RAISE NOTICE 'تم تطبيق إعدادات نقطة البيع بنجاح!';
    RAISE NOTICE 'يمكنك الآن استخدام نظام إعدادات نقطة البيع في التطبيق.';
END
$$;