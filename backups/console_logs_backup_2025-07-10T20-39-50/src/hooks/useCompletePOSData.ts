...
export const useCompletePOSData = (options: POSDataOptions = {}) => {
  ...
  const refreshData = async (): Promise<void> => {
    console.log('🔄 [refreshData] بدء عملية تحديث البيانات...');
    try {
      const result = await refetch();
      console.log('✅ [refreshData] تم تحديث البيانات بنجاح:', {
      ...
      console.error('❌ [refreshData] خطأ في تحديث البيانات:', error);
      throw error;
    }
  };
  ...
  const updateProductStockInCache = (
    ...
    console.log('🔄 [updateProductStockInCache] تحديث المخزون:', {
      productId,
      colorId,
      sizeId,
      quantityChange,
    ...
    console.log('✅ [updateProductStockInCache] تم تحديث الـ cache وإجبار re-render');
  };
  ...
  const getUpdatedProduct = useCallback((productId: string) => {
    ...
    if (!cachedData?.success || !cachedData.data) {
      // إذا لم يكن هناك cache، ابحث في المنتجات الأصلية
      return posData?.products.find(p => p.id === productId);
    }
    return cachedData.data.products.find(p => p.id === productId);
  ...
  const updatedProducts = useMemo(() => {
    ...
    // إذا كان لدينا بيانات محلية مُحدثة وبيانات أصلية
    if (cachedData?.success && cachedData.data && posData?.products) {
      // استخدام البيانات المُحدثة من cache إذا كانت أحدث
      const cacheTimestamp = queryClient.getQueryState(['complete-pos-data', currentOrganization?.id])?.dataUpdatedAt || 0;
      const originalTimestamp = Date.now() - 30000; // افتراض أن البيانات الأصلية أقل من 30 ثانية
      
      // إذا كان الـ cache أحدث، استخدمه
      if (cacheTimestamp > originalTimestamp) {
        console.log('🔄 [updatedProducts] استخدام البيانات المُحدثة من cache');
        return cachedData.data.products;
      }
    }
    
    // إذا لم يكن هناك cache أو البيانات الأصلية أحدث، استخدم البيانات الأصلية
    console.log('🔄 [updatedProducts] استخدام البيانات الأصلية من API');
