...
const GameOrderReceiptPrint: React.FC<GameOrderReceiptPrintProps> = ({
  ...
  // دالة لضمان ظهور QR codes عند الطباعة
  const ensureQRCodesVisible = () => {
    // إضافة أنماط إضافية لضمان ظهور QR codes
  ...
  React.useEffect(() => {
    // تحقق من صحة الروابط
    console.log('🔗 روابط QR Codes:');
    console.log('Customer Tracking:', `${storeUrl}/game-tracking/${order.tracking_number}`);
    console.log('Start Download:', `${storeUrl}/game-download-start/${order.id}`);
    console.log('Complete Order:', `${storeUrl}/game-complete/${order.id}`);
      ...
      >
        <button 
          onClick={() => {
            ...
            console.log('🖼️ QR Server API URLs:');
            console.log('Customer QR:', `https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(storeUrl + '/game-tracking/' + order.tracking_number)}`);
            console.log('Start QR:', `https://api.qrserver.com/v1/create-qr-code/?size=50x50&data=${encodeURIComponent(storeUrl + '/game-download-start/' + order.id)}`);
            console.log('Complete QR:', `https://api.qrserver.com/v1/create-qr-code/?size=50x50&data=${encodeURIComponent(storeUrl + '/game-complete/' + order.id)}`);
