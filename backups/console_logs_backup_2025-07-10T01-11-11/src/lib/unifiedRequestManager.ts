...
const createDirectRestRequest = async (key: string): Promise<any> => {
  ...
  try {
    ...
    // معالجة خاصة للإعدادات - إرجاع العنصر الأول من الـ array
    if (key.includes('settings') && Array.isArray(data) && data.length > 0) {
      return data[0]; // إرجاع الإعدادات الأولى إذا كان الطلب للإعدادات
    }
    ...
    // معالجة خاصة للاشتراكات - إرجاع العنصر الأول من الـ array
    if (key.includes('subscriptions') && Array.isArray(data) && data.length > 0) {
      return data[0]; // إرجاع الاشتراك الأول إذا كان الطلب للاشتراكات
    }
