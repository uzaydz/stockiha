...
import { updateProductStockQuantity, verifyProductColorsAndSizes } from './productVariants';
...
export type Product = Database['public']['Tables']['products']['Row'] & {
  category?: { id: string; name: string; slug: string; image_url?: string } | string;
    ...
    try {
      ...
      if (searchQuery.trim()) {
        ...
        if (normalizedSearchQuery.length >= 2) {
          ...
          // البحث في الباركود
          searchWords.forEach(word => {
            searchConditions.push(`barcode.ilike.%${word}%`);
          });
  ...
  if (colors && colors.length > 0) {
    ...
    if (validColors.length > 0) {
      ...
      if (insertColorsError) {
        ...
        // حفظ المقاسات لكل لون بشكل متزامن
        const sizeInsertPromises: PromiseLike<any>[] = [];
        ...
        for (let i = 0; i < validColors.length; i++) {
          ...
          if (color.sizes && color.sizes.length > 0 && insertedColor) {
            ...
            // إضافة promise لحفظ مقاسات هذا اللون
            const insertPromise = supabase
              .from('product_sizes')
              .insert(sizesToInsert)
              .then(({ error }) => {
                if (error) {
                  toast.error(`تحذير: فشل حفظ مقاسات اللون ${color.name}: ${error.message}`);
                }
              });
            
            sizeInsertPromises.push(insertPromise);
          }
        }
        
        // انتظار حفظ جميع المقاسات
        if (sizeInsertPromises.length > 0) {
          await Promise.all(sizeInsertPromises);
        }
        
        // تحديث stock_quantity للمنتج تلقائياً بعد إضافة الألوان والمقاسات
        await updateProductStockQuantity(createdProduct.id);
        
        // التحقق من صحة حفظ البيانات
        try {
          const verification = await verifyProductColorsAndSizes(createdProduct.id);
          if (!verification.success) {
            console.warn('تحذير:', verification.message);
          }
        } catch (verifyError) {
          console.error('خطأ في التحقق من البيانات:', verifyError);
        }
      }
    }
  }
...
// دالة مسح الcache المتعلق بمؤسسة معينة - محدثة
export const clearProductsCache = (organizationId?: string) => {
  if (!organizationId) return;

  // مسح cache results
  const keysToDelete: string[] = [];
  for (const [key] of resultsCache) {
  ...
  // مسح الطلبات المعلقة المتعلقة بنفس المؤسسة
  const ongoingKeysToDelete: string[] = [];
  for (const [key] of ongoingRequests) {
    if (key.includes(organizationId)) {
      ongoingKeysToDelete.push(key);
    }
  }
  
  ongoingKeysToDelete.forEach(key => {
    ongoingRequests.delete(key);
  });
  
};

// إضافة دالة مسح الcache للنافذة العالمية
