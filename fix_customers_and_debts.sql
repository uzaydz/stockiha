-- ملف إصلاح مشكلة العملاء والديون
-- هذا الملف يحل مشكلة العملاء الذين تم إضافتهم كمستخدمين بدلاً من عملاء

-- 1. إنشاء العملاء المفقودين من جدول المستخدمين
-- هذا سيقوم بإنشاء سجلات عملاء لكل المستخدمين الذين لديهم طلبات ولكن لا يوجد لهم سجل في جدول العملاء
INSERT INTO customers (id, name, email, phone, created_at, updated_at, organization_id)
SELECT 
    u.id,
    u.name,
    u.email,
    '', -- لا يوجد رقم هاتف لنضع قيمة فارغة
    CURRENT_TIMESTAMP,
    CURRENT_TIMESTAMP,
    u.organization_id
FROM users u
...
WHERE c.id IS NULL
GROUP BY u.id, u.name, u.email, u.organization_id;

-- 2. تحديث جميع الطلبات ذات المبالغ المتبقية لتكون معتبرة كديون جزئية
UPDATE orders 
...
-- 4. التأكد من أن قيمة remaining_amount محسوبة بشكل صحيح
UPDATE orders 
SET remaining_amount = total - COALESCE(amount_paid, 0) 
WHERE total > COALESCE(amount_paid, 0);

-- 5. تحديث قيمة amount_paid لتكون 0 بدلاً من NULL للطلبات ذات المبالغ المتبقية
UPDATE orders 
SET amount_paid = 0 
WHERE remaining_amount > 0 AND amount_paid IS NULL;

-- 6. تعديل وظيفة get_partial_payments_summary لتشمل الديون من المستخدمين والعملاء
...
-- 7. تعديل وظيفة get_debts_by_customer لتشمل الديون من المستخدمين والعملاء
...
-- 8. تعديل وظيفة get_customer_debts لتشمل الديون من المستخدمين والعملاء
