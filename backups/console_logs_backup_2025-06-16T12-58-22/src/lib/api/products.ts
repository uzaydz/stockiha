...
export const updateProduct = async (id: string, updates: UpdateProduct): Promise<Product> => {
  ...
  if (colors && colors.length > 0) {
    ...
    if (deleteColorsError) {
      console.error('خطأ في حذف الألوان:', deleteColorsError);
    }
    ...
    if (validColors.length > 0) {
      ...
      if (insertColorsError) {
        ...
        console.log('✅ تم حفظ الألوان بنجاح:', insertedColors.length);
        ...
        for (let i = 0; i < validColors.length; i++) {
          ...
          if (color.sizes && color.sizes.length > 0 && insertedColor) {
            console.log(`💾 حفظ ${color.sizes.length} مقاسات للون ${color.name}`);
            ...
            if (insertSizesError) {
              console.error(`❌ خطأ في حفظ مقاسات اللون ${color.name}:`, insertSizesError);
              toast.error(`تحذير: فشل حفظ مقاسات اللون ${color.name}`);
            } else {
              console.log(`✅ تم حفظ مقاسات اللون ${color.name} بنجاح`);
            }
          }
        }
