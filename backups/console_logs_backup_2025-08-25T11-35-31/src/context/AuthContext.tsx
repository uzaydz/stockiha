...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // ref لمنع إرسال الأحداث المكررة
  const eventSentRef = useRef<Set<string>>(new Set());

  // دالة مساعدة لإرسال الأحداث بدون تكرار
  const sendOrganizationReadyEvent = useCallback((org: any, source: string) => {
    const eventKey = `${org?.id || 'null'}-${source}`;
    if (eventSentRef.current.has(eventKey)) {
      if (process.env.NODE_ENV === 'development') {
        console.log(`⏭️ [AuthContext] تجاهل إرسال حدث مكرر: ${eventKey}`);
      }
      return; // تم إرسال هذا الحدث من قبل
    }

    if (process.env.NODE_ENV === 'development') {
      console.log(`📢 [AuthContext] إرسال حدث authOrganizationReady من ${source}:`, org?.name);
    }

    eventSentRef.current.add(eventKey);
    setTimeout(() => {
  ...
  // مراقبة تغيير المؤسسة وتحديث authReady
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
    ...
    // تحديث authReady عندما تكون البيانات جاهزة
    if (userProfile && organization && !profileLoading && !isLoadingProfile) {
      if (!dataLoadingComplete && !authReady) {
        if (process.env.NODE_ENV === 'development') {
        ...
        // إرسال حدث لإعلام TenantContext بتحديث المؤسسة
        sendOrganizationReadyEvent(organization, 'data-loading-complete');
      }
    }
  }, [organization, userProfile, isLoadingProfile, isLoadingOrganization, profileLoading, orgLoading, dataLoadingComplete, authReady, sendOrganizationReadyEvent]);
  ...
  useEffect(() => {
    if (userProfile && !profileLoading && isLoadingProfile) {
      ...
      // إذا كانت المؤسسة جاهزة بالفعل، أرسل الحدث
      if (organization && !orgLoading && !isLoadingOrganization) {
        sendOrganizationReadyEvent(organization, 'profile-loaded');
      }
    }
  }, [userProfile, profileLoading, isLoadingProfile, organization, orgLoading, isLoadingOrganization, sendOrganizationReadyEvent]);
  ...
  // الاستماع للأحداث من useUserOrganization
  useEffect(() => {
    const handleOrganizationLoaded = (event: CustomEvent) => {
      ...
      // إذا كانت جميع البيانات جاهزة، أرسل الحدث
      if (userProfile && loadedOrg && !profileLoading && !isLoadingProfile) {
        sendOrganizationReadyEvent(loadedOrg, 'organization-loaded');
      }
    };
  ...
  }, [userProfile, profileLoading, isLoadingProfile, sendOrganizationReadyEvent]);
  ...
  /**
   * مزامنة الجلسة مع hook - محسن لمنع الحلقات اللانهائية
   */
  useEffect(() => {
    // فقط إذا كانت الجلسة مختلفة حقاً وليست null
    if (hookSession && hookSession !== session && 
        hookSession.access_token !== session?.access_token) {
  ...
  useEffect(() => {
    ...
    }, 30000); // زيادة من 10 ثوانٍ إلى 30 ثانية
