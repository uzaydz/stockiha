...
// Get all product categories for current organization - محسن مع UnifiedRequestManager
export const getProductCategories = async (organizationId?: string): Promise<string[]> => {
  try {
    if (import.meta.env.DEV) {
      console.log('🔍 [Inventory] جلب فئات المنتجات باستخدام UnifiedRequestManager...');
    }
    
    // الحصول على معرف المؤسسة من context إذا لم يتم تمريره
    let orgId = organizationId;
    if (!orgId) {
      // محاولة الحصول على المعرف من المستخدم الحالي أو السياق
    ...
    if (!orgId) {
      if (import.meta.env.DEV) {
        console.warn('⚠️ [Inventory] لا يوجد معرف مؤسسة - استخدام الطريقة التقليدية');
      }
      ...
      if (import.meta.env.DEV) {
        console.log('✅ [Inventory] تم جلب', categories.length, 'فئة باستخدام الطريقة التقليدية');
      }
      
      return categories;
    }
    
    // استخدام UnifiedRequestManager
    const categoriesData = await UnifiedRequestManager.getProductCategories(orgId);
    ...
    const categories = categoriesData.map(cat => cat.name).filter(Boolean);
    
    if (import.meta.env.DEV) {
      console.log('✅ [Inventory] تم جلب', categories.length, 'فئة باستخدام UnifiedRequestManager');
    }
...
async function checkServerConnection(): Promise<boolean> {
  try {
    ...
    // 2. محاولة الاتصال بـ Supabase عبر عملية بسيطة وآمنة
    try {
      // استخدام auth.getUser() كطريقة آمنة للتحقق من الاتصال
      const { data, error } = await supabase.auth.getUser();
      
      // إذا لم يكن هناك خطأ في الاتصال، فالاتصال متاح
      ...
      // إذا كان الخطأ متعلق بالشبكة، فالاتصال غير متاح
      if (supabaseError.message?.includes('network') || 
          supabaseError.message?.includes('fetch')) {
        return false;
      }
      
      // للأخطاء الأخرى، نعتبر الاتصال متاح
      return true;
    }
  } catch (error) {
    return false;
...
// استدعاء دالة التهيئة تلقائيًا عند استيراد الوحدة - معطل مؤقتاً
// initInventorySystem();
