...
class RequestDeduplicator {
  ...
  async execute<T>(
    ...
    if (!forceRefresh && useCache) {
      ...
      if (cached) {
        if (process.env.NODE_ENV === 'development') {
          console.log(`🎯 [RequestDeduplicator] استخدام الكاش للطلب: ${key}`);
        }
        return cached;
      }
    }
    ...
    if (pending) {
      if (process.env.NODE_ENV === 'development') {
        console.log(`⏳ [RequestDeduplicator] انتظار طلب معلق: ${key}`);
      }
    ...
    // إنشاء طلب جديد
    if (process.env.NODE_ENV === 'development') {
      console.log(`🚀 [RequestDeduplicator] تنفيذ طلب جديد: ${key}`);
    }
    ...
    promise.then(() => {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`⚡ [RequestDeduplicator] تم الانتهاء من الطلب ${key} في ${executionTime.toFixed(2)}ms`);
      }
    });
  ...
  private async executeRequest<T>(
    ...
    try {
      ...
      console.error(`❌ [RequestDeduplicator] فشل الطلب: ${key}`, error);
      throw error;
    } finally {
      // إزالة الطلب المعلق
  ...
  clearCache(key: string): void {
    this.cache.delete(key);
    console.log(`🧹 [RequestDeduplicator] تم مسح الكاش: ${key}`);
  }
  ...
  clearAllCache(): void {
    this.cache.clear();
    console.log(`🧹 [RequestDeduplicator] تم مسح جميع الكاش`);
  }
