...
const ProductPurchaseOptimized = () => {
  ...
  // استخدام Hook المحسن لجلب البيانات
  const {
  ...
  const isLoading = isStateLoading || isFetchingData || isOrganizationLoading;
  const error = stateError || fetchError;

  // تسجيل مؤقت للتصحيح
  React.useEffect(() => {
  }, [slug, currentOrganization?.id, isOrganizationLoading, isFetchingData, fetchError, fetchedProduct]);

  // تحديث البيانات عند جلبها
  React.useEffect(() => {
    if (fetchedProduct && !productData) {
      setProduct(fetchedProduct);
      
      // تعيين اللون الافتراضي
      if (fetchedProduct.product_colors?.length > 0) {
        setSelectedColor(fetchedProduct.product_colors[0]);
      }
      
      // تعيين الحقول المخصصة إذا وجدت
      if (fetchedProduct.form_template_id) {
        // يمكن جلب الحقول المخصصة هنا
      }
    }
  }, [fetchedProduct, productData, setProduct, setSelectedColor]);

  // تحديث الأحجام عند تغيير اللون
  React.useEffect(() => {
    const colorSizes = (selectedColor as any)?.product_sizes;
    if (colorSizes) {
      setSizes(colorSizes);
      if (colorSizes.length > 0 && !selectedSize) {
  ...
  }, [selectedColor, selectedSize, setSizes, setSelectedSize]);
    ...
    <ProductPurchaseDataProvider productId={effectiveProduct?.id || product?.id}>
      ...
      <div className="container mx-auto py-4 px-4 md:px-6 relative z-20">
        {isLoading ? (
          ...
          <ErrorState error={typeof error === 'string' ? error : error?.message || 'حدث خطأ'} onBack={() => navigate(-1)} />
                ...
                <div className="md:sticky md:top-24">
                  {(() => {
                    ...
                    const additionalImagesList = [
                      ...
                      ...(product.product_colors?.filter((color: any) => 
                        color.image_url && 
...
export default ProductPurchaseOptimized;
