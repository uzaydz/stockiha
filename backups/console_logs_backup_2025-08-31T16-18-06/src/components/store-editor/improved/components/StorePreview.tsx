...
import FeaturedProductsPreview from '@/components/store-preview/FeaturedProductsPreview'
  ...
  const renderedComponent = useMemo(() => {
    ...
    switch (component.type) {
      ...
      case 'featured_products':
        // استخدام FeaturedProductsPreview للمعاينة بدلاً من FeaturedProducts
        content = (
          <FeaturedProductsPreview
            title={component.settings.title || 'منتجاتنا المميزة'}
            description={component.settings.description || 'اكتشف أفضل ما لدينا من منتجات مختارة بعناية'}
            selectionMethod={component.settings.selectionMethod || 'automatic'}
            selectionCriteria={component.settings.selectionCriteria || 'featured'}
            selectedProducts={component.settings.selectedProducts || []}
            displayCount={component.settings.displayCount || 4}
            displayType={component.settings.displayType || 'grid'}
            showPrices={component.settings.showPrices !== false}
            showRatings={component.settings.showRatings !== false}
