...
const POSAdvanced = () => {
  ...
  const {
    ...
    page: 1,
    limit: 100, // تحميل المزيد من المنتجات للبحث المحلي
    search: '', // بدون بحث لجلب جميع المنتجات
    categoryId: categoryFilter
  });

  // البحث المحلي السريع في المنتجات المحملة
            ...
            <Suspense fallback={<Skeleton className="h-full w-full rounded-lg" />}>
              <div className="flex-1 w-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-xl border border-border/50 shadow-lg transition-all duration-300 hover:shadow-xl overflow-hidden">
                ...
                {allProducts && allProducts.length > 0 && (
                  <div className="px-6 py-2 bg-primary/5 border-b text-sm text-muted-foreground">
                    📦 {allProducts.length} منتج محمل محلياً | 
                    🔍 {filteredProducts.length} نتيجة بحث | 
                    📄 صفحة {currentPage} من {Math.ceil(filteredProducts.length / pageSize)}
