  ...
  const fetchPromise = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        console.log('🔍 getProductsPaginated raw data sample:', {
          id: sampleProduct.id,
          name: sampleProduct.name,
          slug: sampleProduct.slug,
          hasSlug: !!sampleProduct.slug,
          allKeys: Object.keys(sampleProduct),
          keyCount: Object.keys(sampleProduct).length,
          slugExists: 'slug' in sampleProduct,
          slugValue: sampleProduct.slug
        });
      }
...
export const updateProduct = async (id: string, updates: UpdateProduct): Promise<Product> => {
  ...
  try {
    ...
    if (mainProductUpdates.shipping_method_type !== undefined) {
      console.log('Updating shipping_method_type to:', mainProductUpdates.shipping_method_type);
      ...
      if (shippingUpdateError) {
        console.error('Failed to update shipping settings:', shippingUpdateError);
        toast.error(`تم تحديث المنتج ولكن فشل تحديث إعدادات الشحن: ${shippingUpdateError.message}`);
      } else {
        console.log('Successfully updated shipping settings');
      }
    }
