...
const ProductPurchasePageV3: React.FC = React.memo(() => {
    ...
    useEffect(() => {
      ...
      const checkForPreloadedData = () => {
        ...
        if (cached && cached.success && cached.data) {
          console.log('🚀 [ProductPurchasePageV3] تم العثور على بيانات محملة مسبقاً:', {
      ...
      if (!checkForPreloadedData()) {
        ...
        if (isProductPagePreloading(actualProductId, organizationId)) {
          console.log('⏳ [ProductPurchasePageV3] preload قيد التشغيل، انتظار...');
          ...
          const checkPreloadComplete = () => {
            ...
            if (attempts > maxAttempts) {
              console.log('⚠️ [ProductPurchasePageV3] انتهت مهلة انتظار preload، المتابعة بالطريقة العادية');
              return;
            }
            ...
            if (isProductPagePreloading(actualProductId, organizationId)) {
              ...
              console.log('⚠️ [ProductPurchasePageV3] انتهى preload، لكن لم توجد بيانات');
            }
          };
  ...
  const effectiveData = useMemo(() => {
    if (isPreloaded && preloadedData) {
      ...
      if (preloadedData.product) {
        ...
        if (!hasBasicData) {
          console.warn('⚠️ [ProductPurchasePageV3] البيانات المحملة مسبقاً لا تحتوي على البيانات الأساسية:', {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('📊 [ProductPurchasePageV3] معلومات البيانات المحملة مسبقاً:', {
            hasDescription: !!product.description,
            hasImages: !!product.images,
            hasVariants: !!product.variants && product.variants.length > 0,
  ...
  useEffect(() => {
    if (isPreloaded && preloadedData && preloadedData.product && !isDataFullyLoaded) {
      ...
      if (hasBasicData) {
        console.log('✅ [ProductPurchasePageV3] تم تحديد اكتمال البيانات المحملة مسبقاً (البيانات الأساسية متوفرة):', {
          hasProduct: !!product,
          hasDescription: !!product.description,
          hasImages: !!product.images,
          hasVariants: !!product.variants && product.variants.length > 0,
    ...
    if (!isPreloaded && effectiveData.product && !effectiveData.isLoading &&
      ...
      console.log('✅ [ProductPurchasePageV3] تم تحديد اكتمال البيانات من effectiveData:', {
        hasProduct: !!effectiveData.product,
        hasDescription: !!effectiveData.product.description,
        hasImages: !!effectiveData.product.images,
        hasVariants: !!effectiveData.product.variants && effectiveData.product.variants.length > 0,
        timestamp: new Date().toISOString()
      });
      setIsDataFullyLoaded(true);
    }
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      console.log('🔍 [ProductPurchasePageV3] effectiveData debug:', {
        productId: actualProductId,
        organizationId,
        isLoading: effectiveData.isLoading,
        hasProduct: !!effectiveData.product,
        hasData: !!effectiveData.data,
        unifiedProductId: effectiveData.product?.id,
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (!effectiveData.product && !effectiveData.isLoading && !isDataFullyLoaded) {
        console.warn('⚠️ [ProductPurchasePageV3] timeout للبيانات - لم يتم تحميل المنتج بعد 8 ثواني');
        setShowError(true);
      }
    }, 8000);
  ...
  useEffect(() => {
    ...
    if (isPreloaded && preloadedData && preloadedData.product) {
      ...
      if (hasBasicData && !isDataFullyLoaded) {
        console.log('✅ [ProductPurchasePageV3] تم اكتمال تحميل البيانات المسبقة (البيانات الأساسية متوفرة):', {
          hasProduct: !!product,
          hasDescription: !!product.description,
          hasImages: !!product.images,
          hasVariants: !!product.variants && product.variants.length > 0,
    ...
    if (allDataReady && !isDataFullyLoaded) {
      console.log('✅ [ProductPurchasePageV3] تم اكتمال تحميل جميع البيانات العادية:', {
        hasProduct: !!effectiveData.product,
        hasDescription: !!effectiveData.product?.description,
        hasImages: !!effectiveData.product?.images,
        hasVariants: !!effectiveData.product?.variants && effectiveData.product.variants.length > 0,
        variantsCount: effectiveData.product?.variants?.length || 0,
        timestamp: new Date().toISOString()
      });
      setIsDataFullyLoaded(true);
  ...
  const stableParams = useMemo(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('🔍 [ProductPurchasePageV3] stableParams debug:', {
        hasRequiredData,
        hasUnifiedData,
        ...
        actualProductId,
        isOrganizationLoading,
        unifiedDataProduct: !!effectiveData.product,
        enabled: hasRequiredData && !hasUnifiedData && !isCurrentlyPreloading
      });
    }
  ...
  useEffect(() => {
    if (effectiveData.error && !showError && retryCount < maxRetries) {
      console.warn('⚠️ [ProductPurchasePageV3] خطأ في البيانات، محاولة إعادة التحميل:', {
        error: effectiveData.error,
        retryCount,
        maxRetries,
        timestamp: new Date().toISOString()
      ...
      const retryTimer = setTimeout(() => {
        if (effectiveData.refetch) {
          effectiveData.refetch().catch(error => {
            console.error('❌ [ProductPurchasePageV3] فشل في إعادة التحميل:', error);
          });
        }
  ...
  useEffect(() => {
    if (effectiveProduct && !effectiveProduct.description) {
      console.warn('⚠️ [ProductPurchasePageV3] المنتج موجود لكن الوصف مفقود:', {
        productId: effectiveProduct.id,
        hasDescription: !!effectiveProduct.description,
        hasImages: !!effectiveProduct.images,
        timestamp: new Date().toISOString()
      });
      ...
      if (effectiveData.refetch && !effectiveData.isLoading) {
        setTimeout(() => {
          console.log('🔄 [ProductPurchasePageV3] محاولة إعادة تحميل البيانات للحصول على الوصف');
          effectiveData.refetch().catch(error => {
            console.error('❌ [ProductPurchasePageV3] فشل في إعادة تحميل البيانات:', error);
          });
        }, 1000);
      }
    }
  ...
  useEffect(() => {
    if (effectiveData.isLoading) {
      console.log('📡 [ProductPurchasePageV3] جاري تحميل البيانات...');
    } else if (effectiveData.product && !effectiveData.isLoading) {
      console.log('✅ [ProductPurchasePageV3] تم تحميل البيانات:', {
        hasProduct: !!effectiveData.product,
        hasDescription: !!effectiveData.product.description,
        hasImages: !!effectiveData.product.images,
        timestamp: new Date().toISOString()
  ...
  useEffect(() => {
    if (effectiveProduct && !state.product) {
      console.log('🔄 [ProductPurchasePageV3] تم استخدام effectiveProduct:', {
        effectiveProductId: effectiveProduct.id,
        effectiveProductName: effectiveProduct.name,
        stateProductId: state.product?.id,
        timestamp: new Date().toISOString()
      });
    }
  ...
  useEffect(() => {
    ...
    if (effectiveData.product && !state.product && !state.loading) {
      console.log('🔄 [ProductPurchasePageV3] إعادة مزامنة البيانات من effectiveData:', {
        unifiedProduct: effectiveData.product?.id,
        stateProduct: state.product?.id,
  ...
  useEffect(() => {
    console.log('🔍 [ProductPurchasePageV3] Debug Info:', {
      actualProductId,
      organizationId,
      isOrganizationLoading,
      isDataFullyLoaded,
    ...
    hasData: !!effectiveData.data,
    hasProduct: !!effectiveData.product,
    productId: effectiveData.product?.id,
    error: effectiveData.error
      },
      state: {
        loading: state.loading,
  ...
  useEffect(() => {
    if (effectiveProduct && !productTracking.isReady && !productTracking.isLoading) {
      ...
      if (!success && process.env.NODE_ENV === 'development') {
        console.warn('⚠️ [ProductPurchasePageV3] فشل في استخراج إعدادات التتبع من المنتج');
      }
    }
  ...
  const handleFormChange = useCallback((data: Record<string, any>) => {
    setPageState(prev => ({ ...prev, submittedFormData: data }));
  ...
  if (shouldShowLoading) {
    console.log('⏳ [ProductPurchasePageV3] عرض شاشة التحميل:', {
      effectiveLoading,
      isDataFullyLoaded,
      unifiedDataLoading: effectiveData.isLoading,
      hasEffectiveProduct: !!effectiveProduct,
      isPreloaded,
      preloadedDataAvailable: !!preloadedData,
      timestamp: new Date().toISOString()
    });
  ...
  if (!isPreloaded && (!effectiveProduct.description || !effectiveProduct.images)) {
    console.warn('⚠️ [ProductPurchasePageV3] البيانات الأساسية غير متوفرة:', {
      hasDescription: !!effectiveProduct.description,
      hasImages: !!effectiveProduct.images,
      hasVariants: !!effectiveProduct.variants && effectiveProduct.variants.length > 0,
      productId: effectiveProduct.id,
      isPreloaded,
      timestamp: new Date().toISOString()
    });
