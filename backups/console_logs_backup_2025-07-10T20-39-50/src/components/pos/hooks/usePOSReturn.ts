...
export const usePOSReturn = ({
  ...
  if (!initRef.current && process.env.NODE_ENV === 'development') {
    console.log('🏗️ [usePOSReturn] إنشاء hook instance:', {
  ...
  const [returnItems, setReturnItems] = useState<CartItem[]>(() => {
    ...
    try {
      ...
      console.log('🔄 [usePOSReturn] استرجاع returnItems من localStorage:', {
        itemsCount: parsed.length,
        items: parsed.map((item: any) => ({ name: item.product?.name, quantity: item.quantity }))
      });
      return parsed;
    } catch (error) {
      console.error('❌ [usePOSReturn] خطأ في استرجاع returnItems من localStorage:', error);
      return [];
    }
  });
  ...
  useEffect(() => {
    console.log('🔄 [usePOSReturn] تغيرت returnItems:', {
      itemsCount: returnItems.length,
      items: returnItems.map(item => ({ name: item.product.name, quantity: item.quantity })),
      timestamp: new Date().toLocaleTimeString()
    });
    ...
    try {
      ...
      console.log('💾 [usePOSReturn] تم حفظ returnItems في localStorage');
    } catch (error) {
      console.error('❌ [usePOSReturn] خطأ في حفظ returnItems:', error);
    }
  }, [returnItems]);
  ...
  useEffect(() => {
    if (isReturnMode) {
      ...
      console.log('🎨 [usePOSReturn] تم تطبيق return-mode class عند mount');
    } else {
      document.body.classList.remove('return-mode');
      console.log('🎨 [usePOSReturn] تم إزالة return-mode class عند mount');
    }
    
    // تنظيف عند unmount
    return () => {
      document.body.classList.remove('return-mode');
      console.log('🎨 [usePOSReturn] تم تنظيف return-mode class عند unmount');
    };
  }, [isReturnMode]);
  ...
  const addItemToReturnCart = useCallback((product: Product) => {
    console.log('🔄 [addItemToReturnCart] بدء إضافة منتج لسلة الإرجاع:', {
    ...
    setReturnItems(currentItems => {
      console.log('📊 [addItemToReturnCart] حالة السلة الحالية:', {
        currentItemsCount: currentItems.length,
        currentItems: currentItems.map(item => ({ name: item.product.name, quantity: item.quantity }))
      });
      ...
      if (existingItem) {
        console.log('✅ [addItemToReturnCart] تحديث كمية منتج موجود في سلة الإرجاع');
        ...
        console.log('📊 [addItemToReturnCart] بعد تحديث الكمية:', {
          updatedItemsCount: updatedItems.length,
          updatedItems: updatedItems.map(item => ({ name: item.product.name, quantity: item.quantity }))
        });
        
        return updatedItems;
      } else {
        console.log('✅ [addItemToReturnCart] إضافة منتج جديد لسلة الإرجاع');
        const newItems = [...currentItems, { product, quantity: 1 }];
        
        console.log('📊 [addItemToReturnCart] بعد إضافة منتج جديد:', {
          oldItemsCount: currentItems.length,
          newItemsCount: newItems.length,
          newItems: newItems.map(item => ({ name: item.product.name, quantity: item.quantity }))
        });
    ...
    console.log('✅ [addItemToReturnCart] تم الانتهاء من إضافة المنتج لسلة الإرجاع');
  }, []);
  ...
  const clearReturnCart = useCallback(() => {
    ...
    try {
      ...
      console.log('🗑️ [clearReturnCart] تم مسح returnItems من localStorage');
    } catch (error) {
      console.error('❌ [clearReturnCart] خطأ في مسح localStorage:', error);
    }
  }, []);
