...
const AddProductDialog = ({ open, onOpenChange, onProductAdded }: AddProductDialogProps) => {
  ...
  // Obtener el ID de المؤسسة إذا لم يكن متوفرًا في السياق
  const fetchOrganizationIdFromDB = async () => {
    try {
      ...
      // Buscar usuarios que tenga una organización válida لاستخدامها كتخطية
      ...
      // Si aún no tenemos un ID, intentemos obtenerlo directamente de المؤسسات
      ...
      // لا نستخدم أي قيم محددة مسبقاً، يجب أن يكون للمستخدم مؤسسة حقيقية
  ...
  useEffect(() => {
    ...
    const getOrganizationId = () => {
      ...
      // Intentar obtener المؤسسة من السياق
      ...
      // Intentar obtener المؤسسة من localStorage (تجربة متعددة المفاتيح)
      const keysToTry = [
      ...
      // Intentar obtener من sessionStorage
      for (const key of keysToTry) {
      ...
      // إذا لم يكن هناك ID صالح، جلبه من قاعدة البيانات
  ...
  // Obtener المؤسسة الحالية إذا لم يتم الحصول عليها بعد
  useEffect(() => {
  ...
  const onSubmit = async (values: ProductFormValues) => {
    ...
    try {
      ...
      if (product) {
        ...
        console.log('✅ تم إنشاء المنتج بنجاح، ID:', product.id);
        
        // مسح الcache المتعلق بالمنتجات لضمان جلب البيانات الجديدة
        if (typeof window !== 'undefined' && (window as any).clearProductsCache) {
          try {
            (window as any).clearProductsCache(organizationId);
            console.log('🧹 تم مسح cache المنتجات');
          } catch (error) {
            console.log('⚠️ فشل في مسح cache المنتجات:', error);
          }
        }
        
        // إرسال حدث تحديث المنتجات لتحديث القائمة فوريًا
        const updateEvent = new CustomEvent('products-updated', {
          detail: {
            operation: 'create',
        ...
        console.log('📢 تم إرسال حدث تحديث المنتجات');
        
        // تأخير قصير للتأكد من أن قاعدة البيانات تم تحديثها ثم تحديث القائمة
        setTimeout(() => {
          console.log('🔄 استدعاء onProductAdded...');
          onProductAdded();
        }, 200); // زيادة التأخير قليلاً
        
        // تحديث إضافي متأخر لضمان التحديث حتى لو كان هناك تأخير
        setTimeout(() => {
          console.log('🔄 تحديث إضافي متأخر للمنتجات...');
          onProductAdded();
        }, 1000);
