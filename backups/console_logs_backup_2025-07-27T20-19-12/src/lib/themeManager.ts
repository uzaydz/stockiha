...
function applyThemeToDOM(theme: UnifiedTheme): void {
  ...
  console.log('🎨 [applyThemeToDOM] بداية تطبيق الثيم:', {
    themeKey,
  ...
  if (currentAppliedTheme === themeKey) {
    console.log('⏭️ [applyThemeToDOM] تم تجاهل التطبيق - نفس الثيم مطبق بالفعل');
    return;
  }
  ...
  if (theme.primaryColor) {
    ...
    console.log('✅ [applyThemeToDOM] تم تطبيق اللون الأساسي على العناصر');
...
export function applyInstantTheme(): void {
  ...
  if (now - lastThemeApplication < THEME_THROTTLE_MS) {
    console.log('🔄 [applyInstantTheme] تم تجاهل التطبيق - throttled');
    return;
  }
  ...
  if (pageType === 'store' || pageType === 'admin') {
    ...
    const orgId = getOrganizationIdSync();
    console.log('🏢 [applyInstantTheme] معرف المؤسسة:', orgId);
    
    // محاولة استرجاع ثيم المؤسسة من التخزين المحلي باستخدام hostname
    const hostname = window.location.hostname;
    const hostKey = getThemeStorageKey(hostname);
    const storedHostTheme = localStorage.getItem(hostKey);
    console.log('🔍 [applyInstantTheme] البحث في localStorage بمفتاح:', hostKey, 'النتيجة:', !!storedHostTheme);
    
    if (storedHostTheme) {
      try {
        const hostThemeData = JSON.parse(storedHostTheme);
        console.log('✅ [applyInstantTheme] تم العثور على ثيم في localStorage:', hostThemeData);
        theme = {
        ...
        console.error('❌ [applyInstantTheme] خطأ في تحليل ثيم localStorage:', e);
      }
    }
    ...
    if (!theme && orgId) {
      ...
      console.log('🔍 [applyInstantTheme] البحث عن ثيم المؤسسة:', orgThemeKey, 'النتيجة:', !!storedOrgTheme);
      if (storedOrgTheme) {
        try {
          theme = JSON.parse(storedOrgTheme);
          console.log('✅ [applyInstantTheme] تم العثور على ثيم المؤسسة:', theme);
        } catch (e) {
          console.error('❌ [applyInstantTheme] خطأ في تحليل ثيم المؤسسة:', e);
        }
      }
    }
    ...
    if (!theme) {
      ...
      if (orgId) {
        console.log('🔍 [applyInstantTheme] لم يتم العثور على ثيم محفوظ، انتظار تحميل إعدادات المؤسسة...');
        ...
        if (orgId === 'b87869bc-a69e-4310-a67a-81c2ab927faf') {
          ...
          console.log('🔄 [themeManager] انتظار تحميل إعدادات المؤسسة:', orgId);
          return; // خروج مبكر بدون تطبيق ثيم افتراضي
        }
      } else {
        console.log('🔍 [applyInstantTheme] لا يوجد معرف مؤسسة، انتظار تحميل البيانات...');
...
export function forceApplyOrganizationTheme(
  ...
  console.log('🎨 [forceApplyOrganizationTheme] المدخلات الخام:', {
    organizationId,
    settings,
    hasActualColors: !!(settings.theme_primary_color && settings.theme_secondary_color)
  });
  ...
  if (!settings.theme_primary_color || !settings.theme_secondary_color) {
    console.warn('⚠️ [forceApplyOrganizationTheme] لا توجد ألوان مخصصة للمؤسسة:', organizationId);
    console.log('🔍 [forceApplyOrganizationTheme] الألوان المتوفرة:', {
      primaryColor: settings.theme_primary_color || 'غير موجود',
      secondaryColor: settings.theme_secondary_color || 'غير موجود'
    });
  ...
  console.log('🎨 [forceApplyOrganizationTheme] تطبيق ألوان المؤسسة:', {
    organizationId,
    primaryColor: settings.theme_primary_color,
    secondaryColor: settings.theme_secondary_color,
    mode: themeMode,
  ...
  console.log('✅ [forceApplyOrganizationTheme] تم تطبيق الثيم بنجاح');
}
