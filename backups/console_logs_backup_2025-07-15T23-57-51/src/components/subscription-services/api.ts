...
export class SubscriptionServicesAPI {
  ...
  static async fetchTransactions(organizationId: string): Promise<SubscriptionTransaction[]> {
    ...
    // تنسيق البيانات - استخدام البيانات المحفوظة في الجدول مباشرة
    const formattedTransactions = (transactionsData || []).map(transaction => ({
      ...transaction,
      // البيانات محفوظة بالفعل في subscription_transactions
      service_name: transaction.service_name || 'خدمة غير محددة',
      provider: transaction.provider || 'غير محدد',
