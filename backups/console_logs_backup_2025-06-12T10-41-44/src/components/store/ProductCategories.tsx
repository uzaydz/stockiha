...
const ProductCategories = ({
  ...
  // استخدام الفئات الافتراضية إذا لم توجد فئات حقيقية
  const finalDisplayCategories = useMemo(() => {
    if (!useRealCategories) return [];
    
    // إذا كانت هناك فئات حقيقية، استخدمها
    if (displayCategories.length > 0) {
      return displayCategories;
    }
    
    // إذا لم توجد فئات حقيقية، استخدم الفئات الافتراضية
    if (!isLoading && realCategories.length === 0) {
      const maxDisplay = settings.displayCount || settings.maxCategories || 6;
      return defaultCategories.slice(0, maxDisplay);
    }
    
    return [];
  }, [useRealCategories, displayCategories, isLoading, realCategories.length, settings.displayCount, settings.maxCategories]);
  ...
  const CategoryCard = ({ category }: { category: ExtendedCategory }) => {
    ...
    return (
      <Link 
        to={realCategories.length > 0 ? `/products?category=${category.id}` : `/products?demo_category=${category.slug}`}
    ...
    <section className="py-12 md:py-16 lg:py-20 bg-muted/30 dark:bg-muted/10">
      <div className="container px-4 mx-auto">
        <div className="text-center mb-10 md:mb-12">
          ...
          {!isLoading && realCategories.length === 0 && finalDisplayCategories.length > 0 && (
