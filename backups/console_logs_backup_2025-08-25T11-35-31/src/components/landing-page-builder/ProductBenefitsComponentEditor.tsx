                  ...
                  <div className="flex items-center justify-between pt-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        إظهار الأيقونات الملونة في الفوائد
                      </p>
                    </div>
                    <Switch
                      id="showIcons"
                      checked={settings.showIcons !== false}
                      onCheckedChange={(checked) => handleSettingsChange('showIcons', checked)}
                    />
                  </div>
                    ...
                    <div className="space-y-4 pt-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <Button
                            type="button"
                            variant={settings.imagePosition === 'bottom' ? 'default' : 'outline'}
                            className="flex flex-col items-center justify-center py-2 h-auto gap-1"
                            onClick={() => handleSettingsChange('imagePosition', 'bottom')}
                          >
                            <div className="w-full h-1 bg-muted rounded-md"></div>
                            <div className="w-full h-3 bg-muted rounded-md mt-1"></div>
                            <div className="w-full h-6 bg-muted rounded-md mb-1 relative">
                            ...
                            <span className="text-xs mt-1">أسفل</span>
                          </Button>
                        </div>
                      </div>
                      ...
                      {/* نمط عرض الصورة */}
                      <div className="space-y-2">
                        <Label className="text-sm font-medium">نمط عرض الصورة</Label>
                        <Select
                          value={settings.imageObjectFit || 'cover'}
                          onValueChange={(value) => handleSettingsChange('imageObjectFit', value)}
                        >
                          <SelectTrigger className="w-full">
                            <SelectValue placeholder="اختر نمط العرض" />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="cover">تغطية كاملة (Cover)</SelectItem>
                            <SelectItem value="contain">احتواء كامل (Contain)</SelectItem>
                            <SelectItem value="fill">ملء كامل (Fill)</SelectItem>
                            <SelectItem value="scale-down">تصغير ذكي (Scale Down)</SelectItem>
                            <SelectItem value="none">الحجم الأصلي (None)</SelectItem>
                          </SelectContent>
                        </Select>
                        <p className="text-xs text-muted-foreground">
                          كيفية عرض الصورة داخل المساحة المخصصة لها
                        </p>
                      </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <Label className="text-sm font-medium">
                            ارتفاع الصورة: <strong>{settings.imageHeight || 128}px</strong>
                          </Label>
                        </div>
                        <Slider
                          min={80}
                          max={300}
                          step={8}
                          value={[settings.imageHeight || 128]}
                          onValueChange={(value) => handleSettingsChange('imageHeight', value[0])}
                          className="py-2"
                        />
                        <p className="text-xs text-muted-foreground">
                          ارتفاع الصورة بالبكسل (80-300px)
                        </p>
                      </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <Label className="text-sm font-medium">
                            استدارة الزوايا: <strong>{settings.imageBorderRadius || 12}px</strong>
                          </Label>
                        </div>
                        <Slider
                          min={0}
                          max={32}
                          step={2}
                          value={[settings.imageBorderRadius || 12]}
                          onValueChange={(value) => handleSettingsChange('imageBorderRadius', value[0])}
                          className="py-2"
                        />
                        <p className="text-xs text-muted-foreground">
                          مدى استدارة زوايا الصورة (0-32px)
                        </p>
                      </div>

                      {/* تأثيرات إضافية */}
                      <div className="space-y-3">
                        <Label className="text-sm font-medium">تأثيرات الصورة</Label>
                        
                        <div className="flex items-center justify-between">
                          <div>
                            <Label className="text-sm">تأثير التمرير</Label>
                            <p className="text-xs text-muted-foreground">
                              تكبير الصورة عند التمرير
                            </p>
                          </div>
                          <Switch
                            checked={settings.imageHoverEffect !== false}
                            onCheckedChange={(checked) => handleSettingsChange('imageHoverEffect', checked)}
                          />
                        </div>

                        <div className="flex items-center justify-between">
                          <div>
                            <Label className="text-sm">إضافة ظل</Label>
                            <p className="text-xs text-muted-foreground">
                              ظل خفيف حول الصورة
                            </p>
                          </div>
                          <Switch
                            checked={settings.imageShadow !== false}
                            onCheckedChange={(checked) => handleSettingsChange('imageShadow', checked)}
                          />
                        </div>

                        <div className="flex items-center justify-between">
                          <div>
                            <Label className="text-sm">إطار للصورة</Label>
                            <p className="text-xs text-muted-foreground">
                              إطار رفيع حول الصورة
                            </p>
                          </div>
                          <Switch
                            checked={settings.imageBorder !== false}
                            onCheckedChange={(checked) => handleSettingsChange('imageBorder', checked)}
                          />
                        </div>
                      </div>
                    </div>
