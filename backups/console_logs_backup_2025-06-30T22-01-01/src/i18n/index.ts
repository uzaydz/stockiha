...
const getDefaultLanguageFromDatabase = async () => {
  try {
    ...
    if (languageCache && 
      ...
      console.log('🏢 استخدام اللغة من cache محلي:', languageCache.language);
    ...
    if (typeof window !== 'undefined' && window.organizationCache) {
      console.log('🔍 فحص cache المنظمة في i18n...');
      ...
      if (storedOrgId) {
        ...
        if (window.organizationCache.has(orgCacheKey)) {
          ...
          if (cached && (Date.now() - cached.timestamp) < 10 * 60 * 1000) { // 10 دقائق
            ...
            console.log('🏢 استخدام cache orgId:', organizationId);
          }
        }
      }
      ...
      if (!organizationData && !currentHost.includes('localhost')) {
        ...
        if (window.organizationCache.has(domainCacheKey)) {
          ...
          if (cached && (Date.now() - cached.timestamp) < 10 * 60 * 1000) {
            organizationData = cached.data;
            organizationId = organizationData.id;
            console.log('🏢 استخدام cache domain:', organizationId);
          }
        }
      }
      ...
      if (!organizationData) {
        ...
        if (window.organizationCache.has(subdomainCacheKey)) {
          ...
          if (cached && (Date.now() - cached.timestamp) < 10 * 60 * 1000) {
            organizationData = cached.data;
            organizationId = organizationData.id;
            console.log('🏢 استخدام cache subdomain:', organizationId);
          }
        }
      }
      ...
      if (!organizationData) {
        console.log('⏳ انتظار قصير لتحميل TenantContext...');
        ...
        if (storedOrgId) {
          ...
          if (window.organizationCache.has(orgCacheKey)) {
            ...
            if (cached && (Date.now() - cached.timestamp) < 10 * 60 * 1000) {
              organizationData = cached.data;
              organizationId = organizationData.id;
              console.log('🏢 استخدام cache بعد الانتظار:', organizationId);
            }
          }
        }
      }
    ...
    if (!organizationId) {
      console.log('🔄 لم نجد في cache، نفذ استدعاء للمنظمة من i18n');
      ...
      try {
        ...
        if (!orgError && orgData) {
          ...
          if (typeof window !== 'undefined' && window.organizationCache) {
            ...
            console.log('🏢 حفظ بيانات المنظمة في cache من i18n:', { organizationId, cacheKey });
          }
        }
      } catch (error) {
        console.warn('تعذر جلب بيانات المنظمة في i18n:', error);
      }
    }
    ...
    if (organizationId) {
      try {
        const supabase = getSupabaseClient();
        // استعلام مباشر لجدول organization_settings
        ...
        if (!settingsError && settingsData && settingsData.default_language) {
          ...
          console.log('🌐 تم جلب اللغة من DB:', settingsData.default_language);
          return settingsData.default_language;
        }
      } catch (error) {
        console.warn('تعذر جلب إعدادات اللغة:', error);
      }
    }
    ...
    console.warn('خطأ في getDefaultLanguageFromDatabase:', error);
...
const updateLanguageFromDatabase = async () => {
  if (typeof window !== 'undefined') {
    try {
      ...
      console.log('🌐 بدء تحديث اللغة بعد تأخير ذكي...');
      ...
      if (shouldUseDefaultLanguage && defaultLanguage !== i18n.language) {
        ...
        console.log('🌐 تم تحديث اللغة إلى:', defaultLanguage);
      }
      
    } catch (error) {
      console.warn('خطأ في updateLanguageFromDatabase:', error);
    }
  }
};
