...
const getQueueInfo = async (orderId: string) => {
  try {
    ...
    // إذا لم يكن هناك مكان تصليح محدد، لا يمكن حساب الطابور
    if (!currentOrder.repair_location_id) {
      return null;
    }
    
    // استخدام أرقام الحالات بدلاً من النصوص العربية لتجنب مشاكل الترميز
    const pendingStatusQuery = supabase
      .from('repair_orders')
      .select('id')
    ...
    const inProgressStatusQuery = supabase
      .from('repair_orders')
      .select('id')
      .eq('repair_location_id', currentOrder.repair_location_id)
      .eq('status', 'جاري التصليح')
      .lt('created_at', currentOrder.created_at);
    
    // تنفيذ الاستعلامات بشكل منفصل
    const [pendingResult, inProgressResult] = await Promise.all([
      pendingStatusQuery,
      inProgressStatusQuery
    ]);
    
    if (pendingResult.error || inProgressResult.error) return null;
    
    const ordersBeforeMe = [...(pendingResult.data || []), ...(inProgressResult.data || [])];
    ...
    const inProgressTotalQuery = supabase
      .from('repair_orders')
      .select('id, created_at')
      .eq('repair_location_id', currentOrder.repair_location_id)
      .eq('status', 'جاري التصليح');
    
    const [pendingTotalResult, inProgressTotalResult] = await Promise.all([
      pendingTotalQuery,
      inProgressTotalQuery
    ]);
    
    if (pendingTotalResult.error || inProgressTotalResult.error) return null;
    
    const totalActiveOrders = [...(pendingTotalResult.data || []), ...(inProgressTotalResult.data || [])];
...
const PublicRepairTracking: React.FC = () => {
  ...
  const { currentSubdomain } = useAuthSafe();
