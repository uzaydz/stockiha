...
const AppWrapper: React.FC<AppWrapperProps> = ({ children }) => {
  ...
  const initializeData = async (isRetry = false) => {
    ...
    try {
      ...
      console.log('🚀 [AppWrapper] تحليل النطاق:', { 
        hostname, 
        isLocalhost, 
        subdomain, 
        hasValidSubdomain
      ...
      if (platformDomains.includes(hostname)) {
        console.log('🏢 [AppWrapper] نطاق أساسي للمنصة - تطبيق initializeApp');
        ...
        console.log('🔄 [AppWrapper] جلب بيانات المنصة...');
        const data = await initializeApp();

        if (data && mountedRef.current) {
          console.log('✅ [AppWrapper] تم جلب بيانات المنصة بنجاح');
          isInitializedRef.current = true;
          setIsReady(true);
          hideLoader();
        } else if (mountedRef.current) {
          console.log('⚠️ [AppWrapper] فشل في جلب بيانات المنصة - متابعة بدون بيانات');
          isInitializedRef.current = true;
          setIsReady(true);
          hideLoader();
        }
      ...
      if (isLocalhost && hasValidSubdomain) {
        console.log('🛠️ [AppWrapper] تطوير محلي مع subdomain:', subdomain);
        ...
        const waitForStoreData = async (attempt = 0): Promise<void> => {
          ...
          if (storedOrgId && storedOrgId !== 'default-organization-id') {
            console.log('✅ [AppWrapper] StoreRouter جهز البيانات:', storedOrgId);
            return;
          }
          
          if (attempt < maxAttempts) {
            console.log(`⏱️ [AppWrapper] انتظار StoreRouter... ${attempt + 1}/${maxAttempts}`);
      ...
      if (isLocalhost && !hasValidSubdomain) {
        console.log('🏠 [AppWrapper] تطوير محلي بدون subdomain - صفحة عامة');
        isInitializedRef.current = true;
        setIsReady(true);
        return;
      }
      ...
      console.log('🏪 [AppWrapper] نطاق مخصص للمتجر - ترك المسؤولية لـ StoreRouter');
      ...
      const waitForStoreRouter = async (attempt = 0): Promise<void> => {
        ...
        if ((storedOrgId && storedOrgId !== 'default-organization-id') || storedSubdomain) {
          console.log('✅ [AppWrapper] StoreRouter حمل بيانات المتجر بنجاح');
          return;
        }
        
        if (attempt < maxAttempts) {
          console.log(`⏱️ [AppWrapper] انتظار StoreRouter للنطاق المخصص... ${attempt + 1}/${maxAttempts}`);
          return await waitForStoreRouter(attempt + 1);
        }
        
        console.log('⚠️ [AppWrapper] انتهت مهلة انتظار StoreRouter - المتابعة على أي حال');
      };
      ...
      if (mountedRef.current) {
        ...
        if (isPlatformDomain) {
          ...
          console.log('❌ [AppWrapper] خطأ في نطاق المنصة');
          setHasError(true);
        } else {
          // للنطاقات المخصصة والمحلية، نتجاهل الخطأ
          console.log('🔄 [AppWrapper] تجاهل الخطأ للنطاق المخصص/المحلي');
          isInitializedRef.current = true;
          setIsReady(true);
        }
      }
