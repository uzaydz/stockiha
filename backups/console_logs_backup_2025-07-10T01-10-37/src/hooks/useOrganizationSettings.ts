...
export const useOrganizationSettings = ({ organizationId }: UseOrganizationSettingsProps): UseOrganizationSettingsReturn => {
  ...
  const debugLog = (message: string, data?: any) => {
    console.log(`🎨 [useOrganizationSettings] ${message}`, data ? data : '');
  };
  ...
  const getCachedSettings = useCallback((orgId: string) => {
    ...
    if (globalCached && (Date.now() - globalCached.timestamp) < GLOBAL_CACHE_DURATION) {
      debugLog('تم العثور على بيانات في الكاش العام:', globalCached.data);
    ...
    if (cached && (Date.now() - cached.timestamp) < CACHE_DURATION) {
      debugLog('تم العثور على بيانات في الكاش المحلي:', cached.data);
      return cached.data;
    }
    debugLog('لم يتم العثور على بيانات في الكاش');
    return null;
  }, []);

  // دالة لحفظ في الكاش
  const setCachedSettings = useCallback((orgId: string, data: OrganizationSettings) => {
    debugLog('حفظ البيانات في الكاش:', { orgId, colors: { primary: data.theme_primary_color, secondary: data.theme_secondary_color } });
  ...
  const fetchSettings = useCallback(async (orgId: string, forceRefresh = false) => {
    if (isFetchingRef.current && !forceRefresh) {
      debugLog('جلب الإعدادات قيد التنفيذ، تم تجاهل الطلب');
      return;
    }
    ...
    if (!forceRefresh) {
      ...
      if (cachedData) {
        debugLog('استخدام البيانات المحفوظة في الكاش');
    ...
    if (activeSettingsRequests.has(requestKey)) {
      debugLog('انتظار طلب نشط موجود');
      try {
        const result = await activeSettingsRequests.get(requestKey);
        return result;
      } catch (error) {
        debugLog('فشل في انتظار الطلب النشط:', error);
      }
    }
    ...
    const fetchPromise = (async () => {
      try {
        ...
        debugLog('بدء جلب إعدادات المؤسسة:', orgId);
        ...
        if (settingsData) {
          ...
          if (settingsData.organization_id !== orgId) {
            debugLog('تحذير: معرف المؤسسة غير متطابق!', {
              expected: orgId,
          ...
          debugLog('لم يتم العثور على إعدادات، استخدام الإعدادات الافتراضية');
        ...
        // جلب بيانات بكسل التتبع
        try {
          debugLog('جلب بيانات بكسل التتبع');
          // TODO: تنفيذ دالة getTrackingPixels
          // const pixelsData = await getTrackingPixels(orgId);
          // if (pixelsData) {
          //   setTrackingPixels(pixelsData);
          //   debugLog('تم جلب بيانات بكسل التتبع بنجاح');
          // }
        } catch (pixelError) {
          debugLog('فشل في جلب بيانات بكسل التتبع:', pixelError);
        }

        setIsLoading(false);
        lastFetchedRef.current = orgId;
        
        debugLog('تم الانتهاء من جلب الإعدادات بنجاح');
        
        return settingsData;
      } catch (error) {
        debugLog('خطأ في جلب الإعدادات:', error);
  ...
  useEffect(() => {
    if (!organizationId) {
      debugLog('لا يوجد معرف مؤسسة');
      return;
    }

    if (lastFetchedRef.current === organizationId && hasRealData) {
      debugLog('تم جلب الإعدادات مسبقاً للمؤسسة:', organizationId);
      return;
    }

    debugLog('بدء جلب الإعدادات للمؤسسة:', organizationId);
