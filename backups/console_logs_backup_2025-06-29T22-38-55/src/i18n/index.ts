...
const getDefaultLanguageFromDatabase = async () => {
  try {
    ...
    if (languageCache && 
      ...
      console.log('🎯 استخدام اللغة من التخزين المؤقت:', languageCache.language);
    ...
    try {
      // استعلام واحد يبحث بالنطاق الكامل أو النطاق الفرعي - إصلاح maybeSingle
      ...
      console.warn('خطأ في البحث عن المؤسسة:', error);
      ...
      try {
        ...
        console.warn('فشل في البحث بالنطاق الفرعي أيضاً:', fallbackError);
      }
    }
    ...
    if (organizationId) {
      try {
        // استعلام مباشر لجدول organization_settings - إصلاح maybeSingle
        ...
        if (!settingsError && settingsData && settingsData.default_language) {
          ...
          console.log('✅ تم جلب اللغة وحفظها في التخزين المؤقت:', settingsData.default_language);
          return settingsData.default_language;
        }
      } catch (error) {
        console.warn('خطأ في جلب إعدادات اللغة:', error);
      }
    }
    ...
    console.warn('خطأ عام في جلب اللغة الافتراضية:', error);
