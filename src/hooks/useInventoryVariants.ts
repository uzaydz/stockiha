...
interface UseInventoryVariantsActions {
  ...
  // إدارة التعديل
  startEditingVariant: (productId: string, variantId?: string, currentQuantity?: number) => void;
...
export function useInventoryVariants(
  ...
  const bulkUpdateVariantsAction = useCallback(async (updates: Array<{
    ...
    try {
      ...
      if (result.success_count > 0) {
        ...
        // تحديث البيانات للمنتجات المتأثرة
        const affectedProducts = [...new Set(updates.map(u => u.product_id))];
        for (const productId of affectedProducts) {
          const cacheKey = `inventory_details_${productId}`;
          cacheRef.current.delete(cacheKey);
        }
        
        if (state.inventoryDetails && affectedProducts.includes(state.inventoryDetails.product_id)) {
          await refreshInventoryDetails();
        }
      }
  ...
  }, [state.inventoryDetails, refreshInventoryDetails]);
  ...
  // تحميل سجل المخزون
  const loadInventoryLog = useCallback(async (productId?: string, variantId?: string) => {
    try {
      setState(prev => ({ ...prev, isLoadingLog: true, error: null }));

      const log = await getInventoryVariantsLog(productId, variantId, 50, 0);
  ...
  // إدارة التعديل
  const startEditingVariant = useCallback((productId: string, variantId?: string, currentQuantity?: number) => {
    setState(prev => ({
      ...prev,
      editingVariant: {
        productId,
