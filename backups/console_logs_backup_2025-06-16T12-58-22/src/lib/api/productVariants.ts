...
export const getProductColors = async (productId: string): Promise<ProductColor[]> => {
  try {
    console.log('جلب ألوان المنتج للمعرف:', productId);
    ...
    console.log('تم جلب الألوان بنجاح:', data);
    return data || [];
  } catch (error) {
    console.error('خطأ عام في جلب ألوان المنتج:', error);
...
export const updateProductStockQuantity = async (productId: string): Promise<void> => {
  try {
    console.log('🔄 تحديث stock_quantity للمنتج:', productId);
    ...
    // حساب مجموع الكميات
    const totalQuantity = colors?.reduce((sum, color) => sum + (color.quantity || 0), 0) || 0;
    console.log('📊 مجموع كميات الألوان:', totalQuantity);
    ...
    if (updateError) {
      console.error('❌ خطأ في تحديث stock_quantity:', updateError);
    } else {
      console.log('✅ تم تحديث stock_quantity بنجاح:', totalQuantity);
    }
  } catch (error) {
    console.error('❌ خطأ عام في updateProductStockQuantity:', error);
  }
};
