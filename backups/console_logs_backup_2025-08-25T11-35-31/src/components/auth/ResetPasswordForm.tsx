...
const ResetPasswordForm = () => {
  ...
  useEffect(() => {
    // التحقق من وجود جلسة صالحة أو رموز صالحة دون إعادة توجيه تلقائي
    const checkSessionAndParams = async () => {
      ...
      if ((!hasValidTokens && !session) || (finalType && finalType !== 'recovery')) {
        if (hashErrorCode === 'otp_expired') {
          setLinkError('الرابط منتهي الصلاحية. يرجى طلب رابط جديد.');
          toast.error('انتهت صلاحية رابط إعادة التعيين');
        } else if (hashErrorCode) {
          setLinkError('حدث خطأ: ' + hashErrorCode);
          toast.error('حدث خطأ: ' + hashErrorCode);
        } else {
          setLinkError('رابط غير صالح لإعادة تعيين كلمة المرور');
  ...
  }, [finalAccessToken, finalRefreshToken, finalToken, finalType]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // السماح إذا كانت هناك جلسة صالحة أو رموز صالحة
    const hasValidTokens = (finalAccessToken && finalRefreshToken) || finalToken;
    if (!hasSession && (!hasValidTokens || (finalType && finalType !== 'recovery'))) {
      toast.error('رابط غير صالح لإعادة تعيين كلمة المرور');
      return;
    }
    ...
    try {
      ...
      if (hasSession) {
        // لدينا جلسة صالحة بالفعل (تمت معالجتها تلقائياً بواسطة Supabase)
  ...
  return (
    ...
    >
      <Card className="shadow-xl border-0 bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm">
        ...
        <CardContent className="space-y-4">
          {linkError && (
            <div className="p-3 rounded-md bg-red-50 border border-red-200 text-red-700 text-sm flex items-start justify-between">
              <span>{linkError}</span>
              <button
                type="button"
                onClick={() => navigate('/forgot-password')}
                className="underline text-red-700 hover:text-red-800 ml-4"
              >
                طلب رابط جديد
              </button>
            </div>
