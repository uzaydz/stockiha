...
// Mock performance monitor for compatibility
class PerformanceMonitor {
  private static instance: PerformanceMonitor;
  ...
  async executeWithSplitting<T>(
    ...
    try {
      ...
      if (duration > maxDuration) {
        console.warn(`⚠️ مهمة طويلة: ${taskName} استغرقت ${duration.toFixed(2)}ms`);
      }
      
      return result;
    } catch (error) {
      console.error(`❌ خطأ في المهمة: ${taskName}`, error);
      throw error;
    }
  }
      ...
      this.taskQueue.push({
        task: async () => {
          try {
            ...
            if (duration > maxDuration) {
              console.warn(`⚠️ مهمة طويلة محسنة: ${taskName} استغرقت ${duration.toFixed(2)}ms`);
            }
