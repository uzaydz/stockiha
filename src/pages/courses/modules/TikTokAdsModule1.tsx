...
import { tiktokAdsModule1Data } from '@/data/tiktokAdsModule1Data';
import { Play, CheckCircle, Clock, Music, Target } from 'lucide-react';

const TikTokAdsModule1: React.FC = () => {
  const navigate = useNavigate();
  const [currentVideoIndex, setCurrentVideoIndex] = useState(0);
  const [completedVideos, setCompletedVideos] = useState<number[]>([]);
  const [isVideoCompleted, setIsVideoCompleted] = useState(false);

  const currentVideo = tiktokAdsModule1Data.videos[currentVideoIndex];
  const storageKey = 'tiktok-ads-module-1-completed-videos';
  ...
  const nextVideo = () => {
    if (currentVideoIndex < tiktokAdsModule1Data.videos.length - 1) {
  ...
  return (
    <POSPureLayout>
      <div className="container mx-auto px-4 py-6 max-w-6xl" dir="rtl">
        {/* Module Header */}
        <div className="bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-900/20 dark:to-purple-900/20 rounded-lg border border-pink-200 dark:border-pink-800 p-6 mb-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <div className="text-sm text-pink-600 dark:text-pink-400 font-medium">
                المحور {tiktokAdsModule1Data.moduleNumber}
              </div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                {tiktokAdsModule1Data.title}
              </h1>
            </div>
          </div>
          <p className="text-gray-600 dark:text-gray-300 mb-4">
            {tiktokAdsModule1Data.description}
          </p>
          <div className="flex flex-wrap gap-4 text-sm">
            <div className="flex items-center gap-2 text-pink-700 dark:text-pink-300">
              <Play className="w-4 h-4" />
              <span>{tiktokAdsModule1Data.totalVideos} فيديوهات</span>
            </div>
            <div className="flex items-center gap-2 text-pink-700 dark:text-pink-300">
              <Clock className="w-4 h-4" />
              <span>{tiktokAdsModule1Data.estimatedDuration}</span>
            </div>
            <div className="flex items-center gap-2 text-pink-700 dark:text-pink-300">
              <CheckCircle className="w-4 h-4" />
              <span>{completedVideos.length} / {tiktokAdsModule1Data.totalVideos} مكتمل</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    {currentVideoIndex < tiktokAdsModule1Data.videos.length - 1 && (
                      <button
                        onClick={nextVideo}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
              <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <div className="text-sm text-gray-500 dark:text-gray-400">
                  {completedVideos.length} من {tiktokAdsModule1Data.videos.length} مكتمل
                </div>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-2">
                  <div 
                    className="bg-pink-600 h-2 rounded-full transition-all"
                    style={{ width: `${(completedVideos.length / tiktokAdsModule1Data.videos.length) * 100}%` }}
                  ></div>
                </div>
              </div>
              
              <div className="divide-y divide-gray-200 dark:divide-gray-700">
                {tiktokAdsModule1Data.videos.map((video, index) => (
                  <button
                    key={video.id}
            ...
            <div className="bg-gradient-to-br from-pink-50 to-purple-50 dark:from-pink-900/20 dark:to-purple-900/20 rounded-lg border border-pink-200 dark:border-pink-800 p-4 mt-6">
              <div className="flex items-center gap-2 mb-3">
                <Target className="w-5 h-5 text-pink-600 dark:text-pink-400" />
                <h4 className="font-semibold text-gray-900 dark:text-white">نصائح للتعلم</h4>
              </div>
              <ul className="text-sm text-gray-600 dark:text-gray-300 space-y-2">
                <li>• تابع كل فيديو بتركيز كامل</li>
                <li>• دون الملاحظات المهمة</li>
                <li>• طبق ما تتعلمه عملياً</li>
                <li>• لا تتردد في إعادة المشاهدة</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <ModuleNavigation 
          ...
          totalVideos={tiktokAdsModule1Data.totalVideos}
        />
      </div>
    </POSPureLayout>
  );
...
export default TikTokAdsModule1;
