...
interface POSDataOptions {
  ...
  search?: string;
  categoryId?: string;
  enabled?: boolean; // إضافة خيار للتحكم في تفعيل الاستعلام
}
...
export const useCompletePOSData = (options: POSDataOptions = {}) => {
  ...
  const {
    ...
    enabled: enabled && !!currentOrganization?.id && (search === undefined || search.length === 0 || search.length >= 2), // البحث فقط إذا كان فارغ أو أكثر من حرفين
    staleTime: 10 * 60 * 1000, // 10 دقائق - زيادة كبيرة لتقليل الاستدعاءات
    gcTime: 20 * 60 * 1000, // 20 دقيقة
    retry: 1, // تقليل عدد المحاولات أكثر
    retryDelay: (attemptIndex) => Math.min(1000 * 2 ** attemptIndex, 3000),
    refetchOnWindowFocus: false, // منع إعادة التحميل عند التركيز على النافذة
    refetchOnMount: false, // منع إعادة التحميل عند التركيب إذا كانت البيانات حديثة
    ...
    networkMode: 'online', // تحسين شبكة
    meta: {
      persist: false // عدم الاحتفاظ في localStorage
    }
  });
