...
export const handle406Error = async (
  ...
  try {
    ...
    if (firstRetry.ok) {
      console.log('نجحت المحاولة الأولى لإصلاح خطأ 406');
      return firstRetry;
    }
    ...
    if (secondRetry.ok) {
      console.log('نجحت المحاولة الثانية لإصلاح خطأ 406');
      return secondRetry;
    }
    ...
    if (minimalRetry.ok) {
      console.log('نجحت المحاولة الثالثة لإصلاح خطأ 406');
      return minimalRetry;
    }
    ...
    console.error('خطأ في معالجة خطأ 406:', error);
    throw error;
  }
};
...
export const retryWithDelay = async <T>(
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (i < maxRetries - 1) {
        console.warn(`المحاولة ${i + 1} فشلت، إعادة المحاولة خلال ${delay}ms`);
