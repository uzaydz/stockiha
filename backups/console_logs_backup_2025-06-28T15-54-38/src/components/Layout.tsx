...
export default function Layout({ children }: LayoutProps) {
  ...
  console.log('🏗️ [Layout] تصيير Layout:', {
    hasUser: !!user,
  ...
  useEffect(() => {
    if (user && isLoading) {
      const timer = setTimeout(() => {
        console.log('⏰ [Layout] التحميل يستغرق وقتاً أطول من المتوقع...');
        // إضافة مؤشر للمشكلة إذا استمر التحميل أكثر من 15 ثانية
        const longTimer = setTimeout(() => {
          console.warn('⚠️ [Layout] التحميل معلق لأكثر من 15 ثانية!');
        }, 15000);
  ...
  if (user && isLoading) {
    console.log('⏳ [Layout] عرض شاشة التحميل...');
    return (
