-- إصلاح نظام الطلبات - fix_order_system_final.sql
-- الإصدار النهائي لإصلاح مشكلة المفتاح الأجنبي customer_id في جدول orders الذي يشير إلى users بدلاً من customers

-- 1. إصلاح قيد المفتاح الأجنبي في جدول الطلبات
-- حذف القيد الموجود أولاً
ALTER TABLE orders DROP CONSTRAINT IF EXISTS orders_customer_id_fkey;

-- إعادة إنشاء القيد ليشير إلى جدول customers
ALTER TABLE orders ADD CONSTRAINT orders_customer_id_fkey 
FOREIGN KEY (customer_id) REFERENCES customers(id);

-- 2. تعديل وظيفة process_online_order لتعمل بشكل صحيح مع القيد الجديد
DROP FUNCTION IF EXISTS process_online_order CASCADE;
CREATE OR REPLACE FUNCTION process_online_order(
  p_full_name TEXT,
  p_phone TEXT,
  p_province TEXT,
  p_address TEXT,
  p_delivery_company TEXT,
  p_payment_method TEXT,
  p_notes TEXT,
  p_product_id UUID,
  p_product_color_id UUID,
  p_quantity INTEGER,
  p_unit_price NUMERIC,
  p_total_price NUMERIC,
  p_delivery_fee NUMERIC,
  p_organization_id UUID
) RETURNS JSON AS $$
DECLARE
  v_customer_id UUID;
  v_address_id UUID;
  v_order_id UUID;
  v_order_item_id UUID;
  v_order_number INTEGER;
  v_order_slug TEXT;
  v_item_slug TEXT;
  v_product_name TEXT;
BEGIN
  -- كل العمليات في معاملة واحدة
  BEGIN
    -- 1. البحث عن العميل أو إنشاء عميل جديد
    SELECT id INTO v_customer_id
    FROM customers
    WHERE phone = p_phone AND organization_id = p_organization_id;
    
    IF v_customer_id IS NULL THEN
      -- إنشاء عميل جديد
      INSERT INTO customers (name, phone, organization_id)
      VALUES (p_full_name, p_phone, p_organization_id)
      RETURNING id INTO v_customer_id;
    END IF;
    
    -- 2. إنشاء عنوان للعميل
    -- التحقق مما إذا كان جدول العناوين يحتوي على عمود customer_id
    IF EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_name = 'addresses' AND column_name = 'customer_id'
    ) THEN
      -- استخدام customer_id
      INSERT INTO addresses (
        customer_id,
        name, 
        street_address, 
        city, 
        state, 
        postal_code, 
        country, 
        phone, 
        is_default,
        organization_id
      )
      VALUES (
        v_customer_id,
        p_full_name,
        p_address,
        '', -- المدينة
        p_province,
        '', -- الرمز البريدي
        'Algeria',
        p_phone,
        TRUE,
        p_organization_id
      )
      RETURNING id INTO v_address_id;
    ELSE
      -- استخدام user_id كحل بديل
      INSERT INTO addresses (
        user_id,
        name, 
        street_address, 
        city, 
        state, 
        postal_code, 
        country, 
        phone, 
        is_default,
        organization_id
      )
      VALUES (
        NULL, -- user_id NULL لتجنب مشاكل قيد المفتاح الأجنبي
        p_full_name,
        p_address,
        '', -- المدينة
        p_province,
        '', -- الرمز البريدي
        'Algeria',
        p_phone,
        TRUE,
        p_organization_id
      )
      RETURNING id INTO v_address_id;
    END IF;
    
    -- 3. إنشاء الطلب مع تحديد قيمة slug مسبقاً
    v_order_slug := 'order-' || floor(random() * 1000000)::text;
    
    INSERT INTO orders (
      customer_id, subtotal, tax, discount, total, 
      status, payment_method, payment_status, 
      shipping_address_id, shipping_method, shipping_cost, 
      notes, is_online, organization_id, slug
    )
    VALUES (
      v_customer_id, p_total_price, 0, 0, (p_total_price + p_delivery_fee),
      'pending', p_payment_method, 'pending',
      v_address_id, p_delivery_company, p_delivery_fee,
      p_notes, TRUE, p_organization_id, v_order_slug
    )
    RETURNING id, customer_order_number INTO v_order_id, v_order_number;
    
    -- 4. الحصول على اسم المنتج من جدول المنتجات
    SELECT name INTO v_product_name FROM products WHERE id = p_product_id;
    
    IF v_product_name IS NULL THEN
      v_product_name := 'منتج #' || p_product_id;
    END IF;
    
    -- 5. إنشاء slug عشوائي لـ order_items
    v_item_slug := 'item-' || floor(random() * 1000000)::text;
    
    -- 6. إضافة عنصر الطلب
    INSERT INTO order_items (
      order_id, product_id, product_name, quantity,
      unit_price, total_price, is_digital, organization_id,
      slug, name
    )
    VALUES (
      v_order_id, p_product_id, v_product_name, p_quantity,
      p_unit_price, p_total_price, FALSE, p_organization_id,
      v_item_slug, v_product_name
    )
    RETURNING id INTO v_order_item_id;

    -- 7. تحديث المخزون
    IF p_product_color_id IS NOT NULL THEN
      UPDATE product_colors
      SET quantity = quantity - p_quantity
      WHERE id = p_product_color_id AND organization_id = p_organization_id;
    ELSE
      UPDATE products
      SET stock_quantity = stock_quantity - p_quantity
      WHERE id = p_product_id AND organization_id = p_organization_id;
    END IF;

    -- 8. إرجاع معلومات الطلب كـ JSON
    RETURN json_build_object(
      'order_id', v_order_id,
      'order_number', v_order_number,
      'order_slug', v_order_slug,
      'customer_id', v_customer_id
    );
  EXCEPTION
    WHEN OTHERS THEN
      -- إضافة معلومات حول الخطأ
      RETURN json_build_object(
        'error', SQLERRM,
        'detail', SQLSTATE,
        'context', 'Error in process_online_order function. SQLState: ' || SQLSTATE || ', Error: ' || SQLERRM,
        'customer_id', v_customer_id,
        'failed_at', CASE 
          WHEN v_customer_id IS NULL THEN 'customer_creation'
          WHEN v_address_id IS NULL THEN 'address_creation'
          WHEN v_order_id IS NULL THEN 'order_creation'
          WHEN v_order_item_id IS NULL THEN 'order_item_creation'
          ELSE 'unknown'
        END
      );
  END;
END;
$$ LANGUAGE plpgsql;

-- 3. إضافة عمود customer_id إلى جدول العناوين إذا لم يكن موجوداً
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'addresses' AND column_name = 'customer_id'
  ) THEN
    -- إضافة عمود customer_id مع قيد المفتاح الأجنبي
    ALTER TABLE addresses ADD COLUMN customer_id UUID REFERENCES customers(id);
    RAISE NOTICE 'تمت إضافة عمود customer_id إلى جدول addresses';
  ELSE
    RAISE NOTICE 'عمود customer_id موجود بالفعل في جدول addresses';
  END IF;
END $$;

-- 4. إضافة وظيفة للتأكد من صحة البيانات في النظام
CREATE OR REPLACE FUNCTION verify_order_system()
RETURNS TABLE (
  test_name TEXT,
  status TEXT,
  message TEXT
) AS $$
DECLARE
  v_order_constraint TEXT;
  v_address_constraint TEXT;
BEGIN
  -- 1. التحقق من قيد orders_customer_id_fkey
  SELECT pg_get_constraintdef(oid) INTO v_order_constraint
  FROM pg_constraint WHERE conname = 'orders_customer_id_fkey';
  
  IF v_order_constraint LIKE '%REFERENCES customers(id)%' THEN
    test_name := 'orders_customer_id_fkey';
    status := 'OK';
    message := 'يشير إلى جدول customers بشكل صحيح: ' || v_order_constraint;
    RETURN NEXT;
  ELSE
    test_name := 'orders_customer_id_fkey';
    status := 'ERROR';
    message := 'مازال يشير إلى جدول غير صحيح: ' || v_order_constraint;
    RETURN NEXT;
  END IF;
  
  -- 2. التحقق من وجود عمود customer_id في جدول العناوين
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'addresses' AND column_name = 'customer_id'
  ) THEN
    test_name := 'addresses_customer_id';
    status := 'OK';
    message := 'عمود customer_id موجود في جدول addresses';
    RETURN NEXT;
  ELSE
    test_name := 'addresses_customer_id';
    status := 'WARNING';
    message := 'عمود customer_id غير موجود في جدول addresses';
    RETURN NEXT;
  END IF;
  
  -- 3. التحقق من وجود وظيفة process_online_order
  IF EXISTS (
    SELECT 1 FROM pg_proc WHERE proname = 'process_online_order'
  ) THEN
    test_name := 'process_online_order';
    status := 'OK';
    message := 'وظيفة process_online_order موجودة';
    RETURN NEXT;
  ELSE
    test_name := 'process_online_order';
    status := 'ERROR';
    message := 'وظيفة process_online_order غير موجودة';
    RETURN NEXT;
  END IF;
END;
$$ LANGUAGE plpgsql;

-- 5. بعد التنفيذ، يمكن فحص حالة النظام باستخدام الاستعلام التالي:
-- SELECT * FROM verify_order_system(); 