      ...
      try {
        ...
        if (employeeExists) {
          ...
          if (employeeByAuthId) {
            ...
            console.log('✅ [orderService] تم العثور على الموظف بـ auth_user_id:', {
              providedId: order.employeeId,
              actualId: employeeByAuthId.id
            });
          } else {
            console.warn('🚨 [orderService] معرف الموظف غير موجود في قاعدة البيانات:', order.employeeId);
          }
        }
      } catch (error) {
        console.warn('🚨 [orderService] خطأ في التحقق من معرف الموظف:', error);
      }
    }
