...
class EnhancedErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    console.group('🚨 Smart Wrapper Error Boundary');
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('Error details recorded:', errorDetails);
    }
  ...
  private handleRetry = () => {
    ...
    if (now - this.state.lastErrorTime < this.errorCooldownTime) {
      console.warn('Retry cooldown period not expired');
      return;
    }
...
export const DataErrorBoundary = memo<{ children: ReactNode; pageType?: PageType }>(({ 
  ...
  <EnhancedErrorBoundary
    ...
    pathname={window.location.pathname}
    onError={(details) => {
      console.error('Data Provider Error:', details);
    }}
    fallback={
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
