...
const POS = () => {
  ...
  const updateItemQuantity = (index: number, quantity: number) => {
    ...
    if (quantity > totalAvailable) {
      ...
      // ✅ تم إصلاح المشكلة: لا يتم تحديث المخزون عند تعديل الكمية
      return;
    }
  ...
  const submitOrder = async (orderDetails: Partial<Order>): Promise<{orderId: string, customerOrderNumber: number}> => {
    ...
    try {
      ...
      // ✅ تم نقل تحديث المخزون إلى النظام المحسن
      // المخزون يتم تحديثه تلقائياً عبر posOrderService.createPOSOrder
              ...
              >
                <TabsList className={cn(
                  ...
                  {isAppEnabled('subscription-services') && (
                    <TabsTrigger 
                      value="subscriptions" 
                      className="flex items-center gap-2 py-3 px-4 transition-all duration-200 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm data-[state=active]:border data-[state=active]:border-border/50"
                    >
