  ...
  useEffect(() => {
    const initializeTracker = async () => {
      try {
        ...
        if (trackingSettings) {
          console.log('🎯 ConversionTracker: استخدام الإعدادات الموجودة من useProductTracking');
        } else {
          console.log('⚠️ ConversionTracker: لم يتم العثور على الإعدادات، سيتم جلبها من قاعدة البيانات');
        }
        
        // إنشاء ConversionTracker مع الإعدادات الموجودة
        trackerRef.current = getConversionTracker(productId, trackingSettings, organizationId);
