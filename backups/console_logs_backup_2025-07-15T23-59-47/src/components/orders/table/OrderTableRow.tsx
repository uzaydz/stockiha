...
const OrdersTableRow = ({
  ...
  const customerContact = (order as any).customer_phone || 
        ...
        {visibleColumns.includes("expand") && (
          <TableCell className="w-10 py-4 px-2">
            <Button
              variant="ghost"
              size="icon"
              className="h-6 w-6 transition-opacity opacity-70 group-hover:opacity-100 text-foreground hover:bg-accent"
              onClick={(e) => {
                e.stopPropagation();
                onToggleExpand?.();
              }}
