...
export const ProductCategoriesEditor: React.FC<ProductCategoriesEditorProps> = ({
  ...
  const loadAvailableCategories = async () => {
    ...
    console.log('بدء تحميل الفئات للمؤسسة:', organizationId)
    setCategoriesLoading(true)
    setCategoriesError(null)
    
    try {
      const categories = await getCategories(organizationId)
      console.log('تم تحميل الفئات:', categories?.length || 0)
      
      // تشخيص الصور
      if (categories && categories.length > 0) {
        console.log('عينة من الفئات مع صورها:', categories.slice(0, 3).map(cat => ({
          name: cat.name,
  ...
  const addCategoryToSelection = (category: Category) => {
    if (!selectedCategories.find(c => c.id === category.id)) {
      ...
      console.log('🔄 تحديث الفئات المحددة:', categoryIds)
              ...
              {hasImage ? (
                <img
                  ...
                  onError={(e) => {
                    ...
                    console.warn('فشل تحميل صورة الفئة:', {
        ...
        <div className="space-y-4">
          <div>
            ...
            <Select
              ...
              onValueChange={(value) => {
                console.log('🔄 تغيير طريقة الاختيار إلى:', value)
