  ...
  try {
    ...
    if (!authData?.user) {
      ...
      try {
        ...
        console.error('Error reading alternative storage:', altStorageError);
      }
    }
    ...
    if (!authData?.user) {
      console.log('🔄 استخدام النظام المحسن للمصادقة...');
    ...
    console.log(`⏱️ Auth data retrieved in ${authDuration}ms`);
    ...
    try {
      ...
      console.log('🔍 getCurrentUserProfile - Auth User ID:', authData.user.id);
      console.log('📊 User query result:', { userById, idError });
      
      if (!idError && userById) {
        userProfile = userById;
        console.log('✅ Found user profile:', userById.email, 'User ID:', userById.id);
        
        // محاولة جلب بيانات وكيل مركز الاتصال إذا كان موجوداً
        try {
          console.log('🔍 Searching for call center agent with user_id:', userById.id);
          ...
          console.log('📊 Agent query result in getCurrentUserProfile:', { agentData, agentError });
          
          if (agentData) {
            console.log('✅ Found agent data in getCurrentUserProfile, Agent ID:', agentData.id);
            userProfile = {
              ...userProfile,
            ...
            console.log('❌ No agent data found in getCurrentUserProfile');
          }
        } catch (agentError) {
          console.error('💥 Error fetching agent data in getCurrentUserProfile:', agentError);
        }
      }
    } catch (error) {
    ...
    console.error('General error in getCurrentUserProfile:', error);
    ...
    try {
      ...
      console.error('Error getting session in fallback:', sessionError);
    }
...
export const getCurrentUserProfileWithAgent = async (): Promise<User | null> => {
  try {
    ...
    if (error && error.code !== 'PGRST116') {
      console.error('Error fetching user with agent data:', error);
    ...
    if (userData && userData.call_center_agents && userData.call_center_agents.length > 0) {
      ...
      if (activeAgents.length > 0) {
        ...
        console.log('User with call center agent data:', {
          userId: userWithAgent.id,
          email: userWithAgent.email,
          role: userWithAgent.role,
          call_center_agent_id: userWithAgent.call_center_agent_id
        });
    ...
    console.log('User is not a call center agent, using regular profile');
    return await getCurrentUserProfile();

  } catch (error) {
    console.error('Error in getCurrentUserProfileWithAgent:', error);
    // fallback إلى الطريقة العادية
    return await getCurrentUserProfile();
  }
};
