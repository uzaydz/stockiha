...
export class EcotrackShippingService extends BaseShippingService {
  ...
  async createShippingOrder(params: CreateOrderParams): Promise<any> {
    ...
    try {
      console.log('🚀 [EcotrackAPI] Sending request:', {
        url: `${this.baseUrl}/api/v1/create/order`,
        body: requestBody,
      ...
      console.log('✅ [EcotrackAPI] Response received:', {
      ...
      console.error('❌ [EcotrackAPI] Error details:', {
      ...
      if (error.response?.data) {
        console.error('❌ [EcotrackAPI] Detailed API error response:', error.response.data);
      }
