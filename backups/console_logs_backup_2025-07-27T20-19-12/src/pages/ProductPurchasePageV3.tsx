...
const logProductPerformanceIssue = (type: string, data: any) => {
  ...
  console.warn(`🚨 [PRODUCT PAGE PERFORMANCE] ${type}:`, data);
...
const logProductPageEvent = (event: string, data: any) => {
  ...
  console.log(`📱 [PRODUCT PAGE] ${event}:`, {
    ...
    metrics: {
      ...
      warnings: PRODUCT_PERFORMANCE_METRICS.warnings.length
    }
  });
};
...
const ProductPurchasePageV3: React.FC = React.memo(() => {
  ...
  // 🔧 إصلاح: استخدام useSharedStoreData للحصول على الإعدادات بسرعة أكبر
  const { organizationSettings: sharedOrgSettings } = useSharedStoreData();
  ...
  const organizationSettings = useMemo(() => 
    sharedOrgSettings || organizationSettingsFromProduct, 
    [sharedOrgSettings, organizationSettingsFromProduct]
  );
  ...
  const [state, actions] = useProductPurchase(stableParams);
  ...
  const {
    ...
    productId: product?.id || null, // استخدام معرف المنتج الحقيقي من البيانات المُحملة
    organizationId: organizationId,
    enabled: true,
    batchInterval: 10 // إرسال البيانات كل 10 ثوانِ (للاختبار)
  });
  ...
  const handleFormSubmit = useCallback(async (data: Record<string, any>) => {
    try {
      ...
      console.log('🚀 [ProductPurchasePageV3] بدء معالجة الطلبية...', {
        organizationId,
        productId: product.id,
        selectedColor: selectedColor ? { id: selectedColor.id, name: selectedColor.name } : null,
        selectedSize: selectedSize ? { id: selectedSize.id, name: selectedSize.size_name } : null,
        quantity,
        priceInfo,
      ...
      console.log('🚀 [ProductPurchasePageV3] إرسال الطلبية:', { organizationId, orderPayload });
      const result = await processOrder(organizationId, orderPayload);
      console.log('✅ [ProductPurchasePageV3] نتيجة الطلبية:', result);
