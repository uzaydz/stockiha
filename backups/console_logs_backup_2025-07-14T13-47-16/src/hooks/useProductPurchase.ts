...
export const useProductPurchase = ({
  ...
  const fetchProduct = useCallback(async () => {
    ...
    try {
      ...
      console.log('🔄 useProductPurchase: بدء جلب بيانات المنتج', {
        productId,
        organizationId,
        dataScope
      });
      ...
      if (!response) {
        console.error('❌ useProductPurchase: لم يتم إرجاع استجابة');
        throw new Error('فشل في الاتصال بالخادم');
      }

      if (!response.success) {
        console.error('❌ useProductPurchase: فشل في جلب البيانات', {
      ...
      if (!productData) {
        console.error('❌ useProductPurchase: بيانات المنتج فارغة');
        throw new Error('بيانات المنتج غير صحيحة');
      }

      console.log('✅ useProductPurchase: تم جلب بيانات المنتج بنجاح', {
      ...
      console.error('❌ useProductPurchase: خطأ في جلب المنتج', {
        error: err,
        productId,
        organizationId
      });
