...
export async function createPOSOrder(orderData: POSOrderData): Promise<POSOrderResult> {
  ...
  try {
    ...
    const cleanedParams = {
      ...
      p_amount_paid: orderData.amountPaid !== undefined ? orderData.amountPaid : orderData.total,
...
async function updateInventoryForOrder(items: OrderItem[], orderId?: string, organizationId?: string) {
  ...
  for (const item of items) {
    ...
    try {
      ...
      const { data: fifoResult, error: fifoError } = await supabase.rpc('process_pos_sale_with_variants_fifo_optimized' as any, {
      ...
      if (fifoError || !(fifoResult as any)?.success) {
        
        // العودة للطريقة البسيطة عند فشل FIFO
        const { error: simpleError } = await supabase
          .from('products')
          .update({ 
            stock_quantity: (supabase as any).sql`GREATEST(0, stock_quantity - ${item.quantity})`,
        ...
        logStockUpdate(
          item.productId,
          item.productName || 'منتج',
          (fifoResult as any).previous_stock || 0,
          (fifoResult as any).new_stock || 0,
          ...
          item.variant_info?.sizeId,
          {
            ...
            orderId,
            quantity: -item.quantity,
            totalCost: (fifoResult as any).total_cost,
            averageCost: (fifoResult as any).average_cost_per_unit,
            variantInfo: item.variant_info,
            hasVariants: (fifoResult as any).has_variants,
            processingOptimized: (fifoResult as any).processing_optimized
          }
        );
      }
    } catch (error) {
