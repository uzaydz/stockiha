...
const fetchPOSProductsWithVariants = async (orgId: string): Promise<POSProductWithVariants[]> => {
  return deduplicateRequest(`pos-products-enhanced-${orgId}`, async () => {
    ...
    try {
      ...
      try {
        // ⚠️ لا نحفظ المنتجات هنا!
        // السبب: RPC get_pos_products_optimized ترجع فقط 50 منتج (p_limit: 50)
        // بينما TauriSyncService.syncProductsToSQLite() تجلب وتحفظ جميع المنتجات
        // حفظ المنتجات هنا يسبب فقدان المنتجات الأخرى
        console.log(`[POSDataContext] ℹ️ تخطي حفظ ${allProducts.length} منتج - TauriSyncService يقوم بذلك`);
...
const fetchPOSOrderStats = async (orgId: string): Promise<any> => {
  if (isOfflineMode()) {
    ...
    try {
      // ⚡ استخدام Delta Sync
      localOrders = await deltaWriteService.getAll<LocalPOSOrder>('orders', orgId);
    } catch (error) {
      localOrders = [];
    }
...
export const POSDataProvider: React.FC<POSDataProviderProps> = ({ children }) => {
  ...
  // ⚡ محسّنة: تحدث الكاش + SQLite + ترسل أحداث التحديث
  // ⚡ تدعم الآن المخزون المتقدم (وزن/متر/صندوق)
  const updateProductStockInCache = useCallback(async (
    productId: string,
    colorId: string | null,
    sizeId: string | null,
    ...
    console.log('[POSDataContext] 📦 تحديث المخزون:', { productId, colorId, sizeId, quantityChange, advancedStockUpdate });

    // 1️⃣ تحديث pos-products-enhanced cache (للتوافق مع الكود القديم)
    queryClient.setQueryData(['pos-products-enhanced', orgId], (oldData: POSProductWithVariants[] | undefined) => {
      ...
      const updatedData = oldData.map(product => {
        ...
        // ⚡ تحديث المخزون المتقدم (وزن/متر/صندوق)
        if (advancedStockUpdate) {
          switch (advancedStockUpdate.type) {
            case 'weight':
              if (advancedStockUpdate.weightChange !== undefined) {
                const currentWeight = (updatedProduct as any).available_weight || 0;
                const newWeight = Math.max(0, currentWeight + advancedStockUpdate.weightChange);
                (updatedProduct as any).available_weight = newWeight;
                console.log('[POSDataContext] ⚖️ تحديث الوزن:', { currentWeight, change: advancedStockUpdate.weightChange, newWeight });
              }
              break;
            case 'meter':
              if (advancedStockUpdate.lengthChange !== undefined) {
                const currentLength = (updatedProduct as any).available_length || 0;
                const newLength = Math.max(0, currentLength + advancedStockUpdate.lengthChange);
                (updatedProduct as any).available_length = newLength;
                console.log('[POSDataContext] 📏 تحديث الأمتار:', { currentLength, change: advancedStockUpdate.lengthChange, newLength });
              }
              break;
            case 'box':
              if (advancedStockUpdate.boxChange !== undefined) {
                const currentBoxes = (updatedProduct as any).available_boxes || 0;
                const newBoxes = Math.max(0, currentBoxes + advancedStockUpdate.boxChange);
                (updatedProduct as any).available_boxes = newBoxes;
                console.log('[POSDataContext] 📦 تحديث الصناديق:', { currentBoxes, change: advancedStockUpdate.boxChange, newBoxes });
              }
              break;
          }
        }
    ...
    // 2️⃣ تحديث SQLite مباشرة عبر DeltaWriteService
    try {
      if (orgId) {
        // استخدام updateStockWithVariants لتحديث المخزون في SQLite + Outbox
        await deltaWriteService.updateStockWithVariants(
          productId,
          colorId,
          sizeId,
          quantityChange
        );
        console.log('[POSDataContext] ✅ تم تحديث SQLite بنجاح');
      }
    } catch (sqliteError) {
      console.error('[POSDataContext] ❌ خطأ في تحديث SQLite:', sqliteError);
      // نستمر حتى لو فشل SQLite - الكاش محدث على الأقل
    }

    // 3️⃣ إبطال local-pos-products cache (الذي يستخدمه useLocalPOSProducts)
    queryClient.invalidateQueries({
      queryKey: ['local-pos-products'],
      refetchType: 'all' // إعادة الجلب من SQLite
    });

    // 4️⃣ إجبار React Query على إعادة التقييم والـ re-render فوراً
    queryClient.invalidateQueries({
      queryKey: ['pos-products-enhanced', orgId],
      exact: true,
    ...
    // 5️⃣ إرسال حدث products-updated لتحديث جميع المستمعين
    console.log('🔍 [POSDataContext] 📢 إرسال حدث products-updated');
    window.dispatchEvent(new CustomEvent('products-updated', {
      detail: { productId, colorId, sizeId, quantityChange, source: 'pos-sale' }
    }));

    // 6️⃣ إجبار re-render إضافي للتأكد من تحديث الواجهة مع تأخير قصير
    setTimeout(() => {
      console.log('🔍 [POSDataContext] ⏱️ setTimeout: إعادة إبطال الكاش بعد 100ms');
      queryClient.invalidateQueries({
        queryKey: ['pos-products-enhanced', orgId],
        exact: true,
        refetchType: 'none'
