-- إصلاح مشكلة slug في دالة process_online_order
...
BEGIN
  ...
  -- 5. الحصول على اسم وslug المنتج
  SELECT name, slug INTO v_product_name, v_product_slug 
  FROM products 
  WHERE id = p_product_id;
  
  -- التأكد من وجود slug، إذا لم يكن موجوداً، استخدم قيمة افتراضية
  ...
  -- 6. إنشاء عنصر الطلب مع جميع الحقول المطلوبة
  INSERT INTO online_order_items (
    order_id,
    product_id,
    product_name,
    name, -- حقل name المطلوب
    slug, -- حقل slug المطلوب
    quantity,
    unit_price,
    total_price,
    organization_id,
  ...
  VALUES (
    ...
    p_product_id,
    v_product_name,
    v_product_name, -- استخدام اسم المنتج لحقل name
    v_product_slug, -- استخدام slug المنتج
    ...
    p_product_color_id,
    v_color_name,
    NULL, -- size_id (لم يتم تمريره في المعاملات الحالية)
    NULL, -- size_name (لم يتم تمريره في المعاملات الحالية)
    COALESCE(p_unit_price, 0)
  )
