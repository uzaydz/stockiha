...
const POSAdvanced = () => {
  ...
  console.log('🔄 [POS Advanced] Data source:', {
    useCompletePOSData: products?.length || 0,
    shouldUsePOSDataContext,
    isLoading,
    hasError: !!error
  });
  ...
  const handleSubmitOrder = useCallback(async (
    ...
    try {
      ...
      if (paidAmount < finalTotal && paidAmount > 0 && discountAmount === 0) {
        ...
        console.log('🎯 [POSAdvanced] تطبيق تخفيض تلقائي:', {
          originalTotal: finalTotal,
          paidAmount,
          automaticDiscount: actualDiscountAmount
        });
      }
      ...
      console.log('🔍 [DEBUG-6] تتبع customer_order_number في POSAdvanced بعد استقبال النتيجة من submitOrder:', {
        resultExists: !!result,
        resultCustomerOrderNumber: result?.customerOrderNumber,
        resultOrderId: result?.orderId,
        resultSlug: result?.slug,
        timestamp: new Date().toISOString(),
        location: 'POSAdvanced.handleSubmitOrder - after submitOrder'
      });
