...
export const AppsProvider: React.FC<AppsProviderProps> = ({ children }) => {
  ...
  const fetchOrganizationApps = async () => {
    ...
    try {
      ...
      try {
        ...
        if (error) {
          console.warn('organization_apps table not found, using mock data:', error);
          throw error;
        }
        ...
        // استخدام بيانات وهمية في حالة عدم وجود الجدول
        console.warn('Using mock organization apps data');
      ...
      console.error('Error in fetchOrganizationApps:', error);
      setOrganizationApps([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const enableApp = async (appId: string): Promise<boolean> => {
    ...
    console.log('Enabling app:', { appId, organizationId });
    ...
    try {
      ...
      try {
        ...
        console.warn('Permission check failed, continuing anyway:', permissionError);
      ...
      try {
        ...
        if (error) {
          console.warn('Database operation failed, using local state:', error);
        ...
        console.warn('Database not available, using local state only');
        // تحديث الحالة المحلية فقط
        const newApp: OrganizationApp = {
          id: Date.now().toString(),
          organization_id: organizationId,
...
export const useApps = (): AppsContextType => {
  ...
  if (context === undefined) {
    ...
    console.warn('useApps called outside of AppsProvider, using fallback values');
    return {
      availableApps: [],
      organizationApps: [],
      isLoading: false,
