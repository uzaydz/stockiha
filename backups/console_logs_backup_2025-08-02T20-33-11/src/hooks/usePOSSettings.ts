...
export const usePOSSettings = ({ organizationId }: UsePOSSettingsProps): UsePOSSettingsReturn => {
  ...
  const {
    ...
    queryFn: async (): Promise<POSSettingsResponse> => {
      ...
      try {
        console.log('🔍 محاولة جلب إعدادات POS للمؤسسة:', currentOrganization.id);
        ...
        console.log('📊 نتيجة استدعاء get_pos_settings:', { data, error });

        if (error) {
          console.error('❌ خطأ في استدعاء get_pos_settings:', error);
          throw new Error(`خطأ في جلب إعدادات POS: ${error.message}`);
        }

        if (!data) {
          console.warn('⚠️ لم يتم إرجاع أي بيانات من get_pos_settings');
          throw new Error('لم يتم إرجاع أي إعدادات من الخادم');
        }

        const responseData = Array.isArray(data) ? data[0] : data;
        console.log('📋 البيانات المعالجة:', responseData);
        ...
        console.log('🔍 usePOSSettings - returning result:', result);
        return result;

      } catch (error) {
        // إذا فشل كل شيء، استخدم الإعدادات الافتراضية
        console.warn('فشل في جلب إعدادات POS، استخدام الإعدادات الافتراضية:', error);
        return {
          success: true,
    ...
    placeholderData: (previousData) => {
      console.log('🔍 usePOSSettings - placeholderData called with:', previousData);
      return previousData;
    },
    networkMode: 'online',
    meta: {
  ...
  console.log('🔍 usePOSSettings - typedResponse:', typedResponse);
  console.log('🔍 usePOSSettings - settings from response:', settings);
  ...
  console.log('🔍 usePOSSettings - final return:', {
  ...
  const saveSettings = useCallback(async () => {
    ...
    try {
      ...
      console.error('خطأ في حفظ إعدادات POS:', error);
      toast({
        title: "خطأ",
