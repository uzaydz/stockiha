...
export function clearDeliveryCache() {
  settingsCache.clear();
  priceCache.clear();
  console.log('🗑️ تم مسح جميع cache التوصيل');
}
...
export function clearSpecificPriceCache(
  ...
  console.log('🗑️ مسح cache محدد:', { priceCacheKey, deleted });
}
...
export async function calculateDeliveryFeesOptimized(
  ...
  try {
    ...
    console.log('🚚 بدء حساب رسوم التوصيل المحسن (شامل جميع الشركات):', input);
    ...
    if (input.selectedProvinceId) {
      ...
      console.log('📍 الولاية المختارة:', result.selectedProvince);
    }
    ...
    if (input.selectedMunicipalityId && input.selectedProvinceId) {
      ...
      console.log('🏘️ البلدية المختارة:', result.selectedMunicipality);
    }
    ...
    if (!result.selectedProvince || !result.selectedMunicipality) {
      ...
      console.log('⚠️ بيانات ناقصة - سعر تقديري:', result);
      return result;
    }

    // 🆕 التحقق من معلومات الشحن للمنتج أولاً
    if (input.productShippingInfo) {
      console.log('🎯 استخدام معلومات الشحن من المنتج:', input.productShippingInfo);
      ...
      if (input.productShippingInfo.type === 'clone' && input.productShippingInfo.unified_price) {
        ...
        if (clonePrice && clonePrice > 0) {
          ...
          console.log('✅ استخدام سعر موحد من Clone:', clonePrice);
          result.isLoading = false;
          return result;
        }
      }
      ...
      console.log('🏢 شركة التوصيل المحددة:', providerCode);
      ...
      if (providerCode === 'yalidine') {
        ...
        console.log('⚠️ شركة توصيل غير مدعومة، استخدام الأسعار المحلية');
    ...
    console.log('🔄 لا توجد معلومات شحن للمنتج، استخدام ياليدين افتراضياً...');
    return await calculateYalidineDelivery(input, result);

  } catch (error) {
    console.error('❌ خطأ في حساب رسوم التوصيل المحسن:', error);
...
async function calculateYalidineDelivery(
  ...
  try {
    ...
    if (cachedSettings && (Date.now() - cachedSettings.timestamp) < CACHE_DURATION) {
      console.log('📦 استخدام إعدادات ياليدين من الـ Cache');
      settingsResult = cachedSettings.data;
    } else {
      console.log('🚀 جلب إعدادات ياليدين جديدة من RPC...');
      ...
      if (settingsResult && settingsResult.success) {
        ...
        console.log('💾 تم حفظ إعدادات ياليدين في الـ Cache');
      }
    }
    ...
    if (settingsResult && settingsResult.success && settingsResult.data) {
      ...
      if (cachedPrice && (Date.now() - cachedPrice.timestamp) < CACHE_DURATION) {
        console.log('📦 استخدام سعر ياليدين من الـ Cache:', cachedPrice.price);
        yalidinePrice = cachedPrice.price;
      } else {
        console.log('✅ نجح RPC إعدادات ياليدين، جلب سعر جديد من API...');
        ...
        if (yalidinePrice !== null) {
          ...
          console.log('💾 تم حفظ سعر ياليدين في الـ Cache');
        }
      }
      ...
      if (yalidinePrice !== null) {
        ...
        console.log('✅ حصلنا على سعر من API ياليدين:', yalidinePrice);
        return result;
      }
    }
    ...
    console.log('⚠️ فشل حساب ياليدين، استخدام أسعار محلية:', error);
    return await calculateFallbackDelivery(input, result);
  }
}
...
async function calculateZRExpressDelivery(
  ...
  try {
    console.log('🚛 حساب رسوم ZR Express...');
    ...
    if (!error && data && data.success && typeof data.price === 'number') {
      ...
      console.log('✅ حصلنا على سعر من API ZR Express:', data.price);
      return result;
    }
    
    // fallback: جدول zr_express_tarification
    console.log('🔄 فشل Edge Function، استخدام جدول ZR Express...');
    ...
    if (!dbError && zrData) {
      ...
      if (price && price > 0) {
        ...
        console.log('✅ حصلنا على سعر من جدول ZR Express:', price);
        return result;
      }
    }
    ...
    console.log('⚠️ فشل حساب ZR Express:', error);
    return await calculateFallbackDelivery(input, result);
  }
}
...
async function calculateEcotrackDelivery(
  ...
  try {
    console.log('🌿 حساب رسوم EcoTrack:', providerCode);
    ...
    if (response.ok) {
      ...
      if (data && data.livraison && Array.isArray(data.livraison) && data.livraison.length > 0) {
        ...
        if (price > 0) {
          ...
          console.log('✅ حصلنا على سعر من API EcoTrack:', price);
          return result;
        }
      }
      ...
      if (data.success && data.data && data.data.length > 0) {
        ...
        if (price > 0) {
          ...
          console.log('✅ حصلنا على سعر من API EcoTrack (بنية قديمة):', price);
          return result;
        }
      }
    }
    ...
    console.log('⚠️ فشل حساب EcoTrack:', error);
    return await calculateFallbackDelivery(input, result);
  }
}
...
async function calculateCustomDelivery(
  ...
  try {
    console.log('🎯 حساب رسوم التوصيل المخصصة...');
    ...
    if (price >= 0) { // يمكن أن يكون السعر 0 (مجاني)
      ...
      console.log('✅ حصلنا على سعر من النظام المخصص:', price);
      return result;
    }
    ...
    console.log('⚠️ فشل حساب التوصيل المخصص:', error);
    return await calculateFallbackDelivery(input, result);
  }
}
...
async function calculateFallbackDelivery(
  ...
  console.log('🔄 استخدام النظام الاحتياطي للأسعار...');
...
export function testDeliveryData() {
  console.log('🧪 اختبار بيانات التوصيل:');
  console.log('- عدد الولايات:', yalidineProvinces.length);
  console.log('- عدد البلديات:', yalidineMunicipalities.length);
  console.log('- الولاية الأولى:', yalidineProvinces[0]);
  console.log('- البلدية الأولى:', yalidineMunicipalities[0]);
  
  // اختبار حقول البحث
  const testProvince = getProvinceById(16); // الجزائر
  const testMunicipalities = getMunicipalitiesByWilayaId(16);
  console.log('- ولاية الجزائر:', testProvince);
  console.log('- بلديات الجزائر:', testMunicipalities.length);
