...
async function fetchYalidineFeesFromAPIOptimized(
  ...
  try {
    ...
    if (cachedData) {
      console.log('⚡ [YalidineCache] تم جلب البيانات من الكاش:', {
        fromWilaya: fromWilayaId,
        toWilaya: toWilayaId,
        cacheStats: yalidineCache.getStats()
      });
      return cachedData;
    }

    console.log('🌐 [YalidineAPI] استدعاء API جديد - لا توجد بيانات في الكاش:', {
      fromWilaya: fromWilayaId,
      toWilaya: toWilayaId
    });
    ...
    try {
      ...
      console.log('📦 [YalidineAPI] البيانات الخام المُستلمة:', {
        hasData: !!rawData,
        dataType: typeof rawData,
        keys: typeof rawData === 'object' ? Object.keys(rawData) : [],
        sampleData: JSON.stringify(rawData).slice(0, 500) + '...'
      });
    } catch (jsonError) {
      console.error('❌ [YalidineAPI] خطأ في تحليل JSON من proxy:', {
    ...
    if (!actualData || Object.keys(actualData).length === 0 || !actualData.per_commune) {
      console.log('⚠️ [YalidineAPI] استجابة غير صالحة:', {
        hasRawData: !!rawData,
        hasKeys: rawData ? Object.keys(rawData).length > 0 : false,
        hasActualData: !!actualData,
        hasPerCommune: actualData ? !!actualData.per_commune : false,
        rawDataStructure: rawData ? Object.keys(rawData) : [],
        actualDataStructure: actualData ? Object.keys(actualData) : []
      });
      return null;
    }
    ...
    console.log('💾 [YalidineCache] تم حفظ البيانات في الكاش:', {
      fromWilaya: fromWilayaId,
      toWilaya: toWilayaId,
      cacheSize: yalidineCache.getStats().size
    });

    return processedData;

  } catch (error) {
    console.error('❌ [YalidineAPI] خطأ في استدعاء API:', error);
    return null;
  }
}
...
async function fetchYalidineFeesFromAPI(
  ...
  try {
    ...
    const cachedData = yalidineCache.get(organizationId, fromWilayaId, toWilayaId);
    if (cachedData) {
      console.log('⚡ [YalidineCache-Legacy] تم جلب البيانات من الكاش');
      return cachedData;
    }
    ...
    if (!response.ok) {
      console.error('❌ [YalidineAPI-Legacy] HTTP Error:', {
        status: response.status,
        statusText: response.statusText,
        url: response.url
      });
    ...
    try {
      ...
      console.error('❌ [YalidineAPI-Legacy] خطأ في تحليل JSON:', {
        status: response.status,
        statusText: response.statusText,
        url: response.url,
        jsonError: jsonError.message
    ...
    if (!actualData || Object.keys(actualData).length === 0 || !actualData.per_commune) {
      console.log('⚠️ [YalidineAPI-Legacy] استجابة غير صالحة:', {
        hasRawData: !!rawData,
        hasActualData: !!actualData,
        hasPerCommune: actualData ? !!actualData.per_commune : false
      });
...
export async function calculateDeliveryPrice(
  ...
  try {
    ...
    if (!apiData) {
      ...
      console.error('❌ [YalidineAPI] فشل في جلب البيانات من API ياليدين:', {
        organizationId,
        originWilayaId,
        toWilayaIdNum,
        toCommuneId,
      ...
      console.warn('⚠️ [YalidineAPI] استخدام السعر الافتراضي:', quickFallbackPrice);
      return quickFallbackPrice;
    }

    // تحليل استجابة API ياليدين الحقيقية (format جديد)
    console.log('🔍 [YalidineAPI] تحليل البيانات المُستلمة:', {
      hasApiData: !!apiData,
    ...
    if (apiData && apiData.success && apiData.data) {
      ...
      console.log('🏘️ [YalidineAPI] بحث عن بيانات البلدية:', {
        requestedCommuneId: toCommuneId,
        hasPerCommune: !!apiData.data.per_commune,
        availableCommuneIds: apiData.data.per_commune ? Object.keys(apiData.data.per_commune) : [],
        communeCount: apiData.data.per_commune ? Object.keys(apiData.data.per_commune).length : 0
      });
      
      if (apiData.data.per_commune && apiData.data.per_commune[toCommuneId]) {
        usedCommuneData = apiData.data.per_commune[toCommuneId];
        console.log('✅ [YalidineAPI] تم العثور على بيانات البلدية المطلوبة:', usedCommuneData);
      } else {
        ...
        if (communeEntries.length > 0) {
          ...
          console.log(`⚠️ [YalidineAPI] البلدية ${toCommuneId} غير متاحة، استخدام البلدية ${firstCommuneId}:`, usedCommuneData);
        } else {
          console.error('❌ [YalidineAPI] لا توجد بيانات بلديات متاحة');
        }
      }

      // استخراج السعر من بيانات البلدية
      console.log('💰 [YalidineAPI] استخراج الأسعار:', {
        hasUsedCommuneData: !!usedCommuneData,
        deliveryType,
        communeData: usedCommuneData,
        generalFees: apiData.data.fees
      });
      
      if (usedCommuneData) {
        if (deliveryType === 'home') {
          basePrice = usedCommuneData.express_home || 0;
          console.log(`🏠 [YalidineAPI] سعر التوصيل للمنزل: ${basePrice} دج`);
        } else if (deliveryType === 'desk') {
          basePrice = usedCommuneData.express_desk || 0;
          console.log(`🏢 [YalidineAPI] سعر التوصيل للمكتب: ${basePrice} دج`);
        }
      } else {
        // استخدام الأسعار العامة من البيانات المحولة كـ fallback أخير
        const fees = apiData.data.fees;
        console.log('⚠️ [YalidineAPI] استخدام الأسعار العامة:', fees);
      ...
      if (basePrice === 0) {
        ...
        console.error('❌ [YalidineAPI] السعر الأساسي = 0، استخدام السعر الافتراضي:', {
          deliveryType,
          usedCommuneData,
          apiDataFees: apiData.data.fees,
          toCommuneId,
          originWilayaId,
          toWilayaIdNum
        });
        const quickFallbackPrice = deliveryType === 'home' ? 650 : 450;
        console.warn(`⚠️ [YalidineAPI] السعر الافتراضي المُستخدم: ${quickFallbackPrice} دج`);
        return quickFallbackPrice;
      }
