...
export const getDefaultColor = (product: CompleteProduct): ProductColor | null => {
  ...
  if (defaultColor) {
    console.log('✅ [getDefaultColor] Found default color with stock:', {
      id: defaultColor.id,
      name: defaultColor.name,
      quantity: defaultColor.quantity,
  ...
  if (firstAvailableColor) {
    console.log('⚠️ [getDefaultColor] Default color out of stock, using first available:', {
      id: firstAvailableColor.id,
      name: firstAvailableColor.name,
      quantity: firstAvailableColor.quantity,
      isDefault: firstAvailableColor.is_default
    });
  ...
  if (originalDefault) {
    console.log('❌ [getDefaultColor] All colors out of stock, returning default (out of stock):', {
      id: originalDefault.id,
      name: originalDefault.name,
      quantity: originalDefault.quantity,
      isDefault: originalDefault.is_default
    });
  ...
  if (fallback) {
    console.log('⚠️ [getDefaultColor] No default found, using first color:', {
      id: fallback.id,
      name: fallback.name,
...
export const getDefaultSize = (color: ProductColor): ProductSize | null => {
  ...
  if (defaultSize) {
    console.log('✅ [getDefaultSize] Found default size with stock:', {
      id: defaultSize.id,
      name: defaultSize.size_name,
      quantity: defaultSize.quantity,
  ...
  if (firstAvailableSize) {
    console.log('⚠️ [getDefaultSize] Default size out of stock, using first available:', {
      id: firstAvailableSize.id,
      name: firstAvailableSize.size_name,
      quantity: firstAvailableSize.quantity,
      isDefault: firstAvailableSize.is_default
    });
  ...
  if (originalDefault) {
    console.log('❌ [getDefaultSize] All sizes out of stock, returning default (out of stock):', {
      id: originalDefault.id,
      name: originalDefault.size_name,
      quantity: originalDefault.quantity,
      isDefault: originalDefault.is_default
    });
    return originalDefault;
  ...
  if (fallback) {
    console.log('⚠️ [getDefaultSize] No default found, using first size:', {
      id: fallback.id,
      name: fallback.size_name,
      quantity: fallback.quantity,
      isDefault: fallback.is_default
    });
  }
...
export const getVariantPrice = (
  ...
  console.log('🔍 [getVariantPrice] Starting calculation with:', {
  ...
  if (colorId) {
    ...
    console.log('🎨 [getVariantPrice] Looking for color:', { colorId, foundColor: !!color, colorDetails: color });
    
    if (color) {
      if (sizeId && color.sizes && color.sizes.length > 0) {
        const size = color.sizes.find(s => s.id === sizeId);
        console.log('📏 [getVariantPrice] Looking for size:', { 
          sizeId, 
          foundSize: !!size, 
          sizeDetails: size,
          hasSizePrice: size?.price !== null && size?.price !== undefined,
          sizePriceValid: size?.price > 0 
        });
        
        if (size && size.price !== null && size.price !== undefined && size.price > 0) {
          console.log('✅ [getVariantPrice] Using size price (forced):', size.price);
          return size.price;
        }
      }
      
      console.log('🎨 [getVariantPrice] Checking color price:', {
        colorPrice: color.price,
        hasColorPrice: color.price !== null && color.price !== undefined,
        colorPriceValid: color.price > 0,
        useVariantPrices: product.variants.use_variant_prices
      });
      
      // استخدام سعر اللون حتى لو لم يكن use_variant_prices مفعل
      if (color.price !== null && color.price !== undefined && color.price > 0) {
        console.log('✅ [getVariantPrice] Using color price (forced):', color.price);
        return color.price;
      }
    }
  }
  ...
  if (!product.variants.use_variant_prices) {
    console.log('✅ [getVariantPrice] No variant prices used and no valid variant found, returning base price:', product.pricing.price);
    return product.pricing.price;
  }

  console.log('⚠️ [getVariantPrice] No valid variant price found, using base price:', product.pricing.price);
  return product.pricing.price;
};
...
export const getFinalPrice = (
  ...
  console.log('💰 [getFinalPrice] Starting final price calculation:', {
    productId: product.id,
  ...
  console.log('💰 [getFinalPrice] Base price from getVariantPrice:', basePrice);
  
  const wholesalePrice = getWholesalePrice(product, safeQuantity);
  console.log('💰 [getFinalPrice] Wholesale price:', wholesalePrice);
  ...
  if (wholesalePrice !== null) {
    ...
    console.log('💰 [getFinalPrice] Using wholesale price:', finalPrice);
  } else {
    console.log('💰 [getFinalPrice] Using base price:', finalPrice);
  }

  const totalPrice = finalPrice * safeQuantity;
  const originalTotalPrice = basePrice * safeQuantity;
  
  console.log('💰 [getFinalPrice] Final calculations:', {
    finalPrice,
    originalQuantity: quantity,
    safeQuantity,
    totalPrice,
  ...
  console.log('✅ [getFinalPrice] Final result:', result);
  return result;
};
