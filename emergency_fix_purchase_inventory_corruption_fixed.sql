-- إصلاح طارئ لفساد المخزون من التصحيحات الخاطئة - النسخة المُصححة
...
BEGIN
    ...
    LOOP
        target_restored_stock := corruption_rec.should_be_stock;
        
        -- إصلاح المخزون
        UPDATE products 
        SET stock_quantity = target_restored_stock,
            updated_at = NOW()
        ...
        INSERT INTO inventory_log (
            ...
            target_restored_stock - corruption_rec.current_stock,
            corruption_rec.current_stock,
            target_restored_stock,
            'adjustment',
            'emergency_auto_fix',
            null,
            format('إصلاح طارئ تلقائي - استعادة من %s إلى %s (حذف سجل فاسد)', 
                   corruption_rec.current_stock, target_restored_stock),
        ...
        -- إرجاع النتيجة
        action := 'تم الإصلاح';
        org_name := corruption_rec.org_name;
        product_name := corruption_rec.product_name;
        old_stock := corruption_rec.current_stock;
        restored_stock := target_restored_stock;
        notes := format('استعادة من مخزون فاسد %s إلى %s', corruption_rec.current_stock, target_restored_stock);
...
-- خلاصة التحليل والتوصيات
SELECT 
    '=== خلاصة التحليل والتوصيات ===' as summary_title;

SELECT 
    'التحليل والتوصيات:' as section,
    'تم إصلاح المشكلة الأساسية في trigger المشتريات وإزالة الدوال الخطيرة' as main_fix,
    'يُنصح بمراقبة النظام لمدة 24 ساعة للتأكد من عدم تكرار المشكلة' as recommendation,
    'تم إنشاء دالة safe_inventory_monitor آمنة للمراقبة بدون تدخل تلقائي' as safety_measure;
