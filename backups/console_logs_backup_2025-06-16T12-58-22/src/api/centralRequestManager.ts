...
import { withCache, LONG_CACHE_TTL, DEFAULT_CACHE_TTL, SHORT_CACHE_TTL } from '@/lib/cache/storeCache';
...
export class CentralRequestManager {
  private static instance: CentralRequestManager;

  static getInstance(): CentralRequestManager {
    if (!this.instance) {
  ...
  private async executeRequest<T>(
    ...
    if (activeRequests.has(key)) {
      console.log(`🔄 انتظار طلب جاري: ${key}`);
      return activeRequests.get(key)!;
    }

    // إنشاء طلب جديد مع cache
    const requestPromise = withCache<T>(key, requestFn, ttl);
  ...
  async getOrganizationBySubdomain(subdomain: string): Promise<any> {
    ...
    return this.executeRequest(
      key,
      async () => {
        console.log(`🔍 جلب بيانات المنظمة: ${subdomain}`);
  ...
  async getOrganizationSettings(organizationId: string): Promise<any> {
    ...
    return this.executeRequest(
      key,
      async () => {
        console.log(`🔍 جلب إعدادات المنظمة: ${organizationId}`);
        ...
        if (error) {
          console.warn('⚠️ خطأ في جلب إعدادات المنظمة:', error);
          return null;
        }
  ...
  async getProductCategories(organizationId: string): Promise<any[]> {
    ...
    return this.executeRequest(
      key,
      async () => {
        console.log(`🔍 جلب فئات المنتجات: ${organizationId}`);
        ...
        if (error) {
          console.warn('⚠️ خطأ في جلب فئات المنتجات:', error);
          return [];
        }
  ...
  async getProductSubcategories(organizationId: string): Promise<any[]> {
    ...
    return this.executeRequest(
      key,
      async () => {
        console.log(`🔍 جلب الفئات الفرعية: ${organizationId}`);
        ...
        if (error) {
          console.warn('⚠️ خطأ في جلب الفئات الفرعية:', error);
          return [];
        }
  ...
  async getFeaturedProducts(organizationId: string): Promise<any[]> {
    ...
    return this.executeRequest(
      key,
      async () => {
        console.log(`🔍 جلب المنتجات المميزة: ${organizationId}`);
        ...
        if (error) {
          console.warn('⚠️ خطأ في جلب المنتجات المميزة:', error);
          return [];
        }
  ...
  async getStoreSettings(organizationId: string): Promise<any[]> {
    ...
    return this.executeRequest(
      key,
      async () => {
        console.log(`🔍 جلب إعدادات المتجر: ${organizationId}`);
        ...
        if (error) {
          console.warn('⚠️ خطأ في جلب إعدادات المتجر:', error);
          return [];
        }
  ...
  async getOrganizationApps(organizationId: string): Promise<any[]> {
    ...
    return this.executeRequest(
      key,
      async () => {
        console.log(`🔍 جلب تطبيقات المنظمة: ${organizationId}`);
        ...
        if (error) {
          console.warn('⚠️ خطأ في جلب تطبيقات المنظمة:', error);
          return [];
        }
  ...
  async getStoreInitData(subdomain: string): Promise<any> {
    ...
    return this.executeRequest(
      key,
      async () => {
        console.log(`🔍 استدعاء get_store_init_data: ${subdomain}`);
  ...
  async getOrganizationTheme(organizationId: string): Promise<any> {
    ...
    return this.executeRequest(
      key,
      async () => {
        console.log(`🔍 استدعاء get_organization_theme: ${organizationId}`);
        ...
        if (error) {
          console.warn('⚠️ خطأ في get_organization_theme:', error);
          return null;
        }
  ...
  async getAllStoreData(subdomain: string): Promise<{
    ...
    console.log(`🚀 بدء جلب جميع بيانات المتجر: ${subdomain}`);
    ...
    console.log('📊 الإحصائيات:', {
      ...
      organizationApps: organizationApps.length
    });
