  ...
  useEffect(() => {
    ...
    const checkUserPermissionsLocally = async () => {
      try {
        ...
        console.warn('خطأ في فحص الصلاحيات، استخدام الفحص الاحتياطي:', error);
  ...
  useEffect(() => {
    if (product && open) {
      ...
      const loadProductDetails = async () => {
        try {
          console.log('بدء تحميل تفاصيل المنتج:', product.id);
          
          // تحميل الألوان
          console.log('🔍 EditProductDialog - قبل استدعاء getProductColors');
          const colors = await getProductColors(product.id);
          console.log('✅ EditProductDialog - تم تحميل الألوان:', colors);
          console.log('📊 EditProductDialog - عدد الألوان:', colors?.length || 0);
          
          setProductColors(colors);
          setOriginalProductColors(colors);
          
          console.log('🎯 EditProductDialog - تم تعيين الألوان في state');
