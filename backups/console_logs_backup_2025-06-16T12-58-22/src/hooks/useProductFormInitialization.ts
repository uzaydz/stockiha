...
export const useProductFormInitialization = ({
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      setIsLoading(true);
      try {
        console.log('🔍 ProductForm - بدء تحميل المنتج:', id);
        const fetchedProductData = await getProductById(id); // Returns Product | null
        
        if (fetchedProductData) {
          console.log('✅ ProductForm - تم تحميل المنتج:', fetchedProductData.name);
          
          // جلب الالوان منفصلاً
          console.log('🔍 ProductForm - بدء تحميل الألوان');
          const colors = await getProductColors(id);
          console.log('✅ ProductForm - تم تحميل الألوان:', colors);
          console.log('📊 ProductForm - عدد الألوان:', colors?.length || 0);
        ...
        console.error('❌ ProductForm - خطأ في تحميل المنتج:', error);
