...
const debugLog = (message: string, data?: any) => {
  if (process.env.NODE_ENV === 'development') {
    console.log(`🎨 [ThemeContext] ${message}`, data || '');
  }
};
...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children, initialOrganizationId }) => {
  ...
  useEffect(() => {
    ...
    // إعادة تطبيق الثيم الحالي عند تغيير المسار فقط إذا لم يكن لدينا ثيم مؤسسة
    debugLog('🔄 [ThemeContext] تطبيق الثيم عند التنقل:', theme);
    applyThemeToDOM(theme);
  }, [location.pathname, theme, initialOrganizationId]);
