...
const StoreBannerOptimized = React.memo<StoreBannerOptimizedProps>(({ heroData, featuredProducts }) => {
  const [imageLoaded, setImageLoaded] = useState(false);
  const [isSmallScreen, setIsSmallScreen] = useState(false);

  // الكشف عن الموبايل مبكراً لتطبيق تخفيفات إضافية
  useEffect(() => {
    try {
      ...
      <div className="relative container mx-auto px-4 navbar-spacer-lg pb-8 sm:navbar-spacer-lg sm:pb-10 md:navbar-spacer md:pb-8 lg:py-12">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center min-h-[40vh] sm:min-h-[50vh] lg:min-h-[60vh]">
          ...
          <BannerImageLazy
            ...
            organizationId={heroData?.organization_id || localStorage.getItem('bazaar_organization_id')}
