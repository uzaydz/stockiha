...
function applyThemeToDOM(theme: Theme) {
  ...
  // استخدام themeManager بدلاً من الدالة المحلية لضمان تطبيق كامل للثيم
  import('@/lib/themeManager').then(({ updateOrganizationTheme, getCurrentTheme }) => {
    // التحقق من وجود ثيم المؤسسة المحفوظ أولاً
    ...
    try {
      ...
      if (storedOrgTheme) {
        ...
        if (orgTheme.organizationId) {
          // استخدام المود المحفوظ في orgTheme بدلاً من theme state
          const correctMode = orgTheme.mode || theme;
          
          debugLog('🔧 [ThemeContext-applyThemeToDOM] استخدام المود الصحيح:', { 
            themeFromState: theme, 
            themeFromStorage: orgTheme.mode, 
            finalMode: correctMode 
          });
          
          // تطبيق الثيم الكامل مع الألوان باستخدام updateOrganizationTheme
          updateOrganizationTheme(orgTheme.organizationId, {
            theme_primary_color: orgTheme.primaryColor || '#10b981',
            theme_secondary_color: orgTheme.secondaryColor || '#8b5cf6',
            theme_mode: correctMode,
            custom_css: orgTheme.customCss || ''
          });
          debugLog('تم تطبيق ثيم المؤسسة الكامل:', { theme: correctMode, organizationId: orgTheme.organizationId });
          return;
        }
      }
    } catch (e) {
    ...
    // fallback إلى الطريقة المحلية البسيطة
    ...
    debugLog('تم تطبيق الثيم البسيط');
  }).catch(e => {
    debugLog('خطأ في تحميل themeManager:', e);
    ...
    debugLog('الثيم الفعلي المطبق (fallback):', effectiveTheme);
    ...
    debugLog('تم تطبيق الثيم البسيط (fallback)');
  });
}
...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children, initialOrganizationId }) => {
      ...
      if (orgSettings) {
        ...
        if ((orgSettings as any).theme_mode) {
          ...
          localStorage.setItem('bazaar_org_theme', JSON.stringify({
            ...
            customCss: (orgSettings as any).custom_css,
            timestamp: Date.now()
          }));
          
          // تطبيق الثيم فوراً إذا كان مختلفاً
          if (orgTheme !== theme) {
            debugLog('تطبيق وضع الثيم الجديد:', { من: theme, إلى: orgTheme });
            setThemeState(orgTheme); // تحديث الحالة مباشرة
            // تطبيق فوري على DOM
  ...
  useEffect(() => {
    ...
    if (savedOrgTheme && ['light', 'dark', 'system'].includes(savedOrgTheme) && savedOrgTheme !== theme) {
      ...
      setThemeState(savedOrgTheme as Theme);
  ...
  // تطبيق الثيم عند تغيير المسار - محسن لتجنب التحديثات المتكررة
  useEffect(() => {
    // التحقق من وجود ثيم مؤسسة نشط أولاً
    const orgThemeKey = 'bazaar_org_theme';
    try {
      const storedOrgTheme = localStorage.getItem(orgThemeKey);
      if (storedOrgTheme) {
        const orgTheme = JSON.parse(storedOrgTheme);
        if (orgTheme.organizationId && initialOrganizationId) {
          // debugLog('🚫 [ThemeContext] تجاهل تطبيق الثيم عند التنقل - يوجد ثيم مؤسسة نشط:', {
          //   orgTheme: orgTheme.mode,
          //   currentTheme: theme,
          //   organizationId: orgTheme.organizationId
          // });
          return; // لا نطبق الثيم إذا كان لدينا ثيم مؤسسة
        }
      }
    } catch (e) {
      // debugLog('خطأ في فحص ثيم المؤسسة:', e);
    }
    
    // تطبيق الثيم فقط عند تغيير الثيم نفسه، وليس عند كل تغيير في المسار
    // debugLog('🔄 [ThemeContext] تطبيق الثيم عند التنقل:', theme);
    applyThemeToDOM(theme);
  }, [theme, initialOrganizationId]); // إزالة location.pathname من dependencies

  // تطبيق ثيم المؤسسة عند تحميل المؤسسة - محسن لتجنب التحديثات المتكررة
  useEffect(() => {
    if (!initialOrganizationId) {
      return;
    }
    ...
    // تطبيق فقط للصفحات التي تحتاج ثيم المؤسسة (فحص واحد عند التحميل)
  ...
  }, [initialOrganizationId, applyOrganizationTheme]); // إزالة location.pathname و isDebug من dependencies
