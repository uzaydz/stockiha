...
const FinancialAnalyticsOptimized: React.FC = () => {
  ...
  console.log('🏠 [FinancialAnalyticsOptimized] Component state:', {
    dateRange: {
    ...
    filters,
    activeTab,
    currentPreset // إضافة الـ preset بوضوح
  });
  ...
  console.log('📊 [FinancialAnalyticsOptimized] Financial data received:', {
  ...
  useEffect(() => {
    console.log('🔄 [useEffect] Date range or preset changed, forcing refetch as backup...', {
      from: dateRange.from.toISOString(),
      to: dateRange.to.toISOString(),
      preset: currentPreset
    });
  ...
  const handleDateRangeChange = useCallback(async (preset: string, customRange?: DateRange) => {
    console.log('📅 [handleDateRangeChange] Changing date range:', { preset, customRange, oldPreset: currentPreset });
    
    if (preset === 'custom' && customRange) {
      console.log('🎯 [handleDateRangeChange] Setting custom preset and range');
      setCurrentPreset('custom');
      setDateRange(customRange);
    } else {
      console.log('🎯 [handleDateRangeChange] Setting preset to:', preset);
      setCurrentPreset(preset); // حفظ الـ preset الجديد
      const newRange = getDateRangePreset(preset);
      console.log('📅 [handleDateRangeChange] New range calculated:', newRange);
      setDateRange(newRange);
    }
    
    // فرض إلغاء cache وإعادة جلب البيانات
    console.log('🔄 [handleDateRangeChange] Invalidating queries and forcing refetch...');
    await queryClient.invalidateQueries({ queryKey: ['financial-analytics-optimized'] });
    
    console.log('✅ [handleDateRangeChange] Date range change completed');
  }, [currentPreset, queryClient]);

  const handleEmployeeChange = useCallback((employeeId: string) => {
    console.log('👤 [handleEmployeeChange] Changing employee:', employeeId);
    setSelectedEmployee(employeeId);
  }, []);

  const handleFiltersChange = useCallback((newFilters: AnalyticsFilters) => {
    console.log('🔍 [handleFiltersChange] Updating filters:', newFilters);
