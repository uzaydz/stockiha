...
export const FacebookCookieManager: React.FC = () => {
  ...
  const checkCookieStatus = useCallback(() => {
    ...
    try {
      ...
      console.warn('فشل في قراءة localStorage:', error);
    }
  ...
  const createFbpCookie = useCallback(() => {
    try {
      ...
      console.log('✅ تم إنشاء _fbp:', fbpValue);
      checkCookieStatus();
      return true;
    } catch (error) {
      console.error('❌ فشل في إنشاء _fbp:', error);
      return false;
    }
  }, [checkCookieStatus]);
  ...
  const createFbcFromFbclid = useCallback((fbclid: string) => {
    try {
      ...
      try {
        ...
        console.warn('فشل في حفظ fbclid في localStorage:', error);
      }
      
      console.log('✅ تم إنشاء _fbc من fbclid:', fbcValue);
      checkCookieStatus();
      return true;
    } catch (error) {
      console.error('❌ فشل في إنشاء _fbc:', error);
      return false;
    }
  }, [checkCookieStatus]);
