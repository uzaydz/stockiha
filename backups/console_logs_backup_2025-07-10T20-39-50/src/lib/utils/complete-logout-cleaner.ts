...
export class CompleteLogoutCleaner {
  ...
  private static async performCompleteCleanup(): Promise<void> {
    try {
      console.log('🧹 بدء التنظيف الشامل والنهائي...');
      ...
      console.log('✅ تم إكمال التنظيف الشامل بنجاح');
  ...
  private static async clearReactQueryCache(): Promise<void> {
    try {
      console.log('🧹 بدء تنظيف React Query Cache...');
      ...
      if (typeof window !== 'undefined' && (window as any).queryClient) {
        ...
        console.log('✅ تم تنظيف React Query Cache');
      } else {
        console.log('⚠️ لم يتم العثور على QueryClient في window');
      }
      ...
      console.error('❌ خطأ في تنظيف React Query Cache:', error);
      // تنظيف سريع كحل بديل
      try {
        if (typeof window !== 'undefined' && (window as any).queryClient) {
        ...
        console.error('❌ فشل في التنظيف السريع أيضاً:', fallbackError);
      }
    }
  }
  ...
  private static async clearUnifiedRequestManagerCache(): Promise<void> {
    try {
      console.log('🧹 تنظيف UnifiedRequestManager cache...');
      ...
      if (unifiedModule && unifiedModule.UnifiedRequestManager) {
        ...
        console.log('✅ تم تنظيف UnifiedRequestManager cache');
      }
      ...
      if (typeof window !== 'undefined') {
        ...
        globalCacheKeys.forEach(key => {
          if ((window as any)[key]) {
            ...
            console.log(`🗑️ حذف متغير عام: ${key}`);
          }
        });
      }
      
    } catch (error) {
      console.warn('⚠️ خطأ في تنظيف UnifiedRequestManager cache:', error);
    }
  }
  ...
  private static clearAllStorage(): void {
    try {
      ...
      console.log('✅ تم تنظيف Storage');
    } catch (error) {
      console.warn('⚠️ فشل في تنظيف Storage:', error);
    }
  }
  ...
  private static async clearIndexedDB(): Promise<void> {
    try {
      ...
      for (const dbName of dbNames) {
        try {
          ...
          await new Promise((resolve, reject) => {
            ...
            deleteReq.onblocked = () => {
              console.warn(`قاعدة البيانات ${dbName} محجوبة`);
              resolve(true);
            };
          });
        } catch (error) {
      ...
      console.warn('⚠️ فشل في تنظيف IndexedDB:', error);
    }
  }
  ...
  private static async clearServiceWorkerCache(): Promise<void> {
    try {
      ...
      console.log('✅ تم تنظيف Service Worker Cache');
    } catch (error) {
      console.warn('⚠️ فشل في تنظيف Service Worker Cache:', error);
    }
  }
  ...
  private static async clearBrowserCache(): Promise<void> {
    try {
      ...
      console.log('✅ تم تنظيف Browser Cache & Cookies');
    } catch (error) {
  ...
  private static resetURL(): void {
    try {
      ...
      console.log('✅ تم إعادة ضبط URL');
    } catch (error) {
      console.warn('⚠️ فشل في إعادة ضبط URL:', error);
    }
  }
  ...
  private static async clearApplicationState(): Promise<void> {
    try {
      console.log('🧹 بدء تنظيف Application State...');
        ...
        globalVarsToDelete.forEach(varName => {
          try {
            if ((window as any)[varName]) {
              ...
              console.log(`🗑️ تم حذف متغير عام: ${varName}`);
            }
          } catch (e) {
      ...
      console.log('✅ تم تنظيف Application State');
    } catch (error) {
      console.error('❌ خطأ في تنظيف Application State:', error);
    }
  }
  ...
  private static async clearOrganizationSpecificData(): Promise<void> {
    try {
      console.log('🧹 بدء تنظيف البيانات المخصصة للمؤسسة...');
      ...
      organizationKeys.forEach(key => {
        if (localStorage.getItem(key)) {
          console.log(`🗑️ إزالة localStorage key: ${key}`);
          localStorage.removeItem(key);
        }
      });
      ...
      organizationKeys.forEach(key => {
        if (sessionStorage.getItem(key)) {
          console.log(`🗑️ إزالة sessionStorage key: ${key}`);
          sessionStorage.removeItem(key);
        }
      });
      ...
      console.log(`🔍 فحص ${keysToCheck.length} مفتاح في localStorage للبحث عن بيانات مؤسسة...`);
        ...
        try {
          ...
          if (value && typeof value === 'string') {
            ...
            if (matches || hasOrgPattern) {
              console.log(`🗑️ إزالة key يحتوي على بيانات مؤسسة: ${key}${matches ? ` (UUID: ${matches.join(', ')})` : ''}`);
              localStorage.removeItem(key);
            }
          }
        } catch (e) {
      ...
      console.log(`🔍 فحص ${sessionKeys.length} مفتاح في sessionStorage للبحث عن بيانات مؤسسة...`);
      ...
      sessionKeys.forEach(key => {
        try {
          ...
          if (value && typeof value === 'string') {
            ...
            if (matches || hasOrgPattern) {
              console.log(`🗑️ إزالة session key يحتوي على بيانات مؤسسة: ${key}${matches ? ` (UUID: ${matches.join(', ')})` : ''}`);
              sessionStorage.removeItem(key);
            }
          }
        } catch (e) {
      ...
      specificKeysToCheck.forEach(keyPattern => {
        keysToCheck.concat(sessionKeys).forEach(key => {
          if (key.includes(keyPattern)) {
            console.log(`🗑️ إزالة cache key مشبوه: ${key}`);
            localStorage.removeItem(key);
            sessionStorage.removeItem(key);
          }
        });
      ...
      console.log('✅ تم تنظيف البيانات المخصصة للمؤسسة');
    } catch (error) {
      console.warn('⚠️ خطأ في تنظيف البيانات المخصصة للمؤسسة:', error);
    }
  }
  ...
  private static forceReactContextReset(): void {
    try {
      console.log('🔄 إجبار React على إعادة ضبط Context...');
      ...
      if (typeof window !== 'undefined') {
        ...
        console.log('✅ تم إجبار React على إعادة ضبط Context');
      }
    } catch (error) {
      console.error('❌ خطأ في إجبار React على إعادة ضبط Context:', error);
  ...
  private static clearReactContextStates(): void {
    try {
      console.log('🔄 تنظيف React Context states...');
      ...
      if (typeof window !== 'undefined') {
        ...
        additionalCaches.forEach(cacheKey => {
          if ((window as any)[cacheKey]) {
            ...
            console.log(`🗑️ مسح cache: ${cacheKey}`);
          }
        });
        ...
        moduleKeysToReset.forEach(moduleKey => {
          if ((window as any)[moduleKey]) {
            ...
            console.log(`🗑️ مسح module cache: ${moduleKey}`);
          }
        });
        ...
        console.log('✅ تم تنظيف React Context states');
      }
    } catch (error) {
      console.error('❌ خطأ في تنظيف React Context states:', error);
    }
  }
  ...
  private static async clearHardcodedData(): Promise<void> {
    try {
      console.log('🗑️ تنظيف البيانات المحددة مسبقاً...');
      ...
      if (typeof window !== 'undefined' && (window as any).organizationCache) {
        ...
        for (const [key, value] of cache.entries()) {
          if (key.includes('fredstore') || 
            ...
            cache.delete(key);
            console.log(`🗑️ حذف من organizationCache: ${key}`);
          }
        }
      }
      ...
      if (typeof window !== 'undefined' && (window as any).queryClient) {
        ...
        queryCache.getAll().forEach((query: any) => {
          if (query.queryKey && query.queryKey.some((key: any) => 
            ...
            console.log(`🗑️ حذف query يحتوي على fredstore: ${JSON.stringify(query.queryKey)}`);
          }
        });
      }
      
      console.log('✅ تم تنظيف البيانات المحددة مسبقاً');
      
    } catch (error) {
      console.error('❌ خطأ في تنظيف البيانات المحددة مسبقاً:', error);
    }
  }
} 