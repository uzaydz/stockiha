...
export function setupAuthErrorFiltering(): void {
  ...
  console.error = (...args: any[]) => {
    ...
    // تجاهل أخطاء Realtime غير الحرجة
    if (message.includes('Realtime') && (message.includes('CLOSED') || message.includes('CHANNEL_ERROR'))) {
      // لا نعرض هذه الأخطاء لأنها تُعالج تلقائياً
      return;
    }
