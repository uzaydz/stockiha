...
export const getPOSOrdersWithReturns = async (
  ...
  try {
    ...
    if (error) {
      console.error('خطأ في جلب طلبيات نقطة البيع:', error)
      throw error
    }
    ...
    if (countError) {
      console.error('خطأ في جلب عدد الطلبيات:', countError)
      throw countError
    }
    ...
    console.error('خطأ في جلب طلبيات نقطة البيع مع المرتجعات:', error)
    throw error
  }
} 