...
export const CustomFormFields: React.FC<CustomFormProps> = ({
  ...
  const updateFieldValue = (fieldName: string, value: string) => {
    ...
    const isSignificantField = ['fullName', 'phone', 'province', 'municipality', 'address'].includes(fieldName);
    ...
    try {
      ...
      if (municipalityField) {
        ...
        if (!municipalities || municipalities.length === 0) {
          ...
          setExtendedFields([...updatedFields]);
          console.log(`لم يتم العثور على بلديات للولاية ${provinceId}`);
          return;
        }
        
        console.log(`تم العثور على ${municipalities.length} بلدية للولاية ${provinceId}`);
        ...
        console.log("البلديات المتاحة:", filteredMunicipalities.map(m => ({ id: m.id, name: m.name })));
        ...
        if (filteredMunicipalities.length > 0) {
          ...
          if (currentSelectionIsValid) {
            ...
            console.log(`تعيين القيمة الافتراضية للبلدية: ${newMunicipalityValue} (${filteredMunicipalities[0].name})`);
          }
        }
        ...
        if (municipalityField.name && newMunicipalityValue) {
          ...
          if (updateFieldValue) {
            ...
            console.log(`تم تعيين قيمة البلدية في النموذج عند التحميل: ${newMunicipalityValue}`);
          }
        }
