...
const ProductReturns: React.FC<ProductReturnsProps> = ({
  ...
  const syncInBackground = async () => {
    ...
    try {
      ...
      // ⚡ جلب جميع عناصر الإرجاع مرة واحدة عبر Delta Sync
      const allReturnItems = await deltaWriteService.getAll<LocalReturnItem>('return_items' as any, currentOrganization.id);
      const convertedReturns: Return[] = await Promise.all(
        localReturns.map(async (localReturn) => {
          const items = allReturnItems.filter(item => item.return_id === localReturn.id);
          ...
          return {
            ...
            _synced: localReturn.synced,
            _syncStatus: localReturn.syncStatus,
            _pendingOperation: localReturn.pendingOperation
          } as Return;
        })
      );
    ...
    try {
      ...
      if (isOnline) {
        try {
          ...
          console.warn('[ProductReturns] Failed to fetch from server, falling back to local:', error);
          // Continue to load from local if server fetch fails
        }
      }
      ...
      // ⚡ جلب جميع عناصر الإرجاع مرة واحدة عبر Delta Sync
      const allReturnItemsLocal = await deltaWriteService.getAll<LocalReturnItem>('return_items' as any, currentOrganization.id);
      const convertedReturns: Return[] = await Promise.all(
        localReturns.map(async (localReturn) => {
          // جلب العناصر
          const items = allReturnItemsLocal.filter(item => item.return_id === localReturn.id);
  ...
  const searchOrder = async () => {
    ...
    try {
      // ⚡ البحث في الطلبيات المحلية عبر Delta Sync
      const localOrders = await deltaWriteService.getAll<LocalPOSOrder>('pos_orders', currentOrganization.id);
      ...
      if (foundOrders.length > 0) {
        ...
        // ⚡ جلب عناصر الطلبية عبر Delta Sync
        const allOrderItems = await deltaWriteService.getAll<LocalPOSOrderItem>('pos_order_items', currentOrganization.id);
  ...
  const fetchReturnItems = async (returnId: string) => {
    ...
    try {
      // ⚡ جلب العناصر من المخزن المحلي عبر Delta Sync
