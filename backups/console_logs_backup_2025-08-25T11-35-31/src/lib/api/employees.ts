...
const getOrganizationId = async (): Promise<string | null> => {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('🔄 [getOrganizationId] جلب معرف المؤسسة');
  }
  
  // استخدام cache إذا كان حديثاً
  if (cachedOrganizationId && (now - lastOrgFetch) < ORG_CACHE_DURATION) {
    if (process.env.NODE_ENV === 'development') {
      console.log('✅ [getOrganizationId] استخدام معرف المؤسسة من الـ cache');
    }
  ...
  try {
    ...
    if (localOrgId) {
      if (process.env.NODE_ENV === 'development') {
        console.log('✅ [getOrganizationId] معرف المؤسسة من التخزين المحلي');
      }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('🔄 [getOrganizationId] جلب معرف المؤسسة من قاعدة البيانات');
    }
    ...
    if (!user) {
      if (process.env.NODE_ENV === 'development') {
        console.warn('⚠️ [getOrganizationId] لا يوجد مستخدم مصادقة');
      }
      return null;
    }
    ...
    if (userData?.organization_id) {
      if (process.env.NODE_ENV === 'development') {
        console.log('✅ [getOrganizationId] تم العثور على معرف المؤسسة من قاعدة البيانات');
      }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn('⚠️ [getOrganizationId] لم يتم العثور على معرف المؤسسة');
    }
    return null;
  } catch (err) {
    if (process.env.NODE_ENV === 'development') {
      console.error('❌ [getOrganizationId] خطأ:', err);
    }
    return null;
  }
};
...
export const createEmployeeOptimized = async (
  ...
  try {
    if (process.env.NODE_ENV === 'development') {
      console.log('🚀 [createEmployeeOptimized] بدء إنشاء الموظف باستخدام RPC محسنة');
    }
    ...
    if (error) {
      console.error('❌ [createEmployeeOptimized] خطأ في RPC:', error);
      throw new Error(error.message);
    }

    if (!data || !data.success) {
      const errorMsg = data?.error || 'فشل في إنشاء الموظف';
      console.error('❌ [createEmployeeOptimized] فشل:', errorMsg);
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('✅ [createEmployeeOptimized] تم إنشاء الموظف بنجاح:', employee.name);
      console.log('📝 [createEmployeeOptimized] الإجراء:', data.action);
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('📧 [createEmployeeOptimized] سيتم إرسال دعوة للموظف عبر البريد الإلكتروني');
      console.log('⚠️ [createEmployeeOptimized] تم تجنب تسجيل الدخول التلقائي للموظف');
    }
    ...
    console.error('❌ [createEmployeeOptimized] خطأ عام:', err);
    throw err;
  }
};
...
export const inviteEmployeeAuth = async (
  ...
  try {
    if (process.env.NODE_ENV === 'development') {
      console.log('📧 [inviteEmployeeAuth] إرسال دعوة للموظف:', email);
    }
    ...
    if (employeeError || !employeeData?.organization_id) {
      console.error('❌ [inviteEmployeeAuth] لم يتم العثور على معرف المؤسسة للموظف:', employeeError);
      return {
        success: false,
    ...
    if (error) {
      console.error('❌ [inviteEmployeeAuth] فشل إرسال الدعوة:', error);
      return {
        success: false,
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('✅ [inviteEmployeeAuth] تم إرسال الدعوة بنجاح');
    }
    ...
    console.error('❌ [inviteEmployeeAuth] خطأ عام:', err);
    return {
      success: false,
...
export const getEmployeesWithStats = async (): Promise<{
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('🚀 [getEmployeesWithStats] بدء تحميل البيانات بـ RPC محسنة');
  }
  ...
  if (employeesCacheValid && statsCacheValid) {
    if (process.env.NODE_ENV === 'development') {
      console.log('✅ [getEmployeesWithStats] استخدام البيانات من الـ cache');
    }
    performanceStats.employeesCacheHits++;
    performanceStats.statsCacheHits++;
    return {
  ...
  try {
    if (process.env.NODE_ENV === 'development') {
      console.log('🔄 [getEmployeesWithStats] تحميل البيانات باستخدام RPC function واحدة');
    }
    ...
    if (!organizationId) {
      if (process.env.NODE_ENV === 'development') {
        console.warn('⚠️ [getEmployeesWithStats] لم يتم العثور على معرف المؤسسة');
      }
      return {
        employees: [],
    ...
    if (error) {
      console.error('❌ [getEmployeesWithStats] خطأ في تحميل البيانات:', error);
      throw new Error(error.message);
    }
    
    if (!data) {
      if (process.env.NODE_ENV === 'development') {
        console.warn('⚠️ [getEmployeesWithStats] لم يتم إرجاع بيانات من الـ RPC');
      }
      return {
        employees: [],
        stats: { total: 0, active: 0, inactive: 0 }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('🔍 [getEmployeesWithStats] البيانات المُسترجعة:', data);
      console.log('🔍 [getEmployeesWithStats] نوع البيانات:', typeof data);
      console.log('🔍 [getEmployeesWithStats] success:', (data as any)?.success);
      console.log('🔍 [getEmployeesWithStats] error:', (data as any)?.error);
      console.log('🔍 [getEmployeesWithStats] debug:', (data as any)?.debug);
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`✅ [getEmployeesWithStats] تم تحميل ${employees.length} موظف بنجاح باستخدام RPC واحدة`);
    }
    
    return { employees, stats };
    
  } catch (err) {
    console.error('❌ [getEmployeesWithStats] خطأ:', err);
    return {
      employees: [],
      stats: { total: 0, active: 0, inactive: 0 }
    };
