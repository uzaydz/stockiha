-- إصلاح مشكلة التحكم في الوصول للنطاقات
-- يجب على المستخدمين الدخول فقط من نطاق مؤسستهم

-- 1. إنشاء دالة للحصول على معرف المؤسسة من النطاق أو النطاق الفرعي
CREATE OR REPLACE FUNCTION get_organization_by_domain(
...
-- 2. تحديث دالة check_user_requires_2fa لتكون أكثر صرامة
...
BEGIN
    ...
    IF p_organization_id IS NULL AND p_domain IS NULL AND p_subdomain IS NULL THEN
        -- السماح فقط للمسؤولين العامين بالدخول بدون نطاق محدد
...
-- 3. إنشاء دالة للتحقق من صلاحية الدخول للنطاق
...
-- 4. منح الصلاحيات
GRANT EXECUTE ON FUNCTION get_organization_by_domain(TEXT, TEXT) TO authenticated, anon;
...
CREATE INDEX IF NOT EXISTS idx_organizations_subdomain ON organizations(LOWER(subdomain));