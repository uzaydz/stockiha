...
export function useStoreInfo() {
  ...
  return useMemo(() => {
    if (!data || !data.organization) {
      console.log('⚠️ [useStoreInfo] البيانات غير متوفرة:', { 
        hasData: !!data, 
...
export function useOrganizationSettings() {
  ...
  return useMemo(() => {
    if (!data || !data.organization) {
      console.log('⚠️ [useOrganizationSettings] البيانات غير متوفرة');
      return null;
    }
...
export function useThemeSettings() {
  ...
  return useMemo(() => {
    if (!data || !data.theme) {
      console.log('⚠️ [useThemeSettings] بيانات الثيم غير متوفرة');
      return null;
    }
    return data.theme;
  }, [data]);
