...
export const usePerformanceOptimizer = (options: UsePerformanceOptimizerOptions = {}) => {
  ...
  useEffect(() => {
    ...
    window.getComputedStyle = function(...args) {
      ...
      if (enableLogging && reflowCountRef.current % 10 === 0) {
        console.warn(`🔄 [PERFORMANCE] Forced reflow detected (${reflowCountRef.current} total)`);
      }
  ...
  const endPerformanceMonitoring = useCallback((operationName: string): PerformanceMetrics | null => {
    ...
    if (enableLogging) {
      console.timeEnd(`${operationName}-${metric.startTime}`);
      
      // تحذيرات الأداء
      if (metric.duration > 100) {
        console.warn(`⚠️ [PERFORMANCE] Slow operation: ${operationName} took ${metric.duration.toFixed(2)}ms`);
      }
      
      if (metric.reflows && metric.reflows > 5) {
        console.warn(`🔄 [PERFORMANCE] High reflow count: ${operationName} caused ${metric.reflows} reflows`);
      }
    }
