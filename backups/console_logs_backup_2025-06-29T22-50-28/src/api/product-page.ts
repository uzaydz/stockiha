...
export const getProductPageData = async (organizationId: string, slug: string): Promise<ProductPageData | null> => {
  console.log('🔍 getProductPageData - بدء الطلب:', { organizationId, slug });
  ...
  return requestCache.get(
    ...
    async () => {
      try {
        console.log('🔍 getProductPageData - داخل requestCache.get');
        
        // التحقق من صحة المعاملات
        if (!organizationId || !slug) {
          console.log('❌ getProductPageData - معاملات غير صحيحة:', { organizationId, slug });
          throw new Error('معاملات غير صحيحة: معرف المؤسسة أو slug مفقود');
        }

        console.log('🔍 getProductPageData - استدعاء Edge Function');
        ...
        if (functionError) {
          console.log('❌ getProductPageData - خطأ من Edge Function:', functionError);
          
          // معالجة أنواع مختلفة من الأخطاء
          const errorStatus = (functionError as any)?.status;
          const errorMessage = functionError.message || '';
          
          console.log('🔍 getProductPageData - تفاصيل الخطأ:', { errorStatus, errorMessage });
          
          if (errorStatus === 404 || errorMessage.includes('404') || errorMessage.includes('Product not found')) {
            console.log('❌ getProductPageData - المنتج غير موجود');
        ...
        console.log('✅ getProductPageData - تم استلام البيانات من Edge Function:', responseData);

        // التحقق من صحة البيانات المُرجعة
        if (!responseData || typeof responseData !== 'object') {
          console.log('❌ getProductPageData - تنسيق استجابة غير صحيح');
          throw new Error('تنسيق استجابة غير صحيح من الخادم');
        }
        
        if (!('product' in responseData)) {
          console.log('❌ getProductPageData - بيانات المنتج مفقودة');
        ...
        if (responseData.product === null) {
          console.log('❌ getProductPageData - المنتج null');
          return null;
        }

        // التحقق من صحة بيانات المنتج
        if (!responseData.product.id) {
          console.log('❌ getProductPageData - بيانات المنتج غير مكتملة:', responseData.product);
          throw new Error('بيانات المنتج غير مكتملة');
        }

        console.log('✅ getProductPageData - نجح التحميل، المنتج:', responseData.product.name);
