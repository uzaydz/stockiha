...
async function upsertData(supabase: SupabaseClient, tableName: string, conflictColumn: string, data: any[], syncResults: any) {
  ...
  if (error) {
    ...
    console.log(`[sync-yalidine] Successfully called upsert for ${tableName}. Upsert result:`, result);
    console.log(`[sync-yalidine] Count of affected rows for ${tableName}:`, count);
...
serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      console.log("[sync-yalidine] Starting centers (Stop Desks) sync...");
      ...
      console.log(`[sync-yalidine] Fetched ${rawCentersData.length} raw centers from Yalidine.`);
      ...
      if (centersToUpsert.length > 0) {
        console.log(`[sync-yalidine] Attempting to upsert ${centersToUpsert.length} processed centers.`);
        await upsertData(supabase, "yalidine_centers_global", "center_id", centersToUpsert, syncResults.centers);
        console.log(`[sync-yalidine] Centers sync completed. Upserted: ${syncResults.centers.synced} (or attempted if result unclear). Errors: ${syncResults.centers.errors}`);
      } else {
        console.log("[sync-yalidine] No new center data to sync after mapping and filtering.");
      }
    ...
    if (centersApiCallSuccessful) {
      try {
        ...
        console.log("[sync-yalidine] Successfully updated last_global_centers_sync timestamp.");
      } catch (tsError) {
