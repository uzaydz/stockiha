...
export class AuthService {
  ...
  async signIn(email: string, password: string): Promise<AuthResult> {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('✅ [AuthService] تسجيل دخول ناجح:', {
          userId: data.user.id,
  ...
  async signUp(email: string, password: string, name: string, currentSubdomain?: string | null): Promise<AuthResult> {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('✅ [AuthService] تسجيل جديد ناجح:', {
          userId: data.user?.id,
          email: data.user?.email
        });
      }
  ...
  async signOut(): Promise<void> {
    ...
    try {
      ...
      if (error) {
        if (process.env.NODE_ENV === 'development') {
          console.error('❌ [AuthService] خطأ في تسجيل الخروج:', error);
        }
      }
    } catch (error) {
      if (process.env.NODE_ENV === 'development') {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('✅ [AuthService] تم تسجيل الخروج وتنظيف البيانات');
    }
