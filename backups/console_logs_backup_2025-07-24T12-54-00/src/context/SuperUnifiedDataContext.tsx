...
const fetchGlobalData = async (organizationId: string, userId?: string): Promise<GlobalData> => {
  try {
    console.log('🔄 بدء جلب البيانات الموحدة...', { organizationId, userId });
    ...
    console.log('🔍 [SuperUnifiedDataContext] البيانات المسترجعة من get_global_data_complete:', {
      hasAppsData: !!result.apps_and_subscription,
      organizationAppsCount: result.apps_and_subscription?.organization_apps?.length || 0,
      organizationApps: result.apps_and_subscription?.organization_apps || 'غير موجود',
      fullAppsData: result.apps_and_subscription
    });
    ...
    console.log('✅ [SuperUnifiedDataContext] تم تجميع البيانات بنجاح:', {
      organizationAppsCount: globalData.apps_and_subscription.organization_apps.length,
      enabledApps: globalData.apps_and_subscription.organization_apps.filter((app: any) => app.is_enabled).length,
      organizationApps: globalData.apps_and_subscription.organization_apps,
      organizationId: globalData.organization_id
    });

    return globalData;

  } catch (error) {
    console.error('🔴 خطأ في fetchGlobalData:', error);
    throw error;
  }
};
...
export const useAppsData = () => {
  ...
  console.log('🔍 [useAppsData] Hook called:', {
    organizationAppsCount: organizationApps?.length || 0,
    enabledAppsCount: organizationApps?.filter((app: any) => app.is_enabled)?.length || 0,
...
export const useIsAppEnabled = (appId: string): boolean => {
  ...
  console.log(`🔍 [useIsAppEnabled] Checking app ${appId}:`, {
    app: app,
    isEnabled: isEnabled,
    organizationAppsCount: organizationApps?.length || 0
  });
