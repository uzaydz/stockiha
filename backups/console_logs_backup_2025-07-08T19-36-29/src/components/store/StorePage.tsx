...
const StorePage = ({ storeData: initialStoreData = {} }: StorePageProps) => {
  ...
  // استخدام البيانات المحسنة من useSharedStoreData
  const { categories: sharedCategories, isLoading: sharedDataLoading, refreshData: refreshSharedData } = useSharedStoreData();
  ...
  const [refreshedCategories, setRefreshedCategories] = useState<any[]>([]);
  
  // جلب الفئات المحدثة مباشرة من قاعدة البيانات
  useEffect(() => {
    const fetchUpdatedCategories = async () => {
      try {
        const orgId = storeData?.organization_details?.id;
        if (!orgId) {
          setRefreshedCategories([]);
          return;
        }
        ...
        if (error) {
          ...
          // استخدام البيانات القديمة كـ fallback
          if (storeData?.categories) {
            setRefreshedCategories(storeData.categories.map(category => ({
              ...category,
        ...
        const updatedCategories = (data || []).map(category => {
          ...
          if (imageUrl) {
            ...
            const timestamp = new Date(category.updated_at || Date.now()).getTime();
          ...
          return {
            ...category,
            imageUrl: imageUrl,
            productsCount: 0, // سيتم تحديثه لاحقاً إذا لزم الأمر
            icon: category.icon || 'folder',
            color: 'from-blue-500 to-indigo-600'
          };
        });
  ...
  }, [storeData?.organization_details?.id, storeData?.categories]);

  const extendedCategories = useMemo(() => {
    // 1. أولوية للفئات من useSharedStoreData (محدثة مع cache busting)
  ...
  }, [sharedCategories, refreshedCategories, storeData?.categories]);
                    ...
                    return (
                      <LazyLoad key={component.id || `component-${index}`}>
                        ...
                        {(component.type === 'product_categories') && (
                          (() => {
                            console.log('🏪 [StorePage] تمرير البيانات لـ ProductCategories:');
                            console.log('categoriesForProps:', categoriesForProps);
                            console.log('categoriesForProps length:', categoriesForProps.length);
                            if (categoriesForProps.length > 0) {
                              console.log('🖼️ [StorePage] صور الفئات المُمررة:');
                              categoriesForProps.forEach((cat, index) => {
                                console.log(`${index + 1}. ${cat.name}: ${cat.imageUrl || cat.image_url}`);
                              });
                            }
                            console.log('sharedCategories:', sharedCategories);
                            console.log('sharedCategories length:', sharedCategories.length);
