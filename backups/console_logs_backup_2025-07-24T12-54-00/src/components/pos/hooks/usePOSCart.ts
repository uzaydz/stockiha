...
export const usePOSCart = ({
  ...
  const addItemToCart = useCallback((product: Product) => {
    console.log('🔍 [usePOSCart.addItemToCart] بدء عملية إضافة منتج:', {
    ...
    if (!activeTabId) {
      console.error('❌ [usePOSCart.addItemToCart] لا يوجد تبويب نشط');
      return;
    }
    ...
    try {
      ...
      console.warn('تعذر الحصول على المخزون المحدث، استخدام مخزون المنتج');
    }

    console.log('📦 [usePOSCart.addItemToCart] فحص المخزون:', {
      productName: product.name,
      originalStock: product.stockQuantity || product.stock_quantity || 0,
      currentStock,
    ...
    if (currentStock <= 0) {
      console.error(`❌ [usePOSCart.addItemToCart] مخزون غير متوفر للمنتج: ${product.name}`);
      // يمكن إضافة toast notification هنا لاحقاً
      return;
    }

    console.log('➕ [usePOSCart.addItemToCart] إضافة المنتج للسلة...');
    
    // إضافة المنتج للسلة مباشرة
    addItemToCartTab(product, 1);
    
    console.log('✅ [usePOSCart.addItemToCart] تم استدعاء addItemToCartTab بنجاح');
  ...
  const clearCart = useCallback(() => {
    console.log('🗑️ [usePOSCart.clearCart] بدء تفريغ السلة:', {
      activeTabId,
    ...
    console.log('✅ [usePOSCart.clearCart] تم استدعاء clearCartTab');
    
    toast.success('تم مسح السلة');
  }, [clearCartTab, activeTabId, cartItems]);
