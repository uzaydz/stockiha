    ...
    const fetchStoreData = async () => {
      try {
        ...
        if (appData?.storeSettings && appData.storeSettings.length > 0) {
          console.log('✅ [useStorePageData] استخدام store_settings المحفوظة من appInitializer:', appData.storeSettings.length);
          ...
          console.log('🔄 [useStorePageData] جلب store_settings من قاعدة البيانات مع التنسيق');
        ...
        if (appData?.products && appData.products.length > 0) {
          console.log('✅ [useStorePageData] استخدام featured products المحفوظة من appInitializer');
          ...
          console.log('🔄 [useStorePageData] جلب featured products من قاعدة البيانات مع التنسيق');
          const { coordinateRequest } = await import('@/lib/api/requestCoordinator');
  ...
  useEffect(() => {
    ...
    const fetchFooterSettings = async () => {
      try {
        ...
        if (appData?.storeSettings && appData.storeSettings.length > 0) {
          console.log('✅ [useStorePageData] استخدام footer settings المحفوظة من appInitializer');
        ...
        // 🔄 إذا لم تتوفر البيانات المحفوظة، جلب من قاعدة البيانات مع التنسيق
        console.log('🔄 [useStorePageData] جلب footer settings من قاعدة البيانات مع التنسيق');
        const { coordinateRequest } = await import('@/lib/api/requestCoordinator');
        ...
        console.error('❌ [useStorePageData] خطأ في جلب footer settings:', error);
      }
    };
