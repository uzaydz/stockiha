...
export class ConflictResolver {
   ...
   * 
   * ⚠️ CRITICAL FIX: تحسين منطق دمج المخزون
   * - استخدام timestamps لتحديد الأحدث بدلاً من الاعتماد على local.stock_quantity دائماً
   * - منع فقدان التحديثات من أجهزة POS أخرى
   */
  private mergeProduct(local: LocalProduct, server: any): LocalProduct {
    console.log(`[ConflictResolver] Merging product ${local.id}`);

    // ⚡ تحديد آخر تحديث للمخزون من كلا المصدرين
    const localStockUpdate = local.last_inventory_update || 
                           (local as any).localUpdatedAt || 
                           local.updated_at || 
                           null;
    const serverStockUpdate = server.last_inventory_update || 
                             server.updated_at || 
                             null;

    // ⚡ مقارنة timestamps لتحديد الأحدث
    let resolvedStock: number;
    let resolvedStockUpdate: string | null = null;

    if (localStockUpdate && serverStockUpdate) {
      const localTs = new Date(localStockUpdate).getTime();
      const serverTs = new Date(serverStockUpdate).getTime();
      
      if (localTs > serverTs) {
        // ⚡ التحديث المحلي أحدث
        resolvedStock = local.stock_quantity ?? server.stock_quantity ?? 0;
        resolvedStockUpdate = localStockUpdate;
        console.log(`[ConflictResolver] Local stock is newer (${localTs} > ${serverTs})`);
      } else if (serverTs > localTs) {
        // ⚡ التحديث من السيرفر أحدث
        resolvedStock = server.stock_quantity ?? local.stock_quantity ?? 0;
        resolvedStockUpdate = serverStockUpdate;
        console.log(`[ConflictResolver] Server stock is newer (${serverTs} > ${localTs})`);
      } else {
        // ⚡ نفس الوقت - نستخدم القيمة الأكبر (أو المحلية إذا كانت متساوية)
        resolvedStock = local.stock_quantity ?? server.stock_quantity ?? 0;
        resolvedStockUpdate = localStockUpdate || serverStockUpdate;
        console.log(`[ConflictResolver] Same timestamp - using local stock`);
      }
    } else if (localStockUpdate) {
      // ⚡ فقط المحلي له timestamp
      resolvedStock = local.stock_quantity ?? server.stock_quantity ?? 0;
      resolvedStockUpdate = localStockUpdate;
      console.log(`[ConflictResolver] Only local has timestamp`);
    } else if (serverStockUpdate) {
      // ⚡ فقط السيرفر له timestamp
      resolvedStock = server.stock_quantity ?? local.stock_quantity ?? 0;
      resolvedStockUpdate = serverStockUpdate;
      console.log(`[ConflictResolver] Only server has timestamp`);
    } else {
      // ⚡ لا يوجد timestamps - نستخدم القيمة المحلية كـ fallback (للتوافق مع الكود القديم)
      resolvedStock = local.stock_quantity ?? server.stock_quantity ?? 0;
      resolvedStockUpdate = new Date().toISOString();
      console.log(`[ConflictResolver] No timestamps - using local stock as fallback`);
    }
    ...
    const merged: LocalProduct = {
      ...
      // 🔹 المخزون المحلول بناءً على timestamps
      stock_quantity: resolvedStock,
      last_inventory_update: resolvedStockUpdate || new Date().toISOString(),
    ...
    console.log(`[ConflictResolver] Product merged - server metadata + timestamp-based stock (${merged.stock_quantity}, updated: ${merged.last_inventory_update})`);
    return merged;
  }
