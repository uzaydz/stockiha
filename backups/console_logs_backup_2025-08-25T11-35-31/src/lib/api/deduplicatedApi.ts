...
export async function getOrganizationSettings(
  ...
  return requestDeduplicator.execute(
    key,
    async () => {
      console.log(`🔍 [API] جلب إعدادات المؤسسة: ${organizationId}`);
      ...
      if (error) {
        console.error(`❌ [API] خطأ في جلب إعدادات المؤسسة:`, error);
        throw error;
      }
...
export async function getOrganizationDefaultLanguage(
  ...
  return requestDeduplicator.execute(
    key,
    async () => {
      console.log(`🌐 [API] جلب اللغة الافتراضية: ${organizationId}`);
      ...
      if (error) {
        console.error(`❌ [API] خطأ في جلب اللغة الافتراضية:`, error);
        return 'ar'; // fallback
      }
...
export async function getUserById(
  ...
  return requestDeduplicator.execute(
    key,
    async () => {
      console.log(`👤 [API] جلب بيانات المستخدم: ${userId}`);
      ...
      if (error) {
        console.error(`❌ [API] خطأ في جلب بيانات المستخدم:`, error);
        throw error;
      }
...
export async function getUserByAuthId(
  ...
  return requestDeduplicator.execute(
    key,
    async () => {
      console.log(`👤 [API] جلب بيانات المستخدم بالمعرف: ${authUserId}`);
      ...
      if (error) {
        console.error(`❌ [API] خطأ في جلب بيانات المستخدم بالمعرف:`, error);
        throw error;
      }
...
export async function getOrganizationUsers(
  ...
  return requestDeduplicator.execute(
    key,
    async () => {
      console.log(`👥 [API] جلب مستخدمي المؤسسة: ${organizationId}`);
      ...
      if (error) {
        console.error(`❌ [API] خطأ في جلب مستخدمي المؤسسة:`, error);
        throw error;
      }
...
export async function getOrganizationById(
  ...
  return requestDeduplicator.execute(
    key,
    async () => {
      console.log(`🏢 [API] جلب بيانات المؤسسة: ${organizationId}`);
      ...
      if (error) {
        console.error(`❌ [API] خطأ في جلب بيانات المؤسسة:`, error);
        throw error;
      }
...
export function clearOrganizationCache(organizationId: string): void {
  ...
  console.log(`🧹 [API] تم مسح كاش المؤسسة: ${organizationId}`);
}
...
export function clearUserCache(userId: string, authUserId?: string): void {
  ...
  console.log(`🧹 [API] تم مسح كاش المستخدم: ${userId}`);
}
...
export async function getProductCompleteDataOptimized(
  ...
  return requestDeduplicator.execute(
    key,
    async () => {
      console.log(`🚀 [deduplicatedApi] جلب بيانات المنتج المحسنة: ${productIdentifier}`);
      
      // 🚀 تحسين: استخدام الدالة المحدثة لتحميل جميع صور الألوان
      console.log('🎯 [deduplicatedApi] بدء جلب البيانات مع جميع صور الألوان');
      ...
      console.log('✅ [deduplicatedApi] تم جلب البيانات مع جميع صور الألوان');

      // 🚀 إضافة logging لتشخيص مشكلة صور الألوان
      if (result?.product?.variants?.colors) {
        console.log('🔍 [deduplicatedApi] بيانات الألوان المستلمة:', result.product.variants.colors.map(color => ({
          id: color.id,
          name: color.name,
          hasImage: 'image_url' in color,
          imageUrl: color.image_url,
          imageUrlAlt: (color as any).imageUrl
        })));
      }
      
      console.log(`⚡ [deduplicatedApi] تم الانتهاء من جلب بيانات المنتج`);
      return result;
    },
    {
...
export async function getStoreInitData(
  ...
  return requestDeduplicator.execute(
    key,
    async () => {
      console.log(`🏪 [API] جلب بيانات تهيئة المتجر: ${orgSubdomain}`);
      ...
      console.log(`⚡ [API] تم الانتهاء من جلب بيانات المتجر في ${executionTime.toFixed(2)}ms`);
      
      if (error) {
        console.error(`❌ [API] خطأ في جلب بيانات تهيئة المتجر:`, error);
        console.warn('🔁 [API] استخدام REST fallback لجلب بيانات المتجر بسبب فشل RPC/Timeout');
...
export function clearProductCache(productIdentifier: string): void {
  ...
  console.log(`🧹 [API] تم مسح كاش المنتج: ${productIdentifier}`);
}
...
export function clearStoreCache(orgSubdomain: string): void {
  ...
  console.log(`🧹 [API] تم مسح كاش المتجر: ${orgSubdomain}`);
}
...
async function getStoreInitDataFallback(orgIdentifier: string): Promise<any> {
  try {
    console.log('🔄 [getStoreInitDataFallback] بدء تجميع بيانات المتجر عبر REST:', { orgIdentifier });
    ...
    console.log('✅ [getStoreInitDataFallback] تم بناء بيانات المتجر عبر REST بنجاح');
    return fallbackData;
  } catch (err) {
    console.error('❌ [getStoreInitDataFallback] فشل في بناء بيانات المتجر عبر REST:', err);
    throw err;
  }
}
...
async function getBasicProductDataFallback(productIdentifier: string, organizationId?: string): Promise<any> {
  try {
    console.log('🔄 [getBasicProductDataFallback] استخدام fallback للبيانات الأساسية:', { productIdentifier, organizationId });
    ...
    console.log('✅ [getBasicProductDataFallback] تم جلب البيانات الأساسية بنجاح');
    return basicResponse;
    
  } catch (error) {
    console.error('❌ [getBasicProductDataFallback] فشل في جلب البيانات الأساسية:', error);
    throw error;
  }
}
