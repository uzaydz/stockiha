...
export async function processOrder(
  ...
  try {
    ...
    console.log('🔍 [processOrder] بيانات الطلبية الواردة:', {
      organizationId,
      orderData: {
        fullName,
        phone,
    ...
    console.log('📤 [processOrder] معاملات الدالة المرسلة لقاعدة البيانات:', {
      functionName: 'process_online_order_new',
      params: {
        ...params,
        p_form_data: formData ? Object.keys(formData) : null,
        p_metadata: metadata ? Object.keys(metadata) : null
      },
      paramsCount: Object.keys(params).length,
      // 🔍 فحص قيم UUID بالتفصيل
      uuidFields: {
        p_product_id: { value: params.p_product_id, type: typeof params.p_product_id, isValid: !!params.p_product_id },
        p_organization_id: { value: params.p_organization_id, type: typeof params.p_organization_id, isValid: !!params.p_organization_id },
        p_product_color_id: { value: params.p_product_color_id, type: typeof params.p_product_color_id, isValid: params.p_product_color_id !== null },
        p_product_size_id: { value: params.p_product_size_id, type: typeof params.p_product_size_id, isValid: params.p_product_size_id !== null },
        p_stop_desk_id: { value: params.p_stop_desk_id, type: typeof params.p_stop_desk_id, isValid: params.p_stop_desk_id !== null }
      }
    });

    // استخدام "as any" لتجاوز تدقيق النوع في TypeScript
    console.log('⚡ [processOrder] بدء استدعاء قاعدة البيانات...');
    const { data, error } = await supabaseClient.rpc('process_online_order_new', params as any);

    // 🚨 CONSOLE LOG: نتيجة استدعاء قاعدة البيانات
    console.log('📥 [processOrder] نتيجة استدعاء قاعدة البيانات:', {
      hasData: !!data,
      hasError: !!error,
      data: data,
      dataType: typeof data,
      dataKeys: data ? Object.keys(data) : null,
      // 🔍 فحص تفصيلي للبيانات المُرجعة
      dataStatus: (data as any)?.status,
      dataError: (data as any)?.error,
      dataDetail: (data as any)?.detail,
      dataId: (data as any)?.id,
      dataOrderId: (data as any)?.order_id,
      dataOrderNumber: (data as any)?.order_number,
      error: error ? {
    ...
    if (data && (data as any)?.status === 'error') {
      console.error('❌ [processOrder] خطأ من قاعدة البيانات:', {
  ...
  if (!error && data) {
    console.log('🔍 [processOrder] بدء فحص المخزون بعد الطلبية...', {
      productId,
      productColorId: toUuidOrNull(productColorId),
      productSizeId: toUuidOrNull(productSizeId)
    });
    ...
    try {
      ...
      console.log('📦 [processOrder] كمية المنتج الرئيسي بعد الطلبية:', productAfter);

      // فحص كمية اللون إذا كان موجود
      const validProductColorId = toUuidOrNull(productColorId);
      if (validProductColorId) {
        console.log('🎨 [processOrder] فحص كمية اللون:', validProductColorId);
        ...
        console.log('🎨 [processOrder] كمية اللون بعد الطلبية:', colorAfter);

      // 🧪 CONSOLE LOG: اختبار خصم المخزون مباشرة باستخدام UPDATE
      try {
        console.log('🧪 [processOrder] اختبار تحديث كمية اللون...');
        ...
        if (updateError) {
          console.error('❌ [processOrder] خطأ في تحديث كمية اللون:', updateError);
        } else {
          console.log('✅ [processOrder] نجح تحديث كمية اللون:', updateResult);
          ...
          console.log('🔄 [processOrder] تم إعادة الكمية كما كانت');
        }
      } catch (testError) {
        console.error('❌ [processOrder] خطأ في اختبار تحديث اللون:', testError);
      }
      }
      ...
      if (validProductSizeId) {
        console.log('📏 [processOrder] فحص كمية المقاس:', validProductSizeId);
        ...
        console.log('📏 [processOrder] كمية المقاس بعد الطلبية:', sizeAfter);
      }
    } catch (checkError) {
      console.error('❌ [processOrder] خطأ في فحص المخزون:', checkError);
    }
  }
