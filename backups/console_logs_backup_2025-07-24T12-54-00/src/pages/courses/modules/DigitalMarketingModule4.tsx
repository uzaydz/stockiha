...
import { module4Data } from '@/data/digitalMarketingModule4Data';
import ModuleNavigation from '@/components/courses/ModuleNavigation';
import type { Video } from '@/data/digitalMarketingModule4Data';

const DigitalMarketingModule4: React.FC = () => {
  const navigate = useNavigate();
  const [currentVideoIndex, setCurrentVideoIndex] = useState(0);
  const [completedVideos, setCompletedVideos] = useState<Set<number>>(new Set());
  const [isVideoLoaded, setIsVideoLoaded] = useState(true);

  const currentVideo = module4Data.videos[currentVideoIndex];
  const progress = (completedVideos.size / module4Data.videos.length) * 100;
  ...
  useEffect(() => {
    localStorage.setItem('module4_progress', JSON.stringify([...completedVideos]));
  }, [completedVideos]);
  ...
  const handleNextVideo = () => {
    if (currentVideoIndex < module4Data.videos.length - 1) {
    ...
    <Layout>
      <div className="container mx-auto px-4 py-6 max-w-7xl">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-300 mb-3">
                  {module4Data.description}
                </p>
                <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                  <div className="flex items-center gap-1">
                    <PlayCircle className="w-4 h-4" />
                    <span>{module4Data.totalVideos} فيديو</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <Clock className="w-4 h-4" />
                    <span>{module4Data.totalDuration}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                      <span>الفيديو {currentVideoIndex + 1} من {module4Data.videos.length}</span>
              ...
              <div className="p-4 border-t border-gray-200 dark:border-gray-700">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    disabled={currentVideoIndex === module4Data.videos.length - 1}
                    className="flex items-center gap-2"
                  >
                    التالي
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
              <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  {completedVideos.size} من {module4Data.videos.length} مكتمل
                </p>
              </div>
              <div className="max-h-96 overflow-y-auto">
                {module4Data.videos.map((video, index) => (
                  <div
                    key={video.id}
        ...
        <ModuleNavigation
          ...
          totalVideos={module4Data.videos.length}
        />
      </div>
    </Layout>
  );
...
export default DigitalMarketingModule4; 