            ...
            <div className="p-3">
              ...
              >
                <Button
                  ...
                  aria-label={
                    componentData.isOutOfStock ? 'غير متوفر' :
                    buyingNow ? 'جارٍ المعالجة...' :
                    uiState.hasForm ? 'انتقل إلى نموذج الطلب' :
                    'اشتر الآن'
                  }
                >
                  ...
                  <div className="flex items-center gap-2.5 w-full">
                    ...
                    <div className="flex-1 text-center">
                      <motion.div 
                        ...
                        transition={{ duration: 0.3 }}
                      >
                        {uiState.orderSuccess ? 'تم بنجاح!' :
                         componentData.isOutOfStock ? 'غير متوفر' :
                         buyingNow ? 'جارٍ المعالجة...' :
                         isCalculatingDelivery ? 'حساب التكلفة...' :
                         uiState.hasForm ? 'اطلب الآن' :
                         'اشتر الآن'}
                      </motion.div>
                      
                      {/* Compact subtitle */}
                      <div className="text-xs opacity-75 mt-0.5">
                        {uiState.orderSuccess ? 'شكراً لثقتك' :
                         componentData.isOutOfStock ? 'سيتم الإعلام عند التوفر' :
                         buyingNow || isCalculatingDelivery ? 'انتظر قليلاً...' :
                         uiState.hasForm ? 'انقر للانتقال' :
                         'اكمل الطلب'}
                      </div>
                    </div>
                  </div>
              ...
              >
                <div className="flex items-center justify-center p-2 bg-gradient-to-r from-green-50/60 to-emerald-50/60 dark:from-green-900/20 dark:to-emerald-900/20 rounded-lg border border-green-200/40 dark:border-green-800/40">
                  <div className="flex items-center gap-2 text-green-700 dark:text-green-300">
                    ...
                    <span className="text-xs font-medium">توفير {componentData.discountPercentage}%</span>
                  </div>
                </div>
              </motion.div>
            )}
                ...
                <div className="flex items-center justify-center p-2.5 bg-gradient-to-r from-amber-50/60 to-orange-50/60 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200/50 dark:border-amber-800/50 rounded-lg">
                  <div className="flex items-center gap-2 text-amber-700 dark:text-amber-300">
                    ...
                    <div className="text-center">
                      <span className="font-medium text-xs">غير متوفر حالياً</span>
                    </div>
                  </div>
                </div>
              </motion.div>
