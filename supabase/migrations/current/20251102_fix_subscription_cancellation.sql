-- Fix subscription cancellation to properly update end_date and organization status
-- Issue 1: When canceling subscription, end_date should be set to NOW()
-- Issue 2: Organization subscription_tier should be set to NULL (not showing trial)
...
BEGIN
  ...
  -- ✅ FIX: أولاً، حذف الـ cache للاشتراكات الملغاة/المنتهية
  BEGIN
  ...
  -- ✅ FIX: ثانياً، حذف جميع الاشتراكات الملغاة/المنتهية السابقة لتجنب unique constraint
  DELETE FROM organization_subscriptions
  WHERE organization_id = p_organization_id
    AND status IN ('canceled', 'expired');

  -- ✅ FIX: ثالثاً، إنهاء جميع الاشتراكات النشطة والتجريبية وتحديث end_date إلى الآن
  ...
  SET
    ...
    subscription_tier = NULL,  -- ✅ إزالة tier لإيقاف ظهور "trial"
...
-- Add comment
COMMENT ON FUNCTION admin_terminate_subscription IS 'Fixed: Sets end_date to NOW() when canceling subscription and properly clears subscription_tier to prevent trial status from showing';
