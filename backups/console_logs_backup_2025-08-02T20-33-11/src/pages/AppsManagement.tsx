...
const AppsManagement: React.FC = () => {
  ...
  useEffect(() => {
    console.log('AppsManagement: organizationApps updated:', organizationApps);
    
    // إضافة تأخير صغير للتأكد من تحديث البيانات
    setTimeout(() => {
      console.log('AppsManagement: Data update completed with delay');
    }, 100);
  }, [organizationApps]);
  ...
  const refreshAppsData = async () => {
    try {
      ...
      await refreshApps();
      console.log('Apps data refreshed successfully');
      
      // إضافة تأخير صغير للتأكد من تحديث البيانات
      setTimeout(() => {
        console.log('Apps data refresh completed with delay');
      }, 100);
    } catch (error) {
      console.error('Error refreshing apps data:', error);
      // في حالة فشل التحديث، نحاول تحديث cache البيانات
      try {
        await queryClient.invalidateQueries({ queryKey: ['global-data'] });
        console.log('Apps data refreshed using fallback method');
      } catch (fallbackError) {
  ...
  const handleAppToggle = async (appId: string, enabled: boolean) => {
    ...
    if (isProcessing || processingAppId === appId) {
      console.log('Already processing app:', appId);
      return;
    }
    ...
    try {
      ...
      if (enabled) {
        ...
        console.log('Disabling app:', appId);
        success = await disableApp(appId);
      }
      
      if (success) {
        console.log('App toggle successful:', appId, enabled);
        ...
        // إضافة تأخير صغير للتأكد من تحديث البيانات
        setTimeout(() => {
          console.log('App toggle completed with delay:', appId, enabled);
        }, 200);
      } else {
        console.error('App toggle failed:', appId, enabled);
        // إضافة toast notification للخطأ
        toast.error(`فشل في ${enabled ? 'تفعيل' : 'إلغاء تفعيل'} التطبيق`);
      }
    } catch (error) {
      console.error('Exception in handleAppToggle:', error);
    } finally {
