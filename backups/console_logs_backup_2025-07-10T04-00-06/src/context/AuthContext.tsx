...
// إضافة دالة console مخصصة لتسجيل الدخول
const loginDebugLog = (message: string, data?: any) => {
  if (process.env.NODE_ENV === 'development') {
    console.log(`🔐 [AuthContext - Login] ${message}`, data ? data : '');
  }
};
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // تحسين دالة fetchUserData مع console شامل
  const fetchUserData = useCallback(async () => {
    if (!user?.id || !session?.access_token || isProcessingToken) {
      loginDebugLog('تجاهل جلب بيانات المستخدم', {
        hasUser: !!user?.id,
    ...
    loginDebugLog('=== بدء جلب بيانات المستخدم ===', {
      userId: user.id,
    ...
    try {
      loginDebugLog('جلب ملف المستخدم من قاعدة البيانات');
      ...
      if (profile) {
        loginDebugLog('✅ تم جلب ملف المستخدم بنجاح', {
        ...
        let org = null;
        if (profile.organization_id) {
          loginDebugLog('جلب بيانات المؤسسة', { organizationId: profile.organization_id });
          
          try {
            org = await getOrganizationById(profile.organization_id);
            
            if (org) {
              loginDebugLog('✅ تم جلب بيانات المؤسسة بنجاح', {
                orgId: org.id,
                orgName: org.name,
                ...
                orgSettings: {
                  ...
                  siteName: org.settings?.site_name
                }
              });
              ...
              loginDebugLog('تم حفظ معرف المؤسسة في localStorage', { orgId: org.id });
              
            } else {
              loginDebugLog('⚠️ لم يتم العثور على بيانات المؤسسة');
              setOrganization(null);
            }
          } catch (orgError) {
            loginDebugLog('❌ خطأ في جلب بيانات المؤسسة', orgError);
            setOrganization(null);
          }
        } else {
          loginDebugLog('⚠️ المستخدم لا ينتمي لأي مؤسسة');
          setOrganization(null);
        }
        ...
        loginDebugLog('تم حفظ بيانات المستخدم في localStorage');
      }
    } catch (error) {
      loginDebugLog('❌ خطأ في جلب بيانات المستخدم', error);
      
      // استخدام البيانات المحفوظة عند الخطأ
      if (savedUserData.userProfile && savedUserData.userProfile.id === user.id) {
        loginDebugLog('استخدام البيانات المحفوظة كبديل');
        setUserProfile(savedUserData.userProfile);
        if (savedUserData.organization) {
          setOrganization(savedUserData.organization);
        }
      ...
      loginDebugLog('=== انتهاء جلب بيانات المستخدم ===');
    }
  ...
  const signIn = async (email: string, password: string) => {
    loginDebugLog('=== بدء عملية تسجيل الدخول ===', { email, timestamp: new Date().toISOString() });
    
    setIsLoading(true);
    try {
      loginDebugLog('مسح البيانات القديمة من localStorage');
      ...
      loginDebugLog('محاولة تسجيل الدخول باستخدام Supabase');

      const client = await getSupabaseClient();
      const { data, error } = await client.auth.signInWithPassword({ email, password });

      if (error) {
        loginDebugLog('❌ فشل تسجيل الدخول', { error: error.message, code: error.status });
        setIsLoading(false);
        return { success: false, error };
      }

      if (!data.session || !data.user) {
        loginDebugLog('❌ بيانات الجلسة أو المستخدم غير متاحة');
        setIsLoading(false);
        return { success: false, error: new Error('بيانات الجلسة غير متاحة') };
      }

      loginDebugLog('✅ تم تسجيل الدخول بنجاح', {
        userId: data.user.id,
      ...
      // تحديث حالة المصادقة
      loginDebugLog('تحديث حالة المصادقة في AuthContext');
      updateAuthState(data.session, data.user);
      
      return { success: true, error: null };
    } catch (error) {
      loginDebugLog('❌ خطأ في تسجيل الدخول', error);
      setIsLoading(false);
