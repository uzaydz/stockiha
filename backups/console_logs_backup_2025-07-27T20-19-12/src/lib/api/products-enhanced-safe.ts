...
export const disableProduct = async (productId: string): Promise<{
  ...
  try {
    ...
    if (userError || !userData?.organization_id) {
      console.error('فشل في الحصول على معرف المؤسسة:', userError);
      return {
        success: false,
        error: {
    ...
    console.log('تم تعطيل المنتج بنجاح:', data);
...
export const deleteProductPermanently = async (productId: string): Promise<{
  ...
  try {
    ...
    if (orderCheckError) {
      console.error('فشل التحقق من الطلبات المرتبطة:', orderCheckError);
