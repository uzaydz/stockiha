...
// استيراد Supabase بطريقة متوافقة مع ES Modules
...
if (typeof window !== 'undefined' && window.__BAZAAR_MAIN_SUPABASE_CLIENT__) {
  ...
  try {
    // Create a single, exported Supabase client instance مع معالجة أخطاء
    ...
    console.error('فشل في إنشاء Supabase client:', error);
    console.error('تفاصيل الخطأ:', {
      url: supabaseUrl ? 'موجود' : 'مفقود',
      key: supabaseAnonKey ? 'موجود' : 'مفقود',
      error: error
    });
    
    // إنشاء عميل وهمي كـ fallback
    mainClient = {
      from: () => ({
