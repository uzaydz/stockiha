...
export const useStorePageData = (): UseStorePageDataReturn => {
  ...
  useEffect(() => {
    console.log('🔍 [useStorePageData] فحص شروط تطبيق الثيم:', {
      hasOrganizationSettings: !!organizationSettings,
      hasCentralOrgId: !!centralOrgId,
    ...
    if (organizationSettings && centralOrgId) {
      const applyTheme = async () => {
        try {
          console.log('🎨 [useStorePageData] تطبيق ثيم المؤسسة:', {
            organizationId: centralOrgId,
            primaryColor: organizationSettings.theme_primary_color,
            secondaryColor: organizationSettings.theme_secondary_color,
            themeMode: organizationSettings.theme_mode
          });
          ...
          console.log('✅ [useStorePageData] تم تطبيق الثيم بنجاح');
        } catch (error) {
          console.error('❌ [useStorePageData] خطأ في تطبيق الثيم:', error);
        }
      };
      
      applyTheme();
    } else {
      console.log('⚠️ [useStorePageData] لا يمكن تطبيق الثيم - الشروط غير متوفرة');
    }
