...
export const usePOSOrder = ({
  ...
  const submitOrder = useCallback(async (orderDetails: Partial<Order>) => {
    console.log('🚀 [POS ORDER] بدء إرسال الطلب:', {
    ...
    try {
      ...
      if (cartItems.length > 0 || selectedServices.length > 0) {
        console.log('📦 [POS ORDER] معالجة المنتجات والخدمات:', {
          cartItemsCount: cartItems.length,
          selectedServicesCount: selectedServices.length
        });
        ...
        const orderData: POSOrderData = {
          ...
          items: cartItems.map(item => {
            ...
            console.log(`📦 [POS ORDER] تحضير العنصر:`, {
        ...
        console.log('📋 [POS ORDER] بيانات الطلب المحضرة:', orderData);
