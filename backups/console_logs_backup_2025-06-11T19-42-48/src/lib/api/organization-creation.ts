...
export const createOrganizationSimple = async (
  ...
  try {
    ...
    if (data) {
      console.log(`[createOrganizationSimple] استدعاء createDefaultStoreComponents للمؤسسة ${data}`);
      const componentsResult = await createDefaultStoreComponents(data);
      console.log(`[createOrganizationSimple] نتيجة إنشاء المكونات: ${componentsResult}`);
    }
...
export const createOrganizationDirect = async (
  ...
  try {
    ...
    // إضافة المكونات الافتراضية للمتجر
    console.log(`[createOrganizationDirect] استدعاء createDefaultStoreComponents للمؤسسة ${organizationId}`);
    const componentsResult = await createDefaultStoreComponents(organizationId);
    console.log(`[createOrganizationDirect] نتيجة إنشاء المكونات: ${componentsResult}`);
...
export const createDefaultStoreComponents = async (organizationId: string): Promise<boolean> => {
  ...
  console.log(`${logPrefix} بدء إنشاء المكونات الافتراضية للمؤسسة`);
  ...
  try {
    ...
    if (orgError || !orgCheck) {
      console.error(`${logPrefix} المؤسسة غير موجودة:`, orgError);
      return false;
    }
    
    console.log(`${logPrefix} تم التحقق من وجود المؤسسة: ${orgCheck.name}`);
    ...
    if (checkError) {
      console.error(`${logPrefix} خطأ في فحص المكونات الموجودة:`, checkError);
    } else {
      console.log(`${logPrefix} المكونات الموجودة مسبقاً: ${existingComponents?.length || 0}`, 
        existingComponents?.map(c => c.component_type) || []);
    }
    ...
    console.log(`${logPrefix} محاولة إدراج ${defaultComponents.length} مكونات:`);
    defaultComponents.forEach((comp, index) => {
      console.log(`${logPrefix} [${index + 1}] ${comp.component_type} (order: ${comp.order_index}, id: ${comp.id})`);
    });
    ...
    console.log(`${logPrefix} انتهاء عملية الإدراج في ${insertDuration}ms`);

    if (insertError) {
      console.error(`${logPrefix} خطأ في إضافة المكونات الافتراضية:`, {
      ...
      console.log(`${logPrefix} محاولة إدراج المكونات بشكل فردي...`);
      let successCount = 0;
      ...
      for (let i = 0; i < defaultComponents.length; i++) {
        ...
        try {
          ...
          if (singleError) {
            console.error(`${logPrefix} فشل إدراج المكون ${component.component_type}:`, singleError);
          } else {
            console.log(`${logPrefix} تم إدراج المكون ${component.component_type} بنجاح`);
            successCount++;
          }
        } catch (singleException) {
          console.error(`${logPrefix} استثناء في إدراج المكون ${component.component_type}:`, singleException);
        }
      }
      
      console.log(`${logPrefix} تم إدراج ${successCount} من أصل ${defaultComponents.length} مكونات بنجاح`);
      return successCount > 0;
    }

    console.log(`${logPrefix} تم إدراج المكونات بنجاح:`, insertedData?.length || 0);
    if (insertedData) {
      insertedData.forEach(comp => {
        console.log(`${logPrefix} ✓ تم إدراج: ${comp.component_type} (order: ${comp.order_index})`);
      });
    }
    ...
    if (finalError) {
      console.error(`${logPrefix} خطأ في التحقق النهائي:`, finalError);
    } else {
      console.log(`${logPrefix} إجمالي المكونات في قاعدة البيانات: ${finalCheck?.length || 0}`);
      finalCheck?.forEach(comp => {
        console.log(`${logPrefix} DB: ${comp.component_type} (order: ${comp.order_index}, active: ${comp.is_active})`);
      });
    }
    ...
    if (actualCount === expectedCount) {
      console.log(`${logPrefix} ✅ تم إنشاء جميع المكونات بنجاح (${actualCount}/${expectedCount})`);
      
      // التحقق النهائي باستخدام أداة المراقبة
      setTimeout(async () => {
        const report = await monitorStoreComponents(organizationId);
        console.log(`${logPrefix} تقرير المراقبة النهائي:`, report);
      }, 1000);
      
      return true;
    } else {
      console.warn(`${logPrefix} ⚠️ لم يتم إنشاء جميع المكونات (${actualCount}/${expectedCount})`);
      
      // إنشاء تقرير مفصل للمشكلة
      setTimeout(async () => {
        const report = await monitorStoreComponents(organizationId);
        console.error(`${logPrefix} تقرير المشكلة:`, report);
      }, 1000);
    ...
    console.error(`${logPrefix} استثناء في إنشاء المكونات الافتراضية:`, {
      message: error instanceof Error ? error.message : 'استثناء غير معروف',
