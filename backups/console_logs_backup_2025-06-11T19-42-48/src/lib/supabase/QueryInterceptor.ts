...
class QueryInterceptor {
      ...
      if (this.enabled) {
        ...
        if (url.includes('supabase.co/rest/v1/')) {
          ...
          if (this.isDuplicateQuery(key)) {
            console.warn(`🔄 Duplicate query detected: ${key}`);
          }
        }
      }
  ...
  generateReport() {
    ...
    console.group('📊 Query Statistics Report');
    ...
    console.table(
      sortedQueries.map(([key, log]) => ({
        Query: key,
        Count: log.count,
        'Avg Rate': `${(log.count / ((Date.now() - log.timestamp) / 1000)).toFixed(2)}/s`,
      }))
    );
    ...
    if (problematicQueries.length > 0) {
      console.warn('⚠️ High-frequency queries detected:');
      problematicQueries.forEach(([key, log]) => {
        console.warn(`  - ${key}: ${log.count} times`);
      });
    }
...
if (process.env.NODE_ENV === 'development') {
  ...
  console.log('🔍 Query Interceptor initialized. Use window.__queryInterceptor to access stats.');
}
