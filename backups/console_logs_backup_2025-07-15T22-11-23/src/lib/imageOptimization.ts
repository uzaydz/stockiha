...
export const optimizeStoreImage = (url: string, type: 'product' | 'category' | 'thumbnail' = 'product'): string => {
  ...
  // إذا كانت الصورة من Supabase، استخدم تحسين بسيط
  if (url.includes('supabase.co') || url.includes('supabase.in')) {
    try {
      const newUrl = new URL(url);
      const configs = {
        product: { width: '400', quality: '80' },
        category: { width: '600', quality: '80' },
        thumbnail: { width: '300', quality: '75' }
      };
      
      const config = configs[type];
      newUrl.searchParams.set('width', config.width);
      newUrl.searchParams.set('quality', config.quality);
  ...
  // للصور الأخرى، استخدم التحسين الكامل
  const configs = {
