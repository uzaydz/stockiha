...
export const useProductDataLoader = ({
  ...
  useEffect(() => {
    ...
    const loadProductWithRetry = async () => {
      console.log('🔍 loadProductWithRetry - بدء التحميل:', {
        slug,
        organizationId,
        isOrganizationLoading,
      ...
      if (!organizationId && isOrganizationLoading) {
        console.log('⏳ loadProductWithRetry - انتظار تحميل المؤسسة');
        setIsLoading(true);
        return;
      }
      ...
      if (!slug || !organizationId) {
        console.log('❌ loadProductWithRetry - بيانات ناقصة:', { slug, organizationId, isOrganizationLoading });
        if (!isOrganizationLoading && !organizationId) {
          console.log('❌ معرف المؤسسة غير متاح');
        }
        setIsLoading(false);
        return;
      }
