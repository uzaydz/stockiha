...
import {
    ...
    Menu, X, AlertCircle, Share2, Bookmark, MoreVertical
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { ScrollArea } from '@/components/ui/scroll-area';
import { cn } from '@/lib/utils';
import { SystemModule, SystemLesson } from '@/data/system-training';

interface SystemTrainingPlayerProps {
    module: SystemModule;
...
const SystemTrainingPlayer: React.FC<SystemTrainingPlayerProps> = ({
    module,
    currentLessonIndex,
    onLessonChange,
    onNextModule,
    ...
    // Styling constants - using semantic classes for theme support
    const theme = {
        primary: 'text-orange-600 dark:text-orange-500',
        primaryBg: 'bg-orange-600 dark:bg-orange-500',
        primaryBorder: 'border-orange-600 dark:border-orange-500',
        // Backgrounds
        ...
        <div className={`flex flex-col h-full ${theme.mainBg} ${theme.textPrimary} overflow-hidden font-sans`}>
            ...
            <header className={`h-16 border-b ${theme.borderColor} bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 flex items-center justify-between px-4 z-20 shrink-0`}>
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="icon" onClick={() => navigate('/dashboard/courses-operations/system-training')} title="Back to Course">
                        <ChevronRight className="w-5 h-5 rtl:rotate-180 text-muted-foreground hover:text-orange-500 transition-colors" />
                    </Button>

                    <div className="flex flex-col">
                        <span className="text-xs font-bold text-orange-500 uppercase tracking-wider">نظام سطوكيها</span>
                ...
                <div className="flex items-center gap-2">
                    <Button
                        variant="ghost"
                        size="sm"
                        className="hidden md:flex gap-2 text-muted-foreground hover:text-orange-500"
                    ...
                    <div className="flex-1 bg-black flex items-center justify-center relative group min-h-[300px] md:min-h-0 shadow-lg">
                        ...
                        {currentLesson.status === 'ready' && currentLesson.videoUrl ? (
                            ...
                            <div className="w-full h-full relative">
                                {/* Here ideally we put an iframe or video tag */}
                            ...
                            <div className="flex flex-col items-center justify-center text-center p-8 max-w-md animate-in fade-in zoom-in duration-500">
                                ...
                                <Button variant="outline" className="border-orange-500/30 text-orange-400 hover:bg-orange-500/10 hover:text-orange-300 gap-2">
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h4 className={cn(
                                                ...
                                                isActive ? "text-orange-500 dark:text-orange-400" : "text-foreground/80 group-hover:text-foreground"
                                            )}>
                                                {lesson.title}
                                            </h4>
                                        ...
                                        {isActive && (
                                            <div className="absolute right-0 top-2 bottom-2 w-1 bg-orange-500 rounded-l-full shadow-[0_0_10px_rgba(249,115,22,0.5)]" />
                                        )}
                                    </div>
                                );
                            })}
...
export default SystemTrainingPlayer;
