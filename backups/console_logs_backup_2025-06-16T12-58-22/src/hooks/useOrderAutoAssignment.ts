...
export const useOrderAutoAssignment = (options: UseOrderAutoAssignmentOptions = {}) => {
  ...
  const autoAssignOrders = useCallback(async () => {
    ...
    try {
      ...
      if (fetchError) {
        console.error('Error fetching unassigned orders:', fetchError);
        onError?.(fetchError);
        return;
      }
      ...
      if (unassignedOrders && unassignedOrders.length > 0) {
        ...
        for (const order of unassignedOrders) {
          try {
            ...
            if (error) {
              console.error('Error auto-assigning order:', order.id, error);
              onError?.(error);
            } else if ((data as any)?.success) {
              console.log('Successfully assigned order:', order.id, 'to agent:', (data as any).agent_id);
            ...
            console.error('Error in auto-assignment for order:', order.id, err);
            onError?.(err);
          }
      ...
      console.error('Error in auto-assignment process:', err);
      onError?.(err);
    } finally {
  ...
  const startAutoAssignment = useCallback(() => {
    ...
    if (enabled && currentOrganization?.id) {
      ...
      console.log(`Auto-assignment started with ${interval}ms interval`);
    }
  ...
  const stopAutoAssignment = useCallback(() => {
    if (intervalRef.current) {
      ...
      console.log('Auto-assignment stopped');
    }
  }, []);
