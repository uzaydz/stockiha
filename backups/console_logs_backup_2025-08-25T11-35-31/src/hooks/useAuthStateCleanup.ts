    ...
    const cleanupAuthState = async () => {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('🧹 [useAuthStateCleanup] تم تطهير الحالة القديمة');
        }

      } catch (err) {
        console.error('❌ [useAuthStateCleanup] خطأ في تطهير الحالة:', err);
      }
    };
...
export const checkAuthStateSync = async (): Promise<boolean> => {
  try {
    ...
    console.error('❌ [checkAuthStateSync] خطأ في فحص تزامن الحالة:', err);
    return false;
  }
};
