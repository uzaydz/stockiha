...
export const ShippingProviderColumn: React.FC<ShippingProviderColumnProps> = ({ 
  ...
  // دالة مشتركة لعرض قائمة الشركات
  const renderProviderList = (providers: typeof enabledProviders, excludeCurrent = false) => {
    // إزالة التكرار بناءً على provider_code
    const uniqueProviders = providers.filter((provider, index, self) => 
      index === self.findIndex(p => p.provider_code === provider.provider_code)
    );
    
    const filteredProviders = excludeCurrent && activeProvider 
      ? uniqueProviders.filter(p => p.provider_code !== activeProvider.code)
      : uniqueProviders;
        ...
        >
          ...
          {isCurrentlyLoading && (
            <span className="text-xs text-blue-600">
              {excludeCurrent ? 'جاري التحويل...' : 'جاري الإرسال...'}
            </span>
          )}
        </DropdownMenuItem>
      );
      ...
      <div className={cn("flex items-center gap-2", className)}>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-64 rounded-lg border shadow-lg">
            ...
            {renderProviderList(enabledProviders, true)}
              
            {(() => {
              const uniqueProviders = enabledProviders.filter((provider, index, self) => 
                index === self.findIndex(p => p.provider_code === provider.provider_code)
              );
              const otherProviders = uniqueProviders.filter(p => p.provider_code !== activeProvider.code);
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56 rounded-lg border shadow-lg">
          ...
          <DropdownMenuSeparator />
                     ...
                     {renderProviderList(enabledProviders, false)}
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
  );
