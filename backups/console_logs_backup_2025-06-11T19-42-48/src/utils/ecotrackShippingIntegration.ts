  ...
  try {
    ...
    if (address?.municipality && !isNaN(parseInt(address.municipality))) {
      ...
      const { data: municipalityData } = await supabase
        ...
        .single();
      municipalityName = municipalityData?.name || 'Alger';
      
      // Clean municipality name for Ecotrack API (remove special characters)
      municipalityName = cleanFrenchText(municipalityName);
    } else {
      municipalityName = city || 'Alger';
    }
    ...
    for (const part of addressParts) {
      ...
      if (part.includes('البلدية:')) {
        municipalityName = part.replace('البلدية:', '').trim() || municipalityName;
      }
    }
    ...
    const orderParams = {
      ...
      TypeColis: 1, // 0: Échange (Exchange), 1: Livraison (Normal delivery)
      Confrimee: 1, // Confirmed
      TProd: productsDescription
    };

    console.log('📦 [EcotrackIntegration] بيانات الطلب:', orderParams);
    console.log('🎯 [EcotrackIntegration] نوع العملية - TypeColis:', orderParams.TypeColis, orderParams.TypeColis === 1 ? '(Livraison)' : '(Échange)');
    console.log('🔍 [EcotrackIntegration] تفاصيل إضافية:', {
      customerName,
      customerPhone,
      city,
      municipalityName,
