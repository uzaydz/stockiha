...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  console.log('[AUTH_PROVIDER] Component rendered/updated.', {
    isLoading,
    user: !!user,
    session: !!session,
    ...
    isExplicitSignOut,
    isProcessingToken,
    hasSavedState: !!savedAuthState.session
  });
  ...
  const updateAuthState = useCallback((newSession: Session | null, newUser: SupabaseUser | null, clearAll: boolean = false) => {
    console.log('[AUTH] updateAuthState called:', { 
      hasNewSession: !!newSession, 
      hasNewUser: !!newUser, 
      clearAll,
      currentSessionId: session?.access_token?.slice(-10),
      newSessionId: newSession?.access_token?.slice(-10)
    });
    ...
    if (!clearAll && session && newSession && user && newUser) {
      if (session.access_token === newSession.access_token && user.id === newUser.id) {
        console.log('[AUTH] Skipping state update - no actual changes detected');
        return;
      }
    }
  ...
  const signOutAndClearState = useCallback(async () => {
    console.log('[AUTH] signOutAndClearState: Starting explicit sign out.');
    setIsExplicitSignOut(true);
    
    try {
      const { error } = await supabase.auth.signOut();
      if (error) {
        console.warn('[AUTH] signOutAndClearState: Supabase signOut failed (but continuing):', error);
      }
    } catch (error) {
      console.warn('[AUTH] signOutAndClearState: Exception during signOut (but continuing):', error);
    }
    
    updateAuthState(null, null, true);
    console.log('[AUTH] signOutAndClearState: State cleared.');
  }, [updateAuthState]);

  // دالة تحديث البيانات
  const refreshData = useCallback(async () => {
    if (!user || !session) {
      console.log('[AUTH] refreshData: No user/session to refresh.');
      return;
    }

    console.log('[AUTH] refreshData: Starting data refresh...');
    setIsLoading(true);
    ...
    try {
      ...
      console.log('[AUTH] refreshData: Refresh completed successfully.');
    } catch (error) {
      console.error('[AUTH] refreshData: Error during data refresh.', error);
      await signOutAndClearState();
    } finally {
      console.log('[AUTH] refreshData: Refresh finished.');
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (savedAuthState.session && savedAuthState.user && initialLoadRef.current) {
      console.log('[AUTH_INIT] Loading saved profile and organization data...');
      ...
      try {
        ...
        if (savedProfile) {
          ...
          setUserProfile(profile);
          console.log('[AUTH_INIT] Loaded saved user profile:', profile);
        }
        ...
        if (savedOrg) {
          ...
          setOrganization(org);
          console.log('[AUTH_INIT] Loaded saved organization:', org);
        }
      } catch (error) {
        console.warn('[AUTH_INIT] Failed to load saved data:', error);
      }
  ...
  useEffect(() => {
    console.log('[AUTH_EFFECT] Initializing AuthProvider...');
    ...
    const handleVisibilityChange = () => {
      ...
      console.log('[AUTH_VISIBILITY] Page visibility changed:', {
        wasVisible,
        isVisible,
        changeType: isVisible ? 'focused' : 'blurred'
      });
    };
    ...
    const initialize = async () => {
      ...
      if (isInitializingRef.current) {
        console.log('[AUTH_INIT] Already initializing, skipping...');
        return;
      }
      ...
      if (savedAuthState.session) {
        console.log('[AUTH_INIT] Found saved session, validating...');
        
        try {
          // التحقق من صحة الجلسة المحفوظة
          const isValid = await validateSessionPeriodically(savedAuthState.session);
          if (isValid) {
            console.log('[AUTH_INIT] Saved session is valid, using it.');
            // الجلسة صحيحة، يمكن بدء جلب البيانات في الخلفية
            if (savedAuthState.user && !userProfile) {
              refreshData();
            }
          } else {
            console.log('[AUTH_INIT] Saved session is invalid, clearing...');
            updateAuthState(null, null, true);
            setIsLoading(true);
          }
        } catch (error) {
          console.warn('[AUTH_INIT] Error validating saved session:', error);
          updateAuthState(null, null, true);
          setIsLoading(true);
        }
      } else {
        ...
        if (lastEventRef.current) {
          ...
          if (event === 'SIGNED_IN' && lastEvent === 'SIGNED_IN' && lastSessionId === sessionId) {
            if ((now - lastTimestamp) < 5000 || isRecentVisibilityChange) {
              console.log(`[AUTH_LISTENER] Ignoring duplicate ${event} event for same session (within 5s or due to page focus)`);
              return;
            }
          }
          ...
          if (lastEvent === event && lastSessionId === sessionId && (now - lastTimestamp) < 1000) {
            console.log(`[AUTH_LISTENER] Ignoring duplicate ${event} event for same session (within 1s)`);
            return;
          }
        }

        console.log(`[AUTH_LISTENER] onAuthStateChange event: ${event}`, { 
          hasSession: !!newSession, 
          isExplicitSignOut,
          sessionId,
          lastEventTime: lastEventRef.current?.timestamp ? now - lastEventRef.current.timestamp : 'N/A'
        });
          ...
          if (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') {
            ...
            if (event === 'SIGNED_IN' && session && newSession) {
              ...
              if (session.access_token === newSession.access_token) {
                console.log('[AUTH_LISTENER] Ignoring SIGNED_IN - same access_token as current session');
                return;
              }
            }
            ...
            if (isExplicitSignOut || (!user && !userProfile)) {
              console.log('[AUTH_LISTENER] Processing SIGNED_OUT event (explicit or no existing user data)');
              updateAuthState(null, null, true);
            } else {
              console.log('[AUTH_LISTENER] Ignoring SIGNED_OUT event - appears to be caused by network error, preserving user state');
              return;
            }
      ...
      if (authToken) {
        console.log('[AUTH_INIT] Found auth_token in URL. Will attempt to set session.');
        ...
        try {
          ...
          if (decodedSession) {
            ...
            if (error) {
              console.error('[AUTH_INIT] Error setting session from token:', error);
              setIsProcessingToken(false);
              await signOutAndClearState();
            } else if (data.session && data.user) {
              console.log('[AUTH_INIT] Session set directly from token return value. User state will be updated.');
              updateAuthState(data.session, data.user);
            } else {
              console.error('[AUTH_INIT] setSession returned no error, but also no session/user data. Signing out.');
              setIsProcessingToken(false);
              await signOutAndClearState();
            }
          }
        } catch (error) {
          console.error('[AUTH_INIT] Failed to decode or set session from auth_token:', error);
          setIsProcessingToken(false);
          await signOutAndClearState();
        }
              } else if (!savedAuthState.session) {
          // 3. If no token and no saved session, check for an existing session in storage.
          console.log('[AUTH_INIT] No auth_token and no saved session. Checking for existing session with getSession().');
          const { data: { session: initialSession } } = await supabase.auth.getSession();
          console.log('[AUTH_INIT] Initial getSession() result:', { hasSession: !!initialSession });
          
          if (!initialSession) {
            setIsLoading(false);
            console.log('[AUTH_INIT] No active session found. App is ready for login.');
          }
        }

        // إنهاء التهيئة
        isInitializingRef.current = false;
        console.log('[AUTH_INIT] Initialization completed.');

      return () => {
        console.log('[AUTH_EFFECT] Cleaning up AuthProvider subscription.');
        isInitializingRef.current = false;
    ...
    const fetchUserData = async () => {
      if (user && session) {
        ...
        if (fetchingUserDataRef.current) {
          console.log('[USER_EFFECT] Already fetching user data, skipping...');
          return;
        }
        ...
        if (userProfile && organization && initialLoadRef.current === false) {
          console.log('[USER_EFFECT] User data already loaded from localStorage, skipping immediate fetch...');
          setIsLoading(false);
          return;
        }

        fetchingUserDataRef.current = true;
        console.log('[USER_EFFECT] User detected. Fetching profile and org data...', { userId: user.id });
        setIsLoading(true);
        
        try {
          const profile = await getCurrentUserProfile();
          console.log('[USER_EFFECT] Fetched user profile:', profile);
          setUserProfile(profile);
          ...
          if (profile?.organization_id) {
            console.log(`[USER_EFFECT] Profile has organization_id: ${profile.organization_id}. Fetching org...`);
            const org = await getOrganizationById(profile.organization_id);
            console.log('[USER_EFFECT] Fetched organization:', org);
            setOrganization(org);
            ...
            if (org) {
             ...
             console.log('[USER_EFFECT] Profile has no organization_id.');
             setOrganization(null);
             localStorage.removeItem('current_organization');
          }
        } catch (error) {
          console.error('[USER_EFFECT] Error fetching user data:', error);
        } finally {
          console.log('[USER_EFFECT] Finished fetching user data.');
          setIsLoading(false);
          setIsProcessingToken(false);
        ...
        if (isProcessingToken) {
          console.log('[USER_EFFECT] Token processing is in progress, waiting for session update...');
          return;
        }

        if (!isLoading) return;
        console.log('[USER_EFFECT] No user detected. Clearing data and stopping load.');
        setUserProfile(null);
        setOrganization(null);
        setIsLoading(false);
      }
