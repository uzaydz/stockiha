...
class ConversionTracker {
  ...
  public organizationId?: string; // معرف المؤسسة
  ...
  constructor(private productId: string, private externalSettings?: any) {
    if (externalSettings) {
      console.log('🎯 ConversionTracker: استخدام الإعدادات الخارجية الموجودة');
  ...
  private async initializeSettings(): Promise<void> {
    try {
      // إذا كانت الإعدادات موجودة بالفعل، لا نحتاج لجلبها مرة أخرى
      if (this.settings) {
        return;
      }
      ...
      // استخدام Supabase client الموحد لتجنب Multiple GoTrueClient instances
      const { getSupabaseClient } = await import('@/lib/supabase');
      const supabase = getSupabaseClient();

      console.log('🔄 ConversionTracker: جلب البيانات من قاعدة البيانات');

      // استدعاء دالة get_product_complete_data مباشرة
      const { data: productData, error } = await supabase.rpc('get_product_complete_data' as any, {
      ...
      if (error) {
        console.error('❌ ConversionTracker: خطأ في جلب بيانات المنتج:', error);
        return;
      }

      if (!productData || !(productData as any).success) {
        console.warn('⚠️ ConversionTracker: لم يتم العثور على بيانات المنتج');
        return;
      }

      const product = (productData as any).data?.product;
      
      if (product) {
        // البحث عن marketing_settings في المكان الصحيح
        const marketingSettings = product.marketing_settings || product.marketing_settings;
  ...
  private async sendToFacebook(event: ConversionEvent): Promise<void> {
    try {
      ...
      if (typeof window !== 'undefined' && window.fbq) {
        ...
        if (event.currency) eventData.currency = event.currency || 'DZD';
  ...
  private async sendToFacebookConversionAPI(event: ConversionEvent, eventId: string): Promise<void> {
    ...
    // إعداد بيانات العميل المحسّنة للتمرير إلى Facebook Conversion API
    const enhancedCustomerData = {
      ...
      // استخدام الاسم الكامل إذا كان متوفراً، وإلا استخدم firstName و lastName منفصلين
      name: event.user_data?.firstName && event.user_data?.lastName 
        ? `${event.user_data.firstName} ${event.user_data.lastName}`.trim()
        : event.user_data?.firstName || '',
      fullName: event.user_data?.firstName && event.user_data?.lastName 
        ? `${event.user_data.firstName} ${event.user_data.lastName}`.trim()
        : event.user_data?.firstName || '',
      customer_name: event.user_data?.firstName && event.user_data?.lastName 
        ? `${event.user_data.firstName} ${event.user_data.lastName}`.trim()
        : event.user_data?.firstName || '',
      ...
      municipality: event.user_data?.city, // إضافة نسخة بمفتاح مختلف
      state: event.user_data?.state,
      province: event.user_data?.state, // إضافة نسخة بمفتاح مختلف
...
export function getConversionTracker(productId: string, settings?: any, organizationId?: string): ConversionTracker {
  const cacheKey = `${productId}_${settings ? 'with_settings' : 'no_settings'}`;
  
  if (!trackers.has(cacheKey)) {
    const tracker = new ConversionTracker(productId, settings);
    if (organizationId) {
      tracker.organizationId = organizationId;
    }
    trackers.set(cacheKey, tracker);
  } else if (settings) {
