-- إصلاح مشكلة تحويل JSONB إلى TEXT[] في حقل images
-- تاريخ: 2024-11-08

-- إعادة إنشاء وظيفة update_product_and_return_full مع إصلاح لمشكلة تحويل الصور
...
BEGIN
  ...
  SET
    ...
    images = CASE 
              WHEN product_updates ? 'images' THEN 
                -- استخدام json_array_elements_text للتعامل مع المصفوفة بشكل صحيح
                ARRAY(SELECT json_array_elements_text(product_updates->'images'))
              ELSE images 
            END,
    ...
    features = CASE 
                WHEN product_updates ? 'features' THEN 
                  ARRAY(SELECT json_array_elements_text(product_updates->'features'))
                ELSE features 
              END,
...
-- إضافة تعليق توضيحي
COMMENT ON FUNCTION update_product_and_return_full IS 'وظيفة لتحديث المنتج وإرجاع البيانات المحدثة مع العلاقات (مع إصلاح مشكلة مصفوفة الصور)'; 