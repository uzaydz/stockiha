...
import { createProductColor, createProductImage, deleteProductColor, deleteProductImage, updateProductColor, getProductSizes, createProductSize, updateProductSize, deleteProductSize } from '@/lib/api/productVariants';
  ...
  const updateProductVariants = async (values: ProductFormValues) => {
    try {
      ...
      try {
        ...
        const existingImages = await getProductSizes(product.id);
        
        // ثانياً: احذف الصور التي لم تعد موجودة
        for (const img of existingImages) {
          if (!additionalImages.includes((img as any).image_url)) {
            await deleteProductImage((img as any).id);
          }
        }
        ...
        for (let i = 0; i < additionalImages.length; i++) {
          ...
          const existingImg = existingImages.find((img: any) => img.image_url === imageUrl);
