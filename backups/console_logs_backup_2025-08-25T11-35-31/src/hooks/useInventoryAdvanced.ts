...
export function useInventoryAdvanced(
  ...
  // التحديث التلقائي البسيط
  useEffect(() => {
    if (!config.enableRealTimeStats || !config.autoRefreshInterval) return;
  ...
  }, [config.enableRealTimeStats, config.autoRefreshInterval, loadStats]);

  // دوال للتوافق مع API السابق - مبسطة
  const startAutoRefresh = useCallback(() => {
    // التحديث التلقائي يعمل الآن تلقائياً عبر useEffect
  }, []);

  const stopAutoRefresh = useCallback(() => {
    // لإيقاف التحديث - يمكن تعديل config
  }, []);
