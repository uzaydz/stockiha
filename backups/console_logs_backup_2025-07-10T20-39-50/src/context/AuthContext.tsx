...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchUserData = useCallback(async () => {
    console.log('🔄 [AuthContext-fetchUserData] === محاولة جلب بيانات المستخدم ===', {
      hasUser: !!user?.id,
    ...
    if (!user || !session) {
      console.log('⚠️ [fetchUserData] لا يوجد مستخدم أو جلسة');
    ...
    try {
      console.log('🔄 [fetchUserData] استدعاء getCurrentUserProfile...');
      ...
      console.log('🔄 [fetchUserData] نتيجة getCurrentUserProfile:', {
        hasProfile: !!profile,
      ...
      // إذا لم نحصل على organizationId، نحاول استعلام مباشر
      if (profile && !profile.organization_id) {
        console.log('⚠️ [fetchUserData] المستخدم بدون organization_id، محاولة استعلام مباشر...');
        
        // محاولة استعلام مباشر بكلا من id و auth_user_id
        const client = await getSupabaseClient();
        const { data: directUser, error: directError } = await client
          .from('users')
          .select('*')
          .or(`id.eq.${user.id},auth_user_id.eq.${user.id}`)
          .single();
        
        console.log('🔄 [fetchUserData] نتيجة الاستعلام المباشر:', {
          hasDirectUser: !!directUser,
          directUserId: directUser?.id,
          directOrgId: directUser?.organization_id,
          directError: directError?.message
        });
        
        if (directUser && directUser.organization_id) {
          profile = directUser;
          console.log('✅ [fetchUserData] تم العثور على organization_id في الاستعلام المباشر:', directUser.organization_id);
        }
      }
      
      if (profile) {
        console.log('✅ [fetchUserData] تم جلب بيانات المستخدم بنجاح:', {
          role: profile.role,
          organizationId: profile.organization_id,
        ...
        let org = null;
        if (profile.organization_id) {
          console.log('🔄 [fetchUserData] جلب بيانات المؤسسة، ID:', profile.organization_id);
          
          try {
            org = await getOrganizationById(profile.organization_id);
            
            if (org) {
              console.log('✅ [fetchUserData] تم جلب بيانات المؤسسة بنجاح:', {
                orgId: org.id,
                orgName: org.name,
                orgSubdomain: org.subdomain,
                hasSettings: !!org.settings,
                primaryColor: org.settings?.theme_primary_color,
                secondaryColor: org.settings?.theme_secondary_color,
                mode: org.settings?.theme_mode
              });
              ...
              console.log('💾 [fetchUserData] تم حفظ معرف المؤسسة في localStorage:', org.id);
              
              // 🎨 تطبيق الثيم مباشرة بعد جلب بيانات المؤسسة
              if (org.settings?.theme_primary_color || org.settings?.theme_secondary_color || org.settings?.theme_mode) {
                console.log('🎨 [fetchUserData] تطبيق ثيم المؤسسة مباشرة:', {
                  primaryColor: org.settings.theme_primary_color,
                  secondaryColor: org.settings.theme_secondary_color,
                  mode: org.settings.theme_mode
                });
                ...
                try {
                  ...
                  updateOrganizationTheme(org.id, {
                    theme_primary_color: org.settings.theme_primary_color,
                    theme_secondary_color: org.settings.theme_secondary_color,
                    theme_mode: org.settings.theme_mode,
                    custom_css: org.settings.custom_css
                  });
                  
                  console.log('✅ [fetchUserData] تم تطبيق ثيم المؤسسة فوراً في AuthContext');
                  
                } catch (themeError) {
                  console.log('❌ [fetchUserData] خطأ في تطبيق ثيم المؤسسة:', themeError);
                }
              } else {
                console.log('⚠️ [fetchUserData] لا توجد إعدادات ثيم في المؤسسة');
              }
            } else {
              console.log('❌ [fetchUserData] لم يتم العثور على بيانات المؤسسة');
            }
            
          } catch (orgError) {
            console.log('❌ [fetchUserData] خطأ في جلب بيانات المؤسسة:', orgError);
            setOrganization(null);
          }
        } else {
          console.log('⚠️ [fetchUserData] المستخدم لا ينتمي لأي مؤسسة');
          setOrganization(null);
        }
      ...
      console.log('🏁 [fetchUserData] === انتهاء عملية جلب بيانات المستخدم ===');
    }
  }, [user?.id, session?.access_token, isProcessingToken, userProfile?.id, organization?.id, savedUserData]);

  // useEffect محسن مع debouncing قوي ومنع التكرار
  useEffect(() => {
    // منع الاستدعاء إذا كان هناك user و userProfile مطابقين بالفعل
    if (user && userProfile && userProfile.id === user.id && hasInitialSessionCheck && !isProcessingToken) {
      return;
    }
  ...
  }, [user?.id, session?.access_token, hasInitialSessionCheck]); // استخدام dependencies محددة بدلاً من fetchUserData كاملة
