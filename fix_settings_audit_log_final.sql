-- الإصلاح النهائي لمشكلة "column "old_values" of relation "settings_audit_log" does not exist"
-- تاريخ: 2024

-- 1. إضافة عمود old_values لإصلاح الخطأ النهائي
ALTER TABLE public.settings_audit_log 
ADD COLUMN IF NOT EXISTS old_values TEXT;

-- 2. إنشاء وتطبيق view للتوافق بين الاسمين old_value و old_values
...
-- إنشاء محفز لنسخ القيمة القديمة إلى العمود الجديد
...
-- 3. حذف وإعادة تعريف محفزات سجل التدقيق
...
-- 4. تحديث الدالة المسؤولة عن تسجيل التغييرات
...
-- 6. إنشاء نسخة نهائية من وظائف متجر المؤسسة مع تعطيل المحفزات

-- 6.1 تهيئة متجر المؤسسة
...
BEGIN
  ...
  -- تعطيل المحفزات مؤقتًا (طريقة أكثر فعالية من تعطيل محفز محدد)
  SET session_replication_role = 'replica';
