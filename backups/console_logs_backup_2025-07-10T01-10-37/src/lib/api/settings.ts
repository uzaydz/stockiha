...
export const updateOrganizationSettings = async (
  ...
  try {
    ...
    // مسح التخزين المؤقت بعد التحديث الناجح
    if (data && typeof window !== 'undefined') {
      try {
        // مسح cache من UnifiedRequestManager
        if ((window as any).clearOrganizationUnifiedCache) {
          (window as any).clearOrganizationUnifiedCache(organizationId);
        }
        
        // مسح cache من storeCache إذا كان متاحاً
        const { clearCacheByPattern } = await import('@/lib/cache/storeCache');
        await clearCacheByPattern(`settings:${organizationId}`);
        await clearCacheByPattern(`unified_org_settings_${organizationId}`);
      } catch (cacheError) {
        // تسجيل صامت للأخطاء
        console.warn('تحذير: فشل في مسح التخزين المؤقت بعد تحديث الإعدادات:', cacheError);
      }
    }
