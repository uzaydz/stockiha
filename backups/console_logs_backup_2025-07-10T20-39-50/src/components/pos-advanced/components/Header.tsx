...
const Header: React.FC<HeaderProps> = ({
  ...
  const handleBarcodeSubmit = (e: React.FormEvent) => {
    ...
    if (!barcode) {
      console.warn('⚠️ [Header] باركود فارغ');
      return;
    }
    
    if (!onBarcodeSearch) {
      console.error('❌ [Header] دالة onBarcodeSearch غير متوفرة');
      return;
    }
    
    console.log('🔍 [Header] استدعاء دالة البحث بالباركود:', barcode);
    console.log('🔗 [Header] دالة البحث:', typeof onBarcodeSearch);
    
    try {
      onBarcodeSearch(barcode);
      setBarcodeInput(''); // تصفير فوري للحقل
      console.log('✅ [Header] تم استدعاء دالة البحث بنجاح');
    } catch (error) {
      console.error('💥 [Header] خطأ في استدعاء دالة البحث:', error);
    }
  };
