...
export async function getYalidineSettingsForProductPurchase(
  ...
  try {
    console.log('🚀 استدعاء RPC محسن لجلب إعدادات ياليدين:', organizationId);
    ...
    if (error) {
      console.error('❌ خطأ في RPC إعدادات ياليدين:', error);
      return null;
    }

    if (!data) {
      console.warn('⚠️ لا توجد بيانات من RPC إعدادات ياليدين');
      return null;
    }

    console.log('✅ نجح RPC إعدادات ياليدين:', data);
    return data as {
      success: boolean;
      data?: {
        yalidine_provider_id: number;
    ...
    console.error('❌ خطأ في استدعاء RPC إعدادات ياليدين:', error);
    return null;
  }
}
...
async function fetchYalidineFeesFromAPIOptimized(
  ...
  try {
    ...
    if (!settingsResult || !settingsResult.success || !settingsResult.data) {
      console.warn('⚠️ فشل في جلب إعدادات ياليدين:', settingsResult?.message || 'Unknown error');
      return null;
    }
    ...
    if (!api_credentials.api_token || !api_credentials.api_key) {
      console.warn('⚠️ بيانات اعتماد ياليدين غير متوفرة');
      return null;
    }
    ...
    if (!response.ok) {
      ...
      console.error('❌ فشل طلب API ياليدين:', response.status, errorText);
      return null;
    }
    ...
    if (!rawData || Object.keys(rawData).length === 0 || !rawData.per_commune) {
      console.warn('⚠️ استجابة فارغة من API ياليدين');
      return null;
    }
    ...
    console.error('❌ خطأ في جلب أسعار ياليدين المحسن:', error);
    return null;
  }
}
...
export async function calculateDeliveryPrice(
  ...
  try {
    ...
    if (!apiData) {
      console.log('⚠️ النسخة المحسنة فشلت، استخدام النسخة القديمة...');
