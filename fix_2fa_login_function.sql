...
-- تسجيل في سجل الأمان
INSERT INTO security_audit_log (user_id, action, details, created_at)
SELECT 
    (SELECT id FROM users WHERE email = 'uzaydz33030@gmail.com' LIMIT 1),
    'SYSTEM_UPDATE',
    'Fixed verify_2fa_for_login function - resolved user ID validation issue',
    NOW()
WHERE EXISTS (SELECT 1 FROM users WHERE email = 'uzaydz33030@gmail.com'); 