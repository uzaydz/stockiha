...
// الحصول على ألوان المنتج مع المقاسات
...
// دالة شاملة لجلب الألوان مع المقاسات مع معالجة أفضل للأخطاء
export const getProductColorsWithSizes = async (productId: string): Promise<ProductColor[]> => {
  try {
    if (!productId) {
      return [];
    }
    ...
    if (colorsError) {
      console.error('خطأ في جلب الألوان:', colorsError);
      toast.error('فشل في جلب ألوان المنتج');
      return [];
    }
    
    // تنظيف البيانات وترتيب المقاسات
    const cleanedColors = (colors || []).map(color => ({
      ...
      sizes: color.sizes ? color.sizes
        ...
        .sort((a: any, b: any) => {
          // ترتيب المقاسات: الافتراضي أولاً، ثم حسب تاريخ الإنشاء
    ...
    console.error('خطأ غير متوقع في جلب ألوان المنتج:', error);
    toast.error('حدث خطأ غير متوقع في جلب ألوان المنتج');
    return [];
  }
};
...
// دالة للتحقق من صحة حفظ الألوان والمقاسات
export const verifyProductColorsAndSizes = async (productId: string): Promise<{ success: boolean; message: string }> => {
  try {
    if (!productId) {
      return { success: false, message: 'معرف المنتج مطلوب' };
    }

    const colors = await getProductColorsWithSizes(productId);
    
    if (colors.length === 0) {
      return { success: true, message: 'لا توجد ألوان للتحقق منها' };
    }
    ...
    for (const color of colors) {
      ...
      // التحقق من البيانات الأساسية للون
      if (!color.name || !color.color_code) {
        issuesFound.push(`اللون ${color.id} لا يحتوي على اسم أو كود صحيح`);
      }
      ...
      if (color.has_sizes && color.sizes && color.sizes.length > 0) {
        for (const size of color.sizes) {
          totalVariants++;
          if (!size.size_name) {
            issuesFound.push(`المقاس ${size.id} في اللون ${color.name} لا يحتوي على اسم`);
          }
        }
      }
    }
    ...
    if (issuesFound.length > 0) {
      return { 
        success: false, 
        message: `تم العثور على ${issuesFound.length} مشكلة: ${issuesFound.join(', ')}` 
      };
    }

    return { 
      success: true, 
      message: `تم التحقق بنجاح من ${totalVariants} متغير للمنتج` 
    };
  } catch (error) {
    return { 
      success: false, 
      message: `فشل في التحقق من البيانات: ${error}` 
    };
  }
};
