...
export const useProductDataLoader = ({
  ...
  console.log('🔍 useProductDataLoader - معاملات الدخل:', {
    slug,
    organizationId,
    isOrganizationLoading,
  ...
  useEffect(() => {
    ...
    const loadProductWithRetry = async () => {
      console.log('🔍 loadProductWithRetry - بدء تحميل المنتج:', {
        slug,
        organizationId,
        isOrganizationLoading,
        aborted: currentAbortController.signal.aborted
      });

      // التحقق من إلغاء الطلب
      if (currentAbortController.signal.aborted) {
        console.log('❌ loadProductWithRetry - تم إلغاء الطلب');
        return;
      }

      // إذا كان organizationId غير متاح ولا يزال يتم تحميله، انتظر قليلاً
      if (!organizationId && isOrganizationLoading) {
        console.log('⏳ loadProductWithRetry - انتظار تحميل معرف المؤسسة');
        setIsLoading(true);
        return;
      }
      ...
      if (!slug || !organizationId) {
        console.log('❌ loadProductWithRetry - معاملات مفقودة:', { slug, organizationId, isOrganizationLoading });
        if (!isOrganizationLoading && !organizationId) {
          console.error('❌ معرف المؤسسة غير متاح');
        }
        setIsLoading(false);
        return;
      }
      ...
      for (let attempt = 0; attempt <= RETRY_CONFIG.maxRetries; attempt++) {
        ...
        try {
          ...
          console.log('🔍 loadProductWithRetry - استجابة getProductPageData:', {
            responseData,
          ...
          if (!responseData || !responseData.product || !responseData.product.id) {
            console.error('❌ loadProductWithRetry - بيانات المنتج غير صحيحة:', {
              responseData,
