...
const getDefaultLanguageFromDatabase = async (useImmediateCache = false): Promise<string> => {
  console.log('🌍 [LANGUAGE DEBUG] بدء جلب اللغة من قاعدة البيانات', {
    useImmediateCache,
  ...
  try {
    ...
    console.log('🌍 [LANGUAGE DEBUG] تحليل hostname', {
      currentHost,
    ...
    if (subdomain === 'localhost' || subdomain === '127' || currentHost.includes('localhost')) {
      subdomain = 'fredstore'; // subdomain للاختبار
      console.log('🌍 [LANGUAGE DEBUG] تطوير محلي، استخدام subdomain ثابت:', subdomain);
    }
    ...
    if (languageCache && languageCache.organizationId === subdomain) {
      ...
      console.log('🌍 [LANGUAGE DEBUG] فحص cache اللغة', {
        cacheExists: true,
        cacheAge,
        cacheDuration,
        cachedLanguage: languageCache.language,
        isValid: cacheAge < cacheDuration
      });
      
      if (cacheAge < cacheDuration) {
        console.log('🌍 [LANGUAGE DEBUG] استخدام اللغة من cache:', languageCache.language);
        return languageCache.language;
      } else {
        console.log('🌍 [LANGUAGE DEBUG] cache منتهي الصلاحية، سيتم جلب بيانات جديدة');
      }
    } else {
      console.log('🌍 [LANGUAGE DEBUG] لا يوجد cache صالح للغة');
    }
    ...
    console.log('🌍 [LANGUAGE DEBUG] البحث في organizationCache');
    
    if (window.organizationCache) {
      console.log('🌍 [LANGUAGE DEBUG] organizationCache موجود، البحث عن البيانات');
      ...
      if (window.organizationCache.has(subdomainKey)) {
        ...
        if (cached && cached.data) {
          ...
          console.log('🌍 [LANGUAGE DEBUG] تم العثور على بيانات المؤسسة من subdomain cache', {
            organizationId,
            defaultLanguage: organizationData.default_language
          });
        }
      }
      ...
      if (!organizationData) {
        ...
        console.log('🌍 [LANGUAGE DEBUG] البحث في localStorage عن معرف المؤسسة:', storedOrgId);
        ...
        if (storedOrgId) {
          ...
          if (window.organizationCache.has(orgCacheKey)) {
            ...
            if (cached && cached.data) {
              ...
              console.log('🌍 [LANGUAGE DEBUG] تم العثور على بيانات المؤسسة من localStorage cache', {
                organizationId,
                defaultLanguage: organizationData.default_language
              });
            }
      ...
      console.log('🌍 [LANGUAGE DEBUG] organizationCache غير موجود');
    }
    ...
    if (organizationData && organizationData.default_language) {
      ...
      console.log('🌍 [LANGUAGE DEBUG] تم العثور على اللغة من organizationCache:', language);
    ...
    console.log('🌍 [LANGUAGE DEBUG] لم يتم العثور على بيانات في cache، جلب من قاعدة البيانات');
    ...
    let query;
    if (organizationId) {
      console.log('🌍 [LANGUAGE DEBUG] استخدام معرف المؤسسة المعروف:', organizationId);
      query = supabase
      ...
      console.log('🌍 [LANGUAGE DEBUG] البحث بـ subdomain:', subdomain);
      // أولاً، ابحث عن المؤسسة بـ subdomain
      ...
      console.log('🌍 [LANGUAGE DEBUG] نتيجة البحث عن المؤسسة', {
        orgData,
        orgError: orgError?.message
      });

      if (orgError || !orgData) {
        console.error('🌍 [LANGUAGE DEBUG] لم يتم العثور على المؤسسة بـ subdomain:', subdomain);
        return 'ar'; // fallback
      }

      // ثم ابحث عن إعدادات اللغة
      query = supabase
        .from('organization_settings')
        .select('default_language, organization_id')
    ...
    console.log('🌍 [LANGUAGE DEBUG] نتيجة استعلام قاعدة البيانات', {
      data,
      error: error?.message,
      hasData: !!data
    });
    ...
    if (error) {
      console.error('🌍 [LANGUAGE DEBUG] خطأ في جلب اللغة من قاعدة البيانات:', error);
      return 'ar'; // fallback
    }
    ...
    if (data) {
      ...
      console.log('🌍 [LANGUAGE DEBUG] اللغة من organization_settings:', detectedLanguage);
      ...
      console.log('🌍 [LANGUAGE DEBUG] تم حفظ اللغة في cache:', languageCache);
    }

    console.log('🌍 [LANGUAGE DEBUG] اللغة النهائية المحددة:', detectedLanguage);
    return detectedLanguage;

  } catch (error) {
    console.error('🌍 [LANGUAGE DEBUG] خطأ عام في جلب اللغة:', error);
    return 'ar'; // fallback
  }
};
...
const getInitialLanguage = async (): Promise<string> => {
  console.log('🌍 [LANGUAGE DEBUG] بدء تحديد اللغة الأولية');
    ...
    try {
      ...
      if (appInitData) {
        ...
        if (parsedData.language) {
          console.log('🌍 [LANGUAGE DEBUG] استخدام اللغة من AppInitializer:', parsedData.language);
          return parsedData.language;
        }
      }
    } catch (error) {
      console.log('🌍 [LANGUAGE DEBUG] لا توجد بيانات AppInitializer، المتابعة بالطريقة العادية');
    }
    
    // 2. إذا لم توجد بيانات AppInitializer، جلب من قاعدة البيانات
    console.log('🌍 [LANGUAGE DEBUG] محاولة جلب اللغة من قاعدة البيانات');
    try {
      const dbLanguage = await getDefaultLanguageFromDatabase(true);
      console.log('🌍 [LANGUAGE DEBUG] نتيجة جلب اللغة من قاعدة البيانات:', dbLanguage);
      
      if (dbLanguage) {
        console.log('🌍 [LANGUAGE DEBUG] استخدام اللغة من قاعدة البيانات:', dbLanguage);
        return dbLanguage;
      }
    } catch (error) {
      console.error('🌍 [LANGUAGE DEBUG] خطأ في جلب اللغة من قاعدة البيانات:', error);
  ...
  console.log('🌍 [LANGUAGE DEBUG] استخدام اللغة الافتراضية: ar');
  return 'ar';
};

// تحسين تهيئة i18n مع دعم async
const initializeI18n = async () => {
  console.log('🌍 [LANGUAGE DEBUG] بدء تهيئة i18n');
  
  const initialLanguage = await getInitialLanguage();
  console.log('🌍 [LANGUAGE DEBUG] اللغة الأولية المحددة:', initialLanguage);
  ...
  try {
    ...
    console.log('🌍 [LANGUAGE DEBUG] تم تهيئة i18n بنجاح');
    return i18nInstance;
  } catch (error) {
    console.error('🌍 [LANGUAGE DEBUG] خطأ في تهيئة i18n:', error);
    throw error;
  }
};
...
i18n.on('languageChanged', (lng) => {
  console.log('🌍 [LANGUAGE DEBUG] تم تغيير اللغة إلى:', lng);
  
  if (typeof window !== 'undefined') {
    localStorage.setItem('i18nextLng', lng);
    localStorage.setItem('i18nextLng_timestamp', Date.now().toString());
    
    console.log('🌍 [LANGUAGE DEBUG] تم حفظ اللغة في localStorage', {
      language: lng,
      timestamp: Date.now()
    });
    ...
    console.log('🌍 [LANGUAGE DEBUG] تم تحديث اتجاه الصفحة', {
      language: lng,
      direction
    });
    ...
    if (languageCache) {
      ...
      console.log('🌍 [LANGUAGE DEBUG] تم تحديث cache اللغة');
    }
  }
});
...
const updateLanguageFromDatabase = async () => {
  ...
  if (isUpdatingLanguageFromDB) {
    console.log('🌍 [LANGUAGE DEBUG] تم تجاهل التحديث - عملية جارية');
    return;
  }
  
  if (currentTime - lastLanguageUpdateFromDBTime < LANGUAGE_UPDATE_FROM_DB_DEBOUNCE) {
    console.log('🌍 [LANGUAGE DEBUG] تم تجاهل التحديث - تم التحديث مؤخراً');
    return;
  }
  ...
  console.log('🌍 [LANGUAGE DEBUG] بدء تحديث اللغة من قاعدة البيانات');
  ...
  if (typeof window !== 'undefined') {
    try {
      ...
      console.log('🌍 [LANGUAGE DEBUG] جلب اللغة الافتراضية من قاعدة البيانات');
      const defaultLanguage = await getDefaultLanguageFromDatabase();
      
      console.log('🌍 [LANGUAGE DEBUG] اللغة المجلبة من قاعدة البيانات:', defaultLanguage);
      
      // تجاهل اختيارات المستخدم وتطبيق لغة المؤسسة دائماً
      console.log('🌍 [LANGUAGE DEBUG] تطبيق لغة المؤسسة بدون اعتبار لتفضيلات المستخدم');
      
      console.log('🌍 [LANGUAGE DEBUG] قرار تطبيق اللغة', {
        defaultLanguage,
        currentLanguage: i18n.language,
        willChange: defaultLanguage !== i18n.language
      });
      
      // تطبيق اللغة الافتراضية دائماً إذا كانت مختلفة
      if (defaultLanguage !== i18n.language) {
        console.log('🌍 [LANGUAGE DEBUG] تطبيق اللغة الجديدة:', defaultLanguage);
        await i18n.changeLanguage(defaultLanguage);
        console.log('🌍 [LANGUAGE DEBUG] تم تطبيق اللغة بنجاح');
      } else {
        console.log('🌍 [LANGUAGE DEBUG] اللغة مطبقة بالفعل، لا حاجة للتغيير');
      }
      
    } catch (error) {
      console.error('🌍 [LANGUAGE DEBUG] خطأ في تحديث اللغة:', error);
    } finally {
...
if (typeof window !== 'undefined') {
  window.addEventListener('appInitDataReady', async (event: CustomEvent) => {
    console.log('🌍 [LANGUAGE DEBUG] تلقي بيانات AppInitializer:', event.detail);
    
    const { language } = event.detail;
    if (language && language !== i18n.language) {
      console.log('🌍 [LANGUAGE DEBUG] تحديث اللغة من AppInitializer:', language);
      try {
        await i18n.changeLanguage(language);
        console.log('🌍 [LANGUAGE DEBUG] تم تحديث اللغة بنجاح من AppInitializer');
      } catch (error) {
        console.error('🌍 [LANGUAGE DEBUG] خطأ في تحديث اللغة من AppInitializer:', error);
      }
    }
  });
}
...
setTimeout(() => {
  const appInitData = localStorage.getItem('bazaar_app_init_data');
  if (!appInitData) {
    console.log('🌍 [LANGUAGE DEBUG] لا توجد بيانات AppInitializer، تشغيل التحديث العادي');
...
if (typeof window !== 'undefined') {
  ...
  const periodicLanguageCheck = setInterval(async () => {
    ...
    if (checkAttempts > maxCheckAttempts) {
      ...
      console.log('🌍 [LANGUAGE DEBUG] انتهاء الفحص الدوري للغة');
      return;
    }
    ...
    try {
          ...
          if (window.organizationCache.has(orgCacheKey)) {
            ...
            if (cached && cached.data) {
              ...
              if (defaultLanguage && 
                ...
                console.log('🌍 [LANGUAGE DEBUG] الفحص الدوري: تطبيق اللغة', defaultLanguage);
      ...
      if (process.env.NODE_ENV === 'development') {
        console.warn('🌍 [LANGUAGE DEBUG] خطأ في الفحص الدوري:', error);
      }
    }
...
if (typeof window !== 'undefined') {
  window.addEventListener('organizationLanguageUpdate', async (event: any) => {
    console.log('🌍 [LANGUAGE DEBUG] تلقي حدث تحديث اللغة من المنظمة:', event.detail);
    
    if (event.detail && event.detail.language) {
      const newLang = event.detail.language;
      const currentTime = Date.now();
      
      console.log('🌍 [LANGUAGE DEBUG] اللغة الجديدة المطلوبة:', newLang);
      
      // منع التكرار - فحص إذا كانت اللغة نفسها أو جاري التغيير
      if (newLang === i18n.language) {
        console.log('🌍 [LANGUAGE DEBUG] اللغة مطبقة بالفعل:', newLang);
        return;
      }
      
      // منع التحديث المتكرر خلال فترة قصيرة
      if (currentTime - lastLanguageChangeTime < LANGUAGE_CHANGE_DEBOUNCE) {
        console.log('🌍 [LANGUAGE DEBUG] تجاهل التحديث - تم تغيير اللغة مؤخراً');
        return;
      }
      
      if (isChangingLanguage) {
        console.log('🌍 [LANGUAGE DEBUG] تغيير اللغة قيد التنفيذ، إضافة إلى القائمة');
      ...
      if (newLang && ['ar', 'en', 'fr'].includes(newLang)) {
        console.log('🌍 [LANGUAGE DEBUG] محاولة تغيير اللغة إلى:', newLang);
        ...
        try {
          ...
          if (i18n.isInitialized) {
            ...
            console.log('🌍 [LANGUAGE DEBUG] تم تغيير اللغة بنجاح إلى:', newLang);
          } else {
            console.warn('🌍 [LANGUAGE DEBUG] i18n غير مهيأ بعد، انتظار التهيئة...');
            ...
            await i18n.changeLanguage(newLang);
            console.log('🌍 [LANGUAGE DEBUG] تم تغيير اللغة بنجاح بعد انتظار التهيئة:', newLang);
          }
        } catch (error) {
          console.error('🌍 [LANGUAGE DEBUG] خطأ في تغيير اللغة:', error);
        } finally {
          ...
          if (languageChangeQueue.length > 0) {
            ...
            if (nextLang && nextLang !== i18n.language) {
              console.log('🌍 [LANGUAGE DEBUG] معالجة طلب تغيير اللغة من القائمة:', nextLang);
        ...
        console.warn('🌍 [LANGUAGE DEBUG] لغة غير مدعومة:', newLang);
      }
    } else {
      console.warn('🌍 [LANGUAGE DEBUG] حدث تحديث اللغة بدون بيانات صحيحة');
    }
  });
}
