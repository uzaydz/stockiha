...
const OptimizedImageEnhanced = memo<OptimizedImageProps>(({ 
  ...
  useEffect(() => {
    ...
    const loadImage = async () => {
      try {
        // تحسين جودة الصورة حسب الإعداد
        let optimizedSrc = src;
        if (quality !== 'auto') {
          const separator = src.includes('?') ? '&' : '?';
          const qualityMap = { low: 60, medium: 80, high: 95 };
          optimizedSrc = `${src}${separator}q=${qualityMap[quality]}`;
        }
  ...
  }, [src, isInView, priority, quality, handleImageLoad, handleImageError]);
    ...
    >
      ...
      {isInView && currentSrc && (
        <img
          ...
          onError={() => handleImageError(new Error('Image load failed'))}
