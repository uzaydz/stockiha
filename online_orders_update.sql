-- إضافة الأعمدة اللازمة لتخزين معلومات اللون والمقاس في جدول online_order_items
...
ADD COLUMN selected_price NUMERIC; -- السعر المختار حسب اللون أو المقاس

-- إضافة علاقات خارجية مع جداول المقاسات والألوان
ALTER TABLE online_order_items
ADD CONSTRAINT fk_color_id FOREIGN KEY (color_id) REFERENCES product_colors(id) ON DELETE SET NULL,
ADD CONSTRAINT fk_size_id FOREIGN KEY (size_id) REFERENCES product_sizes(id) ON DELETE SET NULL;

-- تحديث وظيفة معالجة الطلبات لتضمين معلومات اللون والمقاس
...
DECLARE
  ...
  v_product_name TEXT;
  v_color_name TEXT;
  v_selected_price NUMERIC := p_unit_price; -- استخدام السعر المدخل
BEGIN
