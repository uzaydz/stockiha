...
// api/_rpc/get_product_complete_data.ts
var onRequestGet;
var init_get_product_complete_data = __esm({
  "api/_rpc/get_product_complete_data.ts"() {
    init_functionsRoutes_0_6169685256887106();
    onRequestGet = /* @__PURE__ */ __name(async ({ request, env, waitUntil }) => {
      try {
        const url = new URL(request.url);
        const product = (url.searchParams.get("product") || "").trim();
        const org = (url.searchParams.get("org") || "").trim() || null;
        const scope = (url.searchParams.get("scope") || "medium").trim();
        if (!product) {
          return new Response(JSON.stringify({ success: false, error: "Missing product parameter" }), {
            status: 400,
            headers: { "Content-Type": "application/json" }
          });
        }
        const ttl = scope === "full" ? 60 : 180;
        const cacheKey = new Request(`${url.origin}/__rpc_cache/get_product_complete_data?product=${encodeURIComponent(product)}&org=${encodeURIComponent(org || "")}&scope=${encodeURIComponent(scope)}`, { method: "GET" });
        const cache = caches.default;
        const cached = await cache.match(cacheKey);
        if (cached) {
          const h = new Headers(cached.headers);
          h.set("Cache-Control", `public, max-age=${ttl}, s-maxage=${ttl}, stale-while-revalidate=120, stale-if-error=600`);
        ...
        const body = {
          p_product_identifier: product,
          p_organization_id: org,
          p_include_inactive: false,
          p_data_scope: scope,
          p_include_large_images: false
        };
        const rpcRes = await fetch(`${supabaseUrl}/rest/v1/rpc/get_product_complete_data_ultra_optimized`, {
          method: "POST",
          headers: {
        ...
        if (!rpcRes.ok) {
          ...
          return new Response(JSON.stringify({ success: false, error: "Upstream error", status: rpcRes.status, details: text }), {
            status: 502,
            headers: { "Content-Type": "application/json" }
          });
        }
        ...
        const headers = new Headers({
          "Content-Type": "application/json",
          "Cache-Control": `public, max-age=${ttl}, s-maxage=${ttl}, stale-while-revalidate=120, stale-if-error=600`,
          "Vary": "Accept, Origin"
        });
        const out = new Response(JSON.stringify(payload), { status: 200, headers });
...
// api/_rpc/get_store_init_data.ts
var onRequestGet2;
var init_get_store_init_data = __esm({
  "api/_rpc/get_store_init_data.ts"() {
    init_functionsRoutes_0_6169685256887106();
    onRequestGet2 = /* @__PURE__ */ __name(async ({ request, env, waitUntil }) => {
      try {
        const url = new URL(request.url);
        ...
        const cacheKey = new Request(`${url.origin}/__rpc_cache/get_store_init_data?org=${encodeURIComponent(org)}`, { method: "GET" });
        const cache = caches.default;
        const cached = await cache.match(cacheKey);
        if (cached) {
          const h = new Headers(cached.headers);
          h.set("Cache-Control", "public, max-age=300, s-maxage=300, stale-while-revalidate=120, stale-if-error=600");
          h.set("Vary", "Accept, Origin");
          return new Response(cached.body, { status: cached.status, headers: h });
        }
        const supabaseUrl = env.SUPABASE_URL || env.VITE_SUPABASE_URL;
        ...
        const token = service || anon;
        const rpcRes = await fetch(`${supabaseUrl}/rest/v1/rpc/get_store_init_data`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "apikey": token,
          ...
          body: JSON.stringify({ org_identifier: org })
        });
        if (!rpcRes.ok) {
          const text = await rpcRes.text();
          return new Response(JSON.stringify({ error: "Upstream error", status: rpcRes.status, details: text }), {
            status: 502,
            headers: { "Content-Type": "application/json" }
          });
        }
        ...
        const headers = new Headers({
          "Content-Type": "application/json",
          "Cache-Control": "public, max-age=300, s-maxage=300, stale-while-revalidate=120, stale-if-error=600",
          "Vary": "Accept, Origin"
        });
        const out = new Response(data, { status: 200, headers });
        waitUntil(cache.put(cacheKey, out.clone()));
        return out;
      } catch (e) {
        return new Response(JSON.stringify({ error: "Proxy failure", message: e?.message || String(e) }), {
          status: 500,
          headers: { "Content-Type": "application/json" }
        });
      }
...
// api/conversion-events/health.ts
...
// api/conversion-events/tiktok.ts
async function onRequestPost(context) {
  ...
  try {
    ...
    const cleanPixelId = String(pixel_id).trim();
    const eventTime = Math.floor(Date.now() / 1e3);
    const eventId = `${product_id}_${event_type}_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`;
    const eventData = {
      event_source: "web",
      event_source_id: cleanPixelId,
      data: [{
        event: mapEventTypeForTiktok(event_type),
        event_id: eventId,
    ...
    const supabaseUrl = env.SUPABASE_URL || env.VITE_SUPABASE_URL || "https://wrnssatuvmumsczyldth.supabase.co";
    ...
    if (supabaseAnonKey) {
      ...
      await supabase.from("conversion_events").insert({
        ...
        platform: "tiktok",
        user_data: user_data || {},
        custom_data: { ...custom_data || {}, pixel_id: cleanPixelId, tiktok_response: tiktokResult },
        event_id: eventId,
        status: tiktokResp.ok ? "sent" : "failed",
        error_message: tiktokResp.ok ? null : JSON.stringify(tiktokResult),
        timestamp: (/* @__PURE__ */ new Date()).toISOString(),
        sent_at: tiktokResp.ok ? (/* @__PURE__ */ new Date()).toISOString() : null
      });
    }
    if (!tiktokResp.ok) {
      return json({
    ...
    return json({ success: true, tiktok_response: tiktokResult, message: "\u062A\u0645 \u0625\u0631\u0633\u0627\u0644 \u0627\u0644\u062D\u062F\u062B \u0628\u0646\u062C\u0627\u062D" });
  } catch (e) {
    return json({ error: "\u062E\u0637\u0623 \u062F\u0627\u062E\u0644\u064A \u0641\u064A \u0627\u0644\u062E\u0627\u062F\u0645", details: e?.message || String(e) }, 500);
  }
}
...
function mapEventTypeForTiktok(eventType) {
  const mapping = {
...
var init_tiktok = __esm({
  "api/conversion-events/tiktok.ts"() {
    ...
    __name(json, "json");
    __name(mapEventTypeForTiktok, "mapEventTypeForTiktok");
  }
});
...
var getCloudflareApiUrl;
var init_cloudflare_domains = __esm({
  "api/cloudflare-domains.ts"() {
    init_functionsRoutes_0_6169685256887106();
    getCloudflareApiUrl = /* @__PURE__ */ __name(() => {
...
async function onRequestPost3(context) {
  ...
  try {
    ...
    if (webhookSecret && signature) {
      ...
      await handleWebhookEvent(payload, env);
    } else {
...
async function handleWebhookEvent(payload, env) {
  const { event_type, domain, status, ssl_status, details } = payload;
  const organization = await findOrganizationByDomain(domain, env);
  if (!organization) {
    console.log(`No organization found for domain: ${domain}`);
    return;
  }
  await updateDomainStatus(organization.id, domain, {
    status,
    ssl_status,
    event_type,
    error_message: details?.error_message,
    verification_errors: details?.verification_errors,
    last_checked: (/* @__PURE__ */ new Date()).toISOString()
  }, env);
  if (event_type === "domain_delegated" && status === "active") {
    await autoSetupCustomHostname(domain, organization.id, env);
  }
  console.log(`Processed webhook for domain ${domain}, event: ${event_type}, status: ${status}`);
}
async function findOrganizationByDomain(domain, env) {
  const supabaseUrl = env.SUPABASE_URL;
  const supabaseKey = env.SUPABASE_SERVICE_ROLE_KEY;
  const response = await fetch(`${supabaseUrl}/rest/v1/organizations?domain=eq.${domain}&select=id,name,domain`, {
    headers: {
...
async function updateDomainStatus(organizationId, domain, statusData, env) {
  const supabaseUrl = env.SUPABASE_URL;
  const supabaseKey = env.SUPABASE_SERVICE_ROLE_KEY;
  const verificationData = {
    ...
    ssl_status: statusData.ssl_status,
    error_message: statusData.error_message,
    verification_errors: statusData.verification_errors,
    last_checked: statusData.last_checked,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  };
  await fetch(`${supabaseUrl}/rest/v1/domain_verifications`, {
    method: "POST",
    headers: {
      "Authorization": `Bearer ${supabaseKey}`,
      "apikey": supabaseKey,
...
async function autoSetupCustomHostname(domain, organizationId, env) {
  try {
    const cloudflareToken = env.CLOUDFLARE_API_TOKEN;
    const zoneId = env.CLOUDFLARE_ZONE_ID;
    if (!cloudflareToken || !zoneId) {
      console.error("Missing Cloudflare credentials for auto setup");
      return;
    }
    await addCustomHostname(domain, organizationId, cloudflareToken, zoneId);
    const wwwDomain = `www.${domain}`;
    await addCustomHostname(wwwDomain, organizationId, cloudflareToken, zoneId);
    console.log(`Auto-setup completed for domain: ${domain}`);
  } catch (error) {
    console.error(`Auto-setup failed for domain ${domain}:`, error);
  }
}
async function addCustomHostname(hostname, organizationId, cloudflareToken, zoneId) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to add custom hostname ${hostname}: ${error}`);
  }
  const result = await response.json();
  console.log(`Custom hostname added: ${hostname}`, result);
  return result;
}
...
var init_cloudflare_webhook = __esm({
  "api/cloudflare-webhook.ts"() {
    init_functionsRoutes_0_6169685256887106();
    __name(onRequestPost3, "onRequestPost");
    __name(handleWebhookEvent, "handleWebhookEvent");
    __name(findOrganizationByDomain, "findOrganizationByDomain");
    __name(updateDomainStatus, "updateDomainStatus");
    __name(autoSetupCustomHostname, "autoSetupCustomHostname");
    __name(addCustomHostname, "addCustomHostname");
    __name(verifyWebhookSignature, "verifyWebhookSignature");
  }
});

// api/conversion-events.ts
async function onRequestPost4(context) {
  ...
  try {
    ...
    if (!supabaseAnonKey) {
      return json2({
        warning: "Supabase credentials missing; event accepted but not persisted",
        accepted: true
      }, 202);
    }
    const supabase = createClient(supabaseUrl, supabaseAnonKey, {
    ...
    const insertData = {
      ...
      event_id: event_id || `${product_id}_${event_type}_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`,
      status: "sent",
      timestamp: (/* @__PURE__ */ new Date()).toISOString(),
      sent_at: (/* @__PURE__ */ new Date()).toISOString()
    };
    const { error } = await supabase.from("conversion_events").insert(insertData);
    if (error) {
      return json2({ warning: "event accepted but not persisted", details: error.message, code: error.code }, 202);
    }
    return json2({ success: true, message: "\u062A\u0645 \u062A\u0633\u062C\u064A\u0644 \u0627\u0644\u062D\u062F\u062B \u0628\u0646\u062C\u0627\u062D" });
  } catch (e) {
    return json2({ error: "\u062E\u0637\u0623 \u062F\u0627\u062E\u0644\u064A \u0641\u064A \u0627\u0644\u062E\u0627\u062F\u0645", details: e?.message || String(e) }, 500);
  }
}
...
var init_conversion_events = __esm({
  "api/conversion-events.ts"() {
    init_functionsRoutes_0_6169685256887106();
    init_module5();
          ...
          required_records: [
            {
              ...
              note: "\u0633\u062C\u0644 CNAME \u0644\u0644\u0646\u0637\u0627\u0642 \u0645\u0639 www"
            },
            {
              type: "CNAME",
              name: domain,
              value: `www.${domain}`,
              note: "\u0633\u062C\u0644 CNAME \u0644\u0644\u0646\u0637\u0627\u0642 \u0628\u062F\u0648\u0646 www (\u0625\u0639\u0627\u062F\u0629 \u062A\u0648\u062C\u064A\u0647 \u0625\u0644\u0649 www)"
            }
          ],
          alternative_solutions: [
            {
              title: "\u0627\u0644\u062D\u0644 \u0627\u0644\u0623\u0648\u0644: \u0625\u0639\u0627\u062F\u0629 \u0627\u0644\u062A\u0648\u062C\u064A\u0647 (\u0627\u0644\u0623\u0641\u0636\u0644)",
              description: "\u0627\u0633\u062A\u062E\u062F\u0627\u0645 Cloudflare Redirect Rules",
              steps: [
                "\u0641\u064A \u0644\u0648\u062D\u0629 \u062A\u062D\u0643\u0645 Cloudflare \u0627\u0630\u0647\u0628 \u0625\u0644\u0649 Rules \u2192 Redirect Rules",
                "\u0623\u0646\u0634\u0626 \u0642\u0627\u0639\u062F\u0629 \u062C\u062F\u064A\u062F\u0629: \u0645\u0646 example.com \u0625\u0644\u0649 www.example.com",
                "\u0627\u0633\u062A\u062E\u062F\u0645 Status Code 301 \u0644\u0644\u062A\u0648\u062C\u064A\u0647 \u0627\u0644\u062F\u0627\u0626\u0645"
              ]
            },
            {
              title: "\u0627\u0644\u062D\u0644 \u0627\u0644\u062B\u0627\u0646\u064A: CNAME Flattening",
              description: "\u062A\u0641\u0639\u064A\u0644 CNAME Flattening \u0641\u064A Cloudflare",
              steps: [
                "\u0623\u0636\u0641 CNAME \u0644\u0644\u0646\u0637\u0627\u0642 \u0627\u0644\u062C\u0630\u0631\u064A \u064A\u0634\u064A\u0631 \u0625\u0644\u0649 www",
                "\u0641\u0639\u0644 CNAME Flattening \u0641\u064A DNS Settings",
    ...
    if (env.CLOUDFLARE_API_TOKEN && env.CLOUDFLARE_ZONE_ID) {
      ...
      const apexResponse = await fetch(
        `https://api.cloudflare.com/client/v4/zones/${env.CLOUDFLARE_ZONE_ID}/dns_records`,
        {
          ...
          body: JSON.stringify(apexRecord)
        }
      );
      const redirectRule = await createApexRedirectRule(domain, env);
      const wwwData = await wwwResponse.json();
      const apexData = await apexResponse.json();
      if (wwwResponse.ok && apexResponse.ok) {
        return new Response(
          JSON.stringify({
            success: true,
            message: "\u062A\u0645 \u0625\u0639\u062F\u0627\u062F \u0627\u0644\u0646\u0637\u0627\u0642 \u0628\u0646\u062C\u0627\u062D \u0641\u064A Cloudflare \u0645\u0639 \u0625\u0639\u0627\u062F\u0629 \u0627\u0644\u062A\u0648\u062C\u064A\u0647 \u0627\u0644\u062A\u0644\u0642\u0627\u0626\u064A",
            domain,
            dns_records: {
              www: wwwData.result,
              apex: apexData.result
            },
        ...
        throw new Error(`Cloudflare API Error: ${wwwData.errors?.[0]?.message || apexData.errors?.[0]?.message || "\u062E\u0637\u0623 \u063A\u064A\u0631 \u0645\u0639\u0631\u0648\u0641"}`);
      }
    } else {
      return new Response(
        JSON.stringify({
...
async function createApexRedirectRule(domain, env) {
  try {
    if (!env.CLOUDFLARE_API_TOKEN || !env.CLOUDFLARE_ZONE_ID) {
      return { success: false, message: "Cloudflare credentials not available" };
    }
    const redirectRule = {
      ...
      action: {
        ...
        parameters: {
          from_value: {
            status_code: 301,
            target_url: {
              expression: `concat("https://www.${domain}", http.request.uri.path, http.request.uri.query_string != "" ? concat("?", http.request.uri.query_string) : "")`
            },
    ...
    if (rulesetsData.success && rulesetsData.result) {
      const redirectRuleset = rulesetsData.result.find(
    ...
    let ruleResponse;
    if (rulesetId) {
      ruleResponse = await fetch(
        `https://api.cloudflare.com/client/v4/zones/${env.CLOUDFLARE_ZONE_ID}/rulesets/${rulesetId}/rules`,
        {
          method: "POST",
          headers: {
            "Authorization": `Bearer ${env.CLOUDFLARE_API_TOKEN}`,
    ...
    if (ruleResponse.ok) {
      return {
        success: true,
        message: "\u062A\u0645 \u0625\u0646\u0634\u0627\u0621 \u0642\u0627\u0639\u062F\u0629 \u0625\u0639\u0627\u062F\u0629 \u0627\u0644\u062A\u0648\u062C\u064A\u0647 \u0628\u0646\u062C\u0627\u062D",
        rule: ruleData.result
      };
    } else {
      console.error("Failed to create redirect rule:", ruleData);
      return {
        success: false,
        message: "\u0641\u0634\u0644 \u0641\u064A \u0625\u0646\u0634\u0627\u0621 \u0642\u0627\u0639\u062F\u0629 \u0625\u0639\u0627\u062F\u0629 \u0627\u0644\u062A\u0648\u062C\u064A\u0647",
        error: ruleData.errors?.[0]?.message || "\u062E\u0637\u0623 \u063A\u064A\u0631 \u0645\u0639\u0631\u0648\u0641"
      };
    }
  } catch (error) {
    ...
    return {
      success: false,
      message: "\u062E\u0637\u0623 \u0641\u064A \u0625\u0646\u0634\u0627\u0621 \u0642\u0627\u0639\u062F\u0629 \u0625\u0639\u0627\u062F\u0629 \u0627\u0644\u062A\u0648\u062C\u064A\u0647",
...
var init_verify_domain = __esm({
  "api/verify-domain.ts"() {
    init_functionsRoutes_0_6169685256887106();
    __name(onRequestPost6, "onRequestPost");
    __name(verifyDomain, "verifyDomain");
    __name(setupDomain, "setupDomain");
    __name(createApexRedirectRule, "createApexRedirectRule");
...
var init_yalidine_fees_proxy = __esm({
  "api/yalidine-fees-proxy.ts"() {
    init_functionsRoutes_0_6169685256887106();
...
async function checkRateLimit(request, env, limit = 100, window2 = 60) {
  ...
  await env.RATE_LIMIT_KV.put(key, (count + 1).toString(), { expirationTtl: window2 });
  return true;
}
...
var SECURITY_HEADERS2, onRequest2;
var init_secure_yalidine_proxy = __esm({
  "api/secure-yalidine-proxy.ts"() {
    init_functionsRoutes_0_6169685256887106();
...
async function onRequest5(context) {
  ...
  if (url.pathname.startsWith("/api/")) {
    ...
    const allowedOrigins = [
      ...
      "https://techocenter.com",
      "https://www.techocenter.com",
      "http://localhost:8080",
      "http://localhost:3000"
    ];
    const isStockihaSubdomain = (() => {
      try {
        const oh = origin ? new URL(origin).hostname : "";
        const rh = referer ? new URL(referer).hostname : "";
        return oh.endsWith(".stockiha.com") || rh.endsWith(".stockiha.com");
      } catch {
        return false;
      }
    })();
    ...
    if (!isDevelopment && !isAllowedOrigin && !isAllowedReferer && !isStockihaSubdomain) {
  ...
  newResponse.headers.set(
    ...
    [
      "default-src 'self'",
      // السماح لسكايربتات الضرورية + مفاتيح hash الحالية للاكواد inline الضرورية
      "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://static.cloudflareinsights.com https://cloudflareinsights.com https://*.cloudflareinsights.com https://connect.facebook.net https://www.googletagmanager.com https://www.google-analytics.com https://analytics.tiktok.com 'sha256-vRKmGhO0Jt0ZJLajz37rSXGS0J2GkdjuvALzlXD9Ymg=' 'sha256-HEP/a7uzebOhHt9bVmS6zyymv90YmGWEAkZHYjfdd6g=' 'sha256-oToowZlM1h/gvo9UizVDTa7MltyVSmKouu4seQ8jBtw=' 'sha256-gkpPSL93wBndrOdeNSZNvORHPYYQJFZFeuGGo/EZIOI=' 'sha256-W0c+JQ5heCERsJkuKUWc8FpJ48oPtbDWDtCTGWzOXhQ='",
      "script-src-elem 'self' 'unsafe-inline' 'unsafe-eval' https://static.cloudflareinsights.com https://cloudflareinsights.com https://*.cloudflareinsights.com https://connect.facebook.net https://www.googletagmanager.com https://www.google-analytics.com https://analytics.tiktok.com",
      "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com",
      "style-src-elem 'self' 'unsafe-inline' https://fonts.googleapis.com",
      // السماح بصور pixel من فيسبوك + كل https
      "img-src 'self' data: https: blob: https://www.facebook.com https://*.facebook.com",
      "font-src 'self' data: https: https://fonts.gstatic.com",
      // السماح باتصالات HTTPS/WSS عامة + النطاقات المطلوبة صراحةً
      ...
      "frame-src 'self' https://www.facebook.com https://connect.facebook.net https://www.google.com",
      "object-src 'none'"
    ].join("; ")
  );
...
var PERFORMANCE_HEADERS, SUSPICIOUS_USER_AGENTS;
...
// ../.wrangler/tmp/pages-WRADwF/functionsRoutes-0.6169685256887106.mjs
var routes;
var init_functionsRoutes_0_6169685256887106 = __esm({
  "../.wrangler/tmp/pages-WRADwF/functionsRoutes-0.6169685256887106.mjs"() {
    init_get_product_complete_data();
    init_get_store_init_data();
    init_health();
    init_tiktok();
    init_cloudflare_config();
    init_cloudflare_domains();
    init_cloudflare_webhook();
    ...
    init_verify_domain();
    init_verify_domain();
    init_yalidine_fees_proxy();
    init_yalidine_fees_proxy();
    init_yalidine_fees_proxy();
    init_csp_report();
    init_secure_yalidine_proxy();
    init_robots_txt();
    init_sitemap_xml();
    init_middleware();
    init_middleware();
