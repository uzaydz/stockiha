...
// تم إزالة productionDebug لتحسين الأداء
// import { logPOSContextStatus } from '@/utils/productionDebug';
import UnifiedRequestManager from '@/lib/unifiedRequestManager';
// تم إزالة performance-monitor لتحسين الأداء
...
const fetchPOSProductsWithVariants = async (orgId: string): Promise<POSProductWithVariants[]> => {
  return deduplicateRequest(`pos-products-enhanced-${orgId}`, async () => {
    ...
    // تحميل التفاصيل الكاملة للمنتجات
    ...
    for (const product of allProducts) {
      ...
      const processedProduct: POSProductWithVariants = {
        // خصائص Product الأساسية
        id: product.id,
        name: product.name,
...
const fetchPOSSubscriptionsEnhanced = async (orgId: string): Promise<SubscriptionService[]> => {
  return deduplicateRequest(`pos-subscriptions-enhanced-${orgId}`, async () => {
    ...
    if (servicesError) {
      // logPOSContextStatus('FETCH_ERROR', { error: servicesError });
      return [];
    }

    if (!servicesData || servicesData.length === 0) {
      // logPOSContextStatus('NO_SUBSCRIPTIONS', { orgId });
      return [];
    }
  ...
  return deduplicateRequest(`pos-settings-enhanced-${orgId}`, async () => {
    ...
    try {
      ...
      try {
        ...
        // logPOSContextStatus('FETCH_ERROR', { error: directError });
        throw directError;
      }
...
export const POSDataProvider: React.FC<POSDataProviderProps> = ({ children }) => {
  ...
  // تطبيقات المؤسسة - محسنة مع Query Keys موحدة
  const {
    data: organizationApps = [],
    isLoading: isAppsLoading,
    error: appsError
  } = useQuery({
    queryKey: QueryKeys.organizationApps(orgId!),
  ...
  const updateProductStock = useCallback(async (
    ...
    try {
      ...
      if (sizeId && colorId) {
        ...
        if (error) {
          // logPOSContextStatus('UPDATE_ERROR', { error: error });
          return false;
        }
      } else if (colorId) {
        // تحديث مخزون اللون
