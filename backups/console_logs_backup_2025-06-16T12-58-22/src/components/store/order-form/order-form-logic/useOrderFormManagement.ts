...
import { getShippingProvidersOptimized } from "@/api/storeRequestOptimizer";
...
export const useOrderFormManagement = (
  ...
  useEffect(() => {
    const loadProvinces = async () => {
      ...
      try {
        ...
        console.warn('خطأ في تحميل الولايات:', error);
      } finally {
  ...
  // تحميل إعدادات مزود الشحن - محسن
  useEffect(() => {
    const loadShippingSettings = async () => {
      if (!tenant?.id || !shippingProviderId) return;

      try {
        // استخدام الخدمة المحسنة لجلب مزودي الشحن
        const providers = await getShippingProvidersOptimized();
        
        // البحث عن المزود المطلوب
        const provider = providers.find(p => p.id === Number(shippingProviderId));
        ...
        if (provider) {
          ...
          console.log('✅ تم تحميل إعدادات مزود الشحن:', provider.name);
        } else {
          console.warn('⚠️ لم يتم العثور على مزود الشحن:', shippingProviderId);
        }
      } catch (error) {
        console.warn('⚠️ خطأ في تحميل إعدادات مزود الشحن:', error);
      }
    };
  ...
  const handleWilayaChange = async (wilayaId: string) => {
    ...
    try {
      ...
      console.warn('⚠️ خطأ في تحميل البلديات:', error);
    } finally {
