-- إصلاح مشكلة "column "new_values" of relation "settings_audit_log" does not exist"
-- تاريخ: 2024

-- 1. إضافة عمود new_values إلى جدول settings_audit_log
ALTER TABLE public.settings_audit_log 
ADD COLUMN IF NOT EXISTS new_values TEXT;

-- 2. تحديث القيم الموجودة بنسخ البيانات من new_value إلى new_values
UPDATE settings_audit_log SET new_values = new_value WHERE new_values IS NULL;

-- 3. إنشاء محفز لنسخ قيم new_value إلى new_values تلقائيًا
CREATE OR REPLACE FUNCTION trigger_copy_new_value_to_new_values()
RETURNS TRIGGER AS $$
BEGIN
...
-- إنشاء محفز لنسخ القيمة الجديدة إلى العمود الجديد
DROP TRIGGER IF EXISTS copy_new_value_to_new_values ON settings_audit_log;
CREATE TRIGGER copy_new_value_to_new_values
BEFORE INSERT OR UPDATE ON settings_audit_log
FOR EACH ROW
EXECUTE FUNCTION trigger_copy_new_value_to_new_values();

-- 4. تحديث وظائف المتجر لاستخدام العمود الجديد
...
BEGIN
  ...
  IF v_result THEN
    ...
    INSERT INTO settings_audit_log (
      ...
      old_values,
      new_value,
      new_values, -- تضمين العمود الجديد
      action_type,
      table_name,
      record_id,
      created_at
...
BEGIN
  ...
  INSERT INTO settings_audit_log (
    ...
    v_old_val_text,
    v_new_val_text,
    v_new_val_text, -- نفس قيمة new_value
    'UPDATE',
    'store_settings',
    NULL,
    NOW()
...
BEGIN
    ...
    VALUES (
        ...
        TG_ARGV[0],
        old_val,
        old_val, -- استخدام نفس القيمة
        new_val,
        new_val, -- استخدام نفس القيمة
