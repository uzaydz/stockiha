...
export function NavbarMain({
  ...
  const orgLogo = logoUrl || finalOrganizationSettings?.logo_url || '';
  const siteName = storeName || finalOrganizationSettings?.site_name || currentOrganization?.name || '';
  const displayTextWithLogo = finalOrganizationSettings?.display_text_with_logo !== false;
  
  // 🔍 Console logs للتحقق من البيانات
  useEffect(() => {
    console.log('🔍 [NavbarMain] فحص البيانات:', {
      storeInfo: !!storeInfo,
      storeName,
      logoUrl,
      organizationSettings: !!organizationSettings,
      organizationSettingsFromProduct: !!organizationSettingsFromProduct,
      finalOrganizationSettings: !!finalOrganizationSettings,
      orgLogo,
      siteName,
      currentOrgName: currentOrganization?.name
    });
  }, [storeInfo, storeName, logoUrl, organizationSettings, organizationSettingsFromProduct, finalOrganizationSettings, orgLogo, siteName, currentOrganization?.name]);
  
  // 🔍 Log مبسط للبيانات النهائية
  useEffect(() => {
    if (finalOrganizationSettings) {
      console.log('✅ [NavbarMain] البيانات النهائية للنافبار:', {
        orgLogo,
        siteName,
        hasLogo: !!orgLogo,
        hasName: !!siteName
  ...
  }, [orgLogo, siteName, finalOrganizationSettings]);
