-- =================================================================
-- 🎯 get_pos_orders_page_data - RPC شامل لصفحة طلبيات نقطة البيع
-- =================================================================
-- المؤلف: AI Assistant
-- التاريخ: 2025-01-27
-- الوصف: RPC واحد يجلب جميع البيانات اللازمة لصفحة POS Orders
-- الهدف: < 150ms لـ 10k سجل مع تجنب N+1 queries
-- =================================================================

-- حذف الدالة القديمة إذا كانت موجودة
DROP FUNCTION IF EXISTS public.get_pos_orders_page_data(UUID, UUID, INTEGER, INTEGER, JSONB, JSONB, JSONB);

CREATE OR REPLACE FUNCTION public.get_pos_orders_page_data(
    p_org_id UUID,
    p_user_id UUID,
    ...
    p_include JSONB DEFAULT '{"stats":true,"settings":true,"subscription":true,"returns":true}'::JSONB
)
RETURNS JSONB
SECURITY DEFINER
...
BEGIN
    ...
    p_page_size := GREATEST(1, LEAST(100, COALESCE(p_page_size, 20))); -- حد أقصى 100
    ...
    IF array_length(v_where_conditions, 1) > 0 THEN
        ...
        -- استبدال المتغيرات في WHERE clause
    ...
    -- ✅ 5. حساب العدد الإجمالي والمفلتر (مُحسن)
    EXECUTE '
    ...
    v_debug_timings := jsonb_set(v_debug_timings, '{counts_ms}', 
        to_jsonb(EXTRACT(epoch FROM (v_checkpoint_time - (v_start_time + (v_debug_timings->>'setup_ms')::TEXT::INTERVAL))) * 1000));
    
    -- ✅ 6. جلب الطلبيات مع جميع البيانات المطلوبة (استعلام واحد محسن)
    EXECUTE '
    ...
    -- ✅ 10. جلب معلومات الاشتراك (إذا مطلوبة)
...
COMMENT ON FUNCTION public.get_pos_orders_page_data IS 
'RPC محسن لجلب جميع بيانات صفحة طلبيات نقطة البيع في استعلام واحد. 
يدعم الفلترة، الترتيب، والتقسيم للصفحات مع تجنب N+1 queries.
الهدف: < 150ms لـ 10k سجل.';