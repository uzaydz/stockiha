...
const POS = () => {
  ...
  const handleRefreshData = useCallback(async () => {
    try {
      logServerSync('POS.handleRefreshData', {
        operation: 'FULL_DATA_REFRESH',
      ...
      logServerSync('POS.handleRefreshData.success', {
        operation: 'FULL_DATA_REFRESH_SUCCESS',
        timestamp: new Date().toISOString()
      });
      ...
      logError(
        `فشل في تحديث البيانات: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`,
        'POS.handleRefreshData',
        {
  ...
  // 🔄 نظام التحديث التلقائي بعد إتمام العمليات
  useEffect(() => {
    ...
    const handleOrderCompletion = async (event: CustomEvent) => {
      ...
      logOrderSubmit(
        orderId,
        0, // total سيتم تحديثه
        'POS.autoRefreshAfterOrder',
        {
          operation: 'AUTO_REFRESH_TRIGGERED',
          orderType: type,
          customerOrderNumber,
      ...
      // تأخير قصير للسماح لقاعدة البيانات بالانتهاء من المعالجة
      orderCompletionTimeout = setTimeout(async () => {
        try {
          await Promise.all([
            refreshProducts(),
            refreshUnifiedPOSData()
          ...
          logServerSync('POS.autoRefreshAfterOrder.success', {
            operation: 'AUTO_REFRESH_SUCCESS',
            orderId,
            orderType: type
          });
        } catch (error) {
          logError(
            `فشل في التحديث التلقائي بعد الطلب: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`,
            'POS.autoRefreshAfterOrder',
            {
              orderId,
              orderType: type,
  ...
  }, [refreshProducts, refreshUnifiedPOSData]);
