...
const PrintReceipt: React.FC<PrintReceiptProps> = ({
        ...
        const printStyles = `
          <style>
            @media print {
              ...
              .print-content { max-width: none !important; }
            }
          </style>
        `;
  ...
  const handleCopyAsText = async () => {
    ...
    const receiptText = `
وصل رقم: ${orderId}
التاريخ: ${new Date().toLocaleDateString('ar-SA')} ${new Date().toLocaleTimeString('ar-SA')}
${employeeName ? `الموظف: ${employeeName}` : ''}
${customerName ? `العميل: ${customerName}` : ''}

المنتجات:
${items.map(item => `- ${item.product.name} × ${item.quantity} = ${formatPriceWithSettings((item.variantPrice || item.wholesalePrice || item.product.price) * item.quantity)}`).join('\n')}
...
${discountAmount > 0 ? `الخصم (${discount}%): -${formatPriceWithSettings(discountAmount)}` : ''}
...
${paymentMethod ? `طريقة الدفع: ${paymentMethod}` : ''}
  ...
  return createPortal(
    <div 
      className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 cursor-pointer hover:bg-black/65 transition-colors duration-200" 
      style={{ 
        zIndex: 99999,
        position: 'fixed',
        top: 0,
    ...
    >
              <div 
          className="bg-white/95 backdrop-blur-md shadow-2xl rounded-xl max-w-md w-full mx-4 max-h-[95vh] overflow-hidden border border-white/20 cursor-default" 
          onClick={(e) => e.stopPropagation()}
        >
        {/* رأسية محسنة */}
        <div className="flex justify-between items-center p-6 border-b border-gray-200/50 bg-gradient-to-r from-blue-50 to-indigo-50">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-100 rounded-lg">
              <Receipt className="h-5 w-5 text-blue-600" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-gray-900">طباعة الوصل</h2>
        ...
        {/* محتوى الوصل */}
        <div className="overflow-auto max-h-[calc(95vh-180px)] p-6">
          {/* محتوى الوصل للطباعة */}
          <div 
            ref={printRef}
            className={`receipt-content print-content transition-all duration-300 ${
        ...
        {/* حالة التحميل */}
        {isLoading && (
          <div className="p-4 bg-blue-50 border-t border-gray-200/50">
        ...
        {/* شريط المساعدة */}
        <div className="p-3 bg-gray-50 border-t border-gray-200/50 text-center">
          <p className="text-xs text-gray-500">
            اضغط <kbd className="px-1.5 py-0.5 bg-gray-200 rounded text-xs font-mono">Esc</kbd> أو انقر خارج النافذة للإغلاق
          </p>
                 </div>
       </div>
     </div>,
