    ...
    const initializeTracker = async () => {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('🔄 [ProductConversionTracker] Waiting for tracking settings...', {
            initial_settings: trackingSettings,
            productId,
            organizationId
          });
        }
        ...
        while ((!trackingSettings) && attempts < maxAttempts) {
          ...
          if (process.env.NODE_ENV === 'development' && attempts % 5 === 0) {
            console.log(`🔄 [ProductConversionTracker] Still waiting... attempt ${attempts}/${maxAttempts}`, {
              current_settings: trackingSettings
            });
          }
        }
        
        if (process.env.NODE_ENV === 'development') {
          console.log('📋 [ProductConversionTracker] Final settings received:', {
            trackingSettings,
            tiktok: trackingSettings?.tiktok,
            events_api_enabled: trackingSettings?.tiktok?.events_api_enabled,
        ...
        if (!trackingSettings) {
          if (process.env.NODE_ENV === 'development') {
            console.warn('⚠️ [ProductConversionTracker] No tracking settings found, using defaults');
          }
          trackingSettings = {
        ...
        const handleSettingsUpdate = (event: any) => {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.log('🔄 [ProductConversionTracker] Settings updated via event:', {
              updatedSettings,
              tiktok: updatedSettings?.tiktok,
              events_api_enabled: updatedSettings?.tiktok?.events_api_enabled
            });
          }
