...
export async function createOrUpdateCustomShipping(
  ...
  try {
    ...
    console.error('خطأ في إنشاء/تحديث إعدادات الشحن المخصصة:', error);
    return { 
      success: false, 
...
export async function getCustomShippingSettings(organizationId: string) {
  try {
    ...
    console.error('خطأ في جلب إعدادات الشحن المخصصة:', error);
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'حدث خطأ غير متوقع',
      data: null 
...
export async function toggleCustomShipping(organizationId: string, isEnabled: boolean) {
  try {
    ...
    console.error('خطأ في تفعيل/إلغاء تفعيل الشحن المخصص:', error);
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'حدث خطأ غير متوقع' 
    };
...
export async function calculateCustomShippingPrice(
  ...
  try {
    ...
    console.error('خطأ في حساب سعر الشحن المخصص:', error);
    return 0;
  }
} 