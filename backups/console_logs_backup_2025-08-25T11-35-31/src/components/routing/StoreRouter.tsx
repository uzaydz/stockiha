...
const StoreRouter = React.memo(() => {
  ...
  useEffect(() => {
    ...
    checkDomain();
  }, [hostname, subdomain, isSubdomainStore, isCustomDomain, retryCount]);
  ...
  if (isLoading) {
    console.log('🔄 [StoreRouter] isLoading = true, فحص نوع النطاق...', { isSubdomainStore, isCustomDomain, subdomain, hostname });
    
    // 🔥 إذا كان متجر، نعرض StorePage مباشرة لتتولى عرض المحتوى عند الانتهاء
    if (isSubdomainStore || isCustomDomain) {
      console.log('🚀 [StoreRouter] عرض المتجر أثناء التحميل:', { isSubdomainStore, isCustomDomain, subdomain, hostname });
      return <StorePage />;
    }
    
    // للنطاقات الأخرى، نعتمد على مؤشر التحميل المركزي
    console.log('⏳ [StoreRouter] انتظار مؤشر التحميل المركزي للنطاق العام');
    return null;
  }
  
  // 🔥 إذا كان متجر، اعرض StorePage
  if (isStore === true) {
    console.log('🚀 [StoreRouter] عرض المتجر بعد اكتمال التحميل');
    return <StorePage />;
  }
  
  // 🔥 لا نعرض صفحة الهبوط إذا كان هناك نطاق فرعي وما زلنا نحمل
  if (isSubdomainStore && isLoading) {
    console.log('🚀 [StoreRouter] عرض المتجر للنطاق الفرعي أثناء التحميل');
    return <StorePage />;
  }

  // 🔥 عرض صفحة الهبوط للنطاقات العامة فقط
  console.log('🚀 [StoreRouter] عرض صفحة الهبوط للنطاق العام:', hostname);
  return <LandingPage />;
});
