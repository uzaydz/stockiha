...
import { SharedStoreDataProvider, MinimalSharedStoreDataProvider } from '@/context/SharedStoreDataContext';
...
const PROVIDER_CONFIGS: Record<PageType, ProviderConfig> = {
  ...
  'dashboard': {
    ...
    organizationData: true, // ✅ مطلوب لـ useOrdersData في POSOrdersOptimized
...
export const SmartProviderWrapper: React.FC<SmartProviderWrapperProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    console.log('✅ [PERFORMANCE] انتهاء useEffect SmartProviderWrapper:', {
      duration: effectDuration,
      'وقت التأثير بالمللي ثانية': effectDuration
    });
  ...
  const { pageType, config } = useMemo(() => {
    ...
    console.log('🎯 [PERFORMANCE] تحديد نوع الصفحة:', {
      pathname: location.pathname,
      pageType: type,
  ...
  const wrappedContent = useMemo(() => {
                ...
                {config.auth ? (
                  <AuthProvider>
                    {config.tenant ? (
                      <TenantProvider>
                        <UserProvider>
                          {/* استخدام النسخة المحسنة لصفحات dashboard */}
                          {pageType === 'dashboard' ? (
    ...
    console.log('✅ [PERFORMANCE] انتهاء إنشاء الـ providers:', {
      ...
      'وقت إنشاء الـ providers بالمللي ثانية': providersDuration,
      pageType,
      totalProviders: Object.values(config).filter(Boolean).length
    });
  ...
  console.log('🏁 [PERFORMANCE] انتهاء SmartProviderWrapper:', {
    totalDuration: totalWrapperDuration,
    'إجمالي وقت الـ wrapper بالمللي ثانية': totalWrapperDuration,
    pageType,
    pathname: location.pathname,
    isAlreadyRendered,
    performanceMetrics: {
      totalRenders: PERFORMANCE_METRICS.totalRenders,
      totalDuplicates: PERFORMANCE_METRICS.totalDuplicates,
      averageTime: PERFORMANCE_METRICS.averageTime.toFixed(2) + 'ms',
      warningsCount: PERFORMANCE_METRICS.warnings.length
    }
  });
