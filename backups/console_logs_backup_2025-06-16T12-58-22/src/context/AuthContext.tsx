...
const addCallCenterAgentData = async (userProfile: UserProfile): Promise<UserProfile> => {
  ...
  /*
  console.log('🔍 Checking for call center agent data for user:', userProfile.email, 'User ID:', userProfile.id);
  ...
  try {
    ...
    console.error('💥 Error fetching call center agent data:', error);
  }
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const savedAuthState = useMemo(() => {
    ...
    console.log('🏗️ AuthProvider ONE-TIME initialization:', {
  ...
  const updateAuthState = useCallback((newSession: Session | null, newUser: SupabaseUser | null, clearAll: boolean = false) => {
    ...
    if (!clearAll && session && newSession && user && newUser) {
      if (session.access_token === newSession.access_token && user.id === newUser.id) {
        console.log('⏭️ Same session and user, skipping update');
        return;
      }
    }
    ...
    if (clearAll) {
      ...
      console.log('💾 Updating auth state with new session/user');
  ...
  useEffect(() => {
    console.log('🎧 إعداد اشتراك AuthSingleton في AuthProvider');
    const subscriptionId = authSingleton.subscribe((authData) => {
      ...
      if (authData.session !== session || authData.user !== user) {
        console.log('🔄 AuthSingleton: تحديث بيانات المصادقة');
    ...
    return () => {
      if (authCacheCleanupRef.current) {
        console.log('🧹 تنظيف اشتراك AuthSingleton');
        authCacheCleanupRef.current();
      }
    };
  }, []);
      ...
      if (savedAuthState.session && savedAuthState.user) {
        ...
        try {
          ...
          if (isValid) {
            ...
            console.log('❌ Saved session is invalid, clearing auth state');
            updateAuthState(null, null, true);
            setIsLoading(false);
            setHasInitialSessionCheck(true);
          }
        ...
        console.log('🔍 No saved session found, will check Supabase session');
        setIsLoading(true);
      }
  ...
  useEffect(() => {
    ...
    if (user && userProfile && userProfile.id === user.id && hasInitialSessionCheck) {
      if (isLoading) {
        console.log('✅ Auth complete: user + profile loaded');
        setIsLoading(false);
      }
    }
    // إذا لم يكن هناك user بعد التحقق الأولي، توقف عن التحميل
      ...
      if (isLoading) {
        console.log('❌ No user found after initial check');
        setIsLoading(false);
      }
    }
    // إذا كان لدينا user فقط (بدون profile) وقد تم الفحص الأولي، نحتاج للانتظار قليلاً
      ...
      console.log('⏳ User found but waiting for profile...');
      // إعطاء وقت محدود لتحميل ال profile
      const profileTimeout = setTimeout(() => {
        if (!userProfile || userProfile.id !== user.id) {
          console.log('⚠️ Profile loading timeout, stopping loading state');
          setIsLoading(false);
        }
      }, 3000); // 3 ثواني
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (user && session) {
        ...
        if (userProfile && userProfile.id === user.id) {
          console.log('📋 User data already loaded for current user');
          setIsLoading(false);
          return;
        }
        ...
        if (savedUserData.userProfile && savedUserData.userProfile.id === user.id && !userProfile) {
          console.log('🔄 Using cached profile while validating...');
