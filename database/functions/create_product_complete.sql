-- ================================================
-- دالة متقدمة لإنشاء المنتج الكامل في معاملة واحدة
-- ================================================

-- حذف الدالة القديمة لتجنب تضارب المعاملات
DROP FUNCTION IF EXISTS create_product_complete(p_product_data jsonb, p_advanced_settings jsonb, p_marketing_settings jsonb, p_colors jsonb, p_images jsonb, p_wholesale_tiers jsonb, p_user_id uuid);

CREATE OR REPLACE FUNCTION create_product_complete(
...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO products (
      ...
      (p_product_data->>'organization_id')::UUID,
      (p_product_data->>'name')::TEXT,
      COALESCE((p_product_data->>'description')::TEXT, ''),
      COALESCE((p_product_data->>'price')::NUMERIC, 0),
      CASE WHEN p_product_data ? 'purchase_price' AND (p_product_data->>'purchase_price') IS NOT NULL AND (p_product_data->>'purchase_price') != '' 
           THEN (p_product_data->>'purchase_price')::NUMERIC ELSE NULL END,
      CASE WHEN p_product_data ? 'compare_at_price' AND (p_product_data->>'compare_at_price') IS NOT NULL AND (p_product_data->>'compare_at_price') != '' 
           THEN (p_product_data->>'compare_at_price')::NUMERIC ELSE NULL END,
      CASE WHEN p_product_data ? 'sku' AND (p_product_data->>'sku') IS NOT NULL AND (p_product_data->>'sku') != '' 
           THEN (p_product_data->>'sku')::TEXT ELSE NULL END,
      CASE WHEN p_product_data ? 'barcode' AND (p_product_data->>'barcode') IS NOT NULL AND (p_product_data->>'barcode') != '' 
           THEN (p_product_data->>'barcode')::TEXT ELSE NULL END,
      (p_product_data->>'category_id')::UUID,
      CASE WHEN p_product_data ? 'subcategory_id' AND (p_product_data->>'subcategory_id') IS NOT NULL AND (p_product_data->>'subcategory_id') != '' 
           THEN (p_product_data->>'subcategory_id')::UUID ELSE NULL END,
      CASE WHEN p_product_data ? 'brand' AND (p_product_data->>'brand') IS NOT NULL AND (p_product_data->>'brand') != '' 
           THEN (p_product_data->>'brand')::TEXT ELSE NULL END,
      COALESCE((p_product_data->>'stock_quantity')::INTEGER, 0),
      COALESCE((p_product_data->>'thumbnail_image')::TEXT, ''),
      COALESCE((p_product_data->>'has_variants')::BOOLEAN, FALSE),
      COALESCE((p_product_data->>'show_price_on_landing')::BOOLEAN, TRUE),
      COALESCE((p_product_data->>'is_featured')::BOOLEAN, FALSE),
      COALESCE((p_product_data->>'is_new')::BOOLEAN, TRUE),
      COALESCE((p_product_data->>'use_sizes')::BOOLEAN, FALSE),
      COALESCE((p_product_data->>'is_sold_by_unit')::BOOLEAN, TRUE),
      CASE WHEN p_product_data ? 'unit_type' AND (p_product_data->>'unit_type') IS NOT NULL AND (p_product_data->>'unit_type') != '' 
           THEN (p_product_data->>'unit_type')::TEXT ELSE NULL END,
      COALESCE((p_product_data->>'use_variant_prices')::BOOLEAN, FALSE),
      CASE WHEN p_product_data ? 'unit_purchase_price' AND (p_product_data->>'unit_purchase_price') IS NOT NULL AND (p_product_data->>'unit_purchase_price') != '' 
           THEN (p_product_data->>'unit_purchase_price')::NUMERIC ELSE NULL END,
      CASE WHEN p_product_data ? 'unit_sale_price' AND (p_product_data->>'unit_sale_price') IS NOT NULL AND (p_product_data->>'unit_sale_price') != '' 
           THEN (p_product_data->>'unit_sale_price')::NUMERIC ELSE NULL END,
      CASE WHEN p_product_data ? 'form_template_id' AND (p_product_data->>'form_template_id') IS NOT NULL AND (p_product_data->>'form_template_id') != '' 
           THEN (p_product_data->>'form_template_id')::UUID ELSE NULL END,
      CASE WHEN p_product_data ? 'shipping_provider_id' AND (p_product_data->>'shipping_provider_id') IS NOT NULL AND (p_product_data->>'shipping_provider_id') != '' 
           THEN (p_product_data->>'shipping_provider_id')::INTEGER ELSE NULL END,
      COALESCE((p_product_data->>'use_shipping_clone')::BOOLEAN, FALSE),
      CASE WHEN p_product_data ? 'shipping_clone_id' AND (p_product_data->>'shipping_clone_id') IS NOT NULL AND (p_product_data->>'shipping_clone_id') != '' 
           THEN (p_product_data->>'shipping_clone_id')::INTEGER ELSE NULL END,
      COALESCE((p_product_data->>'is_digital')::BOOLEAN, FALSE),
      CASE WHEN p_product_data ? 'features' AND (p_product_data->>'features') IS NOT NULL 
           THEN ARRAY(SELECT jsonb_array_elements_text((p_product_data->>'features')::JSONB))
           ELSE '{}'::text[] END,
      COALESCE((p_product_data->>'specifications')::JSONB, '{}'::JSONB),
      CASE WHEN p_product_data ? 'name_for_shipping' AND (p_product_data->>'name_for_shipping') IS NOT NULL AND (p_product_data->>'name_for_shipping') != '' 
           THEN (p_product_data->>'name_for_shipping')::TEXT ELSE NULL END,
      COALESCE((p_product_data->>'allow_retail')::BOOLEAN, TRUE),
      COALESCE((p_product_data->>'allow_wholesale')::BOOLEAN, FALSE),
      COALESCE((p_product_data->>'allow_partial_wholesale')::BOOLEAN, FALSE),
      CASE WHEN p_product_data ? 'wholesale_price' AND (p_product_data->>'wholesale_price') IS NOT NULL AND (p_product_data->>'wholesale_price') != '' 
           THEN (p_product_data->>'wholesale_price')::NUMERIC ELSE NULL END,
      CASE WHEN p_product_data ? 'partial_wholesale_price' AND (p_product_data->>'partial_wholesale_price') IS NOT NULL AND (p_product_data->>'partial_wholesale_price') != '' 
           THEN (p_product_data->>'partial_wholesale_price')::NUMERIC ELSE NULL END,
      CASE WHEN p_product_data ? 'min_wholesale_quantity' AND (p_product_data->>'min_wholesale_quantity') IS NOT NULL AND (p_product_data->>'min_wholesale_quantity') != '' 
           THEN (p_product_data->>'min_wholesale_quantity')::INTEGER ELSE NULL END,
      CASE WHEN p_product_data ? 'min_partial_wholesale_quantity' AND (p_product_data->>'min_partial_wholesale_quantity') IS NOT NULL AND (p_product_data->>'min_partial_wholesale_quantity') != '' 
           THEN (p_product_data->>'min_partial_wholesale_quantity')::INTEGER ELSE NULL END,
      COALESCE((p_product_data->>'slug')::TEXT, LOWER(REPLACE((p_product_data->>'name')::TEXT, ' ', '-')) || '-' || EXTRACT(EPOCH FROM NOW())::TEXT),
      CASE 
    ...
    -- جلب بيانات المنتج المُنشأ للحصول على has_variants
    SELECT * INTO v_product_record FROM products WHERE id = v_product_id;
    
    -- الحصول على قيمة has_variants
    v_has_variants := v_product_record.has_variants;

            -- 2. إنشاء الإعدادات المتقدمة (باستخدام الحقول الموجودة فعلاً)
        ...
        END IF;
            ...
            -- 3. إنشاء إعدادات التسويق (فقط إذا كانت موجودة)
    ...
    IF p_colors IS NOT NULL THEN
      ...
      -- حساب إجمالي المخزون إذا كان المنتج له متغيرات
      IF v_has_variants THEN
        SELECT COALESCE(SUM(quantity), 0) INTO v_total_stock
        FROM product_colors 
        WHERE product_id = v_product_id;
        
        -- تحديث stock_quantity في جدول المنتجات
        UPDATE products 
    ...
    SELECT jsonb_build_object(
      ...
      'created_at', v_product_record.created_at,
      'total_stock', CASE WHEN v_has_variants THEN v_total_stock ELSE v_product_record.stock_quantity END,
      'message', 'Product created successfully in single transaction'
    ) INTO v_result;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      RETURN jsonb_build_object(
        'success', false,
        'error', SQLERRM,
        'message', 'Failed to create product: ' || SQLERRM
      );
  END;
END;
$$; 