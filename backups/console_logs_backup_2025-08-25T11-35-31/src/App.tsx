...
// 🎯 مكون تحميل محسن للتطبيق - يعرض المتجر أو المنتج مباشرة عند الكشف المبكر
const AppLoader = () => {
  // 🔥 الكشف المبكر للنطاق والصفحة
  const detectEarlyContent = () => {
    try {
      ...
      // فحص ما إذا كان في صفحة منتج
      const isProductPage = pathname.includes('/product-purchase-max-v2/') ||
      ...
      if (!isPublicDomain && !hostname.includes('localhost')) {
        ...
        if (isProductPage) {
          // في صفحة منتج - عرض منتج أساسي بدلاً من التحميل
          return <ProductPageSkeleton showInstantContent={true} />;
        } else {
          // في الصفحة الرئيسية - عرض المتجر مباشرة
          return <StorePage />;
        }
      }
    } catch (error) {
