...
export const useProductTracking = (options: ProductTrackingOptions) => {
  const { productId, organizationId, autoLoadSettings = true, enableDebugMode = true, existingProductData } = options;
  ...
  // تحميل إعدادات التتبع من الخادم أو البيانات الموجودة
  const loadTrackingSettings = useCallback(async () => {
    ...
    try {
      ...
      // استخدام البيانات الموجودة إذا توفرت
      if (existingProductData?.marketing_settings) {
        marketingSettings = existingProductData.marketing_settings;
        if (enableDebugMode) {
          console.log('🎯 useProductTracking: استخدام البيانات الموجودة بدلاً من إعادة الجلب');
        }
      } else {
        // الجلب من قاعدة البيانات فقط إذا لم تتوفر البيانات
        const { supabase } = await import('@/lib/supabase-unified');

        // استدعاء دالة get_product_complete_data مباشرة
        const { data: productData, error } = await (supabase as any).rpc('get_product_complete_data', {
        ...
        if (!(productData as any)?.success) {
          throw new Error((productData as any)?.error?.message || 'فشل في جلب بيانات المنتج');
        }

        // البحث عن marketing_settings في المكان الصحيح
        marketingSettings = (productData as any).data?.product?.marketing_settings || (productData as any).product?.marketing_settings;
        
        if (enableDebugMode) {
          console.log('🔄 useProductTracking: جلب البيانات من قاعدة البيانات');
        }
      }
      ...
      console.log('📤 useProductTracking: حفظ الإعدادات في window.__productTrackingSettings');
    } catch (err) {
  ...
  }, [productId, organizationId, enableDebugMode, existingProductData]);
  ...
  // تحميل الإعدادات عند توفر البيانات الموجودة
  useEffect(() => {
    if (existingProductData?.marketing_settings && !settingsLoadedRef.current) {
      loadTrackingSettings();
    }
  }, [existingProductData, loadTrackingSettings]);
