...
interface ServiceFormData {
  ...
  description: string;
  provider: string;
  service_type: 'streaming' | 'gaming' | 'software' | 'music' | 'education' | 'cloud' | 'other';
  category_id: string;
  delivery_method: 'manual' | 'automatic' | 'both';
...
export const AddServiceDialog: React.FC<AddServiceDialogProps> = ({
  ...
  const [currentCategories, setCurrentCategories] = useState<SubscriptionServiceCategory[]>(categories);

  // تحديث الفئات عند تغيير categories prop
  useEffect(() => {
  ...
  const handleCategoriesUpdate = (updatedCategories: SubscriptionServiceCategory[]) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.category_id) {
      toast({
        title: "خطأ",
        description: "يرجى اختيار فئة للخدمة أو إنشاء فئة جديدة",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      // التأكد من وجود الفئة في قاعدة البيانات قبل إنشاء الخدمة
      const { data: categoryCheck, error: categoryError } = await (supabase as any)
      ...
      if (categoryError || !categoryCheck) {
        toast({
          title: "خطأ",
          description: "الفئة المختارة غير صحيحة. يرجى اختيار فئة أخرى أو إنشاء فئة جديدة.",
          variant: "destructive",
        });
        return;
      }
      ...
      // إدراج الخدمة (profit_amount و profit_margin محسوبان تلقائياً)
      const { data: serviceData, error: serviceError } = await supabase
        ...
        .insert({
          ...
          sold_quantity: 0,
          reserved_quantity: 0,
          available_quantity: formData.total_quantity || 0,
