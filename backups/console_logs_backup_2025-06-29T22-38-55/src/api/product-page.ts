...
export const getProductPageData = async (organizationId: string, slug: string): Promise<ProductPageData | null> => {
  console.log('🔍 getProductPageData - بدء جلب بيانات المنتج:', { organizationId, slug });
  ...
  return requestCache.get(
    ...
    async () => {
      try {
        console.log('🔍 getProductPageData - التحقق من المعاملات:', { organizationId, slug });
        
        // التحقق من صحة المعاملات
        if (!organizationId || !slug) {
          console.error('❌ getProductPageData - معاملات غير صحيحة:', { organizationId, slug });
          throw new Error('معاملات غير صحيحة: معرف المؤسسة أو slug مفقود');
        }

        console.log('🔍 getProductPageData - استدعاء Edge Function:', { slug, organization_id: organizationId });
        
        // محاولة Edge Function أولاً (إذا كان متاحاً)
        let responseData: any = null;
        let functionError: any = null;
        
        try {
          // التحقق من وجود functions API
          if (supabase.functions && typeof supabase.functions.invoke === 'function') {
            ...
            console.warn('⚠️ Edge Functions غير متاح في هذا العميل');
            functionError = new Error('Edge Functions not available');
          }
        } catch (edgeError) {
          console.warn('⚠️ Edge Function غير متاح، استخدام RPC مباشرة:', edgeError);
          functionError = edgeError;
        }

        console.log('🔍 getProductPageData - استجابة Edge Function:', {
          responseData,
          functionError,
        ...
        if (functionError || !responseData) {
          console.log('🔄 getProductPageData - محاولة RPC مباشرة كـ fallback');
          
          // محاولة RPC function مباشرة كـ fallback
          try {
            ...
            console.log('🔍 getProductPageData - استجابة RPC:', {
              rpcData,
              rpcError,
              hasData: !!rpcData
            });
            ...
            if (rpcError) {
              console.error('❌ getProductPageData - خطأ في RPC:', rpcError);
              throw new Error(`RPC Error: ${rpcError.message}`);
            }
            
            if (!rpcData) {
              console.log('ℹ️ getProductPageData - المنتج غير موجود (RPC)');
              return null;
            }
            ...
            console.log('✅ getProductPageData - نجح RPC fallback:', {
            ...
            console.error('❌ getProductPageData - فشل RPC fallback أيضاً:', fallbackError);
            
            // معالجة أنواع مختلفة من الأخطاء الأصلية
            ...
            if (errorStatus === 404 || errorMessage.includes('404') || errorMessage.includes('Product not found')) {
              console.log('ℹ️ getProductPageData - المنتج غير موجود (404)');
