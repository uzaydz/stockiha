...
export const useCompletePOSData = (options: POSDataOptions = {}) => {
  ...
  const {
    ...
    enabled: !!currentOrganization?.id && (search === undefined || search.length === 0 || search.length >= 2), // البحث فقط إذا كان فارغ أو أكثر من حرفين
    staleTime: 5 * 60 * 1000, // 5 دقائق للبحث
