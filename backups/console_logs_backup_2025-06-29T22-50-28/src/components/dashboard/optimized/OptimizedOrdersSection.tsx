...
import { useDashboardOrders, useDashboardOnlineOrders } from '@/context/DashboardDataContext';
...
const OptimizedOnlineOrdersCard = () => {
  const { onlineOrders, isLoading, error } = useDashboardOnlineOrders();
  ...
  // التأكد من أن onlineOrders هو مصفوفة
...
const OptimizedOrdersSection: React.FC = () => {
  const { orders, isLoading: isOrdersLoading, error: ordersError } = useDashboardOrders();
  const { isLoading: isOnlineOrdersLoading, error: onlineOrdersError } = useDashboardOnlineOrders();

  const isLoading = isOrdersLoading || isOnlineOrdersLoading;
  const hasError = ordersError || onlineOrdersError;
  ...
  if (hasError) {
    return <OrdersError error={hasError} onRetry={() => window.location.reload()} />;
  }
