  ...
  useEffect(() => {
    if (dbComponents && dbComponents.length > 0 && components.length === 0) {
      ...
      const formattedComponents = dbComponents.map(comp => {
        ...
        console.log(`تحويل النوع من قاعدة البيانات: ${comp.type} -> ${type}`);
      ...
      console.log('تم تحميل المكونات من قاعدة البيانات:', formattedComponents);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const convertedComponents = components.map(comp => {
        ...
        console.log(`تحويل النوع: ${comp.type} -> ${componentTypeForDB}`);
      ...
      console.log('المكونات المحولة للحفظ:', convertedComponents);
      ...
      for (const comp of convertedComponents) {
        try {
          ...
          if (checkError) {
            console.error('خطأ في التحقق من المكون:', checkError);
            continue;
          }
          ...
          if (existingComponent) {
            try {
              ...
              if (updateError) {
                console.error('خطأ في تحديث المكون:', updateError);
              }
            } catch (error) {
              console.error('استثناء في تحديث المكون:', error);
            }
          } else {
            ...
            if (typeCheckError) {
              console.error('خطأ في التحقق من نوع المكون:', typeCheckError);
            }
            ...
            if (typeExists) {
              ...
              if (typeUpdateError) {
                console.error('خطأ في تحديث المكون الموجود من نفس النوع:', typeUpdateError);
              }
            } else {
              ...
              if (insertError) {
                console.error('خطأ في إضافة المكون الجديد:', insertError);
              }
            }
          }
        } catch (error) {
          console.error('استثناء في إضافة المكون:', error);
        }
      }
      ...
      console.error('خطأ في حفظ المكونات:', error);
