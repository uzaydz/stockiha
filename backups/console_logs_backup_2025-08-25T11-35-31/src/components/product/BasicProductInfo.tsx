...
export default function BasicProductInfo({ form }: BasicProductInfoProps) {
  ...
  useEffect(() => {
    if (watchedName && !isSlugManual && !isFromProductInfoGenerator) {
      ...
      console.log('🔍 [useEffect] Auto-generating slug:', {
        watchedName,
        currentSlug,
        isSlugManual,
        isFromProductInfoGenerator
      });
      
      if (!currentSlug || currentSlug === 'product' || currentSlug === '') {
        const generatedSlug = generateSlugFromText(watchedName);
        console.log('🔍 [useEffect] Generated slug:', generatedSlug);
        ...
        setTimeout(() => {
          ...
          console.log('🔍 [useEffect] Saved slug in form:', savedSlug);
        }, 100);
      }
    }
