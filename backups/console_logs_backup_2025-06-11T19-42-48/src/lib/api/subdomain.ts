...
export const getOrganizationBySubdomain = async (subdomain: string): Promise<Organization | null> => {
  ...
  return withCache<Organization | null>(
    ...
    async () => {
      try {
        console.log(`[getOrganizationBySubdomain] البحث عن المؤسسة بالـ subdomain: ${subdomain}`);
        ...
        if (error) {
          ...
          if (error.code !== 'PGRST116') { // PGRST116: "The result contains 0 rows"
            console.log(`[getOrganizationBySubdomain] خطأ في البحث عن ${subdomain}:`, error.message);
          } else {
            console.log(`[getOrganizationBySubdomain] لم يتم العثور على مؤسسة بالـ subdomain: ${subdomain}`);
          }
          return null;
        }
        
        if (data) {
          console.log(`[getOrganizationBySubdomain] تم العثور على المؤسسة: ${data.name} (${data.id})`);
        }
        
        return data as Organization || null;
      } catch (error) {
        console.log(`[getOrganizationBySubdomain] خطأ في تحميل المؤسسة ${subdomain}:`, error.message);
        return null;
      }
    },
