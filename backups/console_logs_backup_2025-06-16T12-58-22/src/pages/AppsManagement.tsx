...
const AppsManagement: React.FC = () => {
  ...
  console.log('🔍 Apps Context State:', {
    availableAppsCount: availableApps.length,
    organizationAppsCount: organizationApps.length,
    isLoading,
    enableAppExists: !!enableApp,
    disableAppExists: !!disableApp,
    isAppEnabledExists: !!isAppEnabled,
    refreshAppsExists: !!refreshApps
  });
  ...
  const handleAppToggle = async (appId: string, enabled: boolean) => {
    console.log('🔄 handleAppToggle called:', { appId, enabled, currentProcessing: isProcessing });
    ...
    if (isProcessing || processingAppId === appId) {
      console.log('⚠️ Already processing app:', appId);
      return;
    }

    console.log('🚀 Starting to toggle app:', app.name);
    ...
    try {
      ...
      if (enabled) {
        console.log('✅ Attempting to enable app:', appId);
        success = await enableApp(appId);
        console.log('✅ Enable result:', success);
      } else {
        console.log('❌ Attempting to disable app:', appId);
        success = await disableApp(appId);
        console.log('❌ Disable result:', success);
      }
      
      if (success) {
        console.log('🎉 App toggled successfully:', app.name, enabled ? 'enabled' : 'disabled');
        // تحديث واجهة المستخدم فوراً
        await refreshApps();
      } else {
        console.error('💥 Failed to toggle app state');
      }
    } catch (error) {
      console.error('💥 Error toggling app:', error);
    } finally {
      console.log('🏁 Finished processing app:', appId);
  ...
  const showAppDetails = (app: AppDefinition) => {
    console.log('showAppDetails called with app:', app);
    setSelectedApp(app);
  ...
  const executeAction = async () => {
    ...
    try {
      if (actionType === 'enable') {
        await enableApp(selectedApp.id);
      } else {
        await disableApp(selectedApp.id);
      }
    } catch (error) {
                        ...
                        <div className="flex items-center gap-2" style={{ zIndex: 20 }}>
                                ...
                                <div className="flex items-center" style={{ zIndex: 21 }}>
                                  <Switch
                                    ...
                                    onCheckedChange={(checked) => {
                                      console.log('🔘 Switch toggled:', app.id, 'from', isEnabled, 'to', checked);
                          ...
                          <Button
                            ...
                            onClick={async (e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              console.log('🔴 Toggle button clicked:', app.id, 'current state:', isEnabled, 'will change to:', !isEnabled);
                ...
                <div className="sticky bottom-0 p-4 sm:p-6 border-t bg-background/95 backdrop-blur-sm">
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto order-1 sm:order-2">
                      ...
                      <Button
                        onClick={async () => {
                          console.log('🔥 Dialog toggle button clicked for:', selectedApp.id);
