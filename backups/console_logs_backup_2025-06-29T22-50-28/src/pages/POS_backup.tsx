...
// import { useAutoRefresh } from '@/hooks/useAutoRefresh'; // مُعطل لتحسين الأداء
...
import { ShoppingCart, Wrench, Settings2, CreditCard, RotateCcw, Package, Calculator as CalculatorIcon, RefreshCw } from 'lucide-react';
...
const POS = () => {
  ...
  // للتوافق مع الكود الحالي - استخدام التبويب النشط
  ...
  // إعداد التحديث التلقائي للبيانات - مُعطل لتحسين الأداء
  // const { refreshData: autoRefreshData } = useAutoRefresh({
  ...
  const addVariantToCart = (
    ...
    if (availableQuantity <= 0) {
      toast.error(`"${variantName || product.name}" غير متوفر في المخزون`);
      return;
    }
  ...
  const removeItemFromCart = (index: number) => {
    ...
    // إرجاع المخزون للمنتج باستخدام تحديث cache مباشر أولاً
    updateProductStockInCache(
      item.product.id, 
      item.colorId || null, 
      item.sizeId || null, 
      -item.quantity // قيمة سالبة لإضافة المخزون مرة أخرى
    );
    
    // النسخة الاحتياطية: إرجاع المخزون في ProductCatalog
  ...
  const updateItemQuantity = (index: number, quantity: number) => {
    ...
    // إضافة الكمية الحالية للمخزون المتاح (لأنها محجوزة في السلة)
    ...
    if (quantity > totalAvailable) {
      ...
      // تحديث المخزون في cache
      updateProductStockInCache(
        item.product.id,
        item.colorId || null,
        item.sizeId || null,
        totalAvailable - oldQuantity // الفرق بين الكمية الجديدة والقديمة
      );
      return;
    }
    
    // تحديث المخزون في cache بناءً على الفرق في الكمية
    if (quantityDiff !== 0) {
      updateProductStockInCache(
        item.product.id,
        item.colorId || null,
        item.sizeId || null,
        quantityDiff // الفرق الموجب أو السالب
      );
    }
  ...
  // Limpiar carrito
  const clearCart = () => {
    // إرجاع المخزون لجميع المنتجات في السلة باستخدام تحديث cache مباشر
    cartItems.forEach(item => {
      // استخدام تحديث cache مباشر من POSDataContext
      updateProductStockInCache(
        item.product.id, 
        item.colorId || null, 
        item.sizeId || null, 
      ...
      // النسخة الاحتياطية: استخدام productCatalogUpdateFunction إذا كانت متوفرة
      if (productCatalogUpdateFunction.current) {
        productCatalogUpdateFunction.current(item.product.id, item.quantity);
      }
    });
  ...
  const removeSubscriptionFromCart = (subscriptionId: string) => {
    removeSubscriptionTab(activeTabId, subscriptionId);
  };

  // تحديث سعر الاشتراك
  const updateSubscriptionPrice = (subscriptionId: string, price: number) => {
    updateSubscriptionPriceTab(activeTabId, subscriptionId, price);
  };
  ...
  const removeServiceFromCart = (serviceId: string) => {
    removeServiceTab(activeTabId, serviceId);
  };

  // Actualizar precio de un servicio
  const updateServicePrice = (serviceId: string, price: number) => {
    updateServicePriceTab(activeTabId, serviceId, price);
  };
  ...
  const submitOrder = async (orderDetails: Partial<Order>): Promise<{orderId: string, customerOrderNumber: number}> => {
    ...
    try {
      ...
      try {
        // تحديث المخزون في cache مباشرة لكل منتج (طرح الكمية المباعة)
        cartItems.forEach(item => {
          updateProductStockInCache(
            item.product.id,
            item.colorId || null,
            item.sizeId || null,
            item.quantity // قيمة موجبة لطرح الكمية من المخزون
          );
        });
      } catch (error) {
      }
      ...
      // مسح السلة بدون إرجاع المخزون (لأن المنتجات تم بيعها فعلاً)
      clearCartTab(activeTabId);
      toast.success('تم إتمام الطلب وتحديث المخزون');
          ...
          <div className="grid grid-cols-12 gap-4 h-full">
            ...
            <div className="col-span-12 md:col-span-4 h-full">
              <div className="flex flex-col gap-4 sticky top-4 h-[calc(100vh-5rem)] overflow-hidden">
                ...
                {/* سلة المشتريات - نظام التبويبات الجديد */}
