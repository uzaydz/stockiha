...
export const useOrdersData = (options: UseOrdersDataOptions = {}) => {
  ...
  // Fetch orders using optimized RPC function
  ...
  const fetchOrderStats = useCallback(async () => {
    console.log('🚀 [fetchOrderStats] بداية استدعاء الدالة');
    if (!currentOrganization?.id) {
      console.log('⚠️ [fetchOrderStats] لا يوجد معرف مؤسسة');
      return;
    }

    try {
      console.log('🔍 [fetchOrderStats] استدعاء الدالة مع:', {
      ...
      if (error) {
        console.error('❌ [fetchOrderStats] خطأ في جلب الإحصائيات:', error);
        throw error;
      }

      console.log('📊 [fetchOrderStats] البيانات المستلمة:', statsData);
      console.log('📊 [fetchOrderStats] البيانات الخام الأولى:', statsData?.[0]);
      ...
      console.log('📈 [fetchOrderStats] الإحصائيات المحسوبة:', {
        counts: calculatedCounts,
        stats: calculatedStats
      });
      
      console.log('🔢 [fetchOrderStats] القيم التفصيلية:', {
  ...
  const returnValue = useMemo(() => ({
    ...
    updateOrderLocally,
    filters,
  }), [state, loadMore, refresh, updateFilters, updateOrderLocally, filters]);
