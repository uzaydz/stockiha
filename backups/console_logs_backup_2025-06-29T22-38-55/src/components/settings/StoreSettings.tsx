...
const StoreSettings = () => {
  ...
  useEffect(() => {
    console.log('🔍 StoreSettings: Dialog opened state changed:', {
      isDialogOpen,
    ...
    if (isDialogOpen && currentOrganization?.id) {
      console.log('📊 StoreSettings: Starting settings diagnostic:', {
  ...
  useEffect(() => {
    console.log('🔄 StoreSettings: Settings monitoring effect triggered:', {
      organizationId: currentOrganization?.id,
      isLoading,
      hasSettings: !!settings,
      settingsType: Array.isArray(settings) ? 'array' : typeof settings,
      settingsKeys: settings ? Object.keys(settings) : null,
      settingsOrgId: settings?.organization_id,
      isDataReady,
      timestamp: new Date().toISOString()
    });
    
    if (currentOrganization?.id) {
      console.log('🏢 StoreSettings: Organization data available:', {
        ...
        settingsStatus: {
          ...
          hasOrgId: settings?.organization_id,
      ...
      if (!isLoading && settings) {
        console.log('📋 StoreSettings: Processing loaded settings:', {
          originalSettings: settings,
          isArray: Array.isArray(settings),
          settingsLength: Array.isArray(settings) ? settings.length : 'not array',
        ...
        if (Array.isArray(settings) && settings.length > 0) {
          console.log('🔧 StoreSettings: Converting array to object:', {
            arrayLength: settings.length,
            firstElement: settings[0]
          });
        ...
        if (fixedSettings && !fixedSettings.organization_id) {
          console.log('⚠️ StoreSettings: Missing organization_id, fixing:', {
            currentOrgId: currentOrganization.id,
            settingsOrgId: fixedSettings.organization_id
          });
          ...
          console.log('🔧 StoreSettings: Organization ID will be fixed on save');
        }
        ...
        if (fixedSettings?.organization_id === currentOrganization.id || 
          ...
          console.log('✅ StoreSettings: Settings successfully processed:', {
            organizationId: fixedSettings.organization_id || currentOrganization.id,
            siteName: fixedSettings.site_name,
            defaultLanguage: fixedSettings.default_language,
            hasLogo: !!fixedSettings.logo_url,
            hasFavicon: !!fixedSettings.favicon_url,
            customJsLength: fixedSettings.custom_js?.length || 0,
            customCssLength: fixedSettings.custom_css?.length || 0
          });
          
          // تعيين البيانات كجاهزة فقط عندما تكون صحيحة
          setIsDataReady(true);
        } else {
          console.error('❌ StoreSettings: Failed to fix organization_id mismatch:', {
            expected: currentOrganization.id,
            actual: fixedSettings?.organization_id
          });
          setIsDataReady(false);
        }
      } else if (!isLoading && !settings) {
        // إذا لم يتم العثور على إعدادات، ضع البيانات الافتراضية
        console.log('📝 StoreSettings: No settings found, using defaults');
        setIsDataReady(true);
      }
      
      // فحص إضافي للإعدادات
      if (!isLoading && (!settings || !settings.organization_id)) {
        console.warn('⚠️ StoreSettings: Settings validation failed:', {
          isLoading,
          hasSettings: !!settings,
          settingsOrgId: settings?.organization_id,
          expectedOrgId: currentOrganization.id,
          possibleIssues: [
            !settings && 'No settings object',
            settings && !settings.organization_id && 'Missing organization_id in settings',
            settings?.organization_id !== currentOrganization.id && 'Organization ID mismatch'
          ].filter(Boolean)
        });
      }
    } else {
      console.warn('⚠️ StoreSettings: No organization data available:', {
        currentOrganization,
        hasId: !!currentOrganization?.id
      });
      setIsDataReady(false);
    }
