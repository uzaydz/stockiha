  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      try {
        ...
        console.log('✅ useAuthSingleton: تم التهيئة بنجاح');
        
      } catch (error) {
        console.error('❌ useAuthSingleton: خطأ في التهيئة:', error);
        setIsReady(true);
        setIsLoading(false);
      }
    };
    ...
    return () => {
      if (subscriptionId) {
        ...
        console.log('🗑️ useAuthSingleton: تم إلغاء الاشتراك');
      }
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const initializeUser = async () => {
      try {
        ...
        console.error('❌ useCurrentUser: خطأ في الحصول على المستخدم:', error);
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const initializeSession = async () => {
      try {
        ...
        console.error('❌ useCurrentSession: خطأ في الحصول على الجلسة:', error);
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const checkAuthStatus = async () => {
      try {
        ...
        console.error('❌ useAuthStatus: خطأ في التحقق من المصادقة:', error);
        setIsLoading(false);
      }
    };
