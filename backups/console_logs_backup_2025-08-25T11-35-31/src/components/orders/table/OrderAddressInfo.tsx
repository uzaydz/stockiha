...
const OrderAddressInfo = memo(({ order }: OrderAddressInfoProps) => {
  ...
  const getProvinceDisplayName = (provinceValue: any) => {
    if (!provinceValue) return '';
    // إذا كان الاسم جاهزاً (نص غير رقمي)، أعرضه كما هو
    if (typeof provinceValue === 'string' && /[A-Za-zأ-ي]/.test(provinceValue)) {
      return provinceValue;
    }
    // استخدم محول محلي للأكواد
    return typeof getProvinceName === 'function' ? getProvinceName(provinceValue) : String(provinceValue);
  };

  const getMunicipalityDisplayName = (municipalityCode: any, provinceCode?: any) => {
    ...
    // إذا كانت القيمة نصية غير رقمية (اسم جاهز من الـ RPC) فأعرضها كما هي
    if (typeof municipalityCode === 'string' && /[A-Za-zأ-ي]/.test(municipalityCode)) {
      return municipalityCode;
    }
    // استخدم المحول المحلي/المحلّي (بدون استدعاءات خارجية)
  ...
  const getAddressComponents = () => {
    ...
    // Province: استخدم قيمة العنوان أولاً (قد تكون name من RPC)، ثم state، ثم form_data
    const province = (order.shipping_address as any)?.province ?? order.shipping_address?.state ?? order.form_data?.province;
    if (province) {
      components.push({
        type: 'province',
    ...
    // Municipality: استخدم قيمة العنوان أولاً (اسم من RPC)، ثم form_data
    const municipality = order.shipping_address?.municipality ?? order.form_data?.municipality;
    if (municipality) {
