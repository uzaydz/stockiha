...
// Global cache للتأكد من عدم تكرار التهيئة
let globalTableInitialized = false;
...
export class ShippingSettingsService {
  constructor() {
    // لا نقوم بالتهيئة في constructor لتجنب الطلبات المكررة
  }

  /**
   * Initialize shipping tables if they don't exist (مع cache عالمي)
   */
  private async initShippingTables(): Promise<void> {
    // إذا تم التهيئة مسبقاً، لا نفعل شيء
    if (globalTableInitialized) return;
    ...
    try {
      ...
      globalTableInitialized = true;
    } catch (error) {
      // إعادة تعيين في حالة الفشل للسماح بإعادة المحاولة
  ...
  private async performInitialization(): Promise<void> {
    try {
      // استخدام cache للتحقق من وجود الجداول
      const tableExists = await withCache<boolean>(
        'shipping_tables_exist',
        async () => {
        ...
        LONG_CACHE_TTL // cache لمدة طويلة
      );

      if (!tableExists) {
                 // إنشاء الجداول فقط إذا لم تكن موجودة
         await (supabase as any).rpc('create_shipping_tables');
      ...
      // تسجيل الخطأ بصمت
      console.warn('Shipping tables initialization warning:', error);
    }
  }
