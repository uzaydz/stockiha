...
const AppsManagement: React.FC = () => {
  ...
  const disableApp = async (appId: string): Promise<boolean> => {
    ...
    try {
      ...
      if (error) {
        console.error('خطأ في إلغاء تفعيل التطبيق:', error);
        return false;
      }

      if (data && Array.isArray(data) && data.length > 0 && data[0].success) {
        console.log('✅ تم إلغاء تفعيل التطبيق بنجاح:', data[0].message);
        return true;
      } else {
        const errorMessage = Array.isArray(data) && data[0]?.message || 'خطأ غير محدد';
        console.error('❌ فشل في إلغاء تفعيل التطبيق:', errorMessage);
        return false;
      }
    } catch (error) {
      console.error('خطأ في إلغاء تفعيل التطبيق:', error);
  ...
  const refreshAppsData = async () => {
    try {
      ...
      console.log('✅ تم تحديث بيانات التطبيقات بنجاح بدون إعادة تحميل الصفحة');
    } catch (error) {
      console.error('خطأ في تحديث التطبيقات:', error);
      // في حالة فشل التحديث المباشر، نحاول الطريقة التقليدية
      try {
        await refreshApps();
      } catch (fallbackError) {
        console.error('خطأ في التحديث الاحتياطي:', fallbackError);
      }
    }
  };
  ...
  // تشخيص البيانات
  console.log('🔍 [AppsManagement] البيانات الحالية:', {
    ...
    enabledApps: organizationApps.filter((app: any) => app.is_enabled).length,
  ...
  const handleAppToggle = async (appId: string, enabled: boolean) => {
    ...
    try {
      ...
      if (success) {
        ...
        console.log('✅ تم تحديث حالة التطبيق بنجاح في handleAppToggle');
      } else {
        console.error('❌ فشل في تنفيذ العملية في handleAppToggle');
      }
    } catch (error) {
      console.error('❌ خطأ في handleAppToggle:', error);
    } finally {
  ...
  const executeAction = async () => {
    ...
    try {
      ...
      if (success) {
        // تحديث البيانات لإظهار التغيير فوراً - بدون إعادة تحميل الصفحة
        await refreshAppsData();
        console.log('✅ تم تحديث حالة التطبيق بنجاح في executeAction');
      } else {
        console.error('❌ فشل في تنفيذ العملية في executeAction');
      }
    } catch (error) {
      console.error('❌ خطأ في executeAction:', error);
    } finally {
      setIsProcessing(false);
      setSelectedApp(null);
    }
