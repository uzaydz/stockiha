-- إصلاح مشكلة إنشاء الموظفين - مشكلة الصلاحيات والـ RLS

-- إصلاح تضارب الدوال - إزالة جميع النسخ القديمة
...
DO $$
BEGIN
    RAISE NOTICE '✅ تم إصلاح دالة create_employee_unified بنجاح!';
    RAISE NOTICE '🚀 الفوائد: إنشاء موظف في استدعاء واحد مع معالجة أفضل للأخطاء';
    RAISE NOTICE '📝 الاستخدام: create_employee_unified(email, password, name, phone, job_title, permissions, organization_id)';
    RAISE NOTICE '🔒 الأمان: التحقق من الصلاحيات والـ RLS';
END;
$$;
