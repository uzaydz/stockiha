...
export default function YalidineSyncPage() {
  ...
  const fetchSyncStatus = async () => {
    ...
    try {
      ...
      console.log("Attempting to fetch sync status with fetch. Authenticated:", isAuthenticated);
      ...
      console.log("Response from get-global-yalidine-sync-info (fetch):", { status: response.status, data: responseData });
  ...
  const handleSaveConfig = async () => {
    ...
    setFeedbackMessage(null);
    console.log("Attempting to save Yalidine config with fetch:", { apiKey, apiToken });
    ...
    try {
      ...
      const responseData = await response.json(); 
      console.log("Response from save-global-yalidine-config (fetch):", { status: response.status, data: responseData });
  ...
  const handleSyncNow = async () => {
    ...
    try {
      ...
      console.log("Response from sync-global-yalidine-data (fetch):", { status: response.status, data });
  ...
  console.log("Button disabled states:", { isSavingConfig, isSyncing, apiKeyIsEmpty: !apiKey, apiTokenIsEmpty: !apiToken, isLoadingStatus });
  console.log("Environment Variables Check:", { supabaseUrlExists: !!supabaseUrl, supabaseAnonKeyExists: !!supabaseAnonKey });
  console.log("Auth state check:", {isAuthenticated});
