...
import { Ruler, Palette, Plus, Check, ArrowLeft, Copy, Settings, Zap } from 'lucide-react';
  ...
  const handleSizesChange = (sizes: ProductSize[]) => {
    ...
    if (selectedColor) {
      ...
      if (newQuantity !== oldQuantity) {
        toast.info(`تم تحديث كمية اللون "${selectedColor.name}" من ${oldQuantity} إلى ${newQuantity}`);
      }
    }
  };
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => {/* يمكن إضافة ميزة قوالب المقاسات */}}
                      className="bg-green-50 dark:bg-green-900/20 hover:bg-green-100 dark:hover:bg-green-900/30 border-green-200 dark:border-green-700 text-green-600 dark:text-green-400"
                    >
                  ...
                  <TooltipContent>
                    <p>استخدام قوالب مقاسات جاهزة</p>
                  </TooltipContent>
                </Tooltip>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => {/* يمكن إضافة ميزة نسخ المقاسات */}}
                      className="bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/30 border-blue-200 dark:border-blue-700 text-blue-600 dark:text-blue-400"
                    >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {color.has_sizes ? (
                        <span className="text-green-600 dark:text-green-400 flex items-center gap-1">
                          ...
                          {loadingSizes[color.id] ? (
                            <span className="text-xs">جاري التحميل...</span>
                          ) : (
                            <span>{color.sizes?.length ? `${color.sizes.length} مقاس` : 'مقاسات مفعلة'}</span>
                          )}
                        </span>
                      ) : (
                    ...
                    <div className="text-xs text-muted-foreground mt-1">
                      الكمية: {color.has_sizes && color.sizes && color.sizes.length > 0 
                        ? `${color.sizes.reduce((sum, size) => sum + size.quantity, 0)} (محسوبة)`
                        : color.quantity
                      } | السعر: {color.price} دج
                    </div>
                  </div>
            ...
            <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-primary/5 to-transparent rounded-lg border border-primary/20 mt-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Badge variant="secondary" className="text-sm">
                  الكمية: {selectedColor.has_sizes && selectedColor.sizes && selectedColor.sizes.length > 0 
                    ? `${selectedColor.sizes.reduce((sum, size) => sum + size.quantity, 0)} (محسوبة)`
