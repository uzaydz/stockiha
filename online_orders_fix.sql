-- online_orders_fix.sql
-- ملف لإصلاح مشكلة الطلبات عبر الإنترنت باستخدام جدول online_orders
...
-- 2. فحص قيود المفتاح الأجنبي على جدول order_items
SELECT 'فحص قيود المفتاح الأجنبي للعناصر:';
SELECT 
    tc.constraint_name, 
    tc.table_name, 
    kcu.column_name, 
...
CREATE INDEX IF NOT EXISTS idx_online_order_items_organization_id ON online_order_items(organization_id);

-- 4. إعادة إنشاء وظيفة معالجة الطلبات الأونلاين لتستخدم جدول online_orders وجدول online_order_items
