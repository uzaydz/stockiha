...
const POSAdvanced = () => {
  ...
  const handleProductWithVariants = (product: Product) => {
    ...
    console.log('🔍 [handleProductWithVariants] تم النقر على منتج:', {
      ...
      isReturnMode: isCurrentlyReturnMode,
      activeTabId,
    ...
    if (isCurrentlyReturnMode) {
      console.log('🔄 [handleProductWithVariants] إضافة للإرجاع:', product.name);
      addItemToReturnCart(product);
    } else {
      console.log('🛒 [handleProductWithVariants] إضافة للسلة العادية:', product.name);
      try {
        addItemToCart(product);
        console.log('✅ [handleProductWithVariants] تم استدعاء addItemToCart بنجاح');
      } catch (error) {
        console.error('❌ [handleProductWithVariants] خطأ في addItemToCart:', error);
      }
    }
  };
