...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children, initialOrganizationId }) => {
  ...
  const setTheme = useCallback((newTheme: Theme) => {
    ...
    if (isDebug) {
      ...
      console.log(`🔄 ThemeContext: Applied theme: ${newTheme} (${(endTime - startTime).toFixed(2)}ms)`);
    }
  }, [theme, isDebug]);
  ...
  const applyOrganizationTheme = useCallback(async () => {
    ...
    try {
      if (isDebug) {
        console.log('🏢 ThemeContext: Loading organization theme for:', initialOrganizationId);
      }
      ...
      if (isDebug) {
        console.log('✅ ThemeContext: Organization theme applied successfully');
      }

    } catch (error) {
      if (isDebug) {
        console.error('❌ ThemeContext: Error applying organization theme:', error);
      }
    } finally {
      // إزالة العلم
  ...
  useEffect(() => {
    ...
    if (isDebug) {
      console.log('✅ ThemeContext: Initial theme setup complete');
    }
  ...
  const fastThemeController = useMemo(() => ({
    ...
    applyImmediate: (targetTheme: Theme) => {
      ...
      Promise.resolve().then(() => {
        ...
        if (isDebug) {
          const endTime = performance.now();
          console.log(`⚡ Fast theme applied in: ${(endTime - startTime).toFixed(2)}ms`);
        }
      });
    },
