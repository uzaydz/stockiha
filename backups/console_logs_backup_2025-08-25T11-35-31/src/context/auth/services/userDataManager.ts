...
export class UserDataManager {
    ...
    if (savedData.userProfile && savedData.userProfile.id === userId) {
      ...
      if (!hasValidPermissions) {
        if (process.env.NODE_ENV === 'development') {
          console.log('⚠️ [UserDataManager] البيانات المحفوظة لا تحتوي على صلاحيات صحيحة، جلب بيانات جديدة');
        }
      } else {
      ...
      if (profile) {
        ...
        if (profile.organization_id) {
          try {
            ...
            if (process.env.NODE_ENV === 'development') {
              console.warn('⚠️ [UserDataManager] فشل في جلب بيانات المؤسسة:', orgError);
            }
          }
        }
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        if (result.userProfile) {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.log('🔄 [UserDataManager] تم تحديث بيانات المستخدم في الخلفية');
          }
        }
      }, 1000);
    } catch (error) {
  ...
  clearUserCache(userId?: string): void {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`🧹 [UserDataManager] تم مسح cache المستخدم ${userId || 'جميع المستخدمين'}`);
    }
  }
  ...
  cleanExpiredCache(): void {
    ...
    if (process.env.NODE_ENV === 'development' && keysToDelete.length > 0) {
      console.log(`🧹 [UserDataManager] تم تنظيف ${keysToDelete.length} عنصر منتهي الصلاحية`);
    }
  }
