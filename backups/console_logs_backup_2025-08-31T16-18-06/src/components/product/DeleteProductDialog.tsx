...
const DeleteProductDialog = ({ product, open, onOpenChange, onProductDeleted }: DeleteProductDialogProps) => {
  ...
  useEffect(() => {
    ...
    const checkPermissions = async () => {
      ...
      try {
        // استخدام الدالة الموحدة للتحقق من صلاحية حذف المنتجات
        const permissionsResult = await hasPermissions(['deleteProducts'], user.id);
        const canDelete = permissionsResult.deleteProducts || false;
