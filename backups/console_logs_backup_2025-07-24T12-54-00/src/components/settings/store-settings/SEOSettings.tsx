...
const SEOSettings: React.FC<SEOSettingsProps> = ({ settings, updateSetting }) => {
  ...
  const [seoSettings, setSeoSettings] = useState<SEOSettings>(() => {
    ...
    // جلب البيانات من custom_js أولاً
    if (settings.custom_js) {
      try {
        const customJsData = JSON.parse(settings.custom_js);
        if (customJsData?.seoSettings) {
          return { ...defaultSettings, ...customJsData.seoSettings };
        }
      } catch (error) {
        console.warn('خطأ في تحليل custom_js:', error);
      }
    }
  ...
  // جلب إعدادات SEO من store_settings عند تحميل المكون
  useEffect(() => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .maybeSingle();
           ...
           console.log('جلب إعدادات SEO من store_settings:', data.settings);
          ...
          console.warn('فشل في جلب إعدادات SEO:', error);
        }
      } catch (error) {
  ...
  // مزامنة إعدادات SEO مع custom_js عند تغيير seoSettings
  useEffect(() => {
    ...
    try {
      if (settings.custom_js) {
        if (settings.custom_js.includes('//') || settings.custom_js.includes('function')) {
          // إذا كان هناك كود JavaScript، لا نحلله كـ JSON
          return;
        } else {
    ...
    customJs.seoSettings = seoSettings;
    updateSetting('custom_js', JSON.stringify(customJs));
  }, [seoSettings, updateSetting]);
  ...
  // حفظ إعدادات SEO في جدول store_settings منفصل
  const saveSEOToStoreSettings = async () => {
    if (!settings.organization_id || !seoSettings) return;

    try {
      const { data, error } = await supabase.rpc('update_store_seo_settings', {
        _organization_id: settings.organization_id,
        _settings: seoSettings
      });

      if (error) {
        console.error('فشل في حفظ إعدادات SEO:', error);
        toast({
          title: 'خطأ في حفظ إعدادات SEO',
          description: 'حدث خطأ أثناء حفظ إعدادات SEO، الرجاء المحاولة مرة أخرى.',
          variant: 'destructive',
        });
      } else {
  ...
  // إضافة زر حفظ منفصل لإعدادات SEO
  const handleSaveSEO = async () => {
    await saveSEOToStoreSettings();
  };
  ...
  "telephone": "${seoSettings.structured_data.business_phone || '[رقم الهاتف]'}"
      ...
      <div className="p-6 border-t bg-muted/50">
        <div className="flex justify-between items-center">
          <div>
            <p className="text-sm text-muted-foreground">
              احفظ إعدادات SEO بشكل منفصل لضمان تطبيقها على المتجر
            </p>
          </div>
          <Button onClick={handleSaveSEO} className="bg-primary hover:bg-primary/90">
