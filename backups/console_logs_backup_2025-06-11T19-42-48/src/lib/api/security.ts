...
export async function enableTwoFactorAuth(verificationCode: string): Promise<{
  ...
  try {
    ...
    console.log('✅ تم التحقق من رمز TOTP بنجاح');
    ...
    if (error) {
      console.error('خطأ في تحديث إعدادات المصادقة:', error);
      return {
        success: false,
  ...
  try {
    ...
    if (settings.backup_codes && Array.isArray(settings.backup_codes)) {
      ...
      if (settings.backup_codes.includes(code) && !usedCodes.includes(code)) {
        ...
        console.log('✅ تم التحقق من backup code بنجاح');
        return { success: true };
      }
    }

    console.log('❌ فشل التحقق من جميع الرموز');
    // Debug info للتطوير
    await debugTOTP(settings.totp_secret);
