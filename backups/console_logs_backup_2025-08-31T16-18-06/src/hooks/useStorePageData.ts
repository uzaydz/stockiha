...
export const useStorePageData = (): UseStorePageDataReturn => {
  ...
  // جلب إعدادات الهيرو من store_settings
  const [heroSettings, setHeroSettings] = useState<any>(null);
  const [heroProducts, setHeroProducts] = useState<any[]>([]);

  // دالة جلب إعدادات الهيرو وتطبيق المنتجات المختارة
  const fetchHeroSettings = async () => {
    ...
    try {
      ...
      if (heroData?.settings) {
        setHeroSettings(heroData.settings);

        // إذا كان نوع المنتجات "selected"، جلب المنتجات المختارة
        const settings = heroData.settings as any;
        if (settings.productsType === 'selected' && settings.selectedProducts?.length > 0) {
          const { data: selectedProductsData, error: productsError } = await supabase
            .from('products')
            .select('*')
            .in('id', settings.selectedProducts)
            .eq('is_active', true)
            .limit(settings.productsLimit || 4);

          if (productsError) {
            console.error('خطأ في جلب المنتجات المختارة:', productsError);
          } else if (selectedProductsData) {
            const convertedProducts = selectedProductsData.map((dbProd: any) => {
              try {
            ...
            setHeroProducts(convertedProducts);
          }
        }
      }
    } catch (error) {
  ...
  // جلب إعدادات الهيرو عند تغيير المؤسسة
  useEffect(() => {
    fetchHeroSettings();
  }, [centralOrgId]);
  ...
  const componentsToRender = useMemo(() => {
    const components = customComponents.length > 0
      ? customComponents
          ...
          .map(component => {
            ...
            // دمج إعدادات الهيرو من store_settings
            if (normalizedType === 'hero' && heroSettings) {
              return {
                ...component,
                settings: {
                  ...component.settings,
      ...
      : defaultStoreComponents.map(component => {
          // دمج إعدادات الهيرو مع المكون الافتراضي
          if (component.type === 'hero' && heroSettings) {
            return {
              ...component,
              settings: {
                ...component.settings,
    ...
    return components;
  }, [customComponents, defaultStoreComponents, heroSettings]);
  ...
  useEffect(() => {
    ...
    // 2) المنتجات المميزة: أولوية للمنتجات المختارة من heroSettings، ثم المميزة من RPC
    if (heroProducts.length > 0) {
      // استخدم المنتجات المختارة من heroSettings
      setFeaturedProducts(heroProducts);
    } else if (sharedFeaturedProducts && sharedFeaturedProducts.length > 0) {
      // استخدم المنتجات المميزة من RPC
  ...
  }, [centralOrgId, sharedComponents, sharedFeaturedProducts, heroProducts]);
