...
const ComponentPreview: React.FC<ComponentPreviewProps> = ({ component }) => {
  ...
  switch (normalizedType) {
    ...
    case 'featuredproducts':
      const featuredDisplayCount = settings.selectionMethod === 'manual' && settings.selectedProducts?.length > 0
        ? Math.max(settings.selectedProducts.length, settings.displayCount || 4)
        : settings.displayCount || 4;
      return <FeaturedProducts {...settings} organizationId={currentOrganization?.id} displayCount={featuredDisplayCount} />;
      
    case 'featured_products':
      const featuredProductsDisplayCount = settings.selectionMethod === 'manual' && settings.selectedProducts?.length > 0
        ? Math.max(settings.selectedProducts.length, settings.displayCount || 4)
        : settings.displayCount || 4;
      return <FeaturedProducts {...settings} organizationId={currentOrganization?.id} displayCount={featuredProductsDisplayCount} />;
