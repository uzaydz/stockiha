...
export class GentleLogoutCleaner {
  ...
  static async performGentleLogout(options: LogoutOptions = {}): Promise<void> {
    if (this.isProcessing) {
      console.log('🔄 تسجيل خروج قيد التنفيذ بالفعل...');
      return;
    }

    this.isProcessing = true;
    console.log('🚪 بدء تسجيل الخروج اللطيف...');

    // إضافة timeout أمان لمنع التعليق
    const safetyTimeout = setTimeout(() => {
      console.warn('⚠️ انتهت مهلة تسجيل الخروج، سيتم الانتقال فوراً');
    ...
    try {
      ...
      console.log('✅ تم تسجيل الخروج اللطيف بنجاح');
    } catch (error) {
      console.error('❌ خطأ في تسجيل الخروج اللطيف:', error);
  ...
  private static showGentleLoadingState(): void {
    try {
      ...
      console.warn('⚠️ لا يمكن إظهار حالة التحميل:', error);
    }
  }
  ...
  private static async gentleClearData(): Promise<void> {
    console.log('🧹 بدء التنظيف اللطيف للبيانات...');
    ...
    try {
      ...
      console.log('✅ تم التنظيف اللطيف للبيانات');
    } catch (error) {
      console.error('❌ خطأ في التنظيف اللطيف:', error);
    }
  }
  ...
  private static async clearStorageGently(): Promise<void> {
    try {
      ...
      console.log('✅ تم تنظيف Storage بشكل لطيف');
    } catch (error) {
      console.error('❌ خطأ في تنظيف Storage:', error);
    }
  }
  ...
  private static async clearReactQueryGently(): Promise<void> {
    try {
      ...
      console.error('❌ خطأ في تنظيف React Query:', error);
    }
  }
  ...
  private static async clearOrganizationDataGently(): Promise<void> {
    try {
      ...
      console.log('✅ تم تنظيف بيانات المؤسسة');
    } catch (error) {
      console.error('❌ خطأ في تنظيف بيانات المؤسسة:', error);
    }
  }
  ...
  private static notifyContextProviders(): void {
    try {
      console.log('📢 إشعار Context providers...');
      ...
      console.log('✅ تم إشعار Context providers');
    } catch (error) {
      console.error('❌ خطأ في إشعار Context providers:', error);
    }
  }
  ...
  private static gentleNavigation(url: string): void {
    try {
      console.log(`🔄 التنقل اللطيف إلى: ${url}`);
      ...
      console.error('❌ خطأ في التنقل اللطيف:', error);
