...
const convertDatabaseProductToStoreProduct = (dbProduct: DBProduct): Product => {
  console.log('🔄 تحويل منتج من قاعدة البيانات:', {
    name: dbProduct.name,
    thumbnail_image: dbProduct.thumbnail_image,
...
const FeaturedProducts = ({
  ...
  useEffect(() => {
    const fetchSelectedProducts = async () => {
      if (
        ...
        setLoading(true);
        try {
          console.log('🎯 FeaturedProducts - جلب المنتجات المحددة يدوياً:', selectedProducts);
          const allProducts = await getProducts(organizationId);
          const filteredProducts = allProducts.filter(product => 
            selectedProducts.includes(product.id)
          );
          console.log('🎯 FeaturedProducts - المنتجات المجلبة:', filteredProducts.length);
          setFetchedProducts(filteredProducts.map(convertDatabaseProductToStoreProduct));
        } catch (error) {
          console.error('خطأ في جلب المنتجات المحددة:', error);
          setFetchedProducts([]);
        } finally {
          setLoading(false);
        }
  ...
  const displayedProducts = useMemo(() => {
    console.log('🎯 FeaturedProducts - البيانات المستلمة:', initialProducts?.length, 'منتجات');
    console.log('🎯 FeaturedProducts - طريقة الاختيار:', selectionMethod, '| العدد المطلوب:', displayCount);
    
    // فحص بيانات الصور في المنتجات المستلمة
    if (initialProducts && initialProducts.length > 0) {
      console.log('🖼️ فحص بيانات الصور:');
    ...
    if (initialProducts && initialProducts.length > 0) {
      ...
      console.log('🎯 FeaturedProducts - استخدام البيانات المُمررة');
    } else if (fetchedProducts.length > 0) {
      // استخدام البيانات المجلبة (من محرر المتجر)
      sourceProducts = fetchedProducts;
      console.log('🎯 FeaturedProducts - استخدام البيانات المجلبة');
      ...
      console.log('🎯 FeaturedProducts - استخدام بيانات ShopContext (احتياطي للاختيار التلقائي)');
      console.log('⚠️ تحذير: يتم جلب', shopProducts.length, 'منتج من ShopContext - يُفضل تمرير البيانات مباشرة');
    ...
    if (sourceProducts.length > 0) {
      ...
      if (selectionMethod === 'manual') {
        console.log('🎯 FeaturedProducts - الاختيار يدوي - عرض جميع المنتجات المُمررة');
        ...
        console.log('🎯 FeaturedProducts - تطبيق الفلترة التلقائية:', selectionCriteria);
      ...
      console.log('🎯 FeaturedProducts - المنتجات النهائية للعرض:', finalProducts.length, 'منتجات (من أصل', filteredProducts.length, ')');
    ...
    console.log('🎯 FeaturedProducts - لا توجد بيانات متاحة');
    return [];
  ...
  useEffect(() => {
    ...
    const handleBrokenImages = () => {
      try {
        ...
        console.error('خطأ في معالجة الصور المكسورة:', error);
      }
    };
