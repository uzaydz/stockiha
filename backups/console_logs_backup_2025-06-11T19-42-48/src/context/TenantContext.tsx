...
const extractSubdomain = async (hostname: string): Promise<string | null> => {
  ...
  if (hostname.includes('localhost')) {
    ...
    if (parts.length >= 2 && parts[0] !== 'localhost' && parts[0] !== 'www' && parts[0] !== '') {
      console.log(`[TenantContext] استخراج subdomain: ${parts[0]} من ${hostname}`);
      return parts[0];
    }
    
    // إذا كان فقط localhost بدون سابدومين
    if (hostnameWithoutPort === 'localhost') {
      console.log(`[TenantContext] localhost فقط، استخدام main`);
      return 'main';
    }
  }
...
const LoadingIndicator = ({ isLoading, error, retryCount }: { 
  ...
  const clearCacheAndReload = () => {
    try {
      ...
      console.error('خطأ في مسح التخزين المحلي:', e);
      window.location.reload();
    }
  };
...
export const TenantProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchOrganizationBySubdomain = useCallback(async (subdomain: string | null) => {
    ...
    try {
      ...
      if (error instanceof Error && error.message === 'timeout') {
        console.warn('انتهت مهلة تحميل بيانات المؤسسة');
      }
      return null;
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (quickOrgCheck && !organization && !loadingOrganization.current) {
      console.log(`[TenantContext] فحص سريع: وُجد معرف مؤسسة في localStorage: ${quickOrgCheck}`);
    }
    
    const loadTenantData = async () => {
      console.log('🏢 [TenantContext] بدء دالة loadTenantData');
      ...
      if (loadingOrganization.current || initialized.current) {
        console.log('⏭️ [TenantContext] تجاهل التحميل - جاري بالفعل أو مكتمل');
        return; // تجنب التحميل المتزامن أو إعادة التحميل غير الضرورية
      }
      
      console.log('🚀 [TenantContext] بدء التحميل الفعلي...');
      setIsLoading(true);
      setError(null); // إعادة تعيين الخطأ
      loadingOrganization.current = true;
      
      console.log(`[TenantContext] بدء عملية تحميل جديدة - عدد المحاولات: ${retryCount.current}`);
      
      // مسح الكاش المتعلق بهذا الـ subdomain
      const currentHostname = window.location.hostname;
      const cacheKeyPattern = `organization_subdomain:${currentSubdomain}`;
      console.log(`[TenantContext] مسح كاش للنطاق: ${cacheKeyPattern}`);
      ...
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
          loadingTimeout.current = setTimeout(() => {
            ...
            console.warn(`[TenantContext] انتهت مهلة تحميل بيانات المؤسسة (${API_TIMEOUTS.ORGANIZATION_LOAD}ms)`);
        ...
        const loadPromise = (async () => {
          console.log(`[TenantContext] بدء تحميل المؤسسة للـ subdomain: ${subdomain}`);
          
          // 0. أولاً - محاولة التحميل من localStorage مباشرة (للحصول على أسرع نتيجة)
          const storedOrgId = localStorage.getItem('bazaar_organization_id');
          if (storedOrgId) {
            console.log(`[TenantContext] محاولة تحميل المؤسسة من localStorage بالـ ID: ${storedOrgId}`);
            try {
              ...
              if (orgById) {
                console.log(`[TenantContext] ✅ تم العثور على المؤسسة بالـ ID:`, orgById.name);
                return updateOrganizationFromData(orgById);
              }
            } catch (error) {
              console.log(`[TenantContext] خطأ في تحميل من localStorage:`, error.message);
            }
          }
          
          // 1. محاولة التحميل من AuthContext
          if (authOrganization) {
            console.log(`[TenantContext] ✅ تم العثور على المؤسسة في AuthContext:`, authOrganization.name);
            return updateOrganizationFromData(authOrganization);
          }

          // 2. محاولة التحميل من النطاق الفرعي أو المجال المخصص
          if (subdomain && subdomain !== 'main') {
            console.log(`[TenantContext] محاولة تحميل المؤسسة بالـ subdomain: ${subdomain}`);
            try {
              const orgData = await fetchOrganizationBySubdomain(subdomain);
              if (orgData) {
                console.log(`[TenantContext] ✅ تم العثور على المؤسسة بالـ subdomain:`, orgData.name);
                return updateOrganizationFromData(orgData);
              }
            } catch (error) {
              console.log(`[TenantContext] خطأ في تحميل بالـ subdomain:`, error.message);
            }
          }

          console.log(`[TenantContext] ❌ لم يتم العثور على المؤسسة بأي طريقة`);
          return null;
        })();
        ...
        if (retryCount.current < maxRetries) {
          ...
          setError(new Error('فشل في تحميل بيانات المؤسسة. يرجى إعادة تحميل الصفحة.'));
