-- الحل النهائي لمشكلة عرض البلديات
-- المشكلة: الدالة تستخدم a.municipality من جدول addresses وهو يحتوي على معرف البلدية وليس الاسم
...
-- إنشاء الدالة مع حل مشكلة البلديات في addresses أيضاً
...
BEGIN
    ...
    SELECT 
        ...
        -- الحل النهائي: حل أسماء البلديات في كلا الحالتين
        CASE 
            WHEN a.id IS NOT NULL THEN 
                jsonb_build_object(
                    ...
                    'state', a.state,
                    'municipality', 
                    -- حل اسم البلدية من معرفها في جدول addresses
                    CASE 
                        WHEN amun.name IS NOT NULL THEN amun.name 
                        ELSE a.municipality 
                    END,
                    'phone', a.phone
                )
            ELSE 
                jsonb_build_object(
                    'province', 
                    CASE 
                        WHEN wprov.name IS NOT NULL THEN wprov.name 
                        ELSE o.form_data->>'province' 
                    END,
                    'municipality', 
                    -- حل اسم البلدية من معرفها في form_data
                    CASE 
                        WHEN wmun.name IS NOT NULL THEN wmun.name 
                        ELSE o.form_data->>'municipality' 
                    END,
    ...
    -- JOIN إضافي لحل أسماء البلديات في جدول addresses
...
-- رسالة النجاح
SELECT 'تم إصلاح المشكلة! الآن سيتم حل أسماء البلديات من جدول addresses أيضاً' as status;

-- اختبار فوري
SELECT 
  customer_order_number,
  shipping_address->>'municipality' as municipality_should_show_biskra,
  form_data->>'municipality' as municipality_id
FROM get_advanced_online_orders(
  '560e2c06-d13c-4853-abcf-d41f017469cf', 2, 0
)
WHERE customer_order_number IS NOT NULL
ORDER BY customer_order_number DESC
LIMIT 2; 