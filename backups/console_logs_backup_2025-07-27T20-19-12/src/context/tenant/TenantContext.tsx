...
export const TenantProvider: React.FC<TenantProviderProps> = React.memo(({ children }) => {
  ...
  const {
    ...
    onSuccess: (org) => {
      ...
      console.log('✅ [TenantProvider] تم تحميل المؤسسة:', org.name);
    },
    onError: (err) => {
      console.error('❌ [TenantProvider] فشل تحميل المؤسسة:', err.message);
    }
  });
  ...
  useEffect(() => {
    if (organization) {
      try {
        console.log('💾 [TenantProvider] حفظ بيانات المؤسسة في الكاش:', organization.name);
        ...
        if (subdomain && subdomain !== 'main') {
          ...
          console.log('💾 [TenantProvider] تم حفظ بيانات المتجر في session storage:', storeInfo);
        }
        ...
        console.log('🔔 [TenantProvider] تم إطلاق حدث تحديث البيانات');
        
      } catch (error) {
        console.error('❌ [TenantProvider] خطأ في حفظ بيانات المؤسسة:', error);
      }
    }
