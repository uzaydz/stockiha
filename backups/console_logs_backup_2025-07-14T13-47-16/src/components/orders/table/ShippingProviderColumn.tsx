...
export const ShippingProviderColumn: React.FC<ShippingProviderColumnProps> = ({ 
  ...
  const getActiveProvider = () => {
    // أولاً، التحقق من البيانات المُحمّلة مسبقاً
    ...
    // أو التحقق من shipping_orders في الطلب
    const orderWithShipping = order as any;
    if (orderWithShipping.shipping_orders && orderWithShipping.shipping_orders.length > 0) {
      const latestShipping = orderWithShipping.shipping_orders[0];
      if (latestShipping.tracking_number && latestShipping.provider) {
        return {
          code: latestShipping.provider.code,
          trackingId: latestShipping.tracking_number,
          providerName: latestShipping.provider.name
        };
      }
    }
