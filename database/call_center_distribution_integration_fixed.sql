-- ========================================
-- Call Center Distribution Integration SQL - Fixed Version
-- تكامل نظام مركز الاتصالات مع توزيع الطلبات - النسخة المصححة
-- ========================================

BEGIN;

-- 1. إنشاء جدول قواعد التوزيع للـ call center (يجب إنشاؤه أولاً)
...
-- 2. إنشاء جدول ربط الطلبات بوكلاء الاتصالات
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'order_distribution_settings' AND column_name = 'call_center_enabled') THEN
        ALTER TABLE order_distribution_settings ADD COLUMN call_center_enabled BOOLEAN DEFAULT FALSE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'order_distribution_settings' AND column_name = 'call_center_settings') THEN
        ALTER TABLE order_distribution_settings ADD COLUMN call_center_settings JSONB DEFAULT '{}';
    END IF;
END
$$;
...
WHERE call_center_settings = '{}'::jsonb OR call_center_settings IS NULL;

-- 6. إضافة أعمدة للجداول الموجودة إذا لم تكن موجودة
DO $$
BEGIN
...
SELECT DISTINCT
    ...
    'region',
    '{"regions": ["قسنطينة", "الجزائر", "وهران", "عنابة", "سطيف"]}'::jsonb,
    '{"priority_level": 3, "max_attempts": 3}'::jsonb,
    1
...
SELECT DISTINCT
    ...
    'order_value',
    '{"min_order_value": 3000}'::jsonb,
    '{"priority_level": 5, "assign_to_top_performers": true}'::jsonb,
    2
FROM call_center_agents
WHERE NOT EXISTS (
    SELECT 1 FROM call_center_distribution_rules 
