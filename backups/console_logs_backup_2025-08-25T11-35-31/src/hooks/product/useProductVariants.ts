...
export const useProductVariants = ({
  ...
  useEffect(() => {
    ...
    if (product && !isInitialized.current) {
      console.log('🔄 [useProductVariants] تهيئة الاختيارات للمنتج الجديد:', product.id);
      resetSelections();
      isInitialized.current = true;
      lastProductId.current = currentProductId;
    } else if (product && currentProductId !== lastProductId.current) {
      // منتج جديد - إعادة التهيئة
      console.log('🔄 [useProductVariants] منتج جديد، إعادة التهيئة:', product.id);
      resetSelections();
      lastProductId.current = currentProductId;
    }
  ...
  const setSelectedSize = useCallback((size: ProductSize | undefined) => {
    ...
    // التحقق من أن المقاس ينتمي للون المحدد
    if (size && !selectedColor.sizes?.some(s => s.id === size.id)) {
      return;
    }
  ...
  const resetSelections = useCallback(() => {
    if (product?.variants?.has_variants) {
      ...
      console.log('🔄 [useProductVariants] resetSelections - اللون الافتراضي:', defaultColor?.id || 'غير متوفر');
      ...
      if (defaultColor) {
        ...
        if (defaultColor.has_sizes) {
          ...
          console.log('🔄 [useProductVariants] resetSelections - المقاس الافتراضي:', defaultSize?.id || 'غير متوفر');
          setSelectedSizeState(defaultSize || undefined);
        } else {
          setSelectedSizeState(undefined);
        }
  ...
  useEffect(() => {
    if (selectedColor && selectedColor.has_sizes) {
      ...
      if (defaultSize && defaultSize !== selectedSize) {
        console.log('🔄 [useProductVariants] تعيين المقاس الافتراضي للون:', selectedColor.id, 'المقاس:', defaultSize.id);
  ...
  useEffect(() => {
    ...
    if (selectedColor && !availableColors.some(c => c.id === selectedColor.id)) {
      console.log('🔄 [useProductVariants] اللون المحدد غير متوفر، إعادة تعيين');
      setSelectedColorState(undefined);
      hasChanges = true;
    }
    
    if (selectedSize && !availableSizes.some(s => s.id === selectedSize.id)) {
      console.log('🔄 [useProductVariants] المقاس المحدد غير متوفر، إعادة تعيين');
      setSelectedSizeState(undefined);
      hasChanges = true;
    }
