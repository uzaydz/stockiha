...
export const usePOSBarcode = ({
  ...
  const processBarcodeScanned = useCallback(async (rawBarcode: string) => {
    ...
    if (isProcessingBarcode.current) {
      console.log('[DEBUG] تم تجاهل معالجة الباركود - معالجة جارية بالفعل');
      return;
    }
    ...
    console.log('[DEBUG] بدء معالجة الباركود:', barcode);
    ...
    try {
      ...
      if (product) {
        ...
        console.log('[DEBUG] تم إضافة المنتج:', product.name);
        return;
      }
      ...
      for (const prod of currentProducts) {
        if (prod.colors && prod.colors.length > 0) {
          ...
          if (color) {
            ...
            console.log('[DEBUG] تم إضافة متغير (لون):', color.name, prod.name);
            return;
          }
            ...
            for (const color of prod.colors) {
              if (color.sizes && color.sizes.length > 0) {
                ...
                if (size) {
                  ...
                  console.log('[DEBUG] تم إضافة متغير (مقاس):', size.size_name, color.name, prod.name);
                  return;
                }
              }
            }
      ...
      if (foundProduct) {
        ...
        console.log('[DEBUG] تم إضافة منتج من قاعدة البيانات:', foundProduct.name);
        return;
      }
      ...
      setTimeout(() => {
        ...
        console.log('[DEBUG] تم إنهاء معالجة الباركود');
  ...
  useEffect(() => {
    if (barcodeBuffer.length > 0) {
      const clearBufferTimeout = setTimeout(() => {
        ...
        setBarcodeBuffer('');
        barcodeBufferRef.current = '';
        console.log('[DEBUG] تم مسح buffer الباركود بسبب انتهاء المهلة');
