...
const NavigationGroup = ({
  ...
  toggleGroup,
  isCollapsed,
  isMobile = false // إضافة قيمة افتراضية
      ...
      >
        ...
        >
          <group.icon className={cn(
            "text-current", 
            isCollapsed ? "w-5 h-5" : isMobile ? "w-3.5 h-3.5" : "w-4 h-4"
          )} />
            ...
            <span className={cn(
              ...
              {group.badge && (
                <motion.span
                  ...
                  className={cn(
                    "mr-1 px-2 py-0.5 text-[10px] rounded-full bg-primary/10 text-primary",
                    // تحسين الحجم للهاتف المحمول
                    isMobile && "px-1.5 py-0.5 text-[9px]"
                  )}
                >
                  {group.badge}
                </motion.span>
          ...
          {showPopup && (
            <motion.div
              ...
              className={cn(
                ...
                "backdrop-blur-sm",
                // تحسين الحجم للهاتف المحمول
                isMobile && "min-w-56 p-2.5"
              )}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="mb-2.5 pb-2 border-b border-sidebar-border">
                <h3 className={cn(
                  "text-sm font-medium text-card-foreground px-1 mb-1 flex items-center gap-1.5",
                  // تحسين حجم الخط للهاتف المحمول
                  isMobile && "text-xs"
                )}>
                  <group.icon className={cn(
                    "w-3.5 h-3.5 ml-1 text-primary",
                    isMobile && "w-3 h-3"
                  )} />
                  {group.group}
                  {group.badge && (
                    <span className={cn(
                      "text-[10px] px-1.5 py-0.5 rounded-full bg-primary/10 text-primary font-normal",
                      // تحسين الحجم للهاتف المحمول
              ...
              <div className={cn(
                "space-y-1 py-1 max-h-[calc(100vh-200px)] overflow-y-auto",
        ...
        <AnimatePresence mode="wait">
          {isGroupActive && (
            ...
            >
              ...
              >
                <div className={cn(
                  "pr-3.5 mr-1 mt-1 space-y-1.5 border-r border-primary/20 pb-1",
                  // تحسين المسافات للهاتف المحمول
                  isMobile && "pr-2.5 mr-0.5 mt-0.5 space-y-1 pb-0.5"
                )}>
                  {filteredItems.map((item) => {
                    const isActive =
                      currentPath === item.href ||
