...
export async function calculateDeliveryFeesOptimized(
  ...
  try {
    ...
    if (input.productShippingInfo) {
      ...
      console.log('🚚 DEBUG - Product Shipping Info Details:', {
        type: input.productShippingInfo?.type,
        code: input.productShippingInfo?.code,
        originalProvider: input.productShippingInfo?.original_provider,
        fullShippingInfo: input.productShippingInfo
      });
      ...
      console.log('🚚 DEBUG - Determined Provider Code:', providerCode);
    ...
    console.log('🚚 DEBUG - ZR Express Delivery Calculation Starting...');
    ...
    // إذا لم تكن هناك معلومات شحن للمنتج، استخدام shippingProvider كـ fallback
    console.log('⚠️ DEBUG - No productShippingInfo, using shippingProvider:', input.shippingProvider);
    
    const fallbackProviderCode = input.shippingProvider?.code?.toLowerCase();
    console.log('🚚 DEBUG - Fallback Provider Code:', fallbackProviderCode);
    
    if (fallbackProviderCode === 'zrexpress') {
      console.log('🚚 DEBUG - Using ZR Express from fallback');
      return await calculateZRExpressDelivery(input, result);
    } else {
      console.log('🚚 DEBUG - Using Yalidine as final fallback');
      return await calculateYalidineDelivery(input, result);
    }
...
async function calculateZRExpressDelivery(
  ...
  console.log('🚚 DEBUG - ZR Express Function Input:', {
    organizationId: input.organizationId,
    selectedProvinceId: input.selectedProvinceId,
    selectedMunicipalityId: input.selectedMunicipalityId,
    deliveryType: input.deliveryType,
    quantity: input.quantity,
    municipalityData: result.selectedMunicipality
  });
  ...
  try {
    ...
    console.log('🚚 DEBUG - ZR Express API Request Body:', requestBody);
    
    console.log('🔍 DEBUG - Supabase URL:', supabase.supabaseUrl);
    console.log('🔍 DEBUG - About to call Edge Function...');
    
        // استدعاء Edge Function مباشر عبر fetch للتحكم الكامل في Headers
    let data, error;
    try {
      console.log('🔄 DEBUG - Calling Edge Function via direct fetch...');
      const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
      const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY;
      
      const fetchResponse = await fetch(`${supabaseUrl}/functions/v1/calculate-zrexpress-shipping`, {
          method: 'POST',
          headers: {
      ...
      console.log('📡 DEBUG - Edge Function response status:', fetchResponse.status);
        
        if (!fetchResponse.ok) {
        const errorText = await fetchResponse.text();
        console.log('❌ DEBUG - Edge Function HTTP error:', errorText);
        throw new Error(`HTTP ${fetchResponse.status}: ${fetchResponse.statusText} - ${errorText}`);
        }
        
        data = await fetchResponse.json();
        error = null;
      console.log('✅ DEBUG - Edge Function succeeded:', data);
      } catch (fetchError) {
      console.log('❌ DEBUG - Edge Function failed:', fetchError);
        data = null;
        error = fetchError;
    }
     
     console.log('🚚 DEBUG - ZR Express API Response:', { 
       data, 
       error,
    ...
    if (!error && data && data.success && typeof data.price === 'number') {
      console.log('✅ DEBUG - ZR Express API Success! Price:', data.price);
      ...
      console.log('❌ DEBUG - ZR Express API Failed, trying database fallback...');
    }
    ...
    console.log('🚚 DEBUG - ZR Express Database Query:', {
      wilayaId: result.selectedMunicipality?.wilaya_id?.toString(),
      data: zrData,
      error: dbError
    });
    ...
    console.log('❌ DEBUG - ZR Express Exception caught:', error);
    console.log('⚠️ DEBUG - Falling back to default delivery calculation...');
    return await calculateFallbackDelivery(input, result);
  }
}
...
async function calculateFallbackDelivery(
  ...
  console.log('🔄 DEBUG - Fallback Delivery Calculation Started');
  console.log('🔄 DEBUG - Fallback Input:', {
    deliveryType: result.deliveryType,
    selectedMunicipality: result.selectedMunicipality?.name,
    hasStopDesk: result.selectedMunicipality?.has_stop_desk,
    weight: input.weight
  });
  ...
  console.log('🔄 DEBUG - Fallback Delivery Result:', {
    basePrice,
    oversizeCharge,
    totalFee: result.deliveryFee,
