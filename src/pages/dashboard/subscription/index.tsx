...
const SubscriptionPage = () => {
  const { user, organization, refreshOrganizationData } = useAuth();
    ...
    const fetchCurrentSubscription = async () => {
      ...
      if (!organization.subscription_id) {
        ...
        try {
          ...
          // تصفية النتائج يدويًا
          const activeSubscriptions = data
            .filter(sub => sub.organization_id === organization.id && sub.status === 'active')
            .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      ...
      // استدعاء جلب بيانات الاشتراك
      fetchSubscriptionDetails(organization.subscription_id);
    };
          ...
          <div className="space-y-6">
            <Card className="mb-6 sm:mb-8">
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  {currentSubscription 
                    ? `أنت مشترك حالياً في الخطة ${currentSubscription.plan?.name}`
                    : isTrialActive 
