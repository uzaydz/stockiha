...
export const useProductPurchase = ({
  ...
  const fetchProduct = useCallback(async () => {
    ...
    if (fetchingRef.current && lastFetchKey.current === fetchKey) {
      console.log('🔄 تجنب استدعاء مكرر للمنتج:', fetchKey);
      return;
    }
    ...
    if (cached && (Date.now() - cached.timestamp) < CACHE_DURATION) {
      console.log('📦 استخدام المنتج من الـ cache:', cacheKey);
    ...
    try {
      ...
      console.log(`🔄 جلب بيانات المنتج: ${productId} (Organization: ${organizationId || 'public'})`);
